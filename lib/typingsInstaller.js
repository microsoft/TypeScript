...
(function (ts) {
    ...
    ts.Diagnostics = {
        ...
        Pipeline_expressions_are_part_of_a_stage_1_ECMAScript_proposal_and_are_subject_to_change_in_future_releases_Set_the_experimentalPipelineStage1_option_to_remove_this_warning: diag(1255, ts.DiagnosticCategory.Error, "Pipeline_expressions_are_part_of_a_stage_1_ECMAScript_proposal_and_are_subject_to_change_in_future_r_1255", "Pipeline expressions are part of a stage 1 ECMAScript proposal and are subject to change in future releases. Set the 'experimentalPipelineStage1' option to remove this warning."),
        ...
        Enables_experimental_support_for_the_stage_1_ECMAScript_pipeline_operator: diag(6187, ts.DiagnosticCategory.Message, "Enables_experimental_support_for_the_stage_1_ECMAScript_pipeline_operator_6187", "Enables experimental support for the stage-1 ECMAScript pipeline operator."),
...
(function (ts) {
    ...
    function getTokenPosOfNode(node, sourceFile, includeJsDoc) {
        ...
        if (node.kind === 287 && node._children.length > 0) {
    ...
    function isSuperCall(n) {
        return n.kind === 182 && n.expression.kind === 98;
    }
    ts.isSuperCall = isSuperCall;
    function isImportCall(n) {
        return n.kind === 182 && n.expression.kind === 92;
    }
    ...
    function getJSDocCommentRanges(node, text) {
        var commentRanges = (node.kind === 147 ||
            node.kind === 146 ||
            node.kind === 187 ||
            node.kind === 188 ||
            node.kind === 186) ?
    ...
    function isPartOfTypeNode(node) {
        ...
        switch (node.kind) {
            ...
            case 72:
                ...
                ts.Debug.assert(node.kind === 72 || node.kind === 144 || node.kind === 180, "'node' was expected to be a qualified name, identifier or property access in 'isPartOfTypeNode'.");
            case 144:
            case 180:
            case 100:
    ...
    function getImmediatelyInvokedFunctionExpression(func) {
        if (func.kind === 187 || func.kind === 188) {
            ...
            if (ts.isPipelineExpression(parent) && parent.right === prev) {
                return parent;
            }
        }
    }
    ...
    function isInExpressionContext(node) {
        ...
        switch (parent.kind) {
            ...
            case 215:
                ...
                return (forStatement.initializer === node && forStatement.initializer.kind !== 228) ||
                    forStatement.condition === node ||
                    forStatement.incrementor === node;
            case 216:
            case 217:
                var forInStatement = parent;
                return (forInStatement.initializer === node && forInStatement.initializer.kind !== 228) ||
    ...
    function getSpecialPropertyAssignmentKind(expr) {
        ...
        if (expr.operatorToken.kind !== 59 || expr.left.kind !== 180) {
            return 0;
        }
        var lhs = expr.left;
        if (lhs.expression.kind === 72) {
        ...
        else if (lhs.expression.kind === 100) {
            return 4;
        }
        else if (lhs.expression.kind === 180) {
            var innerPropertyAccess = lhs.expression;
            if (innerPropertyAccess.expression.kind === 72) {
    ...
    function getJSDocCommentsAndTags(node) {
        ...
        function getJSDocCommentsAndTagsWorker(node) {
            ...
            var isSourceOfAssignmentExpressionStatement = parent && parent.parent &&
                parent.kind === 195 &&
                parent.operatorToken.kind === 59 &&
                parent.parent.kind === 211;
            if (isSourceOfAssignmentExpressionStatement) {
                getJSDocCommentsAndTagsWorker(parent.parent);
            }
            var isModuleDeclaration = node.kind === 234 &&
                parent && parent.kind === 234;
            var isPropertyAssignmentExpression = parent && parent.kind === 262;
    ...
    function getParameterSymbolFromJSDoc(node) {
        ...
        var parameter = ts.find(func.parameters, function (p) {
            return p.name.kind === 72 && p.name.escapedText === name;
        });
        return parameter && parameter.symbol;
    }
    ts.getParameterSymbolFromJSDoc = getParameterSymbolFromJSDoc;
    function getJSDocHost(node) {
        ts.Debug.assert(node.parent.kind === 276);
    ...
    function isRestParameter(node) {
        if (isInJavaScriptFile(node)) {
            if (node.type && node.type.kind === 275 ||
                ts.forEach(ts.getJSDocParameterTags(node), function (t) { return t.typeExpression && t.typeExpression.type.kind === 275; })) {
                return true;
            }
        }
    ...
    function isInAmbientContext(node) {
        while (node) {
            if (hasModifier(node, 2) || (node.kind === 266 && node.isDeclarationFile)) {
                return true;
            }
            node = node.parent;
        }
    ...
    function isAliasSymbolDeclaration(node) {
        return node.kind === 238 ||
            node.kind === 237 ||
            node.kind === 240 && !!node.name ||
            node.kind === 241 ||
            node.kind === 243 ||
            node.kind === 247 ||
            node.kind === 244 && exportAssignmentIsAlias(node);
    }
    ...
    function getAllAccessorDeclarations(declarations, accessor) {
        ...
        if (hasDynamicName(accessor)) {
            ...
            ts.forEach(declarations, function (member) {
                if ((member.kind === 154 || member.kind === 155)
    ...
    function isEntityNameExpression(node) {
        return node.kind === 72 ||
            node.kind === 180 && isEntityNameExpression(node.expression);
    }
    ts.isEntityNameExpression = isEntityNameExpression;
    function isRightSideOfQualifiedNameOrPropertyAccess(node) {
        return (node.parent.kind === 144 && node.parent.right === node) ||
            (node.parent.kind === 180 && node.parent.name === node);
    }
    ts.isRightSideOfQualifiedNameOrPropertyAccess = isRightSideOfQualifiedNameOrPropertyAccess;
    function isEmptyObjectLiteral(expression) {
        return expression.kind === 179 &&
...
(function (ts) {
    ...
    function getNameOfDeclaration(declaration) {
        ...
        if (ts.isJSDocPropertyLikeTag(declaration) && declaration.name.kind === 144) {
...
(function (ts) {
    ...
    function isPipelineExpression(node) {
        return isBinaryExpression(node)
            && node.operatorToken.kind === 55;
    }
    ts.isPipelineExpression = isPipelineExpression;
...
(function (ts) {
    ...
    function isBlockStatement(node) {
        ...
        if (node.parent !== undefined) {
            if (node.parent.kind === 225 || node.parent.kind === 261) {
                return false;
            }
        }
    ...
    function isJSDocNode(node) {
        return node.kind >= 268 && node.kind <= 286;
    }
    ts.isJSDocNode = isJSDocNode;
    function isJSDocCommentContainingNode(node) {
        return node.kind === 276 || isJSDocTag(node);
    }
    ts.isJSDocCommentContainingNode = isJSDocCommentContainingNode;
    function isJSDocTag(node) {
        return node.kind >= 277 && node.kind <= 286;
    }
...
(function (ts) {
    ...
    (function (Parser) {
        ...
        function createSourceFile(fileName, languageVersion, scriptKind) {
            var sourceFile = new SourceFileConstructor(266, 0, sourceText.length);
            nodeCount++;
        ...
        function createNode(kind, pos) {
            ...
            return ts.isNodeKind(kind) ? new NodeConstructor(kind, pos, pos) :
                kind === 72 ? new IdentifierConstructor(kind, pos, pos) :
        ...
        function nextTokenCanFollowModifier() {
            ...
            if (token() === 85) {
                ...
                return token() !== 39 && token() !== 119 && token() !== 17 && canFollowModifier();
            }
            if (token() === 80) {
        ...
        function isValidHeritageClauseObjectLiteral() {
            ...
            if (nextToken() === 18) {
                var next = nextToken();
                return next === 26 || next === 17 || next === 86 || next === 109;
            }
            return true;
        }
        ...
        function isListTerminator(kind) {
            ...
            switch (kind) {
                ...
                case 22:
                    return token() === 18;
                case 3:
                    return token() === 18 || token() === 74 || token() === 80;
                case 7:
                    return token() === 17 || token() === 86 || token() === 109;
                case 8:
                    return isVariableDeclaratorListTerminator();
                case 18:
                    return token() === 29 || token() === 19 || token() === 17 || token() === 86 || token() === 109;
                case 11:
        ...
        function isStartOfMappedType() {
            ...
            return token() === 21 && nextTokenIsIdentifier() && nextToken() === 93;
        }
        ...
        function parseMappedType() {
            ...
            parseExpected(17);
            node.readonlyToken = parseOptionalToken(132);
            parseExpected(21);
        ...
        function parseNonArrayType() {
            switch (token()) {
                ...
                case 51:
                    return parseJSDocNodeWithType(272);
                case 13:
                case 9:
                case 8:
                case 102:
        ...
        function parseInitializer(inParameter, requireEqualsToken) {
            if (token() !== 59) {
                ...
                if (inParameter && requireEqualsToken) {
                    var result = createMissingNode(72, true, ts.Diagnostics._0_expected, "=");
        ...
        function isParenthesizedArrowFunctionExpression() {
            if (token() === 19 || token() === 27 || token() === 121) {
        ...
        function isUnParenthesizedAsyncArrowFunctionWorker() {
            if (token() === 121) {
                ...
                var expr = parseBinaryExpressionOrHigher(0);
                if (!scanner.hasPrecedingLineBreak() && expr.kind === 72 && token() === 36) {
                    return 1;
                }
            }
            return 0;
        ...
        function parseParenthesizedArrowFunctionExpressionHead(allowAmbiguity) {
            ...
            fillSignature(57, isAsync | (allowAmbiguity ? 0 : 8), node);
            if (!node.parameters) {
                return undefined;
            }
        ...
        function parseConditionalExpressionRest(leftOperand) {
            ...
            node.colonToken = parseExpectedToken(57, false, ts.Diagnostics._0_expected, ts.tokenToString(57));
        ...
        function parseLeftHandSideExpressionOrHigher() {
            var expression;
            if (token() === 92 && lookAhead(nextTokenIsOpenParenOrLessThan)) {
        ...
        function parseJsxAttributes() {
            var jsxAttributes = createNode(255);
        ...
        function tryParseAccessorDeclaration(fullStart, decorators, modifiers) {
            ...
            else if (parseContextualModifier(136)) {
                return parseAccessorDeclaration(155, fullStart, decorators, modifiers);
            }
            return undefined;
        }
        function parseObjectLiteralElement() {
            ...
            if (dotDotDotToken) {
                var spreadElement = createNode(264, fullStart);
            ...
            var isShorthandPropertyAssignment = tokenIsIdentifier && (token() === 26 || token() === 18 || token() === 59);
            if (isShorthandPropertyAssignment) {
                var shorthandDeclaration = createNode(263, fullStart);
                ...
                var propertyAssignment = createNode(262, fullStart);
        ...
        function parseNewExpression() {
            ...
            parseExpected(95);
            if (parseOptional(23)) {
                var node_1 = createNode(205, fullStart);
                node_1.keywordToken = 95;
        ...
        function parseSwitchStatement() {
            ...
            node.expression = allowInAnd(parseExpression);
            parseExpected(20);
            var caseBlock = createNode(236, scanner.getStartPos());
            parseExpected(17);
        ...
        function parseExpressionOrLabeledStatement() {
            ...
            if (expression.kind === 72 && parseOptional(57)) {
                var labeledStatement = createNode(223, fullStart);
                labeledStatement.label = expression;
                labeledStatement.statement = parseStatement();
                return addJSDocComment(finishNode(labeledStatement));
            }
            else {
                var expressionStatement = createNode(211, fullStart);
        ...
        function isDeclaration() {
            while (true) {
                switch (token()) {
                    ...
                    case 142:
                        nextToken();
                        return token() === 17 || token() === 72 || token() === 85;
                    case 92:
                        nextToken();
            ...
            switch (token()) {
                ...
                default:
                    if (decorators || modifiers) {
                        var node = createMissingNode(248, true, ts.Diagnostics.Declaration_expected);
                        node.pos = fullStart;
                        node.decorators = decorators;
                        node.modifiers = modifiers;
                        return finishNode(node);
        ...
        function parseMethodDeclaration(fullStart, decorators, modifiers, asteriskToken, name, questionToken, diagnosticMessage) {
            var method = createNode(152, fullStart);
        ...
        function isClassMemberStart() {
            ...
            if (idToken !== undefined) {
                if (!ts.isKeyword(idToken) || idToken === 136 || idToken === 126) {
                    return true;
                }
                switch (token()) {
                    case 19:
        ...
        function parseClassElement() {
            ...
            if (decorators || modifiers) {
                var name = createMissingNode(72, true, ts.Diagnostics.Declaration_expected);
                return parsePropertyDeclaration(fullStart, decorators, modifiers, name, undefined);
            }
            ts.Debug.fail("Should not have attempted to parse class member declaration.");
        }
        function parseClassExpression() {
            return parseClassDeclarationOrExpression(scanner.getStartPos(), undefined, undefined, 200);
        }
        function parseClassDeclaration(fullStart, decorators, modifiers) {
            return parseClassDeclarationOrExpression(fullStart, decorators, modifiers, 230);
        }
        ...
        function parseEnumMember() {
            var node = createNode(265, scanner.getStartPos());
            node.name = parsePropertyName();
        ...
        function parseModuleBlock() {
            var node = createNode(235, scanner.getStartPos());
            if (parseExpected(17)) {
                node.statements = parseList(1, parseStatement);
                parseExpected(18);
            }
        ...
        function parseNamespaceExportDeclaration(fullStart, decorators, modifiers) {
            var exportDeclaration = createNode(237, fullStart);
        ...
        function parseImportDeclarationOrImportEqualsDeclaration(fullStart, decorators, modifiers) {
            ...
            var importDeclaration = createNode(239, fullStart);
        ...
        function parseImportEqualsDeclaration(fullStart, decorators, modifiers, identifier) {
            var importEqualsDeclaration = createNode(238, fullStart);
        ...
        function parseExportDeclaration(fullStart, decorators, modifiers) {
            var node = createNode(245, fullStart);
            node.decorators = decorators;
            node.modifiers = modifiers;
            if (parseOptional(39)) {
                parseExpected(141);
        ...
        function setExternalModuleIndicator(sourceFile) {
            sourceFile.externalModuleIndicator = ts.forEach(sourceFile.statements, function (node) {
                return ts.hasModifier(node, 1)
                    || node.kind === 238 && node.moduleReference.kind === 249
                    || node.kind === 239
                    || node.kind === 244
                    || node.kind === 245
                    ? node
        ...
        (function (JSDocParser) {
            ...
            function parseJSDocTypeExpression(requireBraces) {
                var result = createNode(268, scanner.getTokenPos());
            ...
            function parseJSDocCommentWorker(start, length) {
                ...
                function createJSDocComment() {
                    var result = createNode(276, start);
                ...
                function parseTag(indent) {
                    ...
                    var atToken = createNode(58, scanner.getTokenPos());
                ...
                function parseNestedTypeLiteral(typeExpression, name) {
                    if (typeExpression && isObjectOrObjectArrayTypeReference(typeExpression.type)) {
                        var typeLiteralExpression = createNode(268, scanner.getTokenPos());
                        var child = void 0;
                ...
                function parseTypedefTag(atToken, tagName) {
                    ...
                    if (typedefTag.fullName) {
                        ...
                        while (true) {
                            if (rightNode.kind === 72 || !rightNode.body) {
                                typedefTag.name = rightNode.kind === 72 ? rightNode : rightNode.name;
                                break;
                            }
                ...
                function tryParseChildTag(target) {
                    ts.Debug.assert(token() === 58);
                    var atToken = createNode(58, scanner.getStartPos());
                    atToken.end = scanner.getTextPos();
                    nextJSDocToken();
                    var tagName = parseJSDocIdentifierName();
                    skipWhitespace();
                ...
                function parseTemplateTag(atToken, tagName) {
                    ...
                    while (true) {
                        ...
                        var typeParameter = createNode(146, name.pos);
...
(function (ts) {
    ...
    ts.optionDeclarations = [
        ...
        },
        {
            ...
            type: "boolean",
            category: ts.Diagnostics.Experimental_Options,
            description: ts.Diagnostics.Enables_experimental_support_for_the_stage_1_ECMAScript_pipeline_operator
        },
        {
            name: "jsxFactory",
            type: "string",
