...
export class API<FromLSP extends boolean = false> implements FormatDiagnosticsHost {
    ...
    get batchRequests(): {
        <const Requests extends readonly APIRequest[]>(requests: Requests): { responses: APIResponseTuple<Requests>; };
        gen<const Requests extends readonly APIRequest[]>(requests: Requests): Generator<ProtocolRequest, { responses: APIResponseTuple<Requests>; }, ProtocolResponse["result"]>;
    } {
        const owner = this;
        return cacheGeneratorMethod(
            owner,
            "batchRequests",
            function <const Requests extends readonly APIRequest[]>(requests: Requests): { responses: APIResponseTuple<Requests>; } {
                const response = owner.client.apiRequest("batchRequests", { requests });
            ...
            function* <const Requests extends readonly APIRequest[]>(requests: Requests): Generator<ProtocolRequest, { responses: APIResponseTuple<Requests>; }, ProtocolResponse["result"]> {
                const response = yield* apiRequest("batchRequests", { requests });
                // we're replacing the `unknown`s in the autogenerated types with much more specific per-request types here, which creates some variance issues for the
                // `batchRequests` method within the batch request object itself, so we cast.
                return response as { responses: APIResponseTuple<Requests>; };
            },
