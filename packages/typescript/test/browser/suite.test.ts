...
const fileExclusions = [
    ...
    {
        mode: "sync",
        file: "sync/shutdown.test.ts",
        tests: 1,
        reason: "This test launches a Node.js child process.",
    },
] as const;
                    ...
                    contents: `
                        ...
                        try {
                            ...
                            globalThis.browserTestResults = await runRegisteredTests([
                                {
                                    pattern: /benchmarks$/i,
                                    reason: "Benchmarks require Node.js process and filesystem APIs.",
                                },
                                {
                                    pattern: /updateSnapshot file systems > (?:full file system is total and does not invoke host callbacks|full file system accepts paths decoded from VS Code document URIs|file system layer bypasses callbacks on hits and falls back on misses)$/,
                                    reason: "The browser WASM adapter mirrors callback files into reactor memory instead of exposing live host callback layering.",
                                },
                                {
...
function browserTestPlugin(mode: "async" | "sync"): Plugin {
    ...
    return {
        ...
        setup(build) {
            ...
            build.onResolve({ filter: /^node:assert$/ }, () => ({ path: "node-assert", namespace: "browser-shim" }));
            build.onResolve({ filter: /^\.\/ast\.bench\.ts$/ }, () => ({ path: "api-bench", namespace: "browser-shim" }));
            build.onResolve({ filter: /^node:fs$/ }, () => ({ path: "node-fs", namespace: "browser-shim" }));
            build.onResolve({ filter: /^node:module$/ }, () => ({ path: "node-module", namespace: "browser-shim" }));
            ...
            build.onResolve({ filter: /^(?:\.\.\/)+generators\/api\.bench\.ts$/ }, () => ({ path: "api-bench", namespace: "browser-shim" }));
            build.onLoad({ filter: /.*/, namespace: "browser-shim" }, args => {
                switch (args.path) {
                    ...
                    case "node-fs":
                        return {
                            contents: `
                                export function globSync() { throw new Error("globSync is not available in a browser"); }
                                export default {
                                    existsSync() { return false; },
                                    readFileSync() { throw new Error("readFileSync is not available in a browser"); },
                                };
                            `,
                        };
                    case "node-module":
