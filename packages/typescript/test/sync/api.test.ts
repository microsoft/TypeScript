...
import "missing";`,
    ...
    test("createProgram resolves raw compiler option paths", () => {
        ...
        const program = api.createProgram(["/src/index.ts", "/src/component.vue"], { compilerOptions: rawOptions });
    ...
    test("createProgram updates an old program with file changes", () => {
        ...
        fs.writeFile!(toRootedFilePath(fileName, undefined), `export const value: string = "valid";`);
...
describe("Snapshot", () => {
    test("updateSnapshot returns snapshot with projects", () => {
        ...
        assert.ok(snapshot.getProject({ uri: "file:///tsconfig.json" }));
    });
...
describe("SourceFile", () => {
    test("relative and absolute identifiers share the project source file cache", () => {
        ...
        try {
            const snapshot = api.updateSnapshot({ openProject: "/outside/tsconfig.json" });
            const program = snapshot.getProject("/outside/tsconfig.json")!.program;
...
describe("updateSnapshot file systems", () => {
    ...
    test("full file system can link node_modules from the host", () => {
        ...
        assert.ok(
            sourceFileNames.includes(toRootedFilePath("/host/node_modules/pkg/index.d.ts", undefined)),
            JSON.stringify({ sourceFileNames, readFileCalls, directoryExistsCalls, fileExistsCalls }),
        );
        assert.equal(
            (project.program.getSourceFile(toRootedFilePath("/host/node_modules/pkg/index.d.ts", undefined)))?.text,
