...
declare module "augmentation" {}`,
    ...
    for (const resolverKind of ["native", "callback", "delegating callback", "static"] as const) {
        test(`Program resolved modules preserve the import phase with ${resolverKind} resolution`, () => {
            using api = spawnAPI({
...
import.source("pkg");`,
            ...
            const moduleResolver = resolverKind === "native" ? fallback : api.createModuleResolver(compilerOptions, {
                ...
                resolveModuleName: (name, directory, mode, options) => {
                    ...
                    return (fallback.resolveModuleName(name, directory, mode, options)).resolvedModule;
                },
            });
            const program = api.createProgram(
            ...
            for (const context of [undefined, snapshot]) {
                for (const importPhase of [ImportPhase.Source, ImportPhase.Evaluation, undefined]) {
                    const resolution = moduleResolver.resolveModuleName("pkg", "/src", ModuleKind.ESNext, { snapshot: context, importPhase });
                    assert.equal(
