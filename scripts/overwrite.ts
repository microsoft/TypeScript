...
export function overwriteOptional(data: string): string {
    const sourceFile = ts.createSourceFile("input.d.ts", data, ts.ScriptTarget.Latest, /*setParentNodes*/ true);
    const fixes: {insertAt: number, remove: [number, number]}[] = [];
        ...
        if (isTraversable(node)) {
            ...
            if (trivia.indexOf("@optional") >= 0 && hasName(node)) {
                fixes.push({
                  ...
                  remove: [node.getStart(sourceFile, /* includeJsDocComment */ true), node.getStart()],
                });
            }
