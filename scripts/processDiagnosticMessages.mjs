...
function buildInfoFileOutput(messageTable, inputFilePathRel) {
    ...
    messageTable.forEach(({ code, category, reportsUnnecessary, elidedInCompatabilityPyramid, reportsDeprecated }, name) => {
        ...
        // TODO(jakebailey): /* @__PURE__ */ should work here, but doesn't.
        result.push(`/** @internal */ export const ${propName} = diag(${code}, DiagnosticCategory.${category}, "${createKey(propName, code)}", ${JSON.stringify(name)}${argReportsUnnecessary}${argElidedInCompatabilityPyramid}${argReportsDeprecated});`);
    });
