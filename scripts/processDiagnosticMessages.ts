...
function buildInfoFileOutput(messageTable: InputDiagnosticMessageTable, nameMap: IIndexable<string>): string {
    ...
    for (var i = 0; i < names.length; i++) {
        ...
        if (diagnosticDetails.code in seen) {
            var sys = ts.sys;
            sys.write('Duplicate diagnostic code ' + diagnosticDetails.code + ' for element "' + name + "'." + sys.newLine);
            sys.write('Previously defined by "' + seen[diagnosticDetails.code] + '".' + sys.newLine);
        }
        seen[diagnosticDetails.code] = name;
        result +=
