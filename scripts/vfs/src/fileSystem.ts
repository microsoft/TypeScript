...
import { Inode, FileInode, isFileInode, DirectoryInode, isDirectoryInode, SymlinkInode, isSymlinkInode } from "./inode";
import { FileSet, File, Directory, Link, Symlink, Mount } from "./fileSet";
import { Stats } from "./stats";
import { FSWatcher, FSWatcherEntry, FSWatcherEntrySet, WatchedFile } from "./watcher";
...
export class FileSystem {
    ...
    private _lazy: {
        ...
        meta?: core.Metadata;
        watchers?: core.SortedMap<string, FSWatcherEntrySet>;
    } = {};
    ...
    public watch(path: string, options?: { recursive?: boolean } | typeof callback, callback?: (eventType: string, filename: string) => void) {
        path = this._resolve(path);
        if (typeof options === "function") callback = options, options = undefined;
    ...
    private _removeWatchers(parent: DirectoryInode | undefined, node: Inode, name: string) {
    ...
    private _notifyChild(parent: DirectoryInode, eventType: "change" | "rename", name: string): void {
    ...
    private _notify(node: Inode, eventType: "change" | "rename", childPath: string | undefined, noExactMatch: boolean) {
        if (typeof childPath === "boolean") noExactMatch = childPath, childPath = undefined;
        if (!this._lazy.watchers) return;
    ...
    private _copyShadowLinks(source: ReadonlyMap<string, Inode>, target: core.SortedMap<string, Inode>, parent: DirectoryInode | undefined) {
        const shadows = this._lazy.shadows || (this._lazy.shadows = new Map<number, Inode>());
    ...
    private _file(syscall: string, fd: number, kind?: "file" | "directory", mode: number = constants.F_OK) {
