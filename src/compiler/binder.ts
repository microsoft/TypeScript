    ...
    function createBinder(): (file: SourceFile, options: CompilerOptions) => void {
        ...
        function bindContainer(node: Mutable<Node>, containerFlags: ContainerFlags) {
            ...
            if (containerFlags & ContainerFlags.IsControlFlowContainer) {
                ...
                const isIIFELike = containerFlags & ContainerFlags.IsFunctionExpression && !hasSyntacticModifier(node, ModifierFlags.Async) &&
                    !(node as FunctionLikeDeclaration).asteriskToken && !!getImmediatelyInvokedFunctionExpression(node) || node.kind === SyntaxKind.ClassStaticBlockDeclaration;
