...
export interface ReusableBuilderProgramState extends ReusableBuilderState {
     ...
     */
    semanticDiagnosticsPerFile?: ts.ReadonlyMap<readonly ReusableDiagnostic[] | readonly Diagnostic[]> | undefined;
    /**
...
export interface BuilderProgramState extends BuilderState {
     ...
     * Cache of bind and check diagnostics for files with their Path being the key
     */
    semanticDiagnosticsPerFile: ts.Map<readonly Diagnostic[]> | undefined;
    /**
     * The map has key by source file's path that has been changed
     */
...
function hasSameKeys<T, U>(map1: ts.ReadonlyMap<T> | undefined, map2: ts.ReadonlyMap<U> | undefined): boolean {
    ...
    return (map1 as ts.ReadonlyMap<T | U>) === map2 || map1 !== undefined && map2 !== undefined && map1.size === map2.size && !forEachKey(map1, key => !map2.has(key));
}
/**
    ...
    return diagnostics.map(diagnostic => {
        ...
        result.relatedInformation = relatedInformation ?
            relatedInformation.length ?
                relatedInformation.map(r => convertToDiagnosticRelatedInformation(r, newProgram, toPath)) : emptyArray :
            undefined;
        return result;
    });
...
function forEachReferencingModulesOfExportOfAffectedFile(state: BuilderProgramState, affectedFile: SourceFile, fn: (state: BuilderProgramState, filePath: Path) => boolean) {
    ...
    if (forEachEntry(state.currentAffectedFilesExportedModulesMap, (exportedModules, exportedFromPath) => exportedModules &&
    ...
    forEachEntry(state.exportedModulesMap, (exportedModules, exportedFromPath) => !state.currentAffectedFilesExportedModulesMap!.has(exportedFromPath) && // If we already iterated this through cache, ignore it
        exportedModules.has(affectedFile.resolvedPath) &&
        forEachFilesReferencingPath(state, (exportedFromPath as Path), seenFileAndExportsOfFile, fn));
}
/**
 ...
 */
/* @internal */
function forEachFilesReferencingPath(state: BuilderProgramState, referencedPath: Path, seenFileAndExportsOfFile: ts.Map<true>, fn: (state: BuilderProgramState, filePath: Path) => boolean) {
 ...
 */
/* @internal */
function forEachFileAndExportsOfFile(state: BuilderProgramState, filePath: Path, seenFileAndExportsOfFile: ts.Map<true>, fn: (state: BuilderProgramState, filePath: Path) => boolean): boolean {
    ...
    // If exported from path is not from cache and exported modules has path, all files referencing file exported from are affected
    if (forEachEntry((state.exportedModulesMap!), (exportedModules, exportedFromPath) => !state.currentAffectedFilesExportedModulesMap!.has(exportedFromPath) && // If we already iterated this through cache, ignore it
        exportedModules.has(filePath) &&
        forEachFileAndExportsOfFile(state, (exportedFromPath as Path), seenFileAndExportsOfFile, fn))) {
        return true;
    }
    // Remove diagnostics of files that import this file (without going to exports of referencing files)
    return !!forEachEntry((state.referencedMap!), (referencesInFile, referencingFilePath) => referencesInFile.has(filePath) &&
    ...
    if (state.exportedModulesMap) {
        ...
        state.exportedModulesMap.forEach((value, key) => {
            ...
            if (newValue === undefined)
                exportedModulesMap[relativeToBuildInfo(key)] = arrayFrom(value.keys(), relativeToBuildInfo);
            // Value in cache and has updated value map, use that
            else if (newValue)
                exportedModulesMap[relativeToBuildInfo(key)] = arrayFrom(newValue.keys(), relativeToBuildInfo);
        });
...
function convertToReusableDiagnostics(diagnostics: readonly Diagnostic[], relativeToBuildInfo: (path: string) => string): readonly ReusableDiagnostic[] {
    ...
    return diagnostics.map(diagnostic => {
        ...
        result.relatedInformation = relatedInformation ?
            relatedInformation.length ?
                relatedInformation.map(r => convertToReusableDiagnosticRelatedInformation(r, relativeToBuildInfo)) : emptyArray :
            undefined;
        return result;
    });
}
...
function addToAffectedFilesPendingEmit(state: BuilderProgramState, affectedFilePendingEmit: Path, kind: BuilderFileEmit) {
    ...
    if (!state.affectedFilesPendingEmitKind)
        state.affectedFilesPendingEmitKind = createMap();
...
/* @internal */
function getMapOfReferencedSet(mapLike: MapLike<readonly string[]> | undefined, toPath: (path: string) => Path): ts.ReadonlyMap<BuilderState.ReferencedSet> | undefined {
    if (!mapLike)
        return undefined;
