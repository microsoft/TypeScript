    ...
    export function createTypeChecker(host: TypeCheckerHost, produceDiagnostics: boolean): TypeChecker {
            ...
            forEach(node.clauses, clause => {
                ...
                if (clause.kind === SyntaxKind.DefaultClause && !hasDuplicateDefaultClause) {
                    if (firstDefaultClause === undefined) {
                        ...
                        var end = clause.statements.length > 0 ? clause.statements[0].start : spanEnd(clause);
        ...
        function checkTryStatement(node: TryStatement) {
            ...
            if (catchClause) {
                ...
                if (catchClause.type) {
                    var sourceFile = getSourceFileOfNode(node);
                    var colonStart = skipTrivia(sourceFile.text, spanEnd(catchClause.name));
            ...
            if (produceDiagnostics) {
                ...
                if (symbol.flags & SymbolFlags.ValueModule
                    ...
                    if (classOrFunc) {
                        ...
                        else if (node.start < classOrFunc.start) {
        ...
        function checkGrammarTypeParameterList(node: FunctionLikeDeclaration, typeParameters: NodeArray<TypeParameterDeclaration>): boolean {
            ...
            if (typeParameters && typeParameters.length === 0) {
                var start = typeParameters.start - "<".length;
                var sourceFile = getSourceFileOfNode(node);
        ...
        function checkGrammarForAtLeastOneTypeArgument(node: Node, typeArguments: NodeArray<TypeNode>): boolean {
            if (typeArguments && typeArguments.length === 0) {
                var sourceFile = getSourceFileOfNode(node);
                var start = typeArguments.start - "<".length;
        ...
        function checkGrammarVariableDeclaration(node: VariableDeclaration) {
            if (isInAmbientContext(node)) {
                ...
                if (node.initializer) {
                    // Error on equals token which immediate precedes the initializer
                    return grammarErrorAtPos(getSourceFileOfNode(node), node.initializer.start - 1, 1, Diagnostics.Initializers_are_not_allowed_in_ambient_contexts);
                }
            }
            else {
                if (!node.initializer) {
        ...
        function grammarErrorOnFirstToken(node: Node, message: DiagnosticMessage, arg0?: any, arg1?: any, arg2?: any): boolean {
            ...
            if (!hasParseDiagnostics(sourceFile)) {
                ...
                var start = scanToken(scanner, node.start);
        ...
        function grammarErrorOnNode(node: Node, message: DiagnosticMessage, arg0?: any, arg1?: any, arg2?: any): boolean {
            ...
            if (!hasParseDiagnostics(sourceFile)) {
                ...
                var start = spanEnd(span) > span.start ? skipTrivia(sourceFile.text, span.start) : span.start;
                diagnostics.push(createFileDiagnostic(sourceFile, start, spanEnd(span) - start, message, arg0, arg1, arg2));
                return true;
            }
        }
