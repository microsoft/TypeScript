...
namespace ts {
    ...
    export function continueWith<T, U, V = never>(value: T | PromiseLike<T>, continuation: (value: T) => U | PromiseLike<U>, onError?: (reason: any) => V | PromiseLike<V>): U | V | PromiseLike<U | V> {
        const then = typeof value === "object" && value !== null && (value as PromiseLike<T>).then;
        if (typeof then === "function") {
            return then(continuation, onError);
        }
        return continuation(value as T);
    }

    export function runWithCancellationToken<T>(func: () => T, onCanceled: () => void): T {
        try {
            return func();
        }
        catch (e) {
    ...
    export async function runWithCancellationTokenAsync<T>(func: () => T | PromiseLike<T>, onCanceled: () => void): Promise<T> {
        try {
            return await func();
        }
        catch (e) {
