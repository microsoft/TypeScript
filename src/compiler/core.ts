import { MapLike, Debug, EqualityComparer, Comparer, SortedReadonlyArray, Comparison, SortedArray, Push, UnderscoreEscapedMap, __String, TextSpan } from "./ts";
...
/* @internal */
export function createMapFromTemplate<T>(template: MapLike<T>): ts.Map<T> {
...
/* @internal */
export function mapMap<T, U>(map: UnderscoreEscapedMap<T>, f: (t: T, key: __String) => [string, U]): ts.Map<U>;
/* @internal */
export function mapMap<T, U>(map: ts.Map<T> | UnderscoreEscapedMap<T>, f: ((t: T, key: string) => [string, U]) | ((t: T, key: __String) => [string, U])): ts.Map<U> {
    const result = createMap<U>();
