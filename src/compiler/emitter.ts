namespace ts {
    ...
    export function createPrinter(printerOptions: PrinterOptions = {}, handlers: PrintHandlers = {}): Printer {
        ...
        function emitVariableDeclaration(node: VariableDeclaration) {
            ...
            emit(node.exclamationToken);
            emitTypeAnnotation(node.type);
            emitInitializer(node.initializer, node.type?.end ?? node.name.extra?.emitExtraFields?.typeNode?.end ?? node.name.end, node, parenthesizer.parenthesizeExpressionForDisallowedComma);
        }
        ...
        function generateName(name: GeneratedIdentifier | GeneratedPrivateIdentifier) {
            ...
            if ((idFields.autoGenerateFlags & GeneratedIdentifierFlags.KindMask) === GeneratedIdentifierFlags.Node) {
                // Node names generate unique names based on their original node
                // and are cached based on that node's id.
                return generateNameCached(getNodeForGeneratedName(name), isPrivateIdentifier(name), idFields.autoGenerateFlags, idFields.autoGeneratePrefix, idFields.autoGenerateSuffix);
            }
            else {
        ...
        function isUniqueLocalName(name: string, container: Node): boolean {
            for (let node: Node | undefined = container; node && isNodeDescendantOf(node, container); node = getBindExtraFields(node)?.nextContainer) {
                const local = getBindExtraFields(node)?.locals?.get(escapeLeadingUnderscores(name));
        ...
        function makeName(name: GeneratedIdentifier | GeneratedPrivateIdentifier) {
            const idFields = getIdentifierExtraFields(name);
            const prefix = formatGeneratedNamePart(idFields.autoGeneratePrefix, generateName);
            const suffix = formatGeneratedNamePart(idFields.autoGenerateSuffix);
            switch (idFields.autoGenerateFlags & GeneratedIdentifierFlags.KindMask) {
                case GeneratedIdentifierFlags.Auto:
                    return makeTempVariableName(TempFlags.Auto, !!(idFields.autoGenerateFlags & GeneratedIdentifierFlags.ReservedInNestedScopes), isPrivateIdentifier(name), prefix, suffix);
                case GeneratedIdentifierFlags.Loop:
                    Debug.assertNode(name, isIdentifier);
                    return makeTempVariableName(TempFlags._i, !!(idFields.autoGenerateFlags & GeneratedIdentifierFlags.ReservedInNestedScopes), /*privateName*/ false, prefix, suffix);
                case GeneratedIdentifierFlags.Unique:
                    return makeUniqueName(
                        idText(name),
                        (idFields.autoGenerateFlags & GeneratedIdentifierFlags.FileLevel) ? isFileLevelUniqueName : isUniqueName,
                        !!(idFields.autoGenerateFlags & GeneratedIdentifierFlags.Optimistic),
                        !!(idFields.autoGenerateFlags & GeneratedIdentifierFlags.ReservedInNestedScopes),
            ...
            return Debug.fail(`Unsupported GeneratedIdentifierKind: ${Debug.formatEnum(idFields.autoGenerateFlags & GeneratedIdentifierFlags.KindMask, (ts as any).GeneratedIdentifierFlags, /*isFlags*/ true)}.`);
        }
