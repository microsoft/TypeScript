...
export function getOutputPathsFor(sourceFile: SourceFile | Bundle, host: EmitHost, forceDtsPaths: boolean): EmitFileNames {
    ...
    if (sourceFile.kind === SyntaxKind.Bundle) {
        ...
        const isJsonEmittedToSameLocation = isJsonFile &&
            comparePaths(sourceFile.fileName, ownOutputFilePath, host.getCurrentDirectory(), !host.useCaseSensitiveFileNames()) ===
                Comparison.EqualTo;
...
/** @internal */
export function getOutputDeclarationFileNameWorker(
    inputFileName: string,
...
export function getCommonSourceDirectoryOfConfig({ options, fileNames }: ParsedCommandLine, ignoreCase: boolean): string {
    return getCommonSourceDirectory(
        ...
        () =>
            filter(
                fileNames,
                file => !(options.noEmitForJsFiles && fileExtensionIsOneOf(file, supportedJSExtensionsFlat)) && !isDeclarationFileName(file),
            ),
...
export function emitFiles(
    ...
    function emitBuildInfo(buildInfoPath: string | undefined) {
        ...
        writeFile(host, emitterDiagnostics, buildInfoPath, getBuildInfoText(buildInfo), /*writeByteOrderMark*/ false, /*sourceFiles*/ undefined, {
            buildInfo,
        });
...
export function createPrinter(printerOptions: PrinterOptions = {}, handlers: PrintHandlers = {}): Printer {
            ...
            switch (node.kind) {
                ...
                case SyntaxKind.JSDocSatisfiesTag:
                    return emitJSDocSimpleTypedTag(
                        node as JSDocTypeTag | JSDocReturnTag | JSDocThisTag | JSDocTypeTag | JSDocThrowsTag | JSDocSatisfiesTag,
                    );
    ...
    function emitParameter(node: ParameterDeclaration) {
        ...
        emitInitializer(
            ...
            node.type ? node.type.end
                : node.questionToken ? node.questionToken.end : node.name ? node.name.end : node.modifiers ? node.modifiers.end : node.pos,
            node,
    ...
    function emitObjectLiteralExpression(node: ObjectLiteralExpression) {
        ...
        const allowTrailingComma =
            currentSourceFile && currentSourceFile.languageVersion >= ScriptTarget.ES5 && !isJsonSourceFile(currentSourceFile) ?
    ...
    function emitPropertyAccessExpression(node: PropertyAccessExpression) {
        ...
        const token = node.questionDotToken ||
            setTextRangePosEnd(factory.createToken(SyntaxKind.DotToken) as DotToken, node.expression.end, node.name.pos);
    ...
    function emitBlockStatements(node: BlockLike, forceSingleLine: boolean) {
        ...
        const format = forceSingleLine || getEmitFlags(node) & EmitFlags.SingleLine ? ListFormat.SingleLineBlockStatements
    ...
    function emitJsxExpression(node: JsxExpression) {
        if (node.expression || (!commentsDisabled && !nodeIsSynthesized(node) && hasCommentsAtPosition(node.pos))) { // preserve empty expressions if they contain comments!
            const isMultiline = currentSourceFile && !nodeIsSynthesized(node) &&
                getLineAndCharacterOfPosition(currentSourceFile, node.pos).line !== getLineAndCharacterOfPosition(currentSourceFile, node.end).line;
            if (isMultiline) {
    ...
    function emitNodeList<Child extends Node, Children extends NodeArray<Child>>(
        ...
        if (isEmpty) {
            ...
            if (
                ...
                !(preserveSourceNewlines && (!parentNode || currentSourceFile && rangeIsOnSingleLine(parentNode, currentSourceFile)))
            ) {
                writeLine();
            }
    ...
    function emitNodeListItems<Child extends Node>(
                ...
                if (separatingLineTerminatorCount > 0) {
                    ...
                    if (shouldEmitInterveningComments && format & ListFormat.DelimitersMask && !positionIsSynthesized(child.pos)) {
                        ...
                        emitTrailingCommentsOfPosition(
                            commentRange.pos,
                            /*prefixSpace*/ !!(format & ListFormat.SpaceBetweenSiblings),
                            /*forceNoNewline*/ true,
                        );
                    }
        ...
        //       ];
        if (
            previousSibling && (parentNode ? parentNode.end : -1) !== previousSibling.end && (format & ListFormat.DelimitersMask) &&
    ...
    function generateNameForModuleOrEnum(node: ModuleDeclaration | EnumDeclaration) {
        ...
        return isUniqueLocalName(name, tryCast(node, canHaveLocals)) ? name
            : makeUniqueName(name, isUniqueName, /*optimistic*/ false, /*scoped*/ false, /*privateName*/ false, /*prefix*/ "", /*suffix*/ "");
    }
    ...
    function emitTrailingCommentsOfPosition(pos: number, prefixSpace?: boolean, forceNoNewline?: boolean) {
        ...
        forEachTrailingCommentToEmit(
            pos,
            prefixSpace ? emitTrailingComment : forceNoNewline ? emitTrailingCommentOfPositionNoNewline : emitTrailingCommentOfPosition,
        );
        exitComment();
    }
