...
module ts {
    ...
    export function emitFiles(resolver: EmitResolver, host: EmitHost, targetSourceFile: SourceFile): EmitResult {
        ...
        function emitJavaScript(jsFilePath: string, root?: SourceFile) {
            ...
            let computedPropertyNameToGeneratedName: string[];
            ...
            function emitExpressionForPropertyName(node: DeclarationName) {
                ...
                if (node.kind === SyntaxKind.StringLiteral) {
                    ...
                    if (nodeIsDecorated(node.parent)) {
                        if (!computedPropertyNameToGeneratedName) {
                            computedPropertyNameToGeneratedName = [];
                        }

                        let generatedName = computedPropertyNameToGeneratedName[node.id];
                        if (generatedName) {
                        ...
                        computedPropertyNameToGeneratedName[node.id] = generatedName;
                        write(generatedName);
                        write(" = ");
                    }
            ...
            function emitClassDeclarationBelowES6(node: ClassDeclaration) {
                ...
                let saveTempVariables = tempVariables;
                let saveTempParameters = tempParameters;
                let saveGeneratedComputedPropertyNames = computedPropertyNameToGeneratedName;
                tempCount = 0;
                tempVariables = undefined;
                tempParameters = undefined;
                computedPropertyNameToGeneratedName = undefined;
                increaseIndent();
                scopeEmitStart(node);
                if (baseTypeNode) {
                    writeLine();
                ...
                tempVariables = saveTempVariables;
                tempParameters = saveTempParameters;
                computedPropertyNameToGeneratedName = saveGeneratedComputedPropertyNames;
                decreaseIndent();
                writeLine();
                emitToken(SyntaxKind.CloseBraceToken, node.members.end);
                scopeEmitEnd();
