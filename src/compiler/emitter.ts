...
namespace ts {
    ...
    export function createPrinter(printerOptions: PrinterOptions = {}, handlers: PrintHandlers = {}): Printer {
        const {
            ...
            onEmitSourceMapOfPosition: emitPos,
            onEmitNode,
            onEmitHelpers,
            onSetSourceFile,
            substituteNode,
        ...
        function emitIdentifierName(node: Identifier) {
            pipelineEmit(EmitHint.IdentifierName, node);
        }
        ...
        function pipelineEmit(hint: EmitHint, node: Node) {
            getInitialPhase(hint)(hint, trySubstituteNode(hint, node));
        }

        function getInitialPhase(hint: EmitHint) {
            return !shouldSkipNotificationPhase() ? pipelineEmitWithNotification : getNextPhaseFollowingNotification(hint);
        }

        function shouldSkipNotificationPhase() {
            return onEmitNode === undefined;
        }

        function getNextPhaseFollowingNotification(hint: EmitHint) {
            return !shouldSkipCommentPhase(hint) ? emitNodeConsideringComments : getNextPhaseFollowingComments(hint);
        }

        function shouldSkipCommentPhase(hint: EmitHint) {
            return disabled || hint === EmitHint.SourceFile;
        }

        function getNextPhaseFollowingComments(hint: EmitHint) {
            return !shouldSkipSourceMapPhase(hint) ? pipelineEmitWithSourceMap : getNextPhaseFollowingSourceMap();
        }

        function shouldSkipSourceMapPhase(hint: EmitHint) {
            return onEmitSourceMapOfNode === undefined
                || hint === EmitHint.SourceFile
                || hint === EmitHint.IdentifierName;
        }

        function getNextPhaseFollowingSourceMap() {
            return pipelineEmitWithHint;
        }

        function pipelineEmitWithNotification(hint: EmitHint, node: Node) {
            onEmitNode(hint, node, getNextPhaseFollowingNotification(hint));
        }

        function pipelineEmitWithSourceMap(hint: EmitHint, node: Node) {
            onEmitSourceMapOfNode(hint, node, getNextPhaseFollowingSourceMap());
        }
        ...
        function emitBlockFunctionBody(body: Block) {
            ...
            const commentFlags = getEmitFlags(body);
            emitLeadingDetachedComments(body.statements, commentFlags);

            if (!disabled && (commentFlags & EmitFlags.NoNestedComments)) {
                disabled = true;
            ...
            emitTrailingDetachedComments(body.statements, commentFlags);

            decreaseIndent();
            writeToken(SyntaxKind.CloseBraceToken, body.statements.end, body);
        }

        function emitBlockFunctionBodyWorker(body: Block) {
            // Emit all the prologue directives (like "use strict").
            const pos = writer.getTextPos();
            const statementOffset = emitPrologueDirectives(body.statements, /*startWithNewLine*/ true);
            emitHelpersIndirect(body);
            if (statementOffset === 0 && pos === writer.getTextPos() && shouldEmitBlockFunctionBodyOnSingleLine(body)) {
                decreaseIndent();
        ...
        function emitSourceFile(node: SourceFile) {
            ...
            const commentFlags = getEmitFlags(node);
            emitLeadingDetachedComments(node.statements, commentFlags);

            const statementOffset = emitPrologueDirectives(node.statements);
            pushNameGenerationScope();
            emitHelpersIndirect(node);
            emitList(node, node.statements, ListFormat.MultiLine, statementOffset);
            popNameGenerationScope();

            emitTrailingDetachedComments(node.statements, commentFlags);
        }
        ...
        function needsIndentation(parent: Node, node1: Node, node2: Node): boolean {
            parent = skipOuterExpressions(parent, OuterExpressionKinds.SynthesizedParentheses);
            node1 = skipOuterExpressions(node1, OuterExpressionKinds.SynthesizedParentheses);
            node2 = skipOuterExpressions(node2, OuterExpressionKinds.SynthesizedParentheses);
        ...
        function emitNodeConsideringComments(hint: EmitHint, node: Node) {
            if (node === undefined) {
                return;
            }

            const commentRange = getCommentRange(node);
            const commentFlags = shouldEmitLeadingOrTrailingComments(node, commentRange);
            if ((commentFlags & EmitFlags.NoComments) === EmitFlags.NoComments) {
                // always emit any explicit leading or trailing comments.
                emitLeadingSynthesizedCommentsOfNode(node);
                emitNodeConsideringNestedComments(hint, node, commentFlags, commentRange);
                emitTrailingSynthesizedCommentsOfNode(node);
            }
            else {
                // Emit leading comments if the position is not synthesized and the node
                // has not opted out from emitting leading comments.
                if ((commentFlags & EmitFlags.NoLeadingComments) === 0) {
                    emitLeadingComments(commentRange.pos, !isNotEmittedStatement(node));
                }
                ...
                // has not opted out from emitting trailing comments.
                if ((commentFlags & EmitFlags.NoTrailingComments) === 0 && !isNotEmittedStatement(node)) {
                    emitTrailingComments(commentRange.end);
                }
            }
        }

        function emitLeadingSynthesizedCommentsOfNode(node: Node) {
            if (extendedDiagnostics) performance.mark("emitLeadingSynthesizedComments");
            forEach(getSyntheticLeadingComments(node), emitLeadingSynthesizedComment);
            if (extendedDiagnostics) performance.measure("commentTime", "emitLeadingSynthesizedComments");
        }
        ...
        function emitTrailingSynthesizedCommentsOfNode(node: Node) {
            if (extendedDiagnostics) performance.mark("emitTrailingSynthesizedComments");
            forEach(getSyntheticTrailingComments(node), emitTrailingSynthesizedComment);
            if (extendedDiagnostics) performance.measure("commentTime", "emitTrailingSynthesizedComments");
        }
        ...
        function emitNodeConsideringNestedComments(hint: EmitHint, node: Node, commentFlags: EmitFlags, commentRange: TextRange) {
            if (commentFlags & EmitFlags.NoNestedComments) {
                disabled = true;
                getNextPhaseFollowingComments(hint)(hint, node);
                disabled = false;
            }
            else {
                emitNodeWithNestedComments(hint, node, commentFlags, commentRange);
            }
        }

        function emitNodeWithNestedComments(hint: EmitHint, node: Node, commentFlags: EmitFlags, commentRange: TextRange) {
            ...
            if ((commentFlags & EmitFlags.NoTrailingComments) === 0) {
        ...
        function emitLeadingDetachedComments(detachedRange: TextRange, commentFlags: EmitFlags) {
            if (extendedDiagnostics) performance.mark("emitLeadingDetachedCommentsOfBody");
            if (!positionIsSynthesized(detachedRange.pos) && (commentFlags & EmitFlags.NoLeadingComments) === 0) {
                emitDetachedCommentsAndUpdateCommentsInfo(detachedRange);
            }
            if (extendedDiagnostics) performance.measure("commentTime", "emitLeadingDetachedCommentsOfBody");
        }

        function emitTrailingDetachedComments(detachedRange: TextRange, commentFlags: EmitFlags) {
            if (disabled) {
                return;
            }
            if (extendedDiagnostics) performance.mark("emitTrailingDetachedComments");
            if (!positionIsSynthesized(detachedRange.end) && (commentFlags & EmitFlags.NoTrailingComments) === 0) {
                emitLeadingComments(detachedRange.end, /*isEmittedNode*/ true);
                if (hasWrittenComment && !writer.isAtStartOfLine()) {
                    writer.writeLine();
                }
            }
            if (extendedDiagnostics) performance.measure("commentTime", "emitTrailingDetachedComments");
        }
        ...
        function emitLeadingComments(pos: number, isEmittedNode: boolean) {
            if (extendedDiagnostics) performance.mark("emitLeadingComments");
            ...
            if (extendedDiagnostics) performance.measure("commentTime", "emitLeadingComments");
        }
        ...
        function emitTripleSlashLeadingComment(commentPos: number, commentEnd: number, kind: SyntaxKind, hasTrailingNewLine: boolean, rangePos: number) {
            if (isTripleSlashComment(currentText, commentPos, commentEnd)) {
        ...
        function emitTrailingComments(pos: number) {
            if (extendedDiagnostics) performance.mark("emitTrailingComments");
            forEachTrailingCommentToEmit(pos, emitTrailingComment);
            if (extendedDiagnostics) performance.measure("commentTime", "emitTrailingComments");
        }
        ...
        function emitTrailingCommentsOfPosition(pos: number) {
            ...
            if (extendedDiagnostics) performance.mark("emitTrailingCommentsOfPosition");
            forEachTrailingCommentToEmit(pos, emitTrailingCommentOfPosition);
            if (extendedDiagnostics) performance.measure("commentTime", "emitTrailingCommentsOfPosition");
        }
    ...
    function shouldEmitLeadingOrTrailingComments(node: Node, commentRange: TextRange) {
        let commentFlags = getEmitFlags(node) & (EmitFlags.NoComments | EmitFlags.NoNestedComments);
        if (isCollapsedRange(commentRange)) {
            commentFlags |= EmitFlags.NoComments;
        }
        else {
            if ((commentFlags & EmitFlags.NoLeadingComments) === 0 && positionIsSynthesized(commentRange.pos)) {
                commentFlags |= EmitFlags.NoLeadingComments;
            }
            if ((commentFlags & EmitFlags.NoTrailingComments) === 0 && positionIsSynthesized(commentRange.end)) {
                commentFlags |= EmitFlags.NoTrailingComments;
            }
        }
        return commentFlags;
    }
     ...
     **/
    function isTripleSlashComment(text: string, commentPos: number, commentEnd: number) {
