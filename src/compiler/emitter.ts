...
export function getFirstProjectOutput(configFile: ts.ParsedCommandLine, ignoreCase: boolean): string {
    if (ts.outFile(configFile.options)) {
        ...
        return ts.Debug.checkDefined(jsFilePath || declarationFilePath, `project ${configFile.options.configFilePath} expected to have at least one output`);
    }
...
export function emitFiles(resolver: ts.EmitResolver, host: ts.EmitHost, targetSourceFile: ts.SourceFile | undefined, { scriptTransformers, declarationTransformers }: ts.EmitTransformers, emitOnly?: boolean | ts.EmitOnly, onlyBuildInfo?: boolean, forceDtsEmit?: boolean): ts.EmitResult {
    ...
    function emitDeclarationFileOrBundle(
        ...
        relativeToBuildInfo: (path: string) => string) {
        if (!sourceFileOrBundle || emitOnly === ts.EmitOnly.Js) return;
        if (!declarationFilePath) {
        ...
        if (emitOnly && !ts.getEmitDeclarations(compilerOptions)) {
...
/*@internal*/
export function createBuildInfo(program: ts.ProgramBuildInfo | undefined, bundle: ts.BundleBuildInfo | undefined): ts.BuildInfo {
...
function createSourceFilesFromBundleBuildInfo(bundle: ts.BundleBuildInfo, buildInfoDirectory: string, host: ts.CompilerHost): readonly ts.SourceFile[] {
...
export function emitUsingBuildInfo(
    ...
    ts.tracing?.push(ts.tracing.Phase.Emit, "emitUsingBuildInfo", {}, /*separateBeginAndEnd*/ true);
...
function emitUsingBuildInfoWorker(
    ...
    const ownPrependInput = ts.createInputFilesWithFileTexts(
        jsFilePath,
        jsFileText,
        sourceMapFilePath,
        sourceMapText,
    ...
    const prependNodes = ts.createPrependNodes(config.projectReferences, getCommandLine, f => host.readFile(f), host);
    ...
    const emitHost: ts.EmitHost = {
        ...
        getBuildInfo: bundle => {
            ...
            if (program && changedDtsText !== undefined && config.options.composite) {
                ...
                (program as ts.ProgramBundleEmitBuildInfo).outSignature = ts.computeSignature(changedDtsText, createHash, changedDtsData);
            }
