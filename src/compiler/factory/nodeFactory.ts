...
export function createNodeFactory(flags: NodeFactoryFlags, baseFactory: ts.BaseNodeFactory): ts.NodeFactory {
    ...
    // @api
    function createIdentifier(text: string, typeArguments?: readonly (ts.TypeNode | ts.TypeParameterDeclaration)[], originalKeywordKind?: ts.SyntaxKind): ts.Identifier {
    ...
    if (!ts.isString(javascriptTextOrReadFileText)) {
        ...
        const getAndCacheBuildInfo = (getText: () => string | undefined) => {
            if (buildInfo === undefined) {
                ...
                buildInfo = result !== undefined ? ts.getBuildInfo(node.buildInfoPath!, result) ?? false : false;
            }
        ...
        Object.defineProperties(node, {
            ...
            declarationText: { get() { return definedTextGetter(ts.Debug.checkDefined(javascriptMapTextOrDeclarationPath)); } },
