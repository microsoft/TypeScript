...
export function createModeAwareCache<T>(): ModeAwareCache<T> {
    ...
    const memoizedReverseKeys = new ts.Map<string, [specifier: string, mode: ts.ModuleKind.CommonJS | ts.ModuleKind.ESNext | undefined]>();
...
/* @internal */
export function zipToModeAwareCache<V>(file: ts.SourceFile, keys: readonly string[] | readonly ts.FileReference[], values: readonly V[]): ModeAwareCache<V> {
    ...
    for (let i = 0; i < keys.length; ++i) {
        ...
        const mode = !ts.isString(entry) ? entry.resolutionMode || file.impliedNodeFormat : ts.getModeForResolutionAtIndex(file, i);
    ...
    function loadModuleFromTargetImportOrExport(target: unknown, subpath: string, pattern: boolean): SearchResult<Resolved> | undefined {
        if (typeof target === "string") {
            ...
            if (!Array.isArray(target)) {
                for (const key of ts.getOwnKeys(target as ts.MapLike<unknown>)) {
                    if (key === "default" || state.conditions.indexOf(key) >= 0 || isApplicableVersionedTypesKey(state.conditions, key)) {
                        const subTarget = (target as ts.MapLike<unknown>)[key];
