...
export interface ModuleResolutionCache extends NonRelativeModuleNameResolutionCache {
    getOrCreateCacheForDirectory(directoryName: string, redirectedReference?: ResolvedProjectReference): ts.Map<ResolvedModuleWithFailedLookupLocations>;
    /*@internal*/ directoryToModuleNameMap: CacheWithRedirects<ts.Map<ResolvedModuleWithFailedLookupLocations>>;
}
/**
...
export interface CacheWithRedirects<T> {
    ownMap: ts.Map<T>;
    redirectsMap: ts.Map<ts.Map<T>>;
    getOrCreateMapOfCacheRedirects(redirectedReference: ResolvedProjectReference | undefined): ts.Map<T>;
    clear(): void;
    setOwnOptions(newOptions: CompilerOptions): void;
    setOwnMap(newOwnMap: ts.Map<T>): void;
}
/*@internal*/
export function createCacheWithRedirects<T>(options?: CompilerOptions): CacheWithRedirects<T> {
    let ownMap: ts.Map<T> = createMap();
    const redirectsMap: ts.Map<ts.Map<T>> = createMap();
    return {
        ownMap,
        redirectsMap,
    ...
    function setOwnMap(newOwnMap: ts.Map<T>) {
        ownMap = newOwnMap;
    }
...
/*@internal*/
export function createModuleResolutionCacheWithMaps(directoryToModuleNameMap: CacheWithRedirects<ts.Map<ResolvedModuleWithFailedLookupLocations>>, moduleNameToDirectoryMap: CacheWithRedirects<PerModuleNameCache>, currentDirectory: string, getCanonicalFileName: GetCanonicalFileName): ModuleResolutionCache {
    ...
    function getOrCreateCacheForDirectory(directoryName: string, redirectedReference?: ResolvedProjectReference) {
        ...
        return getOrCreateCache<ts.Map<ResolvedModuleWithFailedLookupLocations>>(directoryToModuleNameMap, redirectedReference, path, createMap);
    }
