namespace ts {
    ...
    export function resolveTypeReferenceDirective(typeReferenceDirectiveName: string, containingFile: string | undefined, options: CompilerOptions, host: ModuleResolutionHost, redirectedReference?: ResolvedProjectReference, cache?: TypeReferenceDirectiveResolutionCache, resolutionMode?: ResolutionMode): ResolvedTypeReferenceDirectiveWithFailedLookupLocations {
        ...
        let result = containingDirectory ? cache?.getFromCache(containingDirectory, typeReferenceDirectiveName, /*mode*/ resolutionMode, redirectedReference) : undefined;
        if (result) {
            if (traceEnabled) {
    ...
    export interface NonRelativeModuleNameResolutionCache extends PackageJsonInfoCache {
        getFromModuleNameCache(directory: string, nonRelativeModuleName: string, mode: ResolutionMode, redirectedReference: ResolvedProjectReference | undefined): ResolvedModuleWithFailedLookupLocations | undefined;
    ...
    /*@internal*/
    export interface CacheWithRedirects<K, T> {
        getOwnMap: () => ESMap<K, T>;
        redirectsKeyToCache: ESMap<RedirectsCacheKey, RedirectsMapAndOptions<K, T>>;
        createMapForCompilerOptions(options: CompilerOptions, optionsKey: RedirectsCacheKey | undefined): ESMap<K, T>;
    ...
    /*@internal*/
    export function createCacheWithRedirects<K, T>(ownOptions: CompilerOptions | undefined): CacheWithRedirects<K, T> {
        ...
        const redirectsMap = new Map<Path, ESMap<K, T>>();
        return {
            getOwnMap,
            redirectsKeyToCache,
            ...
            clear,
            update,
            clearRedirectsMap: () => redirectsMap.clear(),
        };
        ...
        function createMapForCompilerOptions(options: CompilerOptions, optionsKey: RedirectsCacheKey | undefined): ESMap<K, T> {
            if (options === ownOptions) return ownMap ??= new Map();
            return getOrCreateRedirectMapAndKey(options, /*path*/ undefined, optionsKey);
        }

        function getOrCreateRedirectMapAndKey(options: CompilerOptions, path: Path | undefined, optionsKey: RedirectsCacheKey | undefined): ESMap<K, T> {
            if (path || options.configFile) {
                const redirects = redirectsMap.get(path || options.configFile!.path);
                if (redirects) return redirects;
            }

            const key = optionsKey || getRedirectsCacheKey(options);
            ...
            if (path || options.configFile) redirectsMap.set(path || options.configFile!.path, map);
            return map;
        }
        ...
        function update(newOwnOptions: CompilerOptions) {
            ...
            ownKey = key;
            ownOptions = newOwnOptions;
            if (newOwnOptions.configFile) redirectsMap.set(newOwnOptions.configFile.path, ownMap);
        }
        ...
        function getOrCreateMapOfCacheRedirects(redirectedReference: ResolvedProjectReference | undefined) {
            return !redirectedReference || redirectedReference.commandLine.options === ownOptions ?
                getOwnMap() :
                getOrCreateRedirectMapAndKey(redirectedReference.commandLine.options, redirectedReference.sourceFile.path, /*optionsKey*/ undefined);
        }
    ...
    function getOrCreateCache<K, T>(cacheWithRedirects: CacheWithRedirects<K, T>, redirectedReference: ResolvedProjectReference | undefined, key: K, create: () => T): T {
    ...
    function createPerDirectoryResolutionCache<T>(currentDirectory: string, getCanonicalFileName: GetCanonicalFileName, options: CompilerOptions | undefined): PerDirectoryResolutionCache<T> {
        ...
        function getFromCache(directoryName: string, name: string, mode: ResolutionMode, redirectedReference?: ResolvedProjectReference) {
            const path = toPath(directoryName, currentDirectory, getCanonicalFileName);
    ...
    export function createModeAwareCache<T>(): ModeAwareCache<T> {
        ...
        function getUnderlyingCacheKey(specifier: string, mode: ResolutionMode) {
            const result = getModeAwareCacheKey(specifier, mode);
    ...
    export function createModuleResolutionCache(
        ...
        function getFromModuleNameCache(directoryName: string, nonRelativeModuleName: string, mode: ResolutionMode, redirectedReference?: ResolvedProjectReference) {
            Debug.assert(!isExternalModuleNameRelative(nonRelativeModuleName));
            const result = moduleNameToDirectoryMap.getMapOfCacheRedirects(redirectedReference)?.get(getModeAwareCacheKey(nonRelativeModuleName, mode))?.get(directoryName);
            if (result) return result;

            const fromOld = oldResolutionCache?.getResolved(toPath(directoryName), nonRelativeModuleName, mode, redirectedReference);
    ...
    export function resolveModuleNameFromCache(moduleName: string, containingFile: string, cache: ModuleResolutionCache, mode?: ResolutionMode): ResolvedModuleWithFailedLookupLocations | undefined {
        ...
        return cache?.getFromCache(containingDirectory, moduleName, mode);
    }
