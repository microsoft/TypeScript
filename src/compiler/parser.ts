...
module ts {
    ...
    function parseSourceFile(fileName: string, sourceText: string, languageVersion: ScriptTarget, syntaxCursor: SyntaxCursor, setParentNodes = false): SourceFile {
        ...
        function parseIndexSignatureDeclaration(modifiers: ModifiersArray): IndexSignatureDeclaration {
            let fullStart = modifiers ? modifiers.start : scanner.getStartPos();
        ...
        function parseUnionTypeOrHigher(): TypeNode {
            ...
            if (token === SyntaxKind.BarToken) {
                ...
                let node = <UnionTypeNode>createNode(SyntaxKind.UnionType, type.start);
                node.types = types;
                type = finishNode(node);
            }
            return type;
        ...
        function parseImportOrExportSpecifier(kind: SyntaxKind): ImportOrExportSpecifier {
            ...
            if (token === SyntaxKind.AsKeyword) {
                ...
                if (isFirstIdentifierNameNotAnIdentifier) {
                    ...
                    parseErrorAtPosition(start, spanEnd(identifierName) - start, Diagnostics.Identifier_expected);
                }
            }
        ...
        function processReferenceComments(sourceFile: SourceFile): void {
            ...
            while (true) {
                ...
                let range = createSpanFromBounds(triviaScanner.getTokenPos(), triviaScanner.getTextPos());
                ...
                if (referencePathMatchResult) {
                    ...
                    if (diagnosticMessage) {
                        sourceFile.parseDiagnostics.push(createFileDiagnostic(sourceFile, range.start, range.length, diagnosticMessage));
                    }
                }
                else {
                    ...
                    if (amdModuleNameMatchResult) {
                        if (amdModuleName) {
                            sourceFile.parseDiagnostics.push(createFileDiagnostic(sourceFile, range.start, range.length, Diagnostics.An_AMD_module_cannot_have_multiple_name_assignments));
                        }
