...
module ts {
    ...
    export function createSourceFile(filename: string, sourceText: string, languageVersion: ScriptTarget, version: string, isOpen: boolean = false): SourceFile {
        ...
        function tryParseParenthesizedArrowFunctionExpression(): Expression {
            ...
            if (triState === Tristate.True) {
                ...
                if (parseExpected(SyntaxKind.EqualsGreaterThanToken) || token === SyntaxKind.OpenBraceToken) {
                    return parseArrowExpressionTail(pos, sig, isAsync);
                }
                else {
        ...
        function parseArrowExpressionTail(pos: number, sig: ParsedSignature, isAsync?: boolean): FunctionExpression {
            var body: Node;

            if (token === SyntaxKind.OpenBraceToken) {
                body = parseFunctionBlock(/*allowYield:*/ false, /*allowAwait*/ isAsync, /* ignoreMissingOpenBrace */ false);
            }
                ...
                body = parseFunctionBlock(/*allowYield:*/ false, /*allowAwait*/ isAsync, /* ignoreMissingOpenBrace */ true);
            }
            else {
                body = isAsync ?
                    doInAwaitContext(parseAssignmentExpression) :
                    doOutsideOfAwaitContext(parseAssignmentExpression);
            }

            return makeFunctionExpression(SyntaxKind.ArrowFunction, pos, /* name */ undefined, sig, body, isAsync ? NodeFlags.Async : undefined);
        }
        ...
        function parsePropertyAssignment(): Declaration {
            ...
            if (isGenerator || token === SyntaxKind.OpenParenToken || token === SyntaxKind.LessThanToken) {
                ...
                var body = parseFunctionBlock(isGenerator, /* ignoreMissingOpenBrace */ false, isAsync);
                // do not propagate property name as name for function expression
                // for scenarios like 
                // var x = 1;
                // var y = { x() { } } 
                // otherwise this will bring y.x into the scope of x which is incorrect
                (<PropertyDeclaration>node).initializer = makeFunctionExpression(SyntaxKind.FunctionExpression, node.pos, undefined, sig, body, isAsync ? NodeFlags.Async : undefined);
                return finishNode(node);
            }
        ...
        function parseFunctionExpression(isAsync?: boolean): FunctionExpression {
            ...
            var body = parseFunctionBlock(/*allowYield:*/ isGenerator, /* ignoreMissingOpenBrace */ false, isAsync);
            return makeFunctionExpression(SyntaxKind.FunctionExpression, pos, name, sig, body, isGenerator ? NodeFlags.Generator : undefined, isAsync);
        }
        ...
        function makeFunctionExpression(kind: SyntaxKind, pos: number, name: Identifier, sig: ParsedSignature, body: Node, flags?: NodeFlags, isAsync?: boolean): FunctionExpression {
        ...
        function parseFunctionDeclaration(fullStart: number, modifiers: ModifiersArray): FunctionLikeDeclaration {
            ...
            node.body = parseFunctionBlockOrSemicolon(isGenerator, isAsync);
            return finishNode(node);
        }
        ...
        function parsePropertyMemberDeclaration(fullStart: number, modifiers: ModifiersArray): Declaration {
            ...
            var isAsync = (flags & NodeFlags.Async) !== 0;
            var isGenerator = parseOptional(SyntaxKind.AsteriskToken);
            if (isGenerator) {
            ...
            if (isGenerator || token === SyntaxKind.OpenParenToken || token === SyntaxKind.LessThanToken) {
                ...
                method.body = parseFunctionBlockOrSemicolon(isGenerator, isAsync);
                return finishNode(method);
            }
            else {
                var property = <PropertyDeclaration>createNode(SyntaxKind.Property, fullStart);
