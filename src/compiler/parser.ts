...
import {
    ...
    ClassStaticBlockDeclaration, CommaListExpression, commentPragmas, CommentRange,
    ...
    getJSDocCommentRanges, getLanguageVariant, getLeadingCommentRanges, getSpellingSuggestion,
    getTextOfNodeFromSourceText, HasJSDoc, HasModifiers, HeritageClause, Identifier, idText, IfStatement, ImportClause,
    ImportDeclaration, ImportEqualsDeclaration, ImportOrExportSpecifier, ImportSpecifier, ImportTypeAssertionContainer,
    ImportTypeNode, IndexedAccessTypeNode, IndexSignatureDeclaration, InferTypeNode, InterfaceDeclaration,
    IntersectionTypeNode, isArray, isAssignmentOperator, isAsyncModifier, isClassMemberModifier, isExportAssignment,
    isExportDeclaration, isExportModifier, isExpressionWithTypeArguments, isExternalModuleReference, isFunctionTypeNode,
    isIdentifierText, isImportDeclaration, isImportEqualsDeclaration, isJSDocFunctionType, isJSDocNullableType,
    isJSDocReturnTag, isJSDocTypeTag, isJsxOpeningElement, isJsxOpeningFragment, isKeyword, isLeftHandSideExpression,
    isLiteralKind, isMetaProperty, isModifierKind, isNonNullExpression, isPrivateIdentifier, isSetAccessorDeclaration,
    isStringOrNumericLiteralLike, isTaggedTemplateExpression, isTemplateLiteralKind, isTypeReferenceNode,
    IterationStatement, JSDoc, JSDocAllType, JSDocAugmentsTag, JSDocAuthorTag, JSDocCallbackTag, JSDocClassTag,
    JSDocComment, JSDocContainer, JSDocDeprecatedTag, JSDocEnumTag, JSDocFunctionType, JSDocImplementsTag,
    JSDocLink, JSDocLinkCode, JSDocLinkPlain, JSDocMemberName, JSDocNameReference, JSDocNamespaceDeclaration,
    JSDocNonNullableType, JSDocNullableType, JSDocOptionalType, JSDocOverrideTag, JSDocParameterTag,
    JSDocPrivateTag, JSDocPropertyLikeTag, JSDocPropertyTag, JSDocProtectedTag, JSDocPublicTag, JSDocReadonlyTag,
    JSDocReturnTag, JSDocSeeTag, JSDocSignature, JSDocSyntaxKind, JSDocTag, JSDocTemplateTag, JSDocText,
    JSDocThisTag, JSDocTypedefTag, JSDocTypeExpression, JSDocTypeLiteral, JSDocTypeTag, JSDocUnknownTag, JSDocUnknownType,
    JSDocVariadicType, JsonMinusNumericLiteral, JsonObjectExpressionStatement, JsonSourceFile, JsxAttribute,
    ...
    NamespaceDeclaration, NamespaceExport, NamespaceExportDeclaration, NamespaceImport, NewExpression, Node,
    NodeArray, NodeFactoryFlags, NodeFlags, nodeIsMissing, nodeIsPresent, NonNullExpression, noop, normalizePath, NoSubstitutionTemplateLiteral,
    NullLiteral, NumericLiteral, objectAllocator, ObjectBindingPattern, ObjectLiteralElementLike,
    ObjectLiteralExpression, OperatorPrecedence, OptionalTypeNode, PackageJsonInfo, ParameterDeclaration,
    ParenthesizedExpression, ParenthesizedTypeNode, PartiallyEmittedExpression, perfLogger, PlusToken,
    PostfixUnaryExpression, PostfixUnaryOperator, PragmaDefinition, PragmaKindFlags, PragmaMap, PragmaPseudoMap,
    ...
    TextRange, textToKeywordObj, ThisExpression, ThisTypeNode, ThrowStatement, toArray, Token, TokenFlags, tokenIsIdentifierOrKeyword,
...
import { createSyntaxCursor, updateSourceFile as updateSourceFileWorker } from "./incrementalParser";
...
export function createSourceFile(fileName: string, sourceText: string, languageVersionOrOptions: ScriptTarget | CreateSourceFileOptions, setParentNodes = false, scriptKind?: ScriptKind): SourceFile {
    ...
    if (languageVersion === ScriptTarget.JSON) {
        result = parseSourceFile(fileName, sourceText, languageVersion, /*syntaxCursor*/ undefined, setParentNodes, ScriptKind.JSON, noop);
    }
    else {
...
export function parseJsonText(fileName: string, sourceText: string): JsonSourceFile {
    return parseJsonTextWorker(fileName, sourceText);
}
...
export function updateSourceFile(sourceFile: SourceFile, newText: string, textChangeRange: TextChangeRange, aggressiveChecks = false): SourceFile {
    const newSourceFile = updateSourceFileWorker(sourceFile, newText, textChangeRange, aggressiveChecks);
...
export function parseIsolatedJSDocComment(content: string, start?: number, length?: number) {
    const result = parseIsolatedJSDocCommentWorker(content, start, length);
...
// parser instances can actually be expensive enough to impact us on projects with many source files.
...
let syntaxCursor: SyntaxCursor | undefined;
...
/** @internal */
export function parseSourceFile(fileName: string, sourceText: string, languageVersion: ScriptTarget, syntaxCursor: SyntaxCursor | undefined, setParentNodes = false, scriptKind?: ScriptKind, setExternalModuleIndicatorOverride?: (file: SourceFile) => void): SourceFile {
    scriptKind = ensureScriptKind(fileName, scriptKind);
    if (scriptKind === ScriptKind.JSON) {
        const result = parseJsonTextWorker(fileName, sourceText, languageVersion, syntaxCursor, setParentNodes);
...
function parseJsonTextWorker(fileName: string, sourceText: string, languageVersion: ScriptTarget = ScriptTarget.ES2015, syntaxCursor?: SyntaxCursor, setParentNodes = false): JsonSourceFile {
...
function initializeState(_fileName: string, _sourceText: string, _languageVersion: ScriptTarget, _syntaxCursor: SyntaxCursor | undefined, _scriptKind: ScriptKind) {
...
function parseSourceFileWorker(languageVersion: ScriptTarget, setParentNodes: boolean, scriptKind: ScriptKind, setExternalModuleIndicator: (file: SourceFile) => void): SourceFile {
    ...
    const sourceFile = createSourceFileWorker(fileName, languageVersion, scriptKind, isDeclarationFile, statements, endOfFileToken, sourceFlags, setExternalModuleIndicator);
...
function addJSDocComment<T extends HasJSDoc>(node: T): T {
    ...
    const jsDoc = mapDefined(getJSDocCommentRanges(node, sourceText), comment => parseJSDocComment(node, comment.pos, comment.end - comment.pos));
...
function reparseTopLevelAwait(sourceFile: SourceFile) {
    ...
    const baseSyntaxCursor = createSyntaxCursor(sourceFile);
...
function createSourceFileWorker(
    fileName: string,
    languageVersion: ScriptTarget,
...
function tokenIsExternalModuleReference() {
...
function parseModuleReference() {
    return tokenIsExternalModuleReference()
...
export function parseJSDocTypeExpressionForTests(content: string, start: number | undefined, length: number | undefined): { jsDocTypeExpression: JSDocTypeExpression, diagnostics: Diagnostic[] } | undefined {
    ...
    const sourceFile = createSourceFileWorker("file.js", ScriptTarget.Latest, ScriptKind.JS, /*isDeclarationFile*/ false, [], factory.createToken(SyntaxKind.EndOfFileToken), NodeFlags.None, noop);
