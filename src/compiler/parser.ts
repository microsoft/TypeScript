...
let TokenConstructor: new <TKind extends TokenSyntaxKind>(kind: TKind, pos: number, end: number) => Token<TKind>;
...
export const parseBaseNodeFactory: BaseNodeFactory = {
    createBaseSourceFileNode: kind =>
        new (SourceFileConstructor ||
            (SourceFileConstructor = objectAllocator.getSourceFileConstructor()))(kind, -1, -1),
    createBaseIdentifierNode: kind =>
        new (IdentifierConstructor ||
            (IdentifierConstructor = objectAllocator.getIdentifierConstructor()))(kind, -1, -1),
    createBasePrivateIdentifierNode: kind =>
        new (PrivateIdentifierConstructor ||
    ...
    createBaseTokenNode: kind =>
        new (TokenConstructor ||= objectAllocator.getTokenConstructor())(
            kind,
            -1,
            -1,
        ),
...
function isAnExternalModuleIndicatorNode(node: Node) {
    return (canHaveModifiers(node) &&
            hasModifierOfKind(node, SyntaxKind.ExportKeyword)) ||
...
function walkTreeForImportMeta(node: Node): Node | undefined {
    return isImportMeta(node)
        ? node
        : forEachChild(node, walkTreeForImportMeta);
}
...
const forEachChildTable: ForEachChildTable = {
    ...
    [SyntaxKind.PropertyAssignment]: function forEachChildInPropertyAssignment<
        T,
    >(
    ...
    [SyntaxKind.FunctionExpression]: function forEachChildInFunctionExpression<
        T,
    >(
    ...
    [SyntaxKind.IndexedAccessType]: function forEachChildInIndexedAccessType<T>(
        ...
        return (
            visitNode(cbNode, node.objectType) ||
    ...
    [SyntaxKind.AsExpression]: function forEachChildInAsExpression<T>(
        ...
        return (
            visitNode(cbNode, node.expression) || visitNode(cbNode, node.type)
        );
    },
    ...
    [SyntaxKind.LabeledStatement]: function forEachChildInLabeledStatement<T>(
        ...
        return (
            visitNode(cbNode, node.label) || visitNode(cbNode, node.statement)
        );
    },
    ...
    [SyntaxKind.EnumMember]: function forEachChildInEnumMember<T>(
        ...
        return (
            visitNode(cbNode, node.name) || visitNode(cbNode, node.initializer)
        );
    },
    ...
    [SyntaxKind.TemplateExpression]: function forEachChildInTemplateExpression<
        T,
    >(
    ...
    [SyntaxKind.TemplateLiteralTypeSpan]: function forEachChildInTemplateLiteralTypeSpan<T>(
        ...
        return (
            visitNode(cbNode, node.type) || visitNode(cbNode, node.literal)
        );
    },
    ...
    [SyntaxKind.MissingDeclaration]: function forEachChildInMissingDeclaration<
        T,
    >(
    ...
    [SyntaxKind.JsxSpreadAttribute]: function forEachChildInJsxSpreadAttribute<
        T,
    >(
    ...
    [SyntaxKind.JsxNamespacedName]: function forEachChildInJsxNamespacedName<T>(
        ...
        return (
            visitNode(cbNode, node.namespace) || visitNode(cbNode, node.name)
        );
    },
    ...
    [SyntaxKind.JSDocNameReference]: function forEachChildInJSDocNameReference<
        T,
    >(
    ...
    [SyntaxKind.JSDocImplementsTag]: function forEachChildInJSDocImplementsTag<
        T,
    >(
...
export function createSourceFile(
    ...
    if (languageVersion === ScriptTarget.JSON) {
        ...
        const setIndicator = format === undefined
...
namespace Parser {
    ...
    var NodeConstructor: new (
        kind: SyntaxKind,
        pos: number,
        end: number,
    ) => Node;
    var TokenConstructor: new <TKind extends TokenSyntaxKind>(
        kind: TKind,
        pos: number,
        end: number,
    ) => Token<TKind>;
    ...
    var baseNodeFactory: BaseNodeFactory = {
        ...
        createBasePrivateIdentifierNode: kind =>
            countNode(
                new PrivateIdentifierConstructor(kind, /*pos*/ 0, /*end*/ 0),
            ),
    ...
    export function parseJsonText(
        ...
        if (token() === SyntaxKind.EndOfFileToken) {
                ...
                switch (token()) {
                    ...
                    case SyntaxKind.StringLiteral:
                        if (
                            ...
                            expression = parseLiteralNode() as
                                | StringLiteral
                                | NumericLiteral;
                            break;
                        }
            ...
            const expression = isArray(expressions)
                ? finishNode(
                    factoryCreateArrayLiteralExpression(expressions),
                    pos,
                )
        ...
        sourceFile.parseDiagnostics = attachFileToDiagnostics(
            parseDiagnostics,
            sourceFile,
        );
        if (jsDocDiagnostics) {
            sourceFile.jsDocDiagnostics = attachFileToDiagnostics(
                jsDocDiagnostics,
                sourceFile,
            );
        }
    ...
    function parseSourceFileWorker(
        ...
        const endOfFileToken = withJSDoc(
    ...
    function reparseTopLevelAwait(sourceFile: SourceFile) {
        ...
        while (start !== -1) {
            ...
            const diagnosticEnd = diagnosticStart >= 0
                ? findIndex(
                    savedParseDiagnostics,
            ...
            speculationHelper(() => {
                ...
                while (token() !== SyntaxKind.EndOfFileToken) {
                    ...
                    const statement = parseListElement(
                        ParsingContext.SourceElements,
                        parseStatement,
                    );
                    ...
                    if (pos >= 0) {
                        ...
                        if (statement.end > nonAwaitStatement.pos) {
                            ...
                            pos = findNextStatementWithoutAwait(
            ...
            start = pos >= 0
                ? findNextStatementWithAwait(sourceFile.statements, pos)
                : -1;
        }
        ...
        return factory.updateSourceFile(
            sourceFile,
            setTextRange(
                factoryCreateNodeArray(statements),
                sourceFile.statements,
            ),
        );

        function containsPossibleTopLevelAwait(node: NodeBase) {
            return (
    ...
    function parseErrorForMissingSemicolonAfter(
        ...
        const expressionText = isIdentifierNode(node)
            ? idText(node)
            : undefined;
        if (
            !expressionText ||
            !isIdentifierText(expressionText, languageVersion)
        ) {
            parseErrorAtCurrentToken(
                Diagnostics._0_expected,
    ...
    function getSpaceSuggestion(expressionText: string) {
        for (const keyword of viableKeywordSuggestions) {
            if (
                expressionText.length > keyword.length + 2 &&
                startsWith(expressionText, keyword)
            ) {
    ...
    function parseOptionalToken<TKind extends TokenSyntaxKind>(
        t: TKind,
    ): Token<TKind>;
    ...
    function parseOptionalTokenJSDoc<TKind extends JSDocSyntaxKind>(
        t: TKind,
    ): Token<TKind>;
    ...
    function parseExpectedToken<TKind extends TokenSyntaxKind>(
        t: TKind,
        diagnosticMessage?: DiagnosticMessage,
        arg0?: string,
    ): Token<TKind>;
    ...
    function parseExpectedTokenJSDoc<TKind extends JSDocSyntaxKind>(
        t: TKind,
    ): Token<TKind>;
    function parseExpectedTokenJSDoc(t: JSDocSyntaxKind): Token<JSDocSyntaxKind> | Node {
    ...
    function parseTokenNode<T extends Node>(): T {
        const pos = getNodePos();
        const kind = token() as TokenSyntaxKind;
        nextToken();
    ...
    function parseTokenNodeJSDoc<T extends Node>(): T {
        const pos = getNodePos();
        const kind = token() as JsxTokenSyntaxKind;
        nextTokenJSDoc();
        return finishNode(factoryCreateToken(kind), pos) as T;
    }
    ...
    function createMissingNode<T extends Node>(
        ...
        const pos = getNodePos();
        const result = kind === SyntaxKind.Identifier
    ...
    function parsePrivateIdentifier(): PrivateIdentifier {
        const pos = getNodePos();
        const node = factoryCreatePrivateIdentifier(
    ...
    function currentNode(
        ...
        const node = syntaxCursor.currentNode(
        ...
        if (canHaveJSDoc(node as Node) && (node as Partial<JSDocContainer>).jsDoc?.jsDocCache) {
            // jsDocCache may include tags from parent nodes, which might have been modified.
            (node as Partial<JSDocContainer>).jsDoc!.jsDocCache = undefined;
        }
    ...
    function parsingContextErrors(context: ParsingContext) {
        switch (context) {
            ...
            case ParsingContext.SwitchClauses:
                return parseErrorAtCurrentToken(
                    Diagnostics.case_or_default_expected,
                );
            case ParsingContext.SwitchClauseStatements:
            ...
            case ParsingContext.EnumMembers:
                return parseErrorAtCurrentToken(
                    Diagnostics.Enum_member_expected,
                );
            case ParsingContext.HeritageClauseElement:
                return parseErrorAtCurrentToken(
            ...
            case ParsingContext.TypeArguments:
                return parseErrorAtCurrentToken(
                    Diagnostics.Type_argument_expected,
                );
            case ParsingContext.TupleElementTypes:
        ...
        while (true) {
            if (isListElement(kind, /*inErrorRecovery*/ false)) {
                ...
                parseExpected(
                    ...
                    getExpectedCommaDiagnostic(kind),
                );
    ...
    function createQualifiedName(
        ...
        return finishNode(
            factory.createQualifiedName(entity, name),
            entity.pos,
        );
    }
    ...
    function parseLiteralNode(): LiteralLikeNode {
    ...
    function parseTemplateHead(isTaggedTemplate: boolean): TemplateHead {
        if (
            !isTaggedTemplate &&
            scanner.getTokenFlags() & TokenFlags.IsInvalid
        ) {
    ...
    function getTemplateLiteralRawText(kind: TemplateLiteralToken["kind"]) {
        const isLast = kind === SyntaxKind.NoSubstitutionTemplateLiteral ||
    ...
    function parseLiteralLikeNode(kind: SyntaxKind): LiteralLikeNode {
        ...
        const node = isTemplateLiteralKind(kind)
            ...
            ? factoryCreateNumericLiteral(
                scanner.getTokenValue(),
                scanner.getNumericLiteralFlags(),
            )
            : kind === SyntaxKind.StringLiteral
    ...
    function parseJSDocNonNullableType(): TypeNode {
        ...
        nextToken();
        return finishNode(
            factory.createJSDocNonNullableType(
                parseNonArrayType(),
                /*postfix*/ false,
            ),
            pos,
        ...
        | JSDocNullableType
    {
        ...
        if (
            ...
            return finishNode(
                factory.createJSDocNullableType(parseType(), /*postfix*/ false),
                pos,
            );
        }
    }
    ...
    function parseJSDocFunctionType(): JSDocFunctionType | TypeReferenceNode {
        ...
        if (lookAhead(nextTokenIsOpenParen)) {
            ...
            return withJSDoc(
                finishNode(
                    factory.createJSDocFunctionType(parameters, type),
                    pos,
                ),
                hasJSDoc,
            );
    ...
    function parseTypeQuery(): TypeQueryNode {
        ...
        const typeArguments = !scanner.hasPrecedingLineBreak()
            ? tryParseTypeArguments()
            : undefined;
        return finishNode(
            factory.createTypeQueryNode(entityName, typeArguments),
            pos,
        );
    }
    ...
    function parseTypeParameter(): TypeParameterDeclaration {
        ...
        const defaultType = parseOptional(SyntaxKind.EqualsToken)
            ? parseType()
            : undefined;
    ...
    function parseTypeLiteral(): TypeLiteralNode {
        const pos = getNodePos();
        return finishNode(
            factory.createTypeLiteralNode(parseObjectTypeMembers()),
            pos,
        );
    }
    ...
    function parseMappedType() {
        ...
        const nameType = parseOptional(SyntaxKind.AsKeyword)
            ? parseType()
            : undefined;
        parseExpected(SyntaxKind.CloseBracketToken);
    ...
    function isTupleElementName() {
        if (token() === SyntaxKind.DotDotDotToken) {
            return (
                tokenIsIdentifierOrKeyword(nextToken()) &&
                isNextTokenColonOrQuestionColon()
            );
        }
        return (
            tokenIsIdentifierOrKeyword(token()) &&
        ...
        | undefined
    {
        ...
        if (token() === SyntaxKind.AbstractKeyword) {
            ...
            const modifier = finishNode(
                factoryCreateToken(SyntaxKind.AbstractKeyword),
                pos,
            );
    ...
    function parseLiteralTypeNode(negative?: boolean): LiteralTypeNode {
        ...
        let expression = token() === SyntaxKind.TrueKeyword ||
    ...
    function parseImportType(): ImportTypeNode {
        ...
        if (parseOptional(SyntaxKind.CommaToken)) {
            ...
            if (
                ...
                currentToken === SyntaxKind.AssertKeyword
            ) {
                nextToken();
            }
            else {
                parseErrorAtCurrentToken(
                    Diagnostics._0_expected,
                    tokenToString(SyntaxKind.WithKeyword),
                );
            }
            parseExpected(SyntaxKind.ColonToken);
    ...
    function isStartOfType(inStartOfParameter?: boolean): boolean {
        switch (token()) {
            ...
            case SyntaxKind.OpenParenToken:
                ...
                return (
                    !inStartOfParameter &&
                    lookAhead(isStartOfParenthesizedOrFunctionType)
                );
            default:
                return isIdentifier();
        }
        ...
        while (!scanner.hasPrecedingLineBreak()) {
            switch (token()) {
                ...
                case SyntaxKind.QuestionToken:
                    ...
                    nextToken();
                    type = finishNode(
                        factory.createJSDocNullableType(type, /*postfix*/ true),
                        pos,
                    );
                    break;
                case SyntaxKind.OpenBracketToken:
    ...
    function parseInferType(): InferTypeNode {
        ...
        return finishNode(
            factory.createInferTypeNode(parseTypeParameterOfInferType()),
            pos,
        );
    }
    ...
    function isStartOfFunctionTypeOrConstructorType(): boolean {
        ...
        return (
            token() === SyntaxKind.NewKeyword ||
            (token() === SyntaxKind.AbstractKeyword &&
                lookAhead(nextTokenIsNewKeyword))
        );
    }
    ...
    function parseAssertsTypePredicate(): TypeNode {
        ...
        const parameterName = token() === SyntaxKind.ThisKeyword
            ? parseThisTypeNode()
            : parseIdentifier();
        const type = parseOptional(SyntaxKind.IsKeyword)
            ? parseType()
            : undefined;
        return finishNode(
            factory.createTypePredicateNode(
    ...
    function parseAssignmentExpressionOrHigher(
        ...
        return parseConditionalExpressionRest(
            expr,
            pos,
            allowReturnTypeInArrowFunction,
        );
    ...
    function parsePossibleParenthesizedArrowFunctionExpression(
        ...
        if (!result) {
            (notParenthesizedArrow || (notParenthesizedArrow = new Set())).add(
                tokenPos,
            );
        }
    ...
    function tryParseAsyncSimpleArrowFunctionExpression(
        ...
        if (token() === SyntaxKind.AsyncKeyword) {
            if (
                lookAhead(isUnParenthesizedAsyncArrowFunctionWorker) ===
                    Tristate.True
            ) {
                const pos = getNodePos();
                const hasJSDoc = hasPrecedingJSDocComment();
    ...
    function parseParenthesizedArrowFunctionExpression(
        ...
        if (!parseExpected(SyntaxKind.OpenParenToken)) {
            ...
            if (!allowAmbiguity) {
                const maybeParameters = parseParametersWorker(
                    isAsync,
                    allowAmbiguity,
                );
                if (!maybeParameters) {
        ...
        const body = lastToken === SyntaxKind.EqualsGreaterThanToken ||
    ...
    function makeSatisfiesExpression(
        left: Expression,
        right: TypeNode,
    ): SatisfiesExpression {
        return finishNode(
            factory.createSatisfiesExpression(left, right),
            left.pos,
        );
    }
    ...
    function parseUpdateExpression(): UpdateExpression {
        ...
        if (
            ...
            nextToken();
            return finishNode(
                factory.createPostfixUnaryExpression(expression, operator),
                expression.pos,
            );
        }
    ...
    function parseLeftHandSideExpressionOrHigher(): LeftHandSideExpression {
        ...
        if (token() === SyntaxKind.ImportKeyword) {
            if (lookAhead(nextTokenIsOpenParenOrLessThan)) {
                ...
                sourceFlags |= NodeFlags.PossiblyContainsDynamicImport;
                expression = parseTokenNode<MemberExpression>();
            }
        ...
        if (token() === SyntaxKind.LessThanToken) {
            ...
            if (typeArguments !== undefined) {
                ...
                if (!isTemplateStartOfTaggedTemplate()) {
                    expression = factory.createExpressionWithTypeArguments(
                        expression,
                        typeArguments,
                    );
                }
    ...
    function parseJsxElementOrSelfClosingElementOrFragment(
        ...
        if (opening.kind === SyntaxKind.JsxOpeningElement) {
            ...
            result = finishNode(
                factory.createJsxElement(opening, children, closingElement),
                pos,
            );
        }
        ...
        if (
            ...
            const topBadPos = typeof topInvalidNodePosition === "undefined"
                ? result.pos
    ...
    function parseJsxOpeningOrSelfClosingElementOrOpeningFragment(
        ...
        const typeArguments = (contextFlags & NodeFlags.JavaScriptFile) === 0
            ? tryParseTypeArguments()
            : undefined;
        ...
        if (token() === SyntaxKind.GreaterThanToken) {
            ...
            // scanning errors
            scanJsxText();
            node = factory.createJsxOpeningElement(
                tagName,
                typeArguments,
                attributes,
            );
            ...
            node = factory.createJsxSelfClosingElement(
                tagName,
                typeArguments,
                attributes,
            );
        ...
        | ThisExpression
    {
        ...
        return isThis
            ? finishNode(factory.createToken(SyntaxKind.ThisKeyword), pos)
            : tagName;
    }
    ...
    function parseJsxExpression(
        ...
        return finishNode(
            factory.createJsxExpression(dotDotDotToken, expression),
            pos,
        );
    }
    ...
    function tryReparseOptionalChain(node: Expression) {
        ...
        if (isNonNullExpression(node)) {
            ...
            while (
                isNonNullExpression(expr) &&
                !(expr.flags & NodeFlags.OptionalChain)
            ) {
    ...
    function parsePropertyAccessExpressionRest(
        ...
        const propertyAccess = isOptionalChain
            ? factoryCreatePropertyAccessChain(
                expression,
                questionDotToken,
                name,
            )
    ...
    function parseElementAccessExpressionRest(
        ...
        const indexedAccess = questionDotToken || tryReparseOptionalChain(expression)
            ? factoryCreateElementAccessChain(
                expression,
                questionDotToken,
                argumentExpression,
            )
            : factoryCreateElementAccessExpression(
                expression,
                argumentExpression,
            );
        ...
        while (true) {
            ...
            if (!questionDotToken) {
                if (
                    ...
                    nextToken();
                    expression = finishNode(
                        factory.createNonNullExpression(expression),
                        pos,
                    );
                    continue;
                }
        ...
        while (true) {
            ...
            if (typeArguments || token() === SyntaxKind.OpenParenToken) {
                ...
                const callExpr = questionDotToken || tryReparseOptionalChain(expression)
                    ? factoryCreateCallChain(
                        expression,
                        questionDotToken,
                        typeArguments,
                        argumentList,
                    )
                    : factoryCreateCallExpression(
                        expression,
                        typeArguments,
                        argumentList,
                    );
    ...
    function parsePrimaryExpression(): PrimaryExpression {
        switch (token()) {
            ...
            case SyntaxKind.SlashEqualsToken:
                if (
                    reScanSlashToken() === SyntaxKind.RegularExpressionLiteral
                ) {
                    return parseLiteralNode();
                }
                break;
    ...
    function parseParenthesizedExpression(): ParenthesizedExpression {
        ...
        parseExpected(SyntaxKind.CloseParenToken);
        return withJSDoc(
            finishNode(factoryCreateParenthesizedExpression(expression), pos),
            hasJSDoc,
        );
    }
    ...
    function parseArrayLiteralExpression(): ArrayLiteralExpression {
        ...
        return finishNode(
            factoryCreateArrayLiteralExpression(elements, multiLine),
            pos,
        );
    }
    ...
    function parseObjectLiteralElement(): ObjectLiteralElementLike {
        ...
        if (parseOptionalToken(SyntaxKind.DotDotDotToken)) {
            ...
            return withJSDoc(
                finishNode(factory.createSpreadAssignment(expression), pos),
                hasJSDoc,
            );
        }
    ...
    function parseObjectLiteralExpression(): ObjectLiteralExpression {
        ...
        return finishNode(
            factoryCreateObjectLiteralExpression(properties, multiLine),
            pos,
        );
    }
    ...
    function parseFunctionExpression(): FunctionExpression {
        ...
        const isAsync = some(modifiers, isAsyncModifier)
            ? SignatureFlags.Await
            : SignatureFlags.None;
        const name = isGenerator && isAsync
    ...
    function parseNewExpressionOrNewDotTarget(): NewExpression | MetaProperty {
        ...
        if (parseOptional(SyntaxKind.DotToken)) {
            const name = parseIdentifierName();
            return finishNode(
                factory.createMetaProperty(SyntaxKind.NewKeyword, name),
                pos,
            );
        }
        ...
        const argumentList = token() === SyntaxKind.OpenParenToken
            ? parseArgumentList()
            : undefined;
        return finishNode(
            factoryCreateNewExpression(expression, typeArguments, argumentList),
            pos,
        );
    }
    ...
    function parseBlock(
        ...
        if (openBraceParsed || ignoreMissingOpenBrace) {
            ...
            return withJSDoc(
                finishNode(
                    factoryCreateBlock(statements, /*multiLine*/ undefined),
                    pos,
                ),
                hasJSDoc,
            );
    ...
    function parseEmptyStatement(): Statement {
        ...
        return withJSDoc(
            finishNode(factory.createEmptyStatement(), pos),
            hasJSDoc,
        );
    }
    ...
    function parseIfStatement(): IfStatement {
        ...
        const elseStatement = parseOptional(SyntaxKind.ElseKeyword)
            ? parseStatement()
            : undefined;
        return withJSDoc(
            finishNode(
                factoryCreateIfStatement(
                    expression,
                    thenStatement,
                    elseStatement,
                ),
    ...
    function parseDoStatement(): DoStatement {
        ...
        return withJSDoc(
            finishNode(factory.createDoStatement(statement, expression), pos),
            hasJSDoc,
        );
    }
    ...
    function parseWhileStatement(): WhileStatement {
        ...
        const statement = parseStatement();
        return withJSDoc(
            finishNode(factoryCreateWhileStatement(expression, statement), pos),
            hasJSDoc,
        );
    }
    ...
    function parseForOrForInOrForOfStatement(): Statement {
        ...
        if (token() !== SyntaxKind.SemicolonToken) {
            if (
                ...
                (token() === SyntaxKind.AwaitKeyword &&
                    lookAhead(
                        nextTokenIsUsingKeywordThenBindingIdentifierOrStartOfObjectDestructuringOnSameLineDisallowOf,
                    ))
            ) {
        ...
        if (
            ...
            parseExpected(SyntaxKind.CloseParenToken);
            node = factoryCreateForOfStatement(
                awaitToken,
                initializer,
                expression,
                parseStatement(),
            ...
            const expression = allowInAnd(parseExpression);
            parseExpected(SyntaxKind.CloseParenToken);
            node = factory.createForInStatement(
                initializer,
                expression,
                parseStatement(),
            );
            ...
            const incrementor = token() !== SyntaxKind.CloseParenToken
                ...
                : undefined;
            parseExpected(SyntaxKind.CloseParenToken);
            node = factoryCreateForStatement(
                initializer,
                condition,
                incrementor,
                parseStatement(),
        ...
        return withJSDoc(
            finishNode(node, pos) as ForStatement | ForInOrOfStatement,
            hasJSDoc,
        );
    }
    ...
    function parseReturnStatement(): ReturnStatement {
        ...
        const expression = canParseSemicolon()
            ? undefined
            : allowInAnd(parseExpression);
        parseSemicolon();
        return withJSDoc(
            finishNode(factory.createReturnStatement(expression), pos),
            hasJSDoc,
        );
    }
    ...
    function parseWithStatement(): WithStatement {
        ...
        const statement = doInsideOfContext(
            NodeFlags.InWithStatement,
            parseStatement,
        );
        return withJSDoc(
            finishNode(factory.createWithStatement(expression, statement), pos),
            hasJSDoc,
        );
    }
    ...
    function parseCaseClause(): CaseClause {
        ...
        return withJSDoc(
            finishNode(factory.createCaseClause(expression, statements), pos),
            hasJSDoc,
        );
    }
    ...
    function parseSwitchStatement(): SwitchStatement {
        ...
        return withJSDoc(
            finishNode(
                factory.createSwitchStatement(expression, caseBlock),
                pos,
            ),
            hasJSDoc,
        );
    ...
    function parseThrowStatement(): ThrowStatement {
        ...
        let expression = scanner.hasPrecedingLineBreak()
            ? undefined
            : allowInAnd(parseExpression);
        ...
        return withJSDoc(
            finishNode(factory.createThrowStatement(expression), pos),
            hasJSDoc,
        );
    }
    ...
    function parseTryStatement(): TryStatement {
        ...
        const catchClause = token() === SyntaxKind.CatchKeyword
            ? parseCatchClause()
            : undefined;
        ...
        if (!catchClause || token() === SyntaxKind.FinallyKeyword) {
            parseExpected(
                ...
                Diagnostics.catch_or_finally_expected,
            );
        ...
        return withJSDoc(
            finishNode(
                factory.createTryStatement(tryBlock, catchClause, finallyBlock),
                pos,
            ),
            hasJSDoc,
        );
    ...
    function parseCatchClause(): CatchClause {
        ...
        return finishNode(
            factory.createCatchClause(variableDeclaration, block),
            pos,
        );
    }
    ...
    function parseDebuggerStatement(): Statement {
        ...
        parseSemicolon();
        return withJSDoc(
            finishNode(factory.createDebuggerStatement(), pos),
            hasJSDoc,
        );
    }
        ...
        while (true) {
            switch (token()) {
                ...
                case SyntaxKind.ReadonlyKeyword:
                    ...
                    if (
                        previousToken === SyntaxKind.DeclareKeyword &&
    ...
    function nextTokenIsBindingIdentifierOrStartOfDestructuringOnSameLine(
        disallowOf?: boolean,
    ) {
        nextToken();
            ...
            case SyntaxKind.ExportKeyword:
                ...
                switch (token()) {
                    ...
                    case SyntaxKind.AsKeyword:
                        return parseNamespaceExportDeclaration(
                            pos,
                            hasJSDoc,
                            modifiersIn,
                        );
    ...
    function parseVariableDeclaration(
        ...
        const type = parseTypeAnnotation();
        const initializer = isInOrOfKeyword(token())
            ? undefined
            : parseInitializer();
        const node = factoryCreateVariableDeclaration(
            name,
            exclamationToken,
            type,
            initializer,
    ...
    function parseVariableDeclarationList(
        ...
        return finishNode(
            factoryCreateVariableDeclarationList(declarations, flags),
            pos,
        );
    }
    ...
    function parseFunctionDeclaration(
        ...
        const name = modifierFlags & ModifierFlags.Default
            ? parseOptionalBindingIdentifier()
            : parseBindingIdentifier();
        const isGenerator = asteriskToken
            ? SignatureFlags.Yield
            : SignatureFlags.None;
        const isAsync = modifierFlags & ModifierFlags.Async
            ? SignatureFlags.Await
            : SignatureFlags.None;
        const typeParameters = parseTypeParameters();
        ...
        const parameters = parseParameters(isGenerator | isAsync);
        const type = parseReturnType(SyntaxKind.ColonToken, /*isType*/ false);
        const body = parseFunctionBlockOrSemicolon(
            isGenerator | isAsync,
    ...
    function parsePropertyDeclaration(
        ...
        const exclamationToken = !questionToken && !scanner.hasPrecedingLineBreak()
    ...
    function parseDecoratorExpression() {
        if (inAwaitContext() && token() === SyntaxKind.AwaitKeyword) {
            ...
            const pos = getNodePos();
            const awaitExpression = parseIdentifier(
                Diagnostics.Expression_expected,
            );
            nextToken();
    ...
    function parseModifiersForArrowFunction(): NodeArray<Modifier> | undefined {
        ...
        if (token() === SyntaxKind.AsyncKeyword) {
            ...
            nextToken();
            const modifier = finishNode(
                factoryCreateToken(SyntaxKind.AsyncKeyword),
                pos,
            );
            modifiers = createNodeArray<Modifier>([modifier], pos);
        }
    ...
    function parseClassElement(): ClassElement {
        ...
        if (token() === SyntaxKind.SemicolonToken) {
            nextToken();
            return withJSDoc(
                finishNode(factory.createSemicolonClassElement(), pos),
                hasJSDoc,
            );
        }
        ...
        if (
            ...
            const constructorDeclaration = tryParseConstructorDeclaration(
                pos,
                hasJSDoc,
                modifiers,
            );
    ...
    function parseHeritageClause(): HeritageClause {
        ...
        const tok = token();
        Debug.assert(
            tok === SyntaxKind.ExtendsKeyword ||
                tok === SyntaxKind.ImplementsKeyword,
    ...
    function parseEnumMember(): EnumMember {
        ...
        return withJSDoc(
            finishNode(factory.createEnumMember(name, initializer), pos),
            hasJSDoc,
        );
    }
    ...
    function parseModuleOrNamespaceDeclaration(
        ...
        const name = flags & NodeFlags.NestedNamespace
            ? parseIdentifierName()
            : parseIdentifier();
    ...
    function parseModuleDeclaration(
        ...
        return parseModuleOrNamespaceDeclaration(
            pos,
            hasJSDoc,
            modifiersIn,
            flags,
    ...
    function parseImportAttributes(
        ...
        if (parseExpected(SyntaxKind.OpenBraceToken)) {
            ...
            return finishNode(
                factory.createImportAttributes(elements, multiLine, token),
                pos,
            );
        }
        else {
    ...
    function parseImportClause(
        ...
        return finishNode(
            factory.createImportClause(isTypeOnly, identifier, namedBindings),
            pos,
        );
    }
    ...
    function parseExternalModuleReference() {
        ...
        parseExpected(SyntaxKind.CloseParenToken);
        return finishNode(
            factory.createExternalModuleReference(expression),
            pos,
        );
    }
    ...
    function parseNamespaceExport(pos: number): NamespaceExport {
        return finishNode(
            factory.createNamespaceExport(parseIdentifierName()),
            pos,
        );
    }
    ...
    export namespace JSDocParser {
        ...
        export function parseJSDocNameReference(): JSDocNameReference {
            ...
            const p2 = getNodePos();
            let entityName: EntityName | JSDocMemberName = parseEntityName(
                /*allowReservedWords*/ false,
            );
            while (token() === SyntaxKind.PrivateIdentifier) {
                reScanHashToken(); // rescan #id as # id
                nextTokenJSDoc(); // then skip the #
                entityName = finishNode(
                    factory.createJSDocMemberName(
                        entityName,
                        parseIdentifier(),
                    ),
                    p2,
        ...
        function parseJSDocCommentWorker(
            ...
            const result = scanner.scanRange(
                start + 3,
                length - 5,
                doJSDocScan,
            );
            ...
            function doJSDocScan() {
                    ...
                    switch (token()) {
                        ...
                        case SyntaxKind.WhitespaceTrivia:
                            ...
                            if (
                                ...
                                comments.push(
                                    whitespace.slice(margin - indent),
                                );
                            }
                ...
                if (parts.length && trimmedComments.length) {
                    parts.push(
                        finishNode(
                            factory.createJSDocText(trimmedComments),
                            linkEnd ?? start,
                            commentsPos,
                        ),
                    );
            ...
            function removeTrailingWhitespace(comments: string[]) {
                while (comments.length) {
                    ...
                    if (trimmed === "") {
                        ...
                        trimmed.length < comments[comments.length - 1].length
                    ) {
            ...
            function isObjectOrObjectArrayTypeReference(
                node: TypeNode,
            ): boolean {
                switch (node.kind) {
                    case SyntaxKind.ObjectKeyword:
            ...
            function parseParameterOrPropertyTag(
                ...
                const comment = parseTrailingTagComments(
                    start,
                    getNodePos(),
                    indent,
                    indentText,
                ...
                const nestedTypeLiteral = parseNestedTypeLiteral(
                    typeExpression,
                    name,
                    target,
                    indent,
            ...
            function parseNestedTypeLiteral(
                ...
                if (
                    ...
                    if (children) {
                        ...
                        return finishNode(
                            factory.createJSDocTypeExpression(literal),
                            pos,
                        );
                    }
                }
            ...
            function parseTypeTag(
                ...
                const comments = indent !== undefined && indentText !== undefined
                    ? parseTrailingTagComments(
                        start,
                        getNodePos(),
                        indent,
            ...
            function parseSeeTag(
                ...
                const nameExpression = isMarkdownOrJSDocLink
                    ? undefined
            ...
            function parseAuthorTag(
                ...
                const comments = parseTrailingTagComments(
                    start,
                    commentEnd,
                    indent,
                    indentText,
                ...
                return finishNode(
                    factory.createJSDocAuthorTag(tagName, allParts),
                    start,
                );
            }
            ...
            function parseSatisfiesTag(
                ...
                const comments = margin !== undefined && indentText !== undefined
                    ? parseTrailingTagComments(
                        start,
                        getNodePos(),
                        margin,
            ...
            function parseTypedefTag(
                ...
                let typeExpression:
                    | JSDocTypeExpression
                    | JSDocTypeLiteral
                    | undefined = tryParseTypeExpression();
                skipWhitespaceOrAsterisk();
                ...
                if (
                    ...
                    while (
                        (child = tryParse(() => parseChildPropertyTag(indent)))
                    ) {
                    ...
                    if (hasChildren) {
                        const isArrayType = typeExpression &&
                            typeExpression.type.kind === SyntaxKind.ArrayType;
                        const jsdocTypeLiteral = factory.createJSDocTypeLiteral(
                            jsDocPropertyTags,
                            isArrayType,
                        );
                        typeExpression = childTypeTag &&
                ...
                if (!comment) {
                    comment = parseTrailingTagComments(
                        start,
                        end,
                        indent,
                        indentText,
                    ...
                    (child = tryParse(
                        () =>
                            parseChildParameterOrPropertyTag(
                                ...
                                indent,
                            ) as JSDocParameterTag | JSDocTemplateTag,
                    ))
                ) {
                    if (child.kind === SyntaxKind.JSDocTemplateTag) {
                        parseErrorAtRange(
            ...
            function parseChildPropertyTag(indent: number) {
                return parseChildParameterOrPropertyTag(
                    ...
                    indent,
                ) as JSDocTypeTag | JSDocPropertyTag | JSDocTemplateTag | false;
            }
            ...
            function parseTemplateTag(
                ...
                const constraint = token() === SyntaxKind.OpenBraceToken
                    ? parseJSDocTypeExpression()
                    : undefined;
            ...
            function parseJSDocIdentifierName(
                ...
                const result = finishNode(
                    factoryCreateIdentifier(text, originalKeywordKind),
                    start,
                    end,
                );
                nextTokenJSDoc();
...
namespace IncrementalParser {
    export function updateSourceFile(
        ...
        Parser.fixupParentReferences(incrementalSourceFile as Node);
        ...
        const delta = textChangeRangeNewSpan(changeRange).length -
            changeRange.span.length;
                ...
                if (aggressiveChecks) {
                    Debug.assert(
                        oldText.substring(range.pos, range.end) ===
                            newText.substring(
                                updatedDirective.range.pos,
                                updatedDirective.range.end,
                            ),
                    );
                }
            }
    ...
    function shouldCheckNode(node: IncrementalNode | Node) {
        switch (node.kind) {
            case SyntaxKind.StringLiteral:
            case SyntaxKind.NumericLiteral:
            case SyntaxKind.Identifier:
    ...
    function checkChangeRange(
        ...
        if (textChangeRange) {
            ...
            if (
                ...
                const oldTextPrefix = oldText.substr(
                    0,
                    textChangeRange.span.start,
                );
                const newTextPrefix = newText.substr(
                    0,
                    textChangeRange.span.start,
                );
    ...
    export interface IncrementalNode extends NodeBase, IncrementalElement {
...
export function isDeclarationFileName(fileName: string): boolean {
    return (
        fileExtensionIsOneOf(fileName, supportedDeclarationExtensions) ||
    ...
    context.pragmas!.forEach((entryOrList, key) => {
        ...
        switch (key) {
            case "reference": {
                ...
                forEach(
                    toArray(entryOrList) as PragmaPseudoMap["reference"][],
                    arg => {
                        const {
                            types,
                            lib,
                        ...
                        if (arg.arguments["no-default-lib"]) {
                            ...
                            const parsed = parseResolutionMode(
                                res,
                                types.pos,
                                types.end,
                                reportDiagnostic,
            ...
            case "amd-module": {
                if (entryOrList instanceof Array) {
                    for (const entry of entryOrList) {
                        ...
                        context.moduleName = (
                            entry as PragmaPseudoMap["amd-module"]
                        ).arguments.name;
                    }
                }
                else {
                    context.moduleName = (
                        entryOrList as PragmaPseudoMap["amd-module"]
                    ).arguments.name;
                }
                break;
            }
...
function extractPragmas(
    ...
    const tripleSlash = range.kind === SyntaxKind.SingleLineCommentTrivia &&
    ...
    const singleLine = range.kind === SyntaxKind.SingleLineCommentTrivia &&
...
export function tagNamesAreEquivalent(
    ...
    return (
        ...
        tagNamesAreEquivalent(
            (lhs as PropertyAccessExpression)
                .expression as JsxTagNameExpression,
