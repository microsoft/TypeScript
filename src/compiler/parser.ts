    ...
    export function createSourceFile(filename: string, sourceText: string, languageVersion: ScriptTarget, version: string, isOpen: boolean = false): SourceFile {
        ...
        function parsePropertyAssignment(): Declaration {
            ...
            if (asteriskToken || token === SyntaxKind.OpenParenToken || token === SyntaxKind.LessThanToken) {
                ...
                var body = parseFunctionBlock(!!asteriskToken, /* ignoreMissingOpenBrace */ false, isAsync);
        ...
        function makeFunctionExpression(kind: SyntaxKind, pos: number, asteriskToken: Node, name: Identifier, sig: ParsedSignature, body: Node, isAsync?: boolean): FunctionExpression {
        ...
        function parsePropertyMemberDeclaration(fullStart: number, modifiers: ModifiersArray): Declaration {
            ...
            if (asteriskToken || token === SyntaxKind.OpenParenToken || token === SyntaxKind.LessThanToken) {
                ...
                method.body = parseFunctionBlockOrSemicolon(!!asteriskToken, isAsync);
                return finishNode(method);
            }
            else {
                var property = <PropertyDeclaration>createNode(SyntaxKind.Property, fullStart);
