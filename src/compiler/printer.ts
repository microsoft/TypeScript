...
namespace ts {
    ...
    export function createPrinter(writer: EmitTextWriter, compilerOptions: CompilerOptions, printerOptions: PrinterOptions = {}): Printer {
        ...
        function setSourceFile(sourceFile: SourceFile) {
            ...
            if (nameGenerator) {
                nameGenerator.setSourceFile(sourceFile);
            }
        }
        ...
        function emitBodyIndirect(node: Node, elements: NodeArray<Node>, emitCallback: (node: Node) => void): void {
            if (onEmitDetachedCommentsOfNode) {
                onEmitDetachedCommentsOfNode(node, elements, emitCallback);
            }
            else {
                emitCallback(node);
            }
        ...
        function emitPropertyAssignment(node: PropertyAssignment) {
            ...
            if (onEmitTrailingCommentsOfPosition && (getEmitFlags(initializer) & EmitFlags.NoLeadingComments) === 0) {
                const commentRange = getCommentRange(initializer);
                onEmitTrailingCommentsOfPosition(commentRange.pos);
            }
            ...
            if (isEmpty) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    if (shouldEmitInterveningComments) {
                        if (onEmitTrailingCommentsOfPosition) {
        ...
        function getNameGenerator() {
            if (!nameGenerator) {
                nameGenerator = createNameGenerator(printerOptions.hasGlobalName);
                nameGenerator.setSourceFile(currentSourceFile);
            }
            return nameGenerator;
        }

        function pushNameGenerationScope() {
            getNameGenerator().pushNameGenerationScope();
        }

        function popNameGenerationScope() {
            getNameGenerator().popNameGenerationScope();
        }

        function generateName(name: GeneratedIdentifier): string {
            return getNameGenerator().generateName(name, getTextOfNode);
        }
    }
