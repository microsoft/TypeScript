...
export function createCompilerHostWorker(options: ts.CompilerOptions, setParentNodes?: boolean, system = ts.sys): ts.CompilerHost {
    ...
    function getSourceFile(fileName: string, languageVersionOrOptions: ts.ScriptTarget | ts.CreateSourceFileOptions, onError?: (message: string) => void): ts.SourceFile | undefined {
...
export function changeCompilerHostLikeToUseCache(
    ...
    const sourceFileCache = new ts.Map<string, ts.SourceFile>();
...
export function isProgramUptoDate(
    ...
    fileExists: (fileName: string) => boolean,
    hasInvalidatedResolution: ts.HasInvalidatedResolution,
...
export function createProgram(rootNamesOrOptions: readonly string[] | ts.CreateProgramOptions, _options?: ts.CompilerOptions, _host?: ts.CompilerHost, _oldProgram?: ts.Program, _configFileParsingDiagnostics?: readonly ts.Diagnostic[]): ts.Program {
    ...
    let actualResolveModuleNamesWorker: (moduleNames: string[], containingFile: ts.SourceFile, containingFileName: string, reusedNames?: string[], redirectedReference?: ts.ResolvedProjectReference) => ts.ResolvedModuleFull[];
    const hasInvalidatedResolution = host.hasInvalidatedResolution || ts.returnFalse;
    if (host.resolveModuleNames) {
        actualResolveModuleNamesWorker = (moduleNames, containingFile, containingFileName, reusedNames, redirectedReference) => host.resolveModuleNames!(ts.Debug.checkEachDefined(moduleNames), containingFileName, reusedNames, redirectedReference, options, containingFile).map(resolved => {
        ...
        const loader = (moduleName: string, resolverMode: ts.ModuleKind.CommonJS | ts.ModuleKind.ESNext | undefined, containingFileName: string, redirectedReference: ts.ResolvedProjectReference | undefined) => ts.resolveModuleName(moduleName, containingFileName, options, host, moduleResolutionCache, redirectedReference, resolverMode).resolvedModule!; // TODO: GH#18217
        actualResolveModuleNamesWorker = (moduleNames, containingFile, containingFileName, _reusedNames, redirectedReference) => loadWithModeAwareCache<ts.ResolvedModuleFull>(ts.Debug.checkEachDefined(moduleNames), containingFile, containingFileName, redirectedReference, loader);
    }
    ...
    function pullDiagnosticsFromCache(names: string[] | readonly ts.FileReference[], containingFile: ts.SourceFile) {
    ...
    function resolveModuleNamesWorker(moduleNames: string[], containingFile: ts.SourceFile, reusedNames: string[] | undefined): readonly ts.ResolvedModuleFull[] {
    ...
    function resolveModuleNamesReusingOldState(moduleNames: string[], file: ts.SourceFile): readonly (ts.ResolvedModuleFull | undefined)[] {
        ...
        for (let i = 0; i < moduleNames.length; i++) {
            ...
            if (file === oldSourceFile && !hasInvalidatedResolution(oldSourceFile.path)) {
                const oldResolvedModule = ts.getResolvedModule(oldSourceFile, moduleName, getModeForResolutionAtIndex(oldSourceFile, i));
        ...
        function moduleNameResolvesToAmbientModuleInNonModifiedFile(moduleName: string, index: number): boolean {
            if (index >= ts.length(oldSourceFile?.imports) + ts.length(oldSourceFile?.moduleAugmentations)) return false; // mode index out of bounds, don't reuse resolution
            const resolutionToFile = ts.getResolvedModule(oldSourceFile, moduleName, oldSourceFile && getModeForResolutionAtIndex(oldSourceFile, index));
    ...
    function tryReuseStructureFromOldProgram(): ts.StructureIsReused {
        ...
        for (const { oldFile: oldSourceFile, newFile: newSourceFile } of modifiedSourceFiles) {
            ...
            const resolutionsChanged = ts.hasChangesInResolutions(moduleNames, resolutions, oldSourceFile.resolvedModules, oldSourceFile, ts.moduleResolutionIsEqualTo);
            ...
            const typeReferenceResolutionsChanged = ts.hasChangesInResolutions(typesReferenceDirectives, typeReferenceResolutions, oldSourceFile.resolvedTypeReferenceDirectiveNames, oldSourceFile, ts.typeDirectiveIsEqualTo);
    ...
    function emitWorker(program: ts.Program, sourceFile: ts.SourceFile | undefined, writeFileCallback: ts.WriteFileCallback | undefined, cancellationToken: ts.CancellationToken | undefined, emitOnlyDtsFiles?: boolean, customTransformers?: ts.CustomTransformers, forceDtsEmit?: boolean): ts.EmitResult {
        ...
        const emitResult = ts.emitFiles(
            ...
            getEmitHost(writeFileCallback),
            sourceFile,
            ts.getTransformers(options, customTransformers, emitOnlyDtsFiles),
            emitOnlyDtsFiles,
            /*onlyBuildInfo*/ false,
            forceDtsEmit
        );
