namespace ts {
    ...
    export function createProgram(rootNamesOrOptions: readonly string[] | CreateProgramOptions, _options?: CompilerOptions, _host?: CompilerHost, _oldProgram?: Program, _configFileParsingDiagnostics?: readonly Diagnostic[]): Program {
        ...
        tracing.on && tracing.push(tracing.Phase.Program, "createProgram", { configFilePath: options.configFilePath, rootDir: options.rootDir }, /*separateBeginAndEnd*/ true);
        ...
        tracing.on && tracing.push(tracing.Phase.Program, "shouldProgramCreateNewSourceFiles", { hasOldProgram: !!oldProgram });
        const shouldCreateNewSourceFile = shouldProgramCreateNewSourceFiles(oldProgram, options);
        tracing.on && tracing.pop();
        // We set `structuralIsReused` to `undefined` because `tryReuseStructureFromOldProgram` calls `tryReuseStructureFromOldProgram` which checks
        // `structuralIsReused`, which would be a TDZ violation if it was not set in advance to `undefined`.
        let structureIsReused: StructureIsReused;
        tracing.on && tracing.push(tracing.Phase.Program, "tryReuseStructureFromOldProgram", {});
        ...
        if (structureIsReused !== StructureIsReused.Completely) {
            ...
            tracing.on && tracing.push(tracing.Phase.Program, "processRootFiles", { count: rootNames.length });
            ...
            if (typeReferences.length) {
                tracing.on && tracing.push(tracing.Phase.Program, "processTypeReferences", { count: typeReferences.length });
        ...
        function resolveModuleNamesWorker(moduleNames: string[], containingFile: SourceFile, reusedNames: string[] | undefined): readonly ResolvedModuleFull[] {
            ...
            tracing.on && tracing.push(tracing.Phase.Program, "resolveModuleNamesWorker", { containingFileName });
        ...
        function resolveTypeReferenceDirectiveNamesWorker(typeDirectiveNames: string[], containingFile: string | SourceFile): readonly (ResolvedTypeReferenceDirective | undefined)[] {
            ...
            tracing.on && tracing.push(tracing.Phase.Program, "resolveTypeReferenceDirectiveNamesWorker", { containingFileName });
        ...
        function emitBuildInfo(writeFileCallback?: WriteFileCallback): EmitResult {
            ...
            tracing.on && tracing.push(tracing.Phase.Emit, "emitBuildInfo", {}, /*separateBeginAndEnd*/ true);
        ...
        function emit(sourceFile?: SourceFile, writeFileCallback?: WriteFileCallback, cancellationToken?: CancellationToken, emitOnlyDtsFiles?: boolean, transformers?: CustomTransformers, forceDtsEmit?: boolean): EmitResult {
            tracing.on && tracing.push(tracing.Phase.Emit, "emit", { path: sourceFile?.path }, /*separateBeginAndEnd*/ true);
        ...
        function findSourceFile(fileName: string, path: Path, isDefaultLib: boolean, ignoreNoDefaultLib: boolean, reason: FileIncludeReason, packageId: PackageId | undefined): SourceFile | undefined {
            tracing.on && tracing.push(tracing.Phase.Program, "findSourceFile", {
                fileName,
        ...
        function processTypeReferenceDirective(
            ...
            tracing.on && tracing.push(tracing.Phase.Program, "processTypeReferenceDirective", { directive: typeReferenceDirective, hasResolved: !!resolveModuleNamesReusingOldState, refKind: reason.kind, refPath: isReferencedFile(reason) ? reason.file : undefined });
