...
export function isIdentifierPart(ch: number, identifierVariant?: LanguageVariant): boolean {
...
/** @internal */
export function isIdentifierText(name: string, identifierVariant?: LanguageVariant): boolean {
    ...
    for (let i = charSize(ch); i < name.length; i += charSize(ch)) {
        if (!isIdentifierPart(ch = codePointAt(name, i), identifierVariant)) {
            return false;
        }
    }
