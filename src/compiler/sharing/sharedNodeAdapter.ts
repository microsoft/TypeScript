...
import { measureActivity } from "../performance";
...
export function adoptSharedSourceFile(sharedFile: SharedSourceFile, factory: NodeFactory, sharedCache?: ReadonlyMap<ShareableNonPrimitive, object>): SourceFile {
    using _ = measureActivity("Parallel: Struct to Object conversion", "beginStructToObject", "endStructToObject");
    let file = sharedCache?.get(sharedFile) as SourceFile | undefined;
    let cache: Map<ShareableNonPrimitive, object>;
    let needsParent: [SharedNode, Node][];
    if (!file) {
    ...
    function visitNodes<T extends SharedNode, U extends Node>(sharedNodes: SharedNodeArray<T>, visit: (value: T) => U): NodeArray<U> {
    ...
    function finishNode<T extends Node>(node: Mutable<T>, shared: SharedNode): T {
        node.pos = shared.pos;
        node.end = shared.end;
