...
export function transformDeclarations(context: TransformationContext) {
    ...
    let lateStatementReplacementMap: ts.Map<VisitResult<LateVisibilityPaintedStatement | ExportAssignment>>;
        ...
        function mapReferencesIntoArray(references: FileReference[], outputFilePath: string): (file: SourceFile) => void {
            return file => {
                ...
                if (declFileName) {
                    ...
                    { ...options, baseUrl: options.baseUrl && toPath(options.baseUrl, host.getCurrentDirectory(), host.getCanonicalFileName) }, currentSourceFile, toPath(outputFilePath, host.getCurrentDirectory(), host.getCanonicalFileName), toPath(declFileName, host.getCurrentDirectory(), host.getCanonicalFileName), host, host.getSourceFiles(), 
                    ...
                    let fileName = getRelativePathToDirectoryOrUrl(outputFilePath, declFileName, host.getCurrentDirectory(), host.getCanonicalFileName, 
    ...
    function collectReferences(sourceFile: SourceFile | UnparsedSource, ret: ts.Map<SourceFile>) {
    ...
    function collectLibs(sourceFile: SourceFile | UnparsedSource, ret: ts.Map<boolean>) {
    ...
    function ensureParameter(p: ParameterDeclaration, modifierMask?: ModifierFlags, type?: TypeNode): ParameterDeclaration {
        ...
        const newParam = updateParameter(p, 
    ...
    function transformImportEqualsDeclaration(decl: ImportEqualsDeclaration) {
        ...
        if (decl.moduleReference.kind === SyntaxKind.ExternalModuleReference) {
            ...
            return updateImportEqualsDeclaration(decl, 
            /*decorators*/ undefined, decl.modifiers, decl.name, updateExternalModuleReference(decl.moduleReference, rewriteModuleSpecifier(decl, specifier)));
        }
        else {
            const oldDiag = getSymbolAccessibilityDiagnostic;
    ...
    function transformImportDeclaration(decl: ImportDeclaration) {
        if (!decl.importClause) {
            ...
            return updateImportDeclaration(decl, 
        ...
        if (!decl.importClause.namedBindings) {
            ...
            return visibleDefaultBinding && updateImportDeclaration(decl, /*decorators*/ undefined, decl.modifiers, updateImportClause(decl.importClause, visibleDefaultBinding, 
        ...
        if ((bindingList && bindingList.length) || visibleDefaultBinding) {
            return updateImportDeclaration(decl, 
            /*decorators*/ undefined, decl.modifiers, updateImportClause(decl.importClause, visibleDefaultBinding, bindingList && bindingList.length ? updateNamedImports(decl.importClause.namedBindings, bindingList) : undefined, decl.importClause.isTypeOnly), rewriteModuleSpecifier(decl, decl.moduleSpecifier));
        }
        // Nothing visible
    }
        ...
        if (isProcessedComponent(input)) {
            switch (input.kind) {
                ...
                case SyntaxKind.Constructor: {
                    ...
                    const ctor = createSignatureDeclaration(SyntaxKind.Constructor, ensureTypeParams(input, input.typeParameters), updateParamsList(input, input.parameters, ModifierFlags.None), 
                    /*type*/ undefined);
                ...
                case SyntaxKind.MethodDeclaration: {
                    ...
                    const sig = (createSignatureDeclaration(SyntaxKind.MethodSignature, ensureTypeParams(input, input.typeParameters), updateParamsList(input, input.parameters), ensureType(input, input.type)) as MethodSignature);
                    sig.name = input.name;
                ...
                case SyntaxKind.GetAccessor: {
                    ...
                    const accessorType = getTypeAnnotationFromAllAccessorDeclarations(input, resolver.getAllAccessorDeclarations(input));
                    return cleanup(updateGetAccessor(input, 
                    /*decorators*/ undefined, ensureModifiers(input), input.name, updateAccessorParamsList(input, hasModifier(input, ModifierFlags.Private)), ensureType(input, accessorType), 
                    /*body*/ undefined));
                }
                case SyntaxKind.SetAccessor: {
                    ...
                    return cleanup(updateSetAccessor(input, 
                    /*decorators*/ undefined, ensureModifiers(input), input.name, updateAccessorParamsList(input, hasModifier(input, ModifierFlags.Private)), 
                    /*body*/ undefined));
                }
                case SyntaxKind.PropertyDeclaration:
                    ...
                    return cleanup(updateProperty(input, 
                ...
                case SyntaxKind.IndexSignature: {
                    return cleanup(updateIndexSignature(input, 
                    /*decorators*/ undefined, ensureModifiers(input), updateParamsList(input, input.parameters), visitNode(input.type, visitDeclarationSubtree) || createKeywordTypeNode(SyntaxKind.AnyKeyword)));
                }
                case SyntaxKind.VariableDeclaration: {
    ...
    function visitDeclarationStatements(input: Node): VisitResult<Node> {
        ...
        switch (input.kind) {
            case SyntaxKind.ExportDeclaration: {
                ...
                // Rewrite external module names if necessary
                return updateExportDeclaration(input, 
    ...
    function transformTopLevelDeclaration(input: LateVisibilityPaintedStatement) {
        ...
        switch (input.kind) {
            case SyntaxKind.TypeAliasDeclaration: // Type aliases get `declare`d if need be (for legacy support), but that's all
                return cleanup(updateTypeAliasDeclaration(input, 
                /*decorators*/ undefined, ensureModifiers(input), input.name, visitNodes(input.typeParameters, visitDeclarationSubtree, isTypeParameterDeclaration), visitNode(input.type, visitDeclarationSubtree, isTypeNode)));
            case SyntaxKind.InterfaceDeclaration: {
                return cleanup(updateInterfaceDeclaration(input, 
                /*decorators*/ undefined, ensureModifiers(input), input.name, ensureTypeParams(input, input.typeParameters), transformHeritageClauses(input.heritageClauses), visitNodes(input.members, visitDeclarationSubtree)));
            }
            case SyntaxKind.FunctionDeclaration: {
                // Generators lose their generator-ness, excepting their return type
                const clean = cleanup(updateFunctionDeclaration(input, 
                ...
                if (clean && resolver.isExpandoFunctionDeclaration(input)) {
                    ...
                    const cleanDeclaration = updateFunctionDeclaration(clean, 
                    /*decorators*/ undefined, modifiers, 
                    /*asteriskToken*/ undefined, clean.name, clean.typeParameters, clean.parameters, clean.type, 
                    /*body*/ undefined);
                    const namespaceDeclaration = updateModuleDeclaration(namespaceDecl, 
            ...
            case SyntaxKind.ModuleDeclaration: {
                ...
                if (inner && inner.kind === SyntaxKind.ModuleBlock) {
                    ...
                    return cleanup(updateModuleDeclaration(input, 
                    /*decorators*/ undefined, mods, isExternalModuleAugmentation(input) ? rewriteModuleSpecifier(input, input.name) : input.name, body));
                }
                else {
                    needsDeclare = previousNeedsDeclare;
                    const mods = ensureModifiers(input);
                    ...
                    return cleanup(updateModuleDeclaration(input, 
                    /*decorators*/ undefined, mods, input.name, (body as ModuleBody)));
                }
            }
            case SyntaxKind.ClassDeclaration: {
                ...
                if (ctor) {
                    ...
                    parameterProperties = compact(flatMap(ctor.parameters, (param) => {
                        if (!hasModifier(param, ModifierFlags.ParameterPropertyModifier) || shouldStripInternal(param))
                            return;
                        ...
                        function walkBindingPattern(pattern: BindingPattern) {
                            ...
                            for (const elem of pattern.elements) {
                                ...
                                /*decorators*/ undefined, ensureModifiers(param), (elem.name as Identifier), 
                ...
                const privateIdentifier = hasPrivateIdentifier ? [
                    createProperty(
                    /*decorators*/ undefined, 
                    /*modifiers*/ undefined, createPrivateIdentifier("#private"), 
