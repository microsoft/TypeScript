...
export function transformDeclarations(context: TransformationContext) {
    ...
    function trackReferencedAmbientModuleFromImport(
        node: ImportDeclaration | ExportDeclaration | ImportEqualsDeclaration | ImportTypeNode,
    ) {
    ...
    function transformDeclarationsForJS(sourceFile: SourceFile, bundled?: boolean) {
        ...
        getSymbolAccessibilityDiagnostic =
            s => (s.errorNode && canProduceDiagnostics(s.errorNode) ? createGetSymbolAccessibilityDiagnosticForNode(s.errorNode)(s) : ({
    ...
    function transformImportDeclaration(decl: ImportDeclaration) {
        ...
        const visibleDefaultBinding = decl.importClause && decl.importClause.name && resolver.isDeclarationVisible(decl.importClause) ?
        ...
        if ((bindingList && bindingList.length) || visibleDefaultBinding) {
            return factory.updateImportDeclaration(
                ...
                factory.updateImportClause(
                    ...
                    decl.importClause.isTypeOnly,
                    visibleDefaultBinding,
                    bindingList && bindingList.length ? factory.updateNamedImports(decl.importClause.namedBindings, bindingList)
                        : undefined,
                ),
                rewriteModuleSpecifier(decl, decl.moduleSpecifier),
                tryGetResolutionModeOverride(decl.attributes),
                ...
                if (lateStatementReplacementMap.has(key)) {
                    ...
                    if (result) {
                        ...
                        if (
                            ...
                            (isArray(result) ? some(result, isExternalModuleIndicator) : isExternalModuleIndicator(result))
                        ) {
    ...
    function stripExportModifiers(statement: Statement): Statement {
        if (
            isImportEqualsDeclaration(statement) || hasEffectiveModifier(statement, ModifierFlags.Default) || !canHaveModifiers(statement)
        ) {
                    ...
                    const heritageClauses = factory.createNodeArray(map(input.heritageClauses, clause => {
                        if (clause.token === SyntaxKind.ExtendsKeyword) {
                            ...
                            const newClause = factory.updateHeritageClause(
                                ...
                                map(
                                    clause.types,
                                    t => factory.updateExpressionWithTypeArguments(
                                        t,
                                        newId,
                        ...
                        return factory.updateHeritageClause(
                            ...
                            visitNodes(
                                factory.createNodeArray(
                                    filter(
                                        clause.types,
                                        t => isEntityNameExpression(t.expression) || t.expression.kind === SyntaxKind.NullKeyword,
                                    ),
                                ),
