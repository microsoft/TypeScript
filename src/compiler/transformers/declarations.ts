...
namespace ts {
    ...
    export function transformDeclarations(context: TransformationContext) {
        ...
        function reportImportTypeNodeResolutionModeOverride() {
            if (!isNightly() && (errorNameNode || errorFallbackNode)) {
                context.addDiagnostic(
                    createDiagnosticForNode(
                        (errorNameNode || errorFallbackNode)!,
                        Diagnostics.The_type_of_this_expression_cannot_be_named_without_a_resolution_mode_assertion_which_is_an_unstable_feature_Use_nightly_TypeScript_to_silence_this_error_Try_updating_with_npm_install_D_typescript_next
                    )
                );
            }
        }
                        ...
                        if (prepend.kind === SyntaxKind.InputFiles) {
                            ...
                            recordTypeReferenceDirectivesIfNecessary(
                                map(
                                    ...
                                    (ref) => [ref.fileName, ref.resolutionMode]
                                )
                            );
                            collectLibs(sourceFile, libs);
                            return sourceFile;
                ...
                if (emittedImports) {
                    for (const importStatement of emittedImports) {
                        if (
                            ...
                            const expr =
                                importStatement.moduleReference.expression;
                            if (
        ...
        function filterBindingPatternInitializersAndRenamings(
            ...
            function visitBindingElement<T extends ArrayBindingElement>(
                elem: T
            ): T;
            function visitBindingElement(
                ...
                if (
                    elem.propertyName &&
                    isIdentifier(elem.propertyName) &&
                    isIdentifier(elem.name) &&
                    !elem.symbol.isReferenced
                ) {
        ...
        function shouldEmitFunctionProperties(input: FunctionDeclaration) {
            ...
            return (
                !overloadSignatures ||
                overloadSignatures.indexOf(input) ===
                    overloadSignatures.length - 1
            );
        }
        ...
        function transformImportEqualsDeclaration(
            ...
            if (
                decl.moduleReference.kind === SyntaxKind.ExternalModuleReference
            ) {
        ...
        function transformImportDeclaration(decl: ImportDeclaration) {
            if (!decl.importClause) {
                ...
                return factory.updateImportDeclaration(
                    ...
                    getResolutionModeOverrideForClauseInNightly(
                        decl.assertClause
                    )
                );
            }
            ...
            if (
                decl.importClause.namedBindings.kind ===
        ...
        function getResolutionModeOverrideForClauseInNightly(
        ...
        function visitDeclarationSubtree(input: Node): VisitResult<Node> {
            ...
            if (isDeclaration(input)) {
                ...
                if (
                    ...
                    !resolver.isLateBound(
                        getParseTreeNode(input) as Declaration
                    )
                ) {
                    return;
                }
            ...
            if (
                isFunctionLike(input) &&
                resolver.isImplementationOfOverload(input)
            )
                return;
            ...
            if (
                ...
                getLineAndCharacterOfPosition(currentSourceFile, input.pos)
                    .line ===
                    getLineAndCharacterOfPosition(currentSourceFile, input.end)
                        .line
            ) {
        ...
        function transformTopLevelDeclaration(
            ...
            switch (input.kind) {
                ...
                case SyntaxKind.FunctionDeclaration: {
                    ...
                    if (
                        ...
                        if (
                            !hasEffectiveModifier(clean, ModifierFlags.Default)
                        ) {
            ...
            function cleanup<T extends Node>(
                ...
                errorNameNode = undefined;
                return (
                    node && setOriginalNode(preserveJsDoc(node, input), input)
                );
            }
        }
    ...
    function maskModifierFlags(
        ...
        let flags =
            (getEffectiveModifierFlags(node) & modifierMask) |
            modifierAdditions;
