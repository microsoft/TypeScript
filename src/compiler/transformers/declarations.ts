...
namespace ts {
    ...
    export function transformDeclarations(context: TransformationContext) {
        ...
        function transformRoot(node: SourceFile | Bundle) {
            ...
            let combinedStatements = setTextRange(factory.createNodeArray(transformAndReplaceLatePaintedStatements(statements)), node.statements);
            refs.forEach(referenceVisitor);
            emittedImports = filter(combinedStatements, isAnyImportSyntax);
            if (isExternalModule(node) && (!resultHasExternalModuleIndicator || (needsScopeFixMarker && !resultHasScopeMarker))) {
                combinedStatements = setTextRange(factory.createNodeArray([...combinedStatements, createEmptyExports()]), combinedStatements);
            }
            ...
            if (isProcessedComponent(input)) {
                switch (input.kind) {
                    ...
                    case SyntaxKind.VariableDeclaration: {
                        ...
                        return cleanup(factory.updateVariableDeclaration(input, input.name, ensureType(input, input.type), ensureNoInitializer(input)));
                    }
            ...
            switch (input.kind) {
                ...
                case SyntaxKind.ExportAssignment: {
                    ...
                    if (input.expression.kind === SyntaxKind.Identifier) {
                        ...
                        const varDecl = factory.createVariableDeclaration(newId, resolver.createTypeOfExpression(input.expression, input, declarationEmitNodeBuilderFlags, symbolTracker), /*initializer*/ undefined);
            ...
            switch (input.kind) {
                ...
                case SyntaxKind.FunctionDeclaration: {
                    ...
                    if (clean && resolver.isExpandoFunctionDeclaration(input)) {
                        ...
                        const declarations = mapDefined(props, p => {
                            ...
                            getSymbolAccessibilityDiagnostic = oldDiag;
                            const varDecl = factory.createVariableDeclaration(unescapeLeadingUnderscores(p.escapedName), type, /*initializer*/ undefined);
                    ...
                    if (inner && inner.kind === SyntaxKind.ModuleBlock) {
                        ...
                        if (!isGlobalScopeAugmentation(input) && !hasScopeMarker(lateStatements) && !resultHasScopeMarker) {
                            if (needsScopeFixMarker) {
                                lateStatements = factory.createNodeArray([...lateStatements, createEmptyExports()]);
                            }
                            else {
                ...
                case SyntaxKind.ClassDeclaration: {
                    ...
                    if (extendsClause && !isEntityNameExpression(extendsClause.expression) && extendsClause.expression.kind !== SyntaxKind.NullKeyword) {
                        ...
                        const varDecl = factory.createVariableDeclaration(newId, resolver.createTypeOfExpression(extendsClause.expression, input, declarationEmitNodeBuilderFlags, symbolTracker), /*initializer*/ undefined);
                        const statement = factory.createVariableStatement(needsDeclare ? [factory.createModifier(SyntaxKind.DeclareKeyword)] : [], factory.createVariableDeclarationList([varDecl], NodeFlags.Const));
                ...
                case SyntaxKind.EnumDeclaration: {
                    return cleanup(factory.updateEnumDeclaration(input, /*decorators*/ undefined, factory.createNodeArray(ensureModifiers(input)), input.name, factory.createNodeArray(mapDefined(input.members, m => {
                        ...
                        const initializer = typeof constValue === "string" ? factory.createStringLiteral(constValue) :
                            typeof constValue === "number" ? factory.createNumericLiteral(constValue) :
                            undefined;
                        return preserveJsDoc(factory.updateEnumMember(m, m.name, initializer), m);
                    }))));
                }
            }
        ...
        function recreateBindingElement(e: ArrayBindingElement) {
            ...
            if (e.name) {
                ...
                if (isBindingPattern(e.name)) {
                    ...
                    return factory.createVariableDeclaration(e.name, ensureType(e, /*type*/ undefined), /*initializer*/ undefined);
                }
            }
        }
        ...
        function ensureAccessor(node: AccessorDeclaration): PropertyDeclaration | undefined {
            ...
            const prop = factory.createPropertyDeclaration(/*decorators*/ undefined, maskModifiers(node, /*mask*/ undefined, (!accessors.setAccessor) ? ModifierFlags.Readonly : ModifierFlags.None), node.name, node.questionToken, ensureType(node, accessorType), /*initializer*/ undefined);
