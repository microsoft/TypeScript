...
export function transformES2015(context: TransformationContext): (x: SourceFile | Bundle) => SourceFile | Bundle {
    ...
    type ThisCapturingVariableDeclaration = VariableDeclaration & {
        readonly name: CapturedThis;
        readonly initializer: Expression;
    };
    type ThisCapturingVariableStatement = VariableStatement & {
    ...
    type TransformedSuperCall = CallExpression & {
        readonly expression: PropertyAccessExpression & { readonly expression: SyntheticSuper; };
    };
    type TransformedSuperCallWithFallback = BinaryExpression & {
        readonly left: TransformedSuperCall;
        readonly right: ThisExpression;
    };
    type ImplicitSuperCall = BinaryExpression & {
        readonly left: BinaryExpression;
        readonly right: TransformedSuperCall;
    };
    type ImplicitSuperCallWithFallback = BinaryExpression & {
        readonly left: ImplicitSuperCall;
        readonly right: ThisExpression;
    };
    type ThisCapturingTransformedSuperCallWithFallback = ThisCapturingAssignmentExpression & {
        readonly right: TransformedSuperCallWithFallback;
    };
    type ThisCapturingImplicitSuperCallWithFallback = ThisCapturingAssignmentExpression & {
        readonly right: ImplicitSuperCallWithFallback;
    };
    ...
    function isThisCapturingTransformedSuperCallWithFallback(
        node: Node,
    ): node is ThisCapturingTransformedSuperCallWithFallback {
        return isThisCapturingAssignment(node) && isTransformedSuperCallWithFallback(node.right);
    }

    /** Tests whether `node` is an expression like `_this = _super.call(this) || this` */
    function isThisCapturingImplicitSuperCallWithFallback(
        node: Node,
    ): node is ThisCapturingImplicitSuperCallWithFallback {
    ...
    function simplifyConstructorInlineSuperReturn(body: Block, original: Block) {
        ...
        for (const statement of original.statements) {
            if (
                statement.transformFlags & TransformFlags.ContainsLexicalSuper && !getSuperCallFromStatement(statement)
            ) {
                return body;
            }
        }
    ...
    function elideUnusedThisCaptureWorker(node: Node): VisitResult<Node | undefined> {
        ...
        switch (node.kind) {
            ...
            case SyntaxKind.PropertyDeclaration: {
                const named = node as AccessorDeclaration | MethodDeclaration | PropertyDeclaration;
                if (isComputedPropertyName(named.name)) {
                    return factory.replacePropertyName(
                        named,
                        visitEachChild(named.name, elideUnusedThisCaptureWorker, /*context*/ undefined),
                    );
                }
                return node;
            }
        ...
        switch (node.kind) {
            ...
            case SyntaxKind.PropertyDeclaration: {
                ...
                if (isComputedPropertyName(named.name)) {
                    return factory.replacePropertyName(
                        named,
                        visitEachChild(named.name, injectSuperPresenceCheckWorker, /*context*/ undefined),
                    );
                }
                return node;
            }
