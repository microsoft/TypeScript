import { ClassDeclaration, MethodDeclaration, GetAccessorDeclaration, SetAccessorDeclaration, ConstructorDeclaration, TransformationContext, getEmitScriptTarget, NodeCheckFlags, UnderscoreEscapedMap, chainBundle, SourceFile, isEffectiveStrictModeSourceFile, visitEachChild, addEmitHelpers, Node, VisitResult, TransformFlags, SyntaxKind, AwaitExpression, FunctionDeclaration, FunctionExpression, ArrowFunction, isPropertyAccessExpression, ElementAccessExpression, isNodeWithPossibleHoistedDeclaration, Debug, CatchClause, createUnderscoreEscapedMap, cloneMap, VariableStatement, createExpressionStatement, ForInStatement, updateForIn, visitNode, isForInitializer, isExpression, isStatement, liftToBlock, ForOfStatement, updateForOf, isToken, ForStatement, updateFor, Expression, setOriginalNode, setTextRange, createYield, updateMethod, visitNodes, isModifier, visitParameterList, getFunctionFlags, FunctionFlags, visitFunctionBody, Statement, updateFunctionDeclaration, updateFunctionExpression, updateArrowFunction, ParameterDeclaration, VariableDeclaration, BindingElement, isIdentifier, isOmittedExpression, ForInitializer, VariableDeclarationList, isVariableDeclarationList, NodeFlags, getInitializedVariables, convertToAssignmentElementTarget, inlineExpressions, map, forEach, setSourceMapRange, createAssignment, AccessorDeclaration, FunctionBody, ConciseBody, FunctionLikeDeclaration, getOriginalNode, isFunctionLike, ScriptTarget, addPrologue, Block, createReturn, insertStatementsAfterStandardPrologue, hasEntries, getNodeId, createBlock, addEmitHelper, some, convertToFunctionBody, updateBlock, createNodeArray, concatenate, isBlock, isConciseBody, TypeNode, getEntityNameFromTypeNode, isEntityName, TypeReferenceSerializationKind, EmitHint, PropertyAccessExpression, CallExpression, createPropertyAccess, createFileLevelUniqueName, isSuperProperty, createCall, createThis, TextRange, LeftHandSideExpression, EmitResolver, PropertyAssignment, unescapeLeadingUnderscores, createPropertyAssignment, createArrowFunction, setEmitFlags, createSuper, EmitFlags, createParameter, createIdentifier, createObjectLiteral, createVariableStatement, createVariableDeclarationList, createVariableDeclaration, createNull, UnscopedEmitHelper, EntityName, createFunctionExpression, createToken, EmitNode, getUnscopedHelperName, createVoidZero, createExpressionFromEntityName, EmitHelper, helperString } from "../ts";
/*@internal*/
...
export function transformES2017(context: TransformationContext) {
    ...
    function visitForInStatementInAsyncBody(node: ForInStatement) {
        return updateForIn(node, isVariableDeclarationListWithCollidingName(node.initializer)
            ? visitVariableDeclarationListWithCollidingNames(node.initializer, /*hasReceiver*/ true)!
            : visitNode(node.initializer, visitor, isForInitializer), visitNode(node.expression, visitor, isExpression), visitNode(node.statement, asyncBodyVisitor, isStatement, liftToBlock));
    }
    function visitForOfStatementInAsyncBody(node: ForOfStatement) {
        return updateForOf(node, visitNode(node.awaitModifier, visitor, isToken), isVariableDeclarationListWithCollidingName(node.initializer)
            ? visitVariableDeclarationListWithCollidingNames(node.initializer, /*hasReceiver*/ true)!
            : visitNode(node.initializer, visitor, isForInitializer), visitNode(node.expression, visitor, isExpression), visitNode(node.statement, asyncBodyVisitor, isStatement, liftToBlock));
    }
    function visitForStatementInAsyncBody(node: ForStatement) {
        const initializer = node.initializer!; // TODO: GH#18217
        return updateFor(node, isVariableDeclarationListWithCollidingName(initializer)
            ? visitVariableDeclarationListWithCollidingNames(initializer, /*hasReceiver*/ false)
            : visitNode(node.initializer, visitor, isForInitializer), visitNode(node.condition, visitor, isExpression), visitNode(node.incrementor, visitor, isExpression), visitNode(node.statement, asyncBodyVisitor, isStatement, liftToBlock));
    }
    /**
    ...
    function visitAwaitExpression(node: AwaitExpression): Expression {
        ...
        return setOriginalNode(setTextRange(createYield(
    ...
    function visitMethodDeclaration(node: MethodDeclaration) {
        ...
        /*decorators*/ undefined, visitNodes(node.modifiers, visitor, isModifier), node.asteriskToken, node.name, 
        /*questionToken*/ undefined, 
        /*typeParameters*/ undefined, visitParameterList(node.parameters, visitor, context), 
        /*type*/ undefined, getFunctionFlags(node) & FunctionFlags.Async
    ...
    function visitFunctionExpression(node: FunctionExpression): Expression {
        return updateFunctionExpression(node, visitNodes(node.modifiers, visitor, isModifier), node.asteriskToken, node.name, 
        /*typeParameters*/ undefined, visitParameterList(node.parameters, visitor, context), 
        /*type*/ undefined, getFunctionFlags(node) & FunctionFlags.Async
            ? transformAsyncFunctionBody(node)
            : visitFunctionBody(node.body, visitor, context));
    ...
    function visitArrowFunction(node: ArrowFunction) {
        return updateArrowFunction(node, visitNodes(node.modifiers, visitor, isModifier), 
        /*typeParameters*/ undefined, visitParameterList(node.parameters, visitor, context), 
        /*type*/ undefined, node.equalsGreaterThanToken, getFunctionFlags(node) & FunctionFlags.Async
            ? transformAsyncFunctionBody(node)
            : visitFunctionBody(node.body, visitor, context));
    }
    ...
    function transformAsyncFunctionBody(node: FunctionLikeDeclaration): ConciseBody {
        ...
        if (!isArrowFunction) {
            ...
            statements.push(createReturn(createAwaiterHelper(context, inHasLexicalThisContext(), hasLexicalArguments, promiseConstructor, transformAsyncFunctionBodyWorker((<Block>node.body), statementOffset))));
    ...
    function createSuperElementAccessInAsyncMethod(argumentExpression: Expression, location: TextRange): LeftHandSideExpression {
        if (enclosingSuperContainerFlags & NodeCheckFlags.AsyncMethodWithSuperBinding) {
            return setTextRange(createPropertyAccess(createCall(createFileLevelUniqueName("_superIndex"), 
            /*typeArguments*/ undefined, [argumentExpression]), "value"), location);
        }
        else {
            return setTextRange(createCall(createFileLevelUniqueName("_superIndex"), 
...
export function createSuperAccessVariableStatement(resolver: EmitResolver, node: FunctionLikeDeclaration, names: UnderscoreEscapedMap<true>) {
    ...
    names.forEach((_, key) => {
        ...
        getterAndSetter.push(createPropertyAssignment("get", createArrowFunction(
        /* modifiers */ undefined, 
        /* typeParameters */ undefined, 
        /* parameters */ [], 
        /* type */ undefined, 
        /* equalsGreaterThanToken */ undefined, setEmitFlags(createPropertyAccess(setEmitFlags(createSuper(), EmitFlags.NoSubstitution), name), EmitFlags.NoSubstitution))));
        if (hasBinding) {
            getterAndSetter.push(createPropertyAssignment("set", createArrowFunction(
            /* modifiers */ undefined, 
            /* typeParameters */ undefined, 
            /* parameters */ [
                createParameter(
            ...
            /* type */ undefined, 
            /* equalsGreaterThanToken */ undefined, createAssignment(setEmitFlags(createPropertyAccess(setEmitFlags(createSuper(), EmitFlags.NoSubstitution), name), EmitFlags.NoSubstitution), createIdentifier("v")))));
        }
    ...
    /* modifiers */ undefined, createVariableDeclarationList([
        createVariableDeclaration(createFileLevelUniqueName("_super"), 
        /* type */ undefined, createCall(createPropertyAccess(createIdentifier("Object"), "create"), 
        /* typeArguments */ undefined, [
            createNull(),
...
function createAwaiterHelper(context: TransformationContext, hasLexicalThis: boolean, hasLexicalArguments: boolean, promiseConstructor: EntityName | Expression | undefined, body: Block) {
    ...
    /*modifiers*/ undefined, createToken(SyntaxKind.AsteriskToken), 
    /*name*/ undefined, 
    /*typeParameters*/ undefined, 
    /*parameters*/ [], 
    /*type*/ undefined, body);
    ...
    return createCall(getUnscopedHelperName("__awaiter"), 
    /*typeArguments*/ undefined, [
