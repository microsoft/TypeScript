...
export function transformESDecorators(
    ...
    function transformClassLike(node: ClassLikeDeclaration) {
        ...
        const extendsExpression = extendsElement &&
        ...
        if (classInfo.staticExtraInitializersName) {
            ...
            setSourceMapRange(
                runStaticInitializersStatement,
        ...
        if (classDecorators) {
            ...
            const classReferenceVarDeclList = factory.createVariableDeclarationList([
    ...
    function prepareConstructor(
        ...
        if (
            classInfo.instanceExtraInitializersName &&
            !classInfo.hasNonAmbientInstanceFields
        ) {
    ...
    >(
        ...
        if (memberDecorators) {
            ...
            // 7. Static field (excl. auto-accessor) element decorators are applied
            // 8. Non-static field (excl. auto-accessor) element decorators are applied
            const statements = isMethodOrAccessor(member) ||
                    isAutoAccessorPropertyDeclaration(member)
                ? isStatic(member)
                    ? (classInfo.staticNonFieldDecorationStatements ??= [])
                    : (classInfo.nonStaticNonFieldDecorationStatements ??= [])
            ...
            if (isMethodOrAccessor(member)) {
                ...
                if (
                    ...
                    memberInfo.memberDescriptorName =
                        descriptorName =
    ...
    function visitPropertyDeclaration(node: PropertyDeclaration) {
        ...
        if (
            !isStatic(node) &&
            classInfo?.instanceExtraInitializersName &&
            !classInfo?.hasInjectedInstanceInitializers
        ) {
            ...
            initializer ??= factory.createVoidZero();
            initializer = factory.createParenthesizedExpression(
                factory.createComma(
                    emitHelpers().createRunInitializersHelper(
                        factory.createThis(),
                        classInfo.instanceExtraInitializersName,
