import { Expression, Statement, Identifier, TransformationContext, getEmitScriptTarget, LiteralExpression, TextRange, CaseClause, chainBundle, SourceFile, TransformFlags, visitEachChild, addEmitHelpers, Node, VisitResult, isFunctionLikeDeclaration, SyntaxKind, DoStatement, WhileStatement, SwitchStatement, LabeledStatement, FunctionDeclaration, FunctionExpression, AccessorDeclaration, VariableStatement, ForStatement, ForInStatement, BreakStatement, ContinueStatement, ReturnStatement, BinaryExpression, ConditionalExpression, YieldExpression, ArrayLiteralExpression, ObjectLiteralExpression, ElementAccessExpression, CallExpression, NewExpression, Debug, setOriginalNode, setTextRange, createFunctionDeclaration, visitParameterList, createFunctionExpression, Block, createTempVariable, addPrologue, insertStatementsAfterStandardPrologue, createReturn, createBlock, getEmitFlags, EmitFlags, getInitializedVariables, setSourceMapRange, createExpressionStatement, inlineExpressions, map, getExpressionAssociativity, Associativity, updatePropertyAccess, PropertyAccessExpression, visitNode, isLeftHandSideExpression, updateElementAccess, isExpression, isCompoundAssignment, createAssignment, createBinary, getNonAssignmentOperatorForCompoundAssignment, updateBinary, isLogicalOperator, getMutableClone, isBinaryExpression, LeftHandSideExpression, createValuesHelper, NodeArray, visitNodes, createArrayLiteral, reduceLeft, createArrayConcat, createObjectLiteral, isObjectLiteralElementLike, startOnNewLine, ObjectLiteralElementLike, createExpressionForObjectLiteralElementLike, isImportCall, forEach, createCallBinding, createFunctionApply, createPropertyAccess, createNew, createVoidZero, isBlock, ExpressionStatement, IfStatement, WithStatement, ThrowStatement, TryStatement, isStatement, VariableDeclarationList, getSynthesizedClone, setCommentRange, VariableDeclaration, isVariableDeclarationList, updateFor, liftToBlock, createLoopVariable, createForIn, createCall, createLiteral, createLessThan, createElementAccess, createPostfixIncrement, updateForIn, idText, createCaseClause, createSwitch, createCaseBlock, EmitHint, isIdentifier, isGeneratedIdentifier, getOriginalNode, getOriginalNodeId, createUniqueName, lastOrUndefined, createMap, createOmittedExpression, NumericLiteral, addSyntheticTrailingComment, setEmitFlags, createParameter, createWith, createThrow, createIf, createLogicalNot, getUnscopedHelperName, createThis, UnscopedEmitHelper } from "../ts";
...
export function transformGenerators(context: TransformationContext) {
    ...
    function visitFunctionDeclaration(node: FunctionDeclaration): Statement | undefined {
        ...
        if (node.asteriskToken) {
            node = setOriginalNode(setTextRange(createFunctionDeclaration(
    ...
    function visitFunctionExpression(node: FunctionExpression): Expression {
        // Currently, we only support generators that were originally async functions.
        if (node.asteriskToken) {
            node = setOriginalNode(setTextRange(createFunctionExpression(
            /*modifiers*/ undefined, 
            /*asteriskToken*/ undefined, node.name, 
            /*typeParameters*/ undefined, visitParameterList(node.parameters, visitor, context), 
            /*type*/ undefined, transformGeneratorFunctionBody(node.body)), 
    ...
    function visitElements(elements: NodeArray<Expression>, leadingElement?: Expression, location?: TextRange, multiLine?: boolean) {
        ...
        if (numInitialElements > 0) {
            ...
            emitAssignment(temp, createArrayLiteral(leadingElement
    ...
    function visitCallExpression(node: CallExpression) {
        if (!isImportCall(node) && forEach(node.arguments, containsYield)) {
            ...
            return setOriginalNode(createFunctionApply(cacheExpression(visitNode(target, visitor, isLeftHandSideExpression)), thisArg, visitElements(node.arguments), 
            /*location*/ node), node);
        }
        return visitEachChild(node, visitor, context);
    }
    function visitNewExpression(node: NewExpression) {
        if (forEach(node.arguments, containsYield)) {
            ...
            return setOriginalNode(setTextRange(createNew(createFunctionApply(cacheExpression(visitNode(target, visitor, isExpression)), thisArg, visitElements((node.arguments!), 
    ...
    function visitForStatement(node: ForStatement) {
        ...
        if (initializer && isVariableDeclarationList(initializer)) {
            ...
            node = updateFor(node, variables.length > 0
                ? inlineExpressions(map(variables, transformInitializedVariable))
                : undefined, visitNode(node.condition, visitor, isExpression), visitNode(node.incrementor, visitor, isExpression), visitNode(node.statement, visitor, isStatement, liftToBlock));
        }
        else {
            node = visitEachChild(node, visitor, context);
        }
    ...
    function transformAndEmitForInStatement(node: ForInStatement) {
        ...
        if (containsYield(node)) {
            ...
            emitStatement(createForIn(key, visitNode(node.expression, visitor, isExpression), createExpressionStatement(createCall(createPropertyAccess(keysArray, "push"), 
            ...
            while (clausesWritten < numClauses) {
                ...
                for (let i = clausesWritten; i < numClauses; i++) {
                    ...
                    if (clause.kind === SyntaxKind.CaseClause) {
                        ...
                        pendingClauses.push(createCaseClause(visitNode(clause.expression, visitor, isExpression), [
    ...
    function transformAndEmitThrowStatement(node: ThrowStatement): void {
        emitThrow(visitNode(node.expression, visitor, isExpression), 
        /*location*/ node);
    }
    ...
    function createInlineBreak(label: Label, location?: TextRange): ReturnStatement {
        ...
        return setTextRange(createReturn(createArrayLiteral([
    ...
    function createInlineReturn(expression?: Expression, location?: TextRange): ReturnStatement {
        return setTextRange(createReturn(createArrayLiteral(expression
    ...
    function createGeneratorResume(location?: TextRange): LeftHandSideExpression {
        return setTextRange(createCall(createPropertyAccess(state, "sent"), 
    ...
    function build() {
        ...
        return createGeneratorHelper(context, setEmitFlags(createFunctionExpression(
        /*modifiers*/ undefined, 
        /*asteriskToken*/ undefined, 
        /*name*/ undefined, 
        /*typeParameters*/ undefined, [createParameter(/*decorators*/ undefined, /*modifiers*/ undefined, /*dotDotDotToken*/ undefined, state)], 
        /*type*/ undefined, createBlock(buildResult, 
    ...
    function appendLabel(markLabelEnd: boolean): void {
        ...
        if (statements) {
            ...
            if (currentExceptionBlock) {
                ...
                statements.unshift(createExpressionStatement(createCall(createPropertyAccess(createPropertyAccess(state, "trys"), "push"), 
    ...
    function writeReturn(expression: Expression | undefined, operationLocation: TextRange | undefined): void {
        lastOperationWasAbrupt = true;
        lastOperationWasCompletion = true;
        writeStatement(setEmitFlags(setTextRange(createReturn(createArrayLiteral(expression
            ? [createInstruction(Instruction.Return), expression]
            : [createInstruction(Instruction.Return)])), operationLocation), EmitFlags.NoTokenSourceMaps));
    }
    /**
    ...
    function writeBreak(label: Label, operationLocation: TextRange | undefined): void {
        lastOperationWasAbrupt = true;
        writeStatement(setEmitFlags(setTextRange(createReturn(createArrayLiteral([
            createInstruction(Instruction.Break),
            createLabel(label)
    ...
    function writeBreakWhenTrue(label: Label, condition: Expression, operationLocation: TextRange | undefined): void {
        writeStatement(setEmitFlags(createIf(condition, setEmitFlags(setTextRange(createReturn(createArrayLiteral([
            createInstruction(Instruction.Break),
            createLabel(label)
    ...
    function writeBreakWhenFalse(label: Label, condition: Expression, operationLocation: TextRange | undefined): void {
        writeStatement(setEmitFlags(createIf(createLogicalNot(condition), setEmitFlags(setTextRange(createReturn(createArrayLiteral([
            createInstruction(Instruction.Break),
            createLabel(label)
        ])), operationLocation), EmitFlags.NoTokenSourceMaps)), EmitFlags.SingleLine));
    }
    ...
    function writeYield(expression: Expression, operationLocation: TextRange | undefined): void {
        ...
        writeStatement(setEmitFlags(setTextRange(createReturn(createArrayLiteral(expression
            ...
            : [createInstruction(Instruction.Yield)])), operationLocation), EmitFlags.NoTokenSourceMaps));
    }
    /**
    ...
    function writeEndfinally(): void {
        lastOperationWasAbrupt = true;
        writeStatement(createReturn(createArrayLiteral([
...
function createGeneratorHelper(context: TransformationContext, body: FunctionExpression) {
    ...
    return createCall(getUnscopedHelperName("__generator"), 
    /*typeArguments*/ undefined, [createThis(), body]);
}
