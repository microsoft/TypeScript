...
namespace ts {
    ...
    export function transformGenerators(context: TransformationContext) {
        ...
        function transformAndEmitContinueStatement(node: ContinueStatement): void {
            const label = findContinueTarget(node.label ? unescapeIdentifier(node.label.text) : undefined);
        ...
        function visitContinueStatement(node: ContinueStatement): Statement {
            if (inStatementContainingYield) {
                const label = findContinueTarget(node.label && unescapeIdentifier(node.label.text));
                if (label > 0) {
        ...
        function transformAndEmitBreakStatement(node: BreakStatement): void {
            const label = findBreakTarget(node.label ? unescapeIdentifier(node.label.text) : undefined);
        ...
        function visitBreakStatement(node: BreakStatement): Statement {
            if (inStatementContainingYield) {
                const label = findBreakTarget(node.label && unescapeIdentifier(node.label.text));
                if (label > 0) {
                    return createInlineBreak(label, /*location*/ node);
                }
            }
        ...
        function transformAndEmitLabeledStatement(node: LabeledStatement) {
            if (containsYield(node)) {
                ...
                //  .endlabeled
                //  .mark endLabel
                beginLabeledBlock(unescapeIdentifier(node.label.text));
                transformAndEmitEmbeddedStatement(node.statement);
                endLabeledBlock();
            }
            else {
        ...
        function visitLabeledStatement(node: LabeledStatement) {
            if (inStatementContainingYield) {
                beginScriptLabeledBlock(unescapeIdentifier(node.label.text));
            }
        ...
        function substituteExpressionIdentifier(node: Identifier) {
            if (!isGeneratedIdentifier(node) && renamedCatchVariables && renamedCatchVariables.has(unescapeIdentifier(node.text))) {
        ...
        function beginCatchBlock(variable: VariableDeclaration): void {
            ...
            if (isGeneratedIdentifier(variable.name)) {
                ...
                const text = unescapeIdentifier((<Identifier>variable.name).text);
