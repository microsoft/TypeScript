...
/*@internal*/
namespace ts {
    export function toES6(transformer: Transformer, statements: NodeArray<Statement>) {
        let { 
            accept,
            visitNode, 
            visitNodes, 
        ...
        */
        ...
        function emitConstructor(node: ClassLikeDeclaration, write: (node: ClassElement) => void): void {
            ...
            emitNode(node, emitConstructorBody, statements, PipelineFlags.LexicalEnvironment)
        ...
        function transformFunctionBody(node: FunctionLikeDeclaration): Block | Expression {
            return isAsyncFunctionLike(node)
                ...
                : <Block | Expression>visitNode(node.body, transformNode, PipelineFlags.LexicalEnvironment);
        }
        ...
        function transformModuleDeclaration(node: ModuleDeclaration, write: (node: Statement) => void) {
            ...
            if (isModuleBlock(body)) {
                ...
                let inner = visitNode<ModuleDeclaration, Statement>(body, transformNode, PipelineFlags.StatementOrBlock);
