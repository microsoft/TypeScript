import { CompilerOptionsValue, ResolvedConfigFileName, Path, createMap, fileExtensionIs, Extension, BuilderProgram, ProgramHost, ParsedCommandLine, DiagnosticReporter, WatchHost, Diagnostic, CancellationToken, ExitStatus, UpToDateStatus, ConfigFileProgramReloadLevel, System, formatColorAndReset, getLocaleTimeString, ForegroundColorEscapeSequences, flattenDiagnosticMessageText, CreateProgram, createProgramHost, returnUndefined, noop, createDiagnosticReporter, maybeBind, EmitAndSemanticDiagnosticsBuilderProgram, sys, WatchStatusReporter, createWatchHost, copyProperties, CompilerOptions, commonOptionsWithBuild, hasProperty, WatchOptions, CompilerHost, GetCanonicalFileName, ParseConfigFileHost, ExtendedConfigCacheEntry, ModuleResolutionCache, WildcardDirectoryWatcher, FileWatcher, WatchFile, WatchType, WatchFilePath, WatchDirectory, createGetCanonicalFileName, createCompilerHostFromProgramHost, setGetSourceFileAsHashVersioned, createModuleResolutionCache, ResolvedProjectReference, resolveModuleName, loadWithLocalCache, ResolvedModuleFull, Debug, createWatchFactory, parseConfigHostFromCompilerHostLike, createCompilerDiagnostic, Diagnostics, getParsedCommandLineOfConfigFile, resolveConfigFileProjectName, resolvePath, emptyArray, arrayToSet, mutateMapSkippingNewValues, closeFileWatcher, closeFileWatcherOf, findIndex, changeCompilerHostLikeToUseCache, WriteFileCallback, CustomTransformers, Program, SourceFile, AffectedFileResult, EmitResult, identity, SemanticDiagnosticsBuilderProgram, getConfigFileParsingDiagnostics, OutputFile, emitFilesAndReportErrors, createDiagnosticCollection, writeFile, getFirstProjectOutput, DiagnosticCollection, listFiles, UpToDateStatusType, emitUsingBuildInfo, isString, assertType, isIncrementalCompilation, arrayIsEqualTo, getFileNamesFromConfigSpecs, getDirectoryPath, updateErrorForNoInputFiles, canJsonReportNoInutFiles, readBuilderProgram, Status, missingFileModifiedTime, getAllProjectOutputs, resolveProjectReferencePath, forEach, getTsBuildInfoEmitOutputFilePath, getBuildInfo, version, DiagnosticMessage, PollingInterval, comparePaths, Comparison, removeFileExtension, containsPath, updateWatchingWildcardDirectories, createMapFromTemplate, hasExtension, isSupportedSourceFileName, mutateMap, arrayToMap, clearMap, arrayFrom, mapDefinedIterator, convertToRelativePath, getErrorCountForSummary, getWatchErrorSummaryDiagnosticMessage } from "./ts";
...
interface FileMap<T, U extends Path = Path> extends ts.Map<T> {
...
function getOrCreateValueMapFromConfigFileMap<T>(configFileMap: ConfigFileMap<ts.Map<T>>, resolved: ResolvedConfigFilePath): ts.Map<T> {
    return getOrCreateValueFromConfigFileMap<ts.Map<T>>(configFileMap, resolved, createMap);
}
...
interface SolutionBuilderState<T extends BuilderProgram = BuilderProgram> {
    ...
    readonly extendedConfigCache: ts.Map<ExtendedConfigCacheEntry>;
    ...
    readonly allWatchedWildcardDirectories: ConfigFileMap<ts.Map<WildcardDirectoryWatcher>>;
    readonly allWatchedInputFiles: ConfigFileMap<ts.Map<FileWatcher>>;
...
function createBuildOrUpdateInvalidedProject<T extends BuilderProgram>(kind: InvalidatedProjectKind.Build | InvalidatedProjectKind.UpdateBundle, state: SolutionBuilderState<T>, project: ResolvedConfigFileName, projectPath: ResolvedConfigFilePath, projectIndex: number, config: ParsedCommandLine, buildOrder: readonly ResolvedConfigFileName[]): BuildInvalidedProject<T> | UpdateBundleProject<T> {
    ...
    function finishEmit(emitterDiagnostics: DiagnosticCollection, emittedOutputs: FileMap<string>, priorNewestUpdateTime: Date, newestDeclarationFileContentChangedTimeIsMaximumDate: boolean, oldestOutputFileName: string, resultFlags: BuildResultFlags) {
...
function buildErrors<T extends BuilderProgram>(state: SolutionBuilderState<T>, resolvedPath: ResolvedConfigFilePath, program: T | undefined, config: ParsedCommandLine, diagnostics: readonly Diagnostic[], errorFlags: BuildResultFlags, errorType: string) {
...
function watchWildCardDirectories(state: SolutionBuilderState, resolved: ResolvedConfigFileName, resolvedPath: ResolvedConfigFilePath, parsed: ParsedCommandLine) {
    if (!state.watch)
        return;
    updateWatchingWildcardDirectories(getOrCreateValueMapFromConfigFileMap(state.allWatchedWildcardDirectories, resolvedPath), createMapFromTemplate(parsed.configFileSpecs!.wildcardDirectories), (dir, flags) => state.watchDirectory(state.hostWithWatch, dir, fileOrDirectory => {
