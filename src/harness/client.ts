import { LanguageServiceHost, RenameInfo, RenameLocation, Debug, LanguageService, createMap, computeLineStarts, getSnapshotText, computePositionOfLineAndCharacter, computeLineAndCharacterOfPosition, TextChange, UserPreferences, QuickInfo, CompletionInfo, CompletionEntry, ScriptElementKind, FormatCodeOptions, FormatCodeSettings, CompletionEntryDetails, map, Symbol, notImplemented, NavigateToItem, PatternMatchKind, DefinitionInfo, DefinitionInfoAndBoundSpan, ImplementationLocation, ReferencedSymbol, ReferenceEntry, EmitOutput, DiagnosticWithLocation, Diagnostic, SourceFile, firstDefined, DiagnosticCategory, isString, RenameInfoOptions, identity, RenameInfoSuccess, createTextSpanFromBounds, RenameInfoFailure, NavigationBarItem, NavigationTree, TextSpan, SignatureHelpItems, DocumentHighlights, OutliningSpan, TodoCommentDescriptor, TodoComment, TextInsertion, CodeFixAction, CodeActionCommand, TextRange, ApplicableRefactorInfo, RefactorEditInfo, FileTextChanges, OrganizeImportsScope, EditorOptions, ClassifiedSpan, Classifications, CallHierarchyItem, mapOneOrMany, CallHierarchyIncomingCall, CallHierarchyOutgoingCall, Program } from "./ts";
...
export class SessionClient implements LanguageService {
    private sequence = 0;
    private lineMaps: ts.Map<number[]> = createMap<number[]>();
    ...
    private getDiagnostics(file: string, command: CommandNames): DiagnosticWithLocation[] {
        ...
        return (<protocol.DiagnosticWithLinePosition[]>response.body).map((entry): DiagnosticWithLocation => {
            const category = firstDefined(Object.keys(DiagnosticCategory), id => isString(id) && entry.category === id.toLowerCase() ? (<any>DiagnosticCategory)[id] : undefined);
            return {
                file: fakeSourceFile,
                start: entry.start,
                length: entry.length,
