import { combine, resolve, dirname } from "./vpath";
import { srcFolder, FileSet, createFromFileSystem, FileSystem } from "./vfs";
import { hasProperty, CompilerOptions, ScriptTarget, ModuleKind, some, formatDiagnostics, getEmitModuleKind, isExternalModuleNameRelative, isUrl, normalizeSlashes } from "./ts";
...
export function evaluateTypeScript(source: string | {
    files: FileSet;
    rootFiles: string[];
    main: string;
}, options?: CompilerOptions, globals?: Record<string, any>) {
    if (typeof source === "string")
        source = { files: { [sourceFile]: source }, rootFiles: [sourceFile], main: sourceFile };
    const fs = createFromFileSystem(IO, /*ignoreCase*/ false, { files: source.files });
    ...
    const result = compileFiles(host, source.rootFiles, compilerOptions);
...
export function evaluateJavaScript(sourceText: string, globals?: Record<string, any>, sourceFile = sourceFileJs) {
    globals = { Symbol: FakeSymbol, ...globals };
    const fs = new FileSystem(/*ignoreCase*/ false, { files: { [sourceFile]: sourceText } });
    return new CommonJsLoader(fs, globals).import(sourceFile);
}

function getLoader(compilerOptions: CompilerOptions, fs: FileSystem, globals: Record<string, any>): Loader<unknown> {
    ...
    switch (moduleKind) {
        ...
        default:
            throw new Error(`ModuleKind '${ModuleKind[moduleKind]}' not supported by evaluator.`);
    }
}
...
abstract class Loader<TModule> {
    ...
    constructor(fs: FileSystem, globals: Record<string, any>) {
        this.fs = fs;
    ...
    protected load(file: string): TModule {
        if (!isExternalModuleNameRelative(file))
    ...
    import(id: string, base = this.fs.cwd()) {
        if (!isExternalModuleNameRelative(id))
...
class CommonJsLoader extends Loader<CommonJSModule> {
    ...
    private resolveIndex(dir: string) {
        const indexFile = resolve(dir, "index.js");
    ...
    private resolveAsDirectory(dir: string) {
        ...
        if (this.isFile(packageFile)) {
            ...
            if (json.main) {
                const main = resolve(dir, json.main);
    ...
    protected evaluate(text: string, file: string, module: CommonJSModule): void {
        ...
        for (const name in this.globals) {
            if (hasProperty(this.globals, name)) {
        ...
        evaluateThunk.call(this.globals, module, module.exports, localRequire, dirname(file), file, ...globalArgs);
    }
}
...
class SystemLoader extends Loader<SystemModule> {
    ...
    private instantiateModule(module: SystemModule, dependencies: string[], registration?: SystemModuleRegisterCallback) {
        ...
        const context: SystemModuleContext = {
            ...
            meta: {
                url: isUrl(module.file) ? module.file : `file:///${normalizeSlashes(module.file).replace(/^\//, "").split("/").map(encodeURIComponent).join("/")}`
            }
        };
            ...
            for (;;) {
                switch (module.state) {
                    ...
                    case SystemModuleState.Instantiated: {
                        ...
                        const base = dirname(module.file);
