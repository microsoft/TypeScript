...
function verifyResolutionsOfProgram(expectedProgram: ts.Program, project: ts.server.Project) {
    ...
    ts.Debug.assert(actual === expected, `Program verification:: ${project.projectName}`);
}
...
function getCacheDetails<T extends ts.ResolvedModuleWithFailedLookupLocations | ts.ResolvedTypeReferenceDirectiveWithFailedLookupLocations>(
    ...
    cache.forEach((resolved, key, mode) => baseline.push(`${indent}  ${key}: ${readableResolutionMode(mode, ":")}${getResolvedFileName(resolved)}`));
}
...
function getProgramStructure(program: ts.Program | undefined) {
    ...
    program?.getSourceFiles().slice().sort((f1, f2) => ts.comparePathsCaseSensitive(f1.path, f2.path)).forEach(f => {
        ...
        getCacheDetails(baseline, "Modules", program.resolvedModules?.get(f.path), r => r.resolvedModule?.resolvedFileName, "    ");
        getCacheDetails(baseline, "TypeRefs", program.resolvedTypeReferenceDirectiveNames?.get(f.path), r => r.resolvedTypeReferenceDirective?.resolvedFileName, "    ");
    });
