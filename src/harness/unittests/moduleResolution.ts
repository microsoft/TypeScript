...
namespace ts {
    ...
    function createModuleResolutionHost(hasDirectoryExists: boolean, ...files: File[]): ModuleResolutionHost {
        const map = Map.from(files, f => f.name);

        if (hasDirectoryExists) {
            const directories = Map.create<string>();
            ...
            return {
                ...
                directoryExists: path => {
                    return Map.has(directories, path);
                },
                fileExists: path => {
                    assert.isTrue(Map.has(directories, getDirectoryPath(path)), `'fileExists' '${path}' request in non-existing directory`);
                    return Map.has(map, path);
                }
            };
        }
        else {
            return { readFile, fileExists: path => Map.has(map, path), };
        }
        function readFile(path: string): string {
            return Map.has(map, path) ? map[path].content : undefined;
        }
    }
        ...
        function test(files: MapLike<string>, currentDirectory: string, rootFiles: string[], expectedFilesCount: number, relativeNamesToCheck: string[]) {
            ...
            const host: CompilerHost = {
                getSourceFile: (fileName: string, languageVersion: ScriptTarget) => {
                    ...
                    return MapLike.has(files, path) ? createSourceFile(fileName, files[path], languageVersion) : undefined;
                },
    ...
    describe("Files with different casing", () => {
        ...
        function test(files: MapLike<string>, options: CompilerOptions, currentDirectory: string, useCaseSensitiveFileNames: boolean, rootFiles: string[], diagnosticCodes: number[]): void {
            ...
            if (!useCaseSensitiveFileNames) {
                ...
                for (const fileName in files) if (MapLike.guard(files, fileName)) {
...
import b = require("./moduleB.ts");
    ...
    describe("Type reference directive resolution: ", () => {
        ...
        it("Reused program keeps errors", () => {
            ...
            const sourceFiles = Map.from(map(files, f => createSourceFile(f.name, f.content, ScriptTarget.ES6)), f => f.fileName);
            const compilerHost: CompilerHost = {
                fileExists : fileName => Map.has(sourceFiles, fileName),
                ...
                getNewLine: () => "\r\n",
                useCaseSensitiveFileNames: () => false,
                readFile: fileName => Map.has(sourceFiles, fileName) ? sourceFiles[fileName].text : undefined
            };
