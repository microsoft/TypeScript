...
namespace ts {
    ...
    function createTestCompilerHost(texts: NamedSourceText[], target: ScriptTarget): CompilerHost {
        const files = new ts.StringMap<SourceFileWithText>();
    ...
    function checkCache<T>(caption: string, program: Program, fileName: string, expectedContent: ts.StringMap<T>, getCache: (f: SourceFile) => ts.StringMap<T>, entryChecker: (expected: T, original: T) => void): void {
        ...
        if (expectedContent === undefined) {
            ...
            const actualCacheSize = ts.stringMapSize(cache);
            const expectedSize = ts.stringMapSize(expectedContent);
    ...
    function checkResolvedModulesCache(program: Program, fileName: string, expectedContent: ts.StringMap<ResolvedModule>): void {
        checkCache("resolved modules", program, fileName, expectedContent, f => f.resolvedModules, checkResolvedModule);
    }

    function checkResolvedTypeDirectivesCache(program: Program, fileName: string, expectedContent: ts.StringMap<ResolvedTypeReferenceDirective>): void {
...
/// <reference types="typerefs1" />`;
        ...
        it("resolution cache follows imports", () => {
            ...
            checkResolvedModulesCache(program_1, "a.ts", createStringMap("b", { resolvedFileName: "b.ts" }));
            ...
            checkResolvedModulesCache(program_4, "a.ts", new StringMap([["b", { resolvedFileName: "b.ts" }], ["c", undefined]]));
        });
        ...
        it("resolved type directives cache follows type directives", () => {
            ...
            const program_1 = newProgram(files, ["/a.ts"], options);
            checkResolvedTypeDirectivesCache(program_1, "/a.ts", createStringMap("typedefs", { resolvedFileName: "/types/typedefs/index.d.ts", primary: true }));
