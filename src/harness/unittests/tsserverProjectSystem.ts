...
namespace ts.projectSystem {
    ...
    describe("tsserverProjectSystem general functionality", () => {
        describe("general", () => {
            ...
            it("changes in closed files are reflected in project structure", () => {
                ...
                host.vfs.writeFile(modifiedFile2.path, modifiedFile2.content);
                host.checkTimeoutQueueLengthAndRun(2);
                checkNumberOfInferredProjects(projectService, 1);
            ...
            it("deleted files affect project structure", () => {
                ...
                host.vfs.removeFile(file2.path);
                host.checkTimeoutQueueLengthAndRun(2);
            ...
            it("can correctly update configured project when set of root files has changed (new file in list of files)", () => {
                ...
                host.vfs.writeFile(modifiedConfigFile.path, modifiedConfigFile.content);
            ...
            it("config file is deleted", () => {
                ...
                host.vfs.removeFile(config.path);
                host.checkTimeoutQueueLengthAndRun(1);
                checkNumberOfProjects(projectService, { inferredProjects: 2 });
                checkProjectActualFiles(projectService.inferredProjects[0], [file1.path]);
            ...
            it("Open ref of configured project when open file gets added to the project as part of configured file update", () => {
                ...
                host.vfs.writeFile(file5.path, file5.content);
            ...
            it("files are properly detached when language service is disabled", () => {
                ...
                host.vfs.writeFile(f4.path, f4.content);
            ...
            it("language service disabled events are triggered", () => {
                ...
                host.vfs.writeFile(configWithExclude.path, configWithExclude.content);
                host.checkTimeoutQueueLengthAndRun(2);
                checkNumberOfProjects(projectService, { configuredProjects: 1 });
    ...
    describe("tsserverProjectSystem Proper errors", () => {
            ...
            it("folder rename updates project structure and reports no errors", () => {
                ...
                host.vfs.rename(`${projectDir}/foo`, `${projectDir}/foo2`);
                host.runQueuedTimeoutCallbacks();
                host.runQueuedTimeoutCallbacks();
                verifyErrorsInApp();
    ...
    describe("tsserverProjectSystem external projects", () => {
            it("correctly handling add/remove tsconfig - 1", () => {
                ...
                host.vfs.rename(f2.path, tsconfig.path);
                projectService.openExternalProject({
                    projectFileName: projectName,
                ...
                host.vfs.rename(tsconfig.path, f2.path);
                projectService.openExternalProject({
                    projectFileName: projectName,
                    rootFiles: toExternalFiles([f1.path, f2.path]),
                    options: {}
            ...
            it("correctly handles changes in lib section of config file", () => {
                ...
                host.vfs.writeFile(config2.path, config2.content);
                host.checkTimeoutQueueLengthAndRun(2);
    ...
    describe("tsserverProjectSystem watching @types", () => {
            it("works correctly when typings are added or removed", () => {
                ...
                // create t2
                host.vfs.writeFile(t2.path, t2.content);
                // run throttled operation
                host.runQueuedTimeoutCallbacks();
    ...
    describe("tsserverProjectSystem rename a module file and rename back", () => {
            it("should restore the states for inferred projects", () => {
                ...
                host.vfs.rename(moduleFileOldPath, moduleFileNewPath);
                host.runQueuedTimeoutCallbacks();
                diags = session.executeCommand(getErrRequest).response as server.protocol.Diagnostic[];
                verifyDiagnostics(diags, [
                ...
                host.vfs.rename(moduleFileNewPath, moduleFileOldPath);
                host.runQueuedTimeoutCallbacks();
            ...
            it("should property handle missing config files", () => {
                ...
                host.vfs.writeFile(config.path, config.content);
                projectService.openExternalProject({ rootFiles: toExternalFiles([f1.path, config.path]), options: {}, projectFileName: projectName });
                projectService.checkNumberOfProjects({ configuredProjects: 1 });
            });
    ...
    describe("tsserverProjectSystem add the missing module file for inferred project", () => {
            it("should remove the `module not found` error", () => {
                ...
                host.vfs.writeFile(moduleFile.path, moduleFile.content);
                host.runQueuedTimeoutCallbacks();
            ...
            session.clearMessages();
                ...
                host.vfs.writeFile(padIndex.path, padIndex.content);
    ...
    describe("tsserverProjectSystem Configure file diagnostics events", () => {
            ...
            it("are generated when the config file changes", () => {
                ...
                serverEventManager.host.vfs.writeFile(configFile.path, configFile.content);
    ...
    describe("tsserverProjectSystem CachingFileSystemInformation", () => {
            ...
            it("loads missing files from disk", () => {
                ...
                callsTrackingHost.clear();
                host.vfs.writeFile(imported.path, imported.content);
                host.runQueuedTimeoutCallbacks();
            ...
            describe("WatchDirectories for config file with", () => {
                function verifyWatchDirectoriesCaseSensitivity(useCaseSensitiveFileNames: boolean) {
                    ...
                    host.vfs.writeFile(file3.path, file3.content);
                    host.runQueuedTimeoutCallbacks();
            ...
            describe("Verify npm install in directory with tsconfig file works when", () => {
                function verifyNpmInstall(timeoutDuringPartialInstallation: boolean) {
                    ...
                    const host = new fakes.FakeServerHost({ lib: true });
                    host.vfs.addFile(app.path, app.content);
                    host.vfs.addFile(tsconfigJson.path, tsconfigJson.content);
                    host.vfs.addFile(packageJson.path, packageJson.content);
                    const projectService = createProjectService(host);
                    ...
                    verifyAfterPartialOrCompleteNpmInstall(0, [], [{ path: "/a/b/node_modules/.staging/typescript-8493ea5d/package.json.3017591594" }]);
                    ...
                    forEach(filesAndFoldersToAdd, f => {
                        ...
                        if (newPath !== f.path) {
                            host.vfs.rename(f.path, newPath);
                        }
                        f.path = newPath;
                    });
                    ...
                    function verifyAfterPartialOrCompleteNpmInstall(timeoutQueueLengthWhenRunningTimeouts: number, filesOrFoldersToAdd: FileOrFolder[], filesOrFoldersToRemove: FileOrFolder[] = []) {
                        for (const fileOrFolder of filesOrFoldersToRemove) {
                            if (isString(fileOrFolder.content)) {
                                host.vfs.removeFile(fileOrFolder.path);
                            }
                            else {
                                host.vfs.removeDirectory(fileOrFolder.path);
                            }
                            const index = filesAndFoldersToAdd.indexOf(fileOrFolder);
                            if (index >= 0) {
                                ts.orderedRemoveItemAt(filesAndFoldersToAdd, index);
                            }
                        }

                        for (const fileOrFolder of filesOrFoldersToAdd) {
                            if (isString(fileOrFolder.content)) {
                                host.vfs.writeFile(fileOrFolder.path, fileOrFolder.content);
                            }
                            else {
                                host.vfs.addDirectory(fileOrFolder.path);
                            }
                            filesAndFoldersToAdd.push(fileOrFolder);
                        }
    ...
    describe("tsserverProjectSystem ProjectsChangedInBackground", () => {
            ...
            function verifyProjectsUpdatedInBackgroundEvent(createSession: (host: fakes.FakeServerHost) => ProjectsUpdatedInBackgroundEventVerifier) {
                it("when adding new file", () => {
                    ...
                    host.vfs.writeFile(commonFile2.path, commonFile2.content);
                    host.runQueuedTimeoutCallbacks();
                    verifyProjectsUpdatedInBackgroundEventHandler([{
                    ...
                    host.vfs.writeFile(commonFile3.path, commonFile3.content);
                    host.runQueuedTimeoutCallbacks();
                    verifyProjectsUpdatedInBackgroundEventHandler([{
                        eventName: server.ProjectsUpdatedInBackgroundEvent,
                        data: {
                ...
                describe("with --out or --outFile setting", () => {
                    function verifyEventWithOutSettings(compilerOptions: CompilerOptions = {}) {
                        ...
                        files.push(f2);
                        host.vfs.writeFile(f2.path, f2.content);
                        host.runQueuedTimeoutCallbacks();
                ...
                describe("with modules and configured project", () => {
                    ...
                    function getInitialState({ configObj = {}, getAdditionalFileOrFolder, firstReloadFileList }: InitialStateParams = {}) {
                        ...
                        function updateContentOfOpenFile(file: FileOrFolder, newContent: string) {
                            ...
                            host.vfs.writeFile(file.path, newContent);
                        }
                    }
                    ...
                    it("should contains only itself if a module file's shape didn't change, and all files referencing it if its shape changed", () => {
                        ...
                        host.vfs.writeFile(moduleFile1.path, moduleFile1.content);
                        verifyProjectsUpdatedInBackgroundEvent();
                    ...
                    it("should return all files if a global file changed shape", () => {
                        ...
                        host.vfs.writeFile(globalFile3.path, globalFile3.content);
                        verifyProjectsUpdatedInBackgroundEvent();
                    });
                    ...
                    it("should detect non-existing code file", () => {
                        ...
                        host.vfs.removeFile(moduleFile2.path);
                        verifyNoProjectsUpdatedInBackgroundEvent();

                        // Create module File2 and see both files are saved
                        host.vfs.addFile(moduleFile2.path, moduleFile2.content);
                        verifyProjectsUpdatedInBackgroundEvent();
                    });
                });
