...
namespace ts {
    ...
    function checkMapKeys(caption: string, map: MapLike<any>, expectedKeys: string[]) {
        assert.equal(MapLike.size(map), expectedKeys.length, `${caption}: incorrect size of map`);
        for (const name of expectedKeys) {
            assert.isTrue(MapLike.has(map, name), `${caption} is expected to contain ${name}, actual keys: ${MapLike.keys(map)}`);
        }
    }
    ...
    class TestServerHost implements server.ServerHost {
        ...
        watchDirectory(directoryName: string, callback: DirectoryWatcherCallback, recursive: boolean): DirectoryWatcher {
            ...
            const callbacks = MapLike.get(this.watchedDirectories, path) || (this.watchedDirectories[path] = []);
        ...
        triggerDirectoryWatcherCallback(directoryName: string, fileName: string): void {
            const path = this.toPath(directoryName);
            const callbacks = MapLike.get(this.watchedDirectories, path);
            if (callbacks) {
        ...
        triggerFileWatcherCallback(fileName: string, removed?: boolean): void {
            ...
            const callbacks = MapLike.get(this.watchedFiles, path);
            if (callbacks) {
                for (const callback of callbacks) {
        ...
        watchFile(fileName: string, callback: FileWatcherCallback) {
            const path = this.toPath(fileName);
            const callbacks = MapLike.get(this.watchedFiles, path) || (this.watchedFiles[path] = []);
