...
interface Array<T> {}`
    ...
    export class TestServerHost implements server.ServerHost, FormatDiagnosticsHost, ModuleResolutionHost {
        ...
        private removeFileOrFolder(fileOrDirectory: File | Folder | SymLink, isRemovableLeafFolder: (folder: Folder) => boolean, isRenaming = false) {
        ...
        private toSymLink(fileOrDirectory: FileOrFolder): SymLink {
            ...
            symLink.symLink = getNormalizedAbsolutePath(fileOrDirectory.symLink!, getDirectoryPath(symLink.fullPath)); // TODO: GH#18217
            return symLink;
        }

        private toFolder(path: string): Folder {
            const folder = this.toFsEntry(path) as Folder;
            folder.entries = [] as ReadonlyArray<FSEntry> as SortedArray<FSEntry>; // See GH#19873
            return folder;
        }
