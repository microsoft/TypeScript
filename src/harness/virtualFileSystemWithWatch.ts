import { Path, combinePaths, getDirectoryPath, patchWriteFileEnsuringDirectory, isString, SortedArray, isArray, MultiMap, FileWatcher, createMap, forEach, arrayFrom, arrayToMap, isNumber, identity, PollingInterval, WatchOptions, arrayToSet, FileWatcherCallback, FsWatchCallback, FormatDiagnosticsHost, ModuleResolutionHost, createMultiMap, RequireResult, HostWatchFile, HostWatchDirectory, createGetCanonicalFileName, toPath, createSystemWatchFunctions, createSingleFileWatcherPerName, getNormalizedAbsolutePath, directorySeparator, clone, returnFalse, FileWatcherEventKind, Debug, getBaseFileName, insertSorted, compareStringsCaseSensitive, filterMutate, FileSystemEntryKind, createFileWatcherCallback, getRelativePathToDirectoryOrUrl, mapDefined, matchFiles, generateDjb2Hash, sys, clear, WatchFileKind, WatchDirectoryKind, PollingWatchKind } from "./ts";
...
export function checkMap<T, U = undefined>(caption: string, actual: MultiMap<T>, expectedKeys: ts.ReadonlyMap<number>, valueTester?: MapValueTester<T, U>): void;
...
export function checkMap<T>(caption: string, actual: ts.Map<T> | MultiMap<T>, expectedKeys: readonly string[], eachKeyCount: undefined): void;
export function checkMap<T, U = undefined>(caption: string, actual: ts.Map<T> | MultiMap<T>, expectedKeysMapOrArray: ts.ReadonlyMap<number> | readonly string[], eachKeyCountOrValueTester?: number | MapValueTester<T, U>, valueTester?: MapValueTester<T, U>) {
    ...
    expectedKeys.forEach((count, name) => {
        ...
        if (!isArray(expectedKeysMapOrArray) || eachKeyCountOrValueTester !== undefined) {
            ...
            if (expectedValues) {
                assert.deepEqual((actual as MultiMap<T>).get(name)!.map(valueMapper), expectedValues.get(name), `${caption}:: expected values mismatch for ${name}`);
            }
        }
    });
}
...
export function checkWatchedFilesDetailed(host: TestServerHost, expectedFiles: ts.ReadonlyMap<number>, expectedDetails?: ts.Map<WatchFileDetails[]>): void;
export function checkWatchedFilesDetailed(host: TestServerHost, expectedFiles: readonly string[], eachFileWatchCount: number, expectedDetails?: ts.Map<WatchFileDetails[]>): void;
export function checkWatchedFilesDetailed(host: TestServerHost, expectedFiles: ts.ReadonlyMap<number> | readonly string[], eachFileWatchCountOrExpectedDetails?: number | ts.Map<WatchFileDetails[]>, expectedDetails?: ts.Map<WatchFileDetails[]>) {
...
export function checkWatchedDirectoriesDetailed(host: TestServerHost, expectedDirectories: ts.ReadonlyMap<number>, recursive: boolean, expectedDetails?: ts.Map<WatchDirectoryDetails[]>): void;
export function checkWatchedDirectoriesDetailed(host: TestServerHost, expectedDirectories: readonly string[], eachDirectoryWatchCount: number, recursive: boolean, expectedDetails?: ts.Map<WatchDirectoryDetails[]>): void;
export function checkWatchedDirectoriesDetailed(host: TestServerHost, expectedDirectories: ts.ReadonlyMap<number> | readonly string[], recursiveOrEachDirectoryWatchCount: boolean | number, recursiveOrExpectedDetails?: boolean | ts.Map<WatchDirectoryDetails[]>, expectedDetails?: ts.Map<WatchDirectoryDetails[]>) {
    ...
    if (isArray(expectedDirectories)) {
        ...
        checkMap(`fsWatches{recursive ? " recursive" : ""}`, recursiveOrExpectedDetails ? host.fsWatchesRecursive : host.fsWatches, expectedDirectories, [expectedDetails, ({ directoryName, fallbackPollingInterval, fallbackOptions }) => ({ directoryName, fallbackPollingInterval, fallbackOptions })]);
    }
}
...
export class TestServerHost implements ServerHost, FormatDiagnosticsHost, ModuleResolutionHost {
    ...
    private fs: ts.Map<FSEntry> = createMap<FSEntry>();
    ...
    snap(): ts.Map<FSEntry> {
        const result = new ts.Map<FSEntry>();
    ...
    writtenFiles?: ts.Map<number>;
    diff(baseline: string[], base: ts.Map<FSEntry> = new ts.Map()) {
...
function diffFsEntry(baseline: string[], oldFsEntry: FSEntry | undefined, newFsEntry: FSEntry | undefined, writtenFiles: ts.Map<any> | undefined): void {
