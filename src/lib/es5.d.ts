...
/**
 * A typed array of fixed-bit values. If the requested number of bytes could not be allocated an exception is raised.
 */
...
interface TypedArrayConstructor<T, A extends TypedArray<T>> {
    ...
    new(length: number): A;
    new(array: ArrayLike<T> | ArrayBufferLike): A;
