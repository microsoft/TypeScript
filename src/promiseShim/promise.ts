...
namespace ts {
    ...
    export function createPromiseShim(): PromiseConstructor {
        ...
        function settlePromise(promise: Promise<unknown>, verb: "fulfill" | "reject", value: unknown) {
            // tslint:disable no-string-literal ban
            ...
            // tslint:enable no-string-literal ban
        }
    ...
    // Export as a module.
    // NOTE: This aligns with the same code in src/services/shims.ts so that when `promise.ts` is stiched into services that
    // we export at the same level.
