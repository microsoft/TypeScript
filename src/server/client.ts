...
namespace ts.server {
    ...
    export class SessionClient implements LanguageService {
        ...
        private getLineMap(fileName: string): number[] {
            let lineMap = _g(this.lineMaps, fileName);
            if (!lineMap) {
                const scriptSnapshot = this.host.getScriptSnapshot(fileName);
                lineMap = _s(this.lineMaps, fileName, ts.computeLineStarts(scriptSnapshot.getText(0, scriptSnapshot.getLength())));
            }
            return lineMap;
        }
        ...
        changeFile(fileName: string, start: number, end: number, newText: string): void {
            ...
            _s(this.lineMaps, fileName, undefined);
