import { Project, NormalizedPath, ConfiguredProject, ServerHost, Logger, ITypingsInstaller, ScriptInfo, ExternalProject, isConfiguredProject, TypingsCache, ScriptInfoVersion, InferredProject, ThrottledOperations, nullTypingsInstaller, emptyArray, toNormalizedPath, LogLevel, SetTypings, InvalidateCachedTypings, PackageInstalledResponse, BeginInstallTypes, EndInstallTypes, ActionSet, ActionInvalidate, isInferredProjectName, Errors, normalizedPathToPath, ProjectKind, isInferredProject, asNormalizedPath, ProjectOptions, countEachFileTypes, getBaseConfigFileName, isDynamicFileName, Msg, ProjectFilesWithTSDiagnostics, protocol, hasNoTypeScriptSource } from "./ts.server";
import { Diagnostic, CompilerOptions, CommandLineOption, createMap, Debug, optionDeclarations, optionsForWatch, createMapFromTemplate, IndentStyle, isString, WatchOptions, ScriptKind, FileExtensionInfo, getAnyExtensionFromPath, some, fileExtensionIs, FileWatcher, HostCancellationToken, Path, ResolvedProjectReference, forEachKey, ProjectReference, forEach, WatchType, TextChange, DocumentRegistry, MultiMap, WatchFactory, combinePaths, getDirectoryPath, createMultiMap, createGetCanonicalFileName, ensureTrailingDirectorySeparator, getDefaultFormatCodeSettings, emptyOptions, createDocumentRegistryInternal, WatchLogLevel, noop, getWatchFactory, toPath, getNormalizedAbsolutePath, DocumentRegistryBucketKey, SourceFile, arrayFrom, FileWatcherEventKind, WatchDirectoryFlags, ConfigFileProgramReloadLevel, removeIgnoredPath, getBaseFileName, isInsideNodeModules, hasExtension, isSupportedSourceFileName, AssertionLevel, mapDefinedIterator, unorderedRemoveItem, isRootedDiskPath, forEachEntry, noopFileWatcher, canWatchDirectory, PollingInterval, containsPath, hasTSFileExtension, TypeAcquisition, convertCompilerOptionsForTelemetry, version, createCachedDirectoryStructureHost, normalizePath, parseJsonText, EndOfFileToken, SyntaxKind, parseJsonSourceFileConfigFileContent, canJsonReportNoInutFiles, getFileNamesFromConfigSpecs, DirectoryStructureHost, contains, startsWith, missingFileModifiedTime, getFileWatcherEventKind, directorySeparator, DocumentPositionMapper, ReadMapFile, getDocumentPositionMapper, returnTrue, DocumentPosition, find, mapDefinedMap, addToSeen, cloneMap, isArray, mapDefined, length, arrayToMap, normalizeSlashes, toFileNameLowerCase, removeFileExtension, removeMinAndVersionNumbers, convertEnableAutoDiscoveryToEnable, TsConfigSourceFile } from "./ts";
import { FormatCodeSettings, ExternalProjectCompilerOptions, CompileOnSaveMixin, ScriptKindName, UserPreferences, ExternalFile, ConfigureRequestArguments, ProjectVersionInfo, ConfigurePluginRequestArguments } from "./ts.server.protocol";
...
function prepareConvertersForEnumLikeCompilerOptions(commandLineOptions: CommandLineOption[]): ts.Map<ts.Map<number>> {
    const map: ts.Map<ts.Map<number>> = createMap<ts.Map<number>>();
...
export function convertWatchOptions(protocolOptions: ExternalProjectCompilerOptions): WatchOptions | undefined {
...
function forEachResolvedProjectReference<T>(project: ConfiguredProject, cb: (resolvedProjectReference: ResolvedProjectReference | undefined, resolvedProjectReferencePath: Path) => T | undefined): T | undefined {
...
export class ProjectService {
     ...
     */
    private readonly externalProjectToConfiguredProjectMap: ts.Map<NormalizedPath[]> = createMap<NormalizedPath[]>();
    /**
     ...
     */
    private readonly projectToSizeMap: ts.Map<number> = createMap<number>();
    /**
    ...
    setCompilerOptionsForInferredProjects(projectCompilerOptions: ExternalProjectCompilerOptions, projectRootPath?: string): void {
    ...
    watchWildcardDirectory(directory: Path, flags: WatchDirectoryFlags, project: ConfiguredProject) {
        ...
        }, flags, this.getWatchOptions(project), WatchType.WildcardDirectory, project);
    }
    ...
    private logConfigFileWatchUpdate(configFileName: NormalizedPath, canonicalConfigFilePath: string, configFileExistenceInfo: ConfigFileExistenceInfo, status: ConfigFileWatcherStatus) {
        ...
        if (configFileExistenceInfo.configFileWatcherForRootOfInferredProject) {
            watches.push(configFileExistenceInfo.configFileWatcherForRootOfInferredProject === noopFileWatcher ?
     ...
     */
    private createConfigFileWatcherOfConfigFileExistence(configFileName: NormalizedPath, canonicalConfigFilePath: string, configFileExistenceInfo: ConfigFileExistenceInfo) {
        configFileExistenceInfo.configFileWatcherForRootOfInferredProject =
            canWatchDirectory((getDirectoryPath(canonicalConfigFilePath) as Path)) ?
                this.watchFactory.watchFile(this.host, configFileName, (_filename, eventKind) => this.onConfigFileChangeForOpenScriptInfo(configFileName, eventKind), PollingInterval.High, this.hostConfiguration.watchOptions, WatchType.ConfigFileForInferredRoot) : noopFileWatcher;
    ...
    private getFilenameForExceededTotalSizeLimitForNonTsFiles<T>(name: string, options: CompilerOptions | undefined, fileNames: T[], propertyReader: FilePropertyReader<T>): string | undefined {
        ...
        for (const f of fileNames) {
            ...
            if (totalNonTsFileSize > maxProgramSizeForNonTsFiles || totalNonTsFileSize > availableSpace) {
                this.logger.info(getExceedLimitMessage({ propertyReader, hasTSFileExtension: hasTSFileExtension, host: this.host }, totalNonTsFileSize)); // eslint-disable-line @typescript-eslint/no-unnecessary-qualifier
    ...
    private createConfiguredProject(configFileName: NormalizedPath) {
        ...
        const project = new ConfiguredProject(configFileName, this, this.documentRegistry, cachedDirectoryStructureHost);
    ...
    private loadConfiguredProject(project: ConfiguredProject, reason: string) {
        ...
        const parsedCommandLine = parseJsonSourceFileConfigFileContent(result, project.getCachedDirectoryStructureHost(), getDirectoryPath(configFilename), 
    ...
    private watchClosedScriptInfo(info: ScriptInfo) {
        ...
        if (!info.isDynamicOrHasMixedContent() &&
            ...
            if (!this.host.getModifiedTime || indexOfNodeModules === -1) {
                info.fileWatcher = this.watchFactory.watchFilePath(this.host, info.fileName, (fileName, eventKind, path) => this.onSourceFileChanged(fileName, eventKind, path), PollingInterval.Medium, this.hostConfiguration.watchOptions, info.path, WatchType.ClosedScriptInfo);
            }
            else {
    ...
    private watchClosedScriptInfoInNodeModules(dir: Path): ScriptInfoInNodeModulesWatcher {
        ...
        }, WatchDirectoryFlags.Recursive, this.hostConfiguration.watchOptions, WatchType.NodeModulesForClosedScriptInfo);
    ...
    private addSourceInfoToSourceMap(sourceFileName: string | undefined, project: Project, sourceInfos?: ts.Map<true>) {
        if (sourceFileName) {
            // Attach as source
    ...
    loadAncestorProjectTree(forProjects?: ts.ReadonlyMap<true>) {
        forProjects = forProjects || mapDefinedMap(this.configuredProjects, project => !project.isInitialLoadPending() || undefined);
    ...
    private ensureProjectChildren(project: ConfiguredProject, seenProjects: ts.Map<true>) {
        if (!addToSeen(seenProjects, project.canonicalConfigFilePath))
            return;
        // Update the project
        updateProjectIfDirty(project);
