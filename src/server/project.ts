import { ScriptInfo, FileStats, ServerHost, NormalizedPath, emptyArray, updateProjectIfDirty, PackageJsonCache, ProjectService, createPackageJsonCache, TypingsCache, Msg, asNormalizedPath, Errors, LogLevel, toNormalizedPath, makeInferredProjectName, ProjectOptions } from "./ts.server";
import { ScriptKind, fileExtensionIs, Extension, Diagnostic, LanguageService, LanguageServiceHost, Path, FileWatcher, ModuleResolutionHost, createMap, Program, SortedReadonlyArray, HasInvalidatedResolution, ResolutionCache, BuilderState, returnFalse, ResolvedProjectReference, ThrottledCancellationToken, normalizeSlashes, combinePaths, DirectoryStructureHost, GetCanonicalFileName, Completions, DocumentRegistry, CompilerOptions, WatchOptions, getDefaultCompilerOptions, createResolutionCache, createLanguageService, InstallPackageOptions, ApplyCodeActionCommandResult, SourceFile, discoverProbableSymlinks, ProjectReference, addRange, IScriptSnapshot, HostCancellationToken, getDirectoryPath, normalizePath, getDefaultLibFileName, ResolvedModuleFull, ResolvedModuleWithFailedLookupLocations, ResolvedTypeReferenceDirective, CachedDirectoryStructureHost, toPath, DirectoryWatcherCallback, WatchDirectoryFlags, WatchType, JsTyping, SourceMapper, DocumentPositionMapper, mapDefined, getNormalizedAbsolutePath, Debug, TypeAcquisition, sort, flatMap, forEach, clearMap, closeFileWatcher, map, getDefaultLibFilePath, perfLogger, enumerateInsertsAndDeletes, getStringComparer, noop, getAutomaticTypeDirectiveNames, timestamp, StructureIsReused, updateMissingFilePathsWatch, removeFileExtension, getDeclarationEmitOutputFilePathWorker, closeFileWatcherOf, forEachKey, consumesNodeCoreModules, arrayIsEqualTo, Statement, isNonGlobalAmbientModule, findIndex, FileWatcherEventKind, PollingInterval, changesAffectModuleResolution, arrayFrom, arrayToMap, forEachEntry, orderedRemoveItem, PluginImport, firstDefined, PackageJsonInfo, forEachAncestorDirectory, Ternary, stripQuotes, sortAndDeduplicate, getOrUpdate, resolutionExtensionIsTSOrJson, isExternalModuleNameRelative, append, parsePackageName, cloneCompilerOptions, every, WildcardDirectoryWatcher, ResolvedProjectReferenceCallbacks, ConfigFileProgramReloadLevel, ConfigFileSpecs, returnTrue, isString, isDeclarationFileName, directorySeparator, startsWith, stringContains, nodeModulesPathPart, ensureTrailingDirectorySeparator, firstDefinedIterator, isJsonEqual, filter, updateWatchingWildcardDirectories, getEffectiveTypeRoots, ExpandResult, updateErrorForNoInputFiles } from "./ts";
import { ProjectFiles, FileWithProjectReferenceRedirectInfo, ProjectVersionInfo } from "./ts.server.protocol";
...
/* @internal */
export interface ProjectFilesWithTSDiagnostics extends ProjectFiles {
...
type GeneratedFileWatcherMap = GeneratedFileWatcher | ts.Map<GeneratedFileWatcher>;
...
export abstract class Project implements LanguageServiceHost, ModuleResolutionHost {
    ...
    private missingFilesMap: ts.Map<FileWatcher> | undefined;
    private generatedFilesMap: GeneratedFileWatcherMap | undefined;
    private plugins: PluginModuleWithName[] = [];
    /*@internal*/
    private packageJsonFilesMap: ts.Map<FileWatcher> | undefined;
    /*@internal*/
    /**
     ...
     */
    private updatedFileNames: ts.Map<true> | undefined;
    /**
     * Set of files that was returned from the last call to getChangesSinceVersion.
     */
    private lastReportedFileNames: ts.Map<boolean> | undefined;
    /**
    ...
    /*@internal*/
    originalConfiguredProjects: ts.Map<true> | undefined;
    /*@internal*/
    ...
    /*@internal*/
    private dirtyFilesForSuggestions: ts.Map<true> | undefined;
    /*@internal*/
    private symlinks: ts.ReadonlyMap<string> | undefined;
    /*@internal*/
    constructor(
    /*@internal*/ readonly projectName: string, readonly projectKind: ProjectKind, readonly projectService: ProjectService, private documentRegistry: DocumentRegistry, hasExplicitListOfFiles: boolean, lastFileExceededProgramSize: string | undefined, private compilerOptions: CompilerOptions, public compileOnSaveEnabled: boolean, protected watchOptions: WatchOptions | undefined, directoryStructureHost: DirectoryStructureHost, currentDirectory: string | undefined, customRealpath?: (s: string) => string) {
    ...
    /*@internal*/
    getProbableSymlinks(files: readonly SourceFile[]): ts.ReadonlyMap<string> {
        return this.symlinks || (this.symlinks = discoverProbableSymlinks(files, this.getCanonicalFileName, this.getCurrentDirectory()));
    }
    ...
    watchDirectoryOfFailedLookupLocation(directory: string, cb: DirectoryWatcherCallback, flags: WatchDirectoryFlags) {
        return this.projectService.watchFactory.watchDirectory(this.projectService.host, directory, cb, flags, this.projectService.getWatchOptions(this), WatchType.FailedLookupLocations, this);
    }
    /*@internal*/
    ...
    watchTypeRootsDirectory(directory: string, cb: DirectoryWatcherCallback, flags: WatchDirectoryFlags) {
        return this.projectService.watchFactory.watchDirectory(this.projectService.host, directory, cb, flags, this.projectService.getWatchOptions(this), WatchType.TypeRoots, this);
    }
    /*@internal*/
    ...
    getFileNamesWithRedirectInfo(includeProjectReferenceRedirectInfo: boolean) {
        return this.getFileNames().map((fileName): FileWithProjectReferenceRedirectInfo => ({
            fileName,
    ...
    updateTypingFiles(typingFiles: SortedReadonlyArray<string>) {
        enumerateInsertsAndDeletes<string, string>(typingFiles, this.typingFiles, getStringComparer(!this.useCaseSensitiveFileNames()), noop, removed => this.detachScriptInfoFromProject(removed));
    ...
    private updateGraphWorker() {
        ...
        if (hasNewProgram) {
            ...
            updateMissingFilePathsWatch(this.program, this.missingFilesMap || (this.missingFilesMap = createMap()), 
                    ...
                    if (outPath) {
                        ...
                        this.generatedFilesMap.forEach((watcher, source) => {
                            ...
                            if (!sourceFile ||
                                ...
                                !this.isValidGeneratedFileWatcher(getDeclarationEmitOutputFilePathWorker(sourceFile.fileName, this.compilerOptions, this.currentDirectory, this.program!.getCommonSourceDirectory(), this.getCanonicalFileName), watcher)) {
                                closeFileWatcherOf(watcher);
                                (this.generatedFilesMap as ts.Map<GeneratedFileWatcher>).delete(source);
                            }
                        });
                    }
                }
    ...
    private createGeneratedFileWatcher(generatedFile: string): GeneratedFileWatcher {
        return {
            ...
            watcher: this.projectService.watchFactory.watchFile(this.projectService.host, generatedFile, () => {
    ...
    getChangesSinceVersion(lastKnownVersion?: number, includeProjectReferenceRedirectInfo?: boolean): ProjectFilesWithTSDiagnostics {
        const includeProjectReferenceRedirectInfoIfRequested = includeProjectReferenceRedirectInfo
            ? (files: ts.Map<boolean>) => arrayFrom(files.entries(), ([fileName, isSourceOfProjectReferenceRedirect]): FileWithProjectReferenceRedirectInfo => ({
                fileName,
                isSourceOfProjectReferenceRedirect
            }))
            : (files: ts.Map<boolean>) => arrayFrom(files.keys());
        // Update the graph only if initial configured project load is not pending
        if (!this.isInitialLoadPending()) {
            updateProjectIfDirty(this);
        }
        const info: ProjectVersionInfo = {
        ...
        if (this.lastReportedFileNames && lastKnownVersion === this.lastReportedVersion) {
            ...
            const externalFiles = this.getExternalFiles().map((f): FileWithProjectReferenceRedirectInfo => ({
            ...
            const added: ts.Map<boolean> = new ts.Map<boolean>();
            const removed: ts.Map<boolean> = new ts.Map<boolean>();
            const updated: string[] = updatedFileNames ? arrayFrom(updatedFileNames.keys()) : [];
            const updatedRedirects: FileWithProjectReferenceRedirectInfo[] = [];
            ...
            return {
                ...
                changes: {
                    ...
                    updated: includeProjectReferenceRedirectInfo
                        ? updated.map((fileName): FileWithProjectReferenceRedirectInfo => ({
                            fileName,
    ...
    protected enableGlobalPlugins(options: CompilerOptions, pluginConfigOverrides: ts.Map<any> | undefined) {
        const host = this.projectService.host;
        if (!host.require) {
    ...
    protected enablePlugin(pluginConfigEntry: PluginImport, searchPaths: string[], pluginConfigOverrides: ts.Map<any> | undefined) {
        this.projectService.logger.info(`Enabling plugin ${pluginConfigEntry.name} from candidate paths: ${searchPaths.join(",")}`);
        const log = (message: string) => this.projectService.logger.info(message);
        let errorLogs: string[] | undefined;
        const logError = (message: string) => { (errorLogs || (errorLogs = [])).push(message); };
        const resolvedModule = firstDefined(searchPaths, searchPath => <PluginModuleFactory | undefined>Project.resolveModule(pluginConfigEntry.name, searchPath, this.projectService.host, log, logError));
        if (resolvedModule) {
    ...
    private watchPackageJsonFile(path: Path) {
        ...
        if (!watchers.has(path)) {
            watchers.set(path, this.projectService.watchFactory.watchFile(this.projectService.host, path, (fileName, eventKind) => {
            ...
            }, PollingInterval.Low, this.projectService.getWatchOptions(this), WatchType.PackageJsonFile));
        }
    }
}
function getUnresolvedImports(program: Program, cachedUnresolvedImportsPerFile: ts.Map<readonly string[]>): SortedReadonlyArray<string> {
    const ambientModules = program.getTypeChecker().getAmbientModules().map(mod => stripQuotes(mod.getName()));
    return sortAndDeduplicate(flatMap(program.getSourceFiles(), sourceFile => extractUnresolvedImportsFromSourceFile(sourceFile, ambientModules, cachedUnresolvedImportsPerFile)));
}
function extractUnresolvedImportsFromSourceFile(file: SourceFile, ambientModules: readonly string[], cachedUnresolvedImportsPerFile: ts.Map<readonly string[]>): readonly string[] {
...
export class InferredProject extends Project {
    ...
    /*@internal*/
    constructor(projectService: ProjectService, documentRegistry: DocumentRegistry, compilerOptions: CompilerOptions, watchOptions: WatchOptions | undefined, projectRootPath: NormalizedPath | undefined, currentDirectory: string | undefined, pluginConfigOverrides: ts.Map<any> | undefined) {
...
export class ConfiguredProject extends Project {
    ...
    private directoriesWatchedForWildcards: ts.Map<WildcardDirectoryWatcher> | undefined;
    readonly canonicalConfigFilePath: NormalizedPath;
    private projectReferenceCallbacks: ResolvedProjectReferenceCallbacks | undefined;
    private mapOfDeclarationDirectories: ts.Map<true> | undefined;
    private symlinkedDirectories: ts.Map<SymlinkedDirectory | false> | undefined;
    private symlinkedFiles: ts.Map<string> | undefined;
    /* @internal */
    ...
    /*@internal*/
    potentialProjectReferences: ts.Map<true> | undefined;
    /*@internal*/
    ...
    constructor(configFileName: NormalizedPath, projectService: ProjectService, documentRegistry: DocumentRegistry, cachedDirectoryStructureHost: CachedDirectoryStructureHost) {
        super(configFileName, ProjectKind.Configured, projectService, documentRegistry, 
        /*hasExplicitListOfFiles*/ false, 
        /*lastFileExceededProgramSize*/ undefined, 
        /*compilerOptions*/ {}, 
        /*compileOnSaveEnabled*/ false, 
        /*watchOptions*/ undefined, cachedDirectoryStructureHost, getDirectoryPath(configFileName), projectService.host.realpath && (s => this.getRealpath(s)));
    ...
    private directoryExistsIfProjectReferenceDeclDir(dir: string) {
        ...
        return forEachKey((this.mapOfDeclarationDirectories!), declDirPath => dirPath === declDirPath ||
    ...
    private handleDirectoryCouldBeSymlink(directory: string) {
        ...
        if (!this.realpath || !stringContains(directory, nodeModulesPathPart))
            return;
        if (!this.symlinkedDirectories)
            this.symlinkedDirectories = createMap();
    ...
    private fileOrDirectoryExistsUsingSource(fileOrDirectory: string, isFile: boolean): boolean {
        ...
        return firstDefinedIterator(this.symlinkedDirectories.entries(), ([directoryPath, symlinkedDirectory]) => {
            ...
            if (isFile && result) {
                if (!this.symlinkedFiles)
                    this.symlinkedFiles = createMap();
                // Store the real path for the file'
                const absolutePath = getNormalizedAbsolutePath(fileOrDirectory, this.currentDirectory);
                this.symlinkedFiles.set(fileOrDirectoryPath, `${symlinkedDirectory.real}${absolutePath.replace(new RegExp(directoryPath, "i"), "")}`);
            }
            return result;
        }) || false;
    }
    ...
    createConfigFileWatcher() {
        this.configFileWatcher = this.projectService.watchFactory.watchFile(this.projectService.host, this.getConfigFilePath(), (_fileName, eventKind) => this.projectService.onConfigChangedForConfiguredProject(this, eventKind), PollingInterval.High, this.projectService.getWatchOptions(this), WatchType.ConfigFile, this);
    }
    /**
    ...
    /*@internal*/
    enablePluginsWithOptions(options: CompilerOptions, pluginConfigOverrides: ts.Map<any> | undefined) {
        const host = this.projectService.host;
        if (!host.require) {
            this.projectService.logger.info("Plugins were requested but not running in environment that supports 'require'. Nothing will be loaded");
            return;
    ...
    /*@internal*/
    watchWildcards(wildcardDirectories: ts.Map<WatchDirectoryFlags>) {
        updateWatchingWildcardDirectories(this.directoriesWatchedForWildcards || (this.directoriesWatchedForWildcards = createMap()), wildcardDirectories, 
    ...
    hasOpenRef() {
        ...
        return forEachEntry(configFileExistenceInfo.openFilesImpactedByConfigFile, (_value, infoPath) => this.containsScriptInfo((this.projectService.getScriptInfoForPath((infoPath as Path))!))) || false;
    }
    /*@internal*/
