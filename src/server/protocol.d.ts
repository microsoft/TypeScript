...
declare namespace ts.server.protocol {
    ...
    */
    ...
    export interface TsdQuery {
        addNamePattern(name: string): void;
    }

    export interface TsdSelection {
    }

    export interface TsdInstallResult {
        written?: { dict: Map<string> };
    }

    export interface TsdApi {
        readConfig(): PromiseLike<void>;
        initConfig(overwrite: boolean): PromiseLike<string[]>;
        select(query: TsdQuery, options: TsdOptions): PromiseLike<TsdSelection>;
        install(selection: TsdSelection, options: TsdOptions): PromiseLike<TsdInstallResult>;
    }

    export interface Tsd {
        getAPI(tsdJsonPath: string): TsdApi;
        Query: { new(): TsdQuery; };
        Options: { new(): TsdOptions; };
    }
}
