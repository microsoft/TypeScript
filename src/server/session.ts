...
function formatConfigFileDiag(diag: Diagnostic, includeFileName: true): DiagnosticWithFileName;
function formatConfigFileDiag(diag: Diagnostic, includeFileName: false): protocol.Diagnostic;
function formatConfigFileDiag(diag: Diagnostic, includeFileName: boolean): protocol.Diagnostic | DiagnosticWithFileName {
...
function combineProjectOutputWhileOpeningReferencedProjects<T>(projects: Projects, defaultProject: Project, action: (project: Project) => readonly T[], getLocation: (t: T) => DocumentPosition, resultsEqual: (a: T, b: T) => boolean): T[] {
    const outputs: T[] = [];
...
type CombineProjectOutputCallback<TLocation extends DocumentPosition | undefined> = (where: ProjectAndLocation<TLocation>, getMappedLocation: (project: Project, location: DocumentPosition) => DocumentPosition | undefined) => void;
...
function addToTodo<TLocation extends DocumentPosition | undefined>(projectAndLocation: ProjectAndLocation<TLocation>, toDo: Push<ProjectAndLocation<TLocation>>, seenProjects: ts.Map<true>): void {
    if (addToSeen(seenProjects, projectAndLocation.project))
        toDo.push(projectAndLocation);
}
function addToSeen(seenProjects: ts.Map<true>, project: Project) {
...
export class Session implements EventSender {
    ...
    private semanticCheck(file: NormalizedPath, project: Project) {
        const diags = isDeclarationFileInJSOnlyNonConfiguredProject(project, file)
            ? emptyArray : project.getLanguageService().getSemanticDiagnostics(file);
    ...
    private getConfigFileDiagnostics(configFile: NormalizedPath, project: Project, includeLinePosition: boolean) {
        ...
        return includeLinePosition ?
            ...
            map(diagnosticsForConfigFile, diagnostic => formatConfigFileDiag(diagnostic, /*includeFileName*/ false));
    }
    ...
    private convertToDiagnosticsWithLinePosition(diagnostics: readonly Diagnostic[], scriptInfo: ScriptInfo | undefined): DiagnosticWithLinePosition[] {
        return diagnostics.map(d => <DiagnosticWithLinePosition>{
            message: flattenDiagnosticMessageText(d.messageText, this.host.newLine),
            start: d.start,
            length: d.length,
            category: diagnosticCategoryName(d),
    ...
    private getEmitOutput(args: FileRequestArgs): EmitOutput {
        const { file, project } = this.getFileAndProject(args);
    ...
    private mapDefinitionInfo(definitions: readonly DefinitionInfo[], project: Project): readonly FileSpanWithContext[] {
    ...
    */
    ...
    private getSyntacticDiagnosticsSync(args: SyntacticDiagnosticsSyncRequestArgs): readonly protocol.Diagnostic[] | readonly DiagnosticWithLinePosition[] {
    ...
    private getSemanticDiagnosticsSync(args: SemanticDiagnosticsSyncRequestArgs): readonly protocol.Diagnostic[] | readonly DiagnosticWithLinePosition[] {
    ...
    private getSuggestionDiagnosticsSync(args: SuggestionDiagnosticsSyncRequestArgs): readonly protocol.Diagnostic[] | readonly DiagnosticWithLinePosition[] {
        const { configFile } = this.getConfigFileAndProject(args);
        if (configFile) {
    ...
    private getReferences(args: FileLocationRequestArgs, simplifiedResult: boolean): ReferencesResponseBody | undefined | readonly ReferencedSymbol[] {
        const file = toNormalizedPath(args.file);
        const projects = this.getProjects(args);
        const position = this.getPositionInFile(args, file);
        ...
        const refs: readonly ReferencesResponseItem[] = flatMap(references, referencedSymbol => referencedSymbol.references.map(({ fileName, textSpan, contextSpan, isWriteAccess, isDefinition }): ReferencesResponseItem => {
            const scriptInfo = Debug.checkDefined(this.projectService.getScriptInfo(fileName));
    ...
    private getCompletionEntryDetails(args: CompletionDetailsRequestArgs, simplifiedResult: boolean): readonly CompletionEntryDetails[] | readonly ts.CompletionEntryDetails[] {
        const { file, project } = this.getFileAndProject(args);
        const scriptInfo = this.projectService.getScriptInfoForNormalizedPath(file)!;
        const position = this.getPosition(args, scriptInfo);
    ...
    private emitFile(args: CompileOnSaveEmitFileRequestArgs) {
        const { file, project } = this.getFileAndProject(args);
        if (!project) {
    ...
    private getFullNavigateToItems(args: NavtoRequestArgs): readonly NavigateToItem[] {
    ...
    private isLocation(locationOrSpan: FileLocationOrRangeRequestArgs): locationOrSpan is FileLocationRequestArgs {
        return (<FileLocationRequestArgs>locationOrSpan).line !== undefined;
    }
    ...
    private organizeImports({ scope }: OrganizeImportsRequestArgs, simplifiedResult: boolean): readonly FileCodeEdits[] | readonly FileTextChanges[] {
    ...
    private handlers = createMapFromTemplate<(request: Request) => HandlerResponse>({
        [CommandNames.Status]: () => {
        ...
        [CommandNames.Saveto]: (request: Request) => {
            const savetoArgs = (<SavetoRequestArgs>request.arguments);
            this.saveToTmp(savetoArgs.file, savetoArgs.tmpfile);
            return this.notRequired();
        },
        [CommandNames.Close]: (request: Request) => {
            const closeArgs = (<FileRequestArgs>request.arguments);
    ...
    public onMessage(message: string) {
        ...
        try {
            request = (<Request>JSON.parse(message));
