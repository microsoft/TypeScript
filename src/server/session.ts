namespace ts.server {
        ...
        private executeAction(action: (next: NextStep) => void) {
            ...
            try {
                if (this.operationHost.isCancellationRequested()) {
                    stop = true;
                    tracing.on && tracing.instant(tracing.Phase.Session, "stepCanceled", { seq: this.requestId, early: true });
                }
                else {
                    tracing.on && tracing.push(tracing.Phase.Session, "stepAction", { seq: this.requestId });
                    action(this);
                ...
                tracing.on && tracing.popAll();

                stop = true;
                // ignore cancellation request
                if (e instanceof OperationCanceledException) {
                    tracing.on && tracing.instant(tracing.Phase.Session, "stepCanceled", { seq: this.requestId });
                }
                else {
                    tracing.on && tracing.instant(tracing.Phase.Session, "stepError", { seq: this.requestId, message: (<Error>e).message });
    ...
    export class Session<TMessage = string> implements EventSender {
        ...
        public event<T extends object>(body: T, eventName: string): void {
            tracing.on && tracing.instant(tracing.Phase.Session, "event", { eventName });
        ...
        */
        ...
        public onMessage(message: TMessage) {
            ...
            try {
                ...
                tracing.on && tracing.instant(tracing.Phase.Session, "request", { seq: request.seq, command: request.command });
                perfLogger.logStartCommand("" + request.command, this.toStringMessage(message).substring(0, 100));

                tracing.on && tracing.push(tracing.Phase.Session, "executeCommand", { seq: request.seq, command: request.command }, /*separateBeginAndEnd*/ true);
                ...
                tracing.on && tracing.instant(tracing.Phase.Session, "response", { seq: request.seq, command: request.command, success: !!response });
                if (response) {
                ...
                if (err instanceof OperationCanceledException) {
                    ...
                    tracing.on && tracing.instant(tracing.Phase.Session, "commandCanceled", { seq: request?.seq, command: request?.command });
                ...
                tracing.on && tracing.instant(tracing.Phase.Session, "commandError", { seq: request?.seq, command: request?.command, message: (<Error>err).message });
