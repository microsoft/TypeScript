import { InstallPackageOptions, Path, ApplyCodeActionCommandResult, TypeAcquisition, SortedReadonlyArray, returnFalse, notImplemented, noop, CompilerOptions, createMap, arrayIsEqualTo, sort } from "./ts";
...
function setIsEqualTo(arr1: string[] | undefined, arr2: string[] | undefined): boolean {
    ...
    const set: ts.Map<boolean> = createMap<boolean>();
    let unique = 0;
    for (const v of arr1!) {
        if (set.get(v) !== true) {
            set.set(v, true);
...
export class TypingsCache {
    private readonly perProjectCache: ts.Map<TypingsCacheEntry> = createMap<TypingsCacheEntry>();
