namespace ts {
    ...
    /* @internal */
    export function getSemanticClassifications(typeChecker: TypeChecker, cancellationToken: CancellationToken, sourceFile: SourceFile, classifiableNames: Set, span: TextSpan): ClassifiedSpan[] {
    ...
    /* @internal */
    export function getEncodedSemanticClassifications(typeChecker: TypeChecker, cancellationToken: CancellationToken, sourceFile: SourceFile, classifiableNames: Set, span: TextSpan): Classifications {
        const result: number[] = [];
        ...
        function processNode(node: Node) {
            ...
            if (node && textSpanIntersectsWith(span, node.getFullStart(), node.getFullWidth())) {
                ...
                if (kind === SyntaxKind.Identifier && !nodeIsMissing(node)) {
                    ...
                    if (_setHas(classifiableNames, identifier.text)) {
