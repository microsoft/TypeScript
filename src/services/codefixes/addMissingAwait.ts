...
import { FileTextChanges, Diagnostics, compact, createMap, CodeFixContext, CodeFixAllContext, Expression, TypeChecker, forEach, SourceFile, TextSpan, CancellationToken, Program, some, isNumber, textSpansEqual, getTokenAtPosition, findAncestor, textSpanEnd, isExpression, createTextSpanFromNode, Symbol, Node, tryCast, isVariableDeclaration, isIdentifier, getAncestor, SyntaxKind, hasModifier, ModifierFlags, Identifier, isPropertyAccessExpression, isBinaryExpression, Diagnostic, find, contains, TypeFlags, NodeFlags, isArrowFunction, isBlock, getSymbolId, createAwait, createParen, isCallOrNewExpression, addToSeen, findPrecedingToken, positionIsASICandidate } from "../ts";
...
/* @internal */
function getDeclarationSiteFix(context: CodeFixContext | CodeFixAllContext, expression: Expression, errorCode: number, checker: TypeChecker, trackChanges: ContextualTrackChangesFunction, fixedDeclarations?: ts.Map<true>) {
    const { sourceFile, program, cancellationToken } = context;
...
/* @internal */
function getUseSiteFix(context: CodeFixContext | CodeFixAllContext, expression: Expression, errorCode: number, checker: TypeChecker, trackChanges: ContextualTrackChangesFunction, fixedDeclarations?: ts.Map<true>) {
...
/* @internal */
function makeChange(changeTracker: ChangeTracker, errorCode: number, sourceFile: SourceFile, checker: TypeChecker, insertionSite: Expression, fixedDeclarations?: ts.Map<true>) {
    if (isBinaryExpression(insertionSite)) {
        ...
        changeTracker.replaceNode(sourceFile, insertionSite.parent.expression, createParen(createAwait(insertionSite.parent.expression)));
