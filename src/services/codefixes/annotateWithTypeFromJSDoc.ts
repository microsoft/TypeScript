...
namespace ts.codefix {
    ...
    function transformJSDocOptionalType(node: JSDocOptionalType) {
        return factory.createUnionTypeNode([visitNode(node.type, transformJSDocType), factory.createTypeReferenceNode("undefined", [])]);
    }

    function transformJSDocNullableType(node: JSDocNullableType) {
        return factory.createUnionTypeNode([visitNode(node.type, transformJSDocType), factory.createTypeReferenceNode("null", [])]);
    }
    ...
    function transformJSDocFunctionType(node: JSDocFunctionType) {
        ...
        return factory.createFunctionTypeNode([], node.parameters.map(transformJSDocParameter), node.type ?? factory.createKeywordTypeNode(SyntaxKind.AnyKeyword));
    }
    ...
    function transformJSDocTypeReference(node: TypeReferenceNode) {
        ...
        if (isIdentifier(node.typeName)) {
            ...
            if ((text === "Array" || text === "Promise") && !node.typeArguments) {
                args = factory.createNodeArray([factory.createTypeReferenceNode("any", [])]);
            }
            else {
