...
namespace ts.codefix {
    ...
    function transformJSDocParameter(node: ParameterDeclaration) {
        ...
        return factory.createParameterDeclaration(RESERVED, node.modifiers, dotdotdot, name, node.questionToken, visitNode(node.type, transformJSDocType), node.initializer);
    }
    ...
    function transformJSDocIndexSignature(node: TypeReferenceNode) {
        ...
        const indexSignature = factory.createTypeLiteralNode([factory.createIndexSignature(RESERVED, /*modifiers*/ undefined, [index], node.typeArguments![1])]);
