import { Diagnostics, CodeFixContext, SourceFile, TypeChecker, getTokenAtPosition, SymbolFlags, Node, ClassDeclaration, SyntaxKind, FunctionDeclaration, VariableDeclaration, VariableDeclarationList, copyLeadingComments, Symbol, ClassElement, createToken, PropertyAccessExpression, Expression, isFunctionLike, Modifier, BinaryExpression, createProperty, FunctionExpression, concatenate, createMethod, ArrowFunction, Block, createBlock, createReturn, isSourceFileJS, createConstructor, createClassDeclaration, filter } from "../ts";
...
registerCodeFix({
    ...
    getCodeActions(context: CodeFixContext) {
        const changes = ChangeTracker.with(context, t => doChange(t, context.sourceFile, context.span.start, context.program.getTypeChecker()));
...
/* @internal */
function doChange(changes: ChangeTracker, sourceFile: SourceFile, position: number, checker: TypeChecker): void {
