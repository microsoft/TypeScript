import { Diagnostics, CodeFixContext, BindingPattern, Type, Identifier, Symbol, TypeChecker, SourceFile, CodeFixContextBase, getTokenAtPosition, FunctionLikeDeclaration, isIdentifier, isVariableDeclaration, isFunctionLikeDeclaration, tryCast, getContainingFunction, createMap, isInJSFile, isBlock, emptyArray, SyntaxKind, CallExpression, Node, forEachChild, isCallExpression, isFunctionLike, Block, ReturnStatement, forEachReturnStatement, isReturnStatementWithFixablePromiseHandler, getNodeId, forEach, isExpression, hasPropertyAccessExpressionWithName, getSymbolId, firstOrUndefined, isParameter, createOptimisticUniqueName, isBindingElement, getSynthesizedDeepClone, getSynthesizedDeepCloneWithRenames, createIdentifier, Expression, Statement, isPropertyAccessExpression, createBlock, createVariableDeclaration, createCatchClause, VariableStatement, UnionReduction, createVariableStatement, createVariableDeclarationList, NodeFlags, createTry, compact, BindingName, flatMap, isOmittedExpression, contains, createAwait, createStatement, createReturn, createAssignment, createCall, SignatureKind, FunctionExpression, ArrowFunction, isReturnStatement, isFixablePromiseHandler, Signature, lastOrUndefined, createExpressionStatement, every } from "../ts";
...
registerCodeFix({
    ...
    getCodeActions(context: CodeFixContext) {
        ...
        const changes = ChangeTracker.with(context, (t) => convertToAsyncFunction(t, context.sourceFile, context.span.start, context.program.getTypeChecker(), context));
...
interface Transformer {
    ...
    readonly originalTypeMap: ts.ReadonlyMap<Type>; // keys are the node id of the identifier
    readonly isInJSFile: boolean;
}
/* @internal */
function convertToAsyncFunction(changes: ChangeTracker, sourceFile: SourceFile, position: number, checker: TypeChecker, context: CodeFixContextBase): void {
    ...
    const synthNamesMap: ts.Map<SynthIdentifier> = createMap();
    const originalTypeMap: ts.Map<Type> = createMap();
...
function getReturnStatementsWithPromiseHandlers(body: Block): readonly ReturnStatement[] {
    ...
    forEachReturnStatement(body, ret => {
        if (isReturnStatementWithFixablePromiseHandler(ret))
            res.push(ret);
    });
    return res;
}
...
/* @internal */
function getConstIdentifiers(synthNamesMap: ts.ReadonlyMap<SynthIdentifier>): Identifier[] {
...
*/
/* @internal */
function getAllPromiseExpressionsToReturn(func: FunctionLikeDeclaration, checker: TypeChecker): ts.Map<true> {
    if (!func.body) {
        return createMap<true>();
    }
    const setOfExpressionsToReturn: ts.Map<true> = createMap<true>();
...
*/
/* @internal */
function renameCollidingVarNames(nodeToRename: FunctionLikeDeclaration, checker: TypeChecker, synthNamesMap: ts.Map<SynthIdentifier>, context: CodeFixContextBase, setOfAllExpressionsToReturn: ts.Map<true>, originalType: ts.Map<Type>, allVarNames: SymbolAndIdentifier[]): FunctionLikeDeclaration {
    const identsToRenameMap: ts.Map<Identifier> = createMap(); // key is the symbol id
    const collidingSymbolMap: ts.Map<Symbol[]> = createMap();
...
/* @internal */
function addNameToFrequencyMap(renamedVarNameFrequencyMap: ts.Map<Symbol[]>, originalName: string, symbol: Symbol) {
...
/* @internal */
function getNewNameIfConflict(name: Identifier, originalNames: ts.ReadonlyMap<Symbol[]>): SynthIdentifier {
...
function createTransformedStatement(prevArgName: SynthBindingName | undefined, rightHandSide: Expression, transformer: Transformer): readonly Statement[] {
    ...
    return [createVariableStatement(/*modifiers*/ undefined, (createVariableDeclarationList([createVariableDeclaration(getSynthesizedDeepClone(getNode(prevArgName)), /*type*/ undefined, rightHandSide)], getFlagOfBindingName(prevArgName, transformer.constIdentifiers))))];
}
...
function removeReturns(stmts: readonly Statement[], prevArgName: SynthBindingName | undefined, transformer: Transformer, seenReturnStatement: boolean): readonly Statement[] {
        ...
        if (isReturnStatement(stmt)) {
            if (stmt.expression) {
                ...
                if (prevArgName === undefined) {
                    ...
                    ret.push(createVariableStatement(/*modifiers*/ undefined, (createVariableDeclarationList([createVariableDeclaration(getNode(prevArgName), /*type*/ undefined, possiblyAwaitedExpression)], getFlagOfBindingName(prevArgName, transformer.constIdentifiers)))));
                }
            }
        }
        else {
    ...
    if (!seenReturnStatement && prevArgName !== undefined) {
        ret.push(createVariableStatement(/*modifiers*/ undefined, (createVariableDeclarationList([createVariableDeclaration(getNode(prevArgName), /*type*/ undefined, createIdentifier("undefined"))], getFlagOfBindingName(prevArgName, transformer.constIdentifiers)))));
    }
    return ret;
}
/* @internal */
