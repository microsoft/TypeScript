import { registerCodeFix, createCodeFixActionWithoutFixAll, moduleSpecifierToValidIdentifier } from "../ts.codefix";
import { Diagnostics, getQuotePreference, SourceFile, QuotePreference, getResolvedModule, importFromModuleSpecifier, SyntaxKind, makeImport, isRequireCall, createPropertyAccess, getSynthesizedDeepClone, TypeChecker, ScriptTarget, createMap, isNonContextualKeyword, SymbolFlags, createIdentifier, PropertyAccessExpression, Identifier, isPropertyAccessExpression, isExportsOrModuleExportsOrAlias, isIdentifier, isBinaryExpression, Statement, VariableStatement, ExpressionStatement, BinaryExpression, flatMap, createVariableStatement, createVariableDeclarationList, BindingName, StringLiteralLike, Node, Debug, isObjectLiteralExpression, createRange, ObjectLiteralExpression, mapAllOrFail, createToken, ObjectLiteralElementLike, createExportSpecifier, emptyUnderscoreEscapedMap, __String, ExportDeclaration, isFunctionExpression, isArrowFunction, isClassExpression, findChildOfKind, Expression, FunctionExpression, ArrowFunction, ClassExpression, arrayFrom, mapIterator, createImportSpecifier, createMultiMap, BindingElement, ImportSpecifier, Modifier, MethodDeclaration, FunctionDeclaration, createFunctionDeclaration, getSynthesizedDeepClones, concatenate, convertToFunctionBody, ClassDeclaration, createClassDeclaration, ImportDeclaration, createVariableDeclaration, NodeFlags, ExportSpecifier, createExportDeclaration, createNamedExports, createLiteral } from "../ts";
...
/* @internal */
function convertFileToEs6Module(sourceFile: SourceFile, checker: TypeChecker, changes: ChangeTracker, target: ScriptTarget, quotePreference: QuotePreference): ModuleExportsChanged {
...
/* @internal */
function forEachExportReference(sourceFile: SourceFile, cb: (node: (PropertyAccessExpression & {
    name: Identifier;
}), isAssignmentLhs: boolean) => void): void {
...
/* @internal */
function convertStatement(sourceFile: SourceFile, statement: Statement, checker: TypeChecker, changes: ChangeTracker, identifiers: Identifiers, target: ScriptTarget, exports: ExportRenames, quotePreference: QuotePreference): ModuleExportsChanged {
...
/* @internal */
function convertVariableStatement(sourceFile: SourceFile, statement: VariableStatement, changes: ChangeTracker, checker: TypeChecker, identifiers: Identifiers, target: ScriptTarget, quotePreference: QuotePreference): void {
...
/* @internal */
function convertAssignment(sourceFile: SourceFile, checker: TypeChecker, assignment: BinaryExpression, changes: ChangeTracker, exports: ExportRenames): ModuleExportsChanged {
    ...
    if (isExportsOrModuleExportsOrAlias(sourceFile, left)) {
        ...
        convertNamedExport(sourceFile, (assignment as BinaryExpression & {
...
/* @internal */
function convertNamedExport(sourceFile: SourceFile, assignment: BinaryExpression & {
    left: PropertyAccessExpression;
}, changes: ChangeTracker, exports: ExportRenames): void {
...
/* @internal */
function convertExportsPropertyAssignment({ left, right, parent }: BinaryExpression & {
    left: PropertyAccessExpression;
}, sourceFile: SourceFile, changes: ChangeTracker): void {
    ...
    if ((isFunctionExpression(right) || isArrowFunction(right) || isClassExpression(right)) && (!right.name || right.name.text === name)) {
        ...
        changes.replaceNodeRangeWithNodes(sourceFile, left.expression, (findChildOfKind(left, SyntaxKind.DotToken, sourceFile)!), [createToken(SyntaxKind.ExportKeyword), createToken(SyntaxKind.ConstKeyword)], { joiner: " ", suffix: " " });
    }
}
 ...
 */
/* @internal */
function convertSingleImport(file: SourceFile, name: BindingName, moduleSpecifier: StringLiteralLike, changes: ChangeTracker, checker: TypeChecker, identifiers: Identifiers, target: ScriptTarget, quotePreference: QuotePreference): readonly Node[] {
    switch (name.kind) {
        case SyntaxKind.ObjectBindingPattern: {
 ...
 */
/* @internal */
function convertSingleIdentifierImport(file: SourceFile, name: Identifier, moduleSpecifier: StringLiteralLike, changes: ChangeTracker, checker: TypeChecker, identifiers: Identifiers, quotePreference: QuotePreference): readonly Node[] {
    ...
    const namedBindings = namedBindingsNames.size === 0 ? undefined : arrayFrom(mapIterator(namedBindingsNames.entries(), ([propertyName, idName]) => createImportSpecifier(propertyName === idName ? undefined : createIdentifier(propertyName), createIdentifier(idName))));
    if (!namedBindings) {
...
/* @internal */
type FreeIdentifiers = ts.ReadonlyMap<readonly Identifier[]>;
/* @internal */
...
function functionExpressionToDeclaration(name: string | undefined, additionalModifiers: readonly Modifier[], fn: FunctionExpression | ArrowFunction | MethodDeclaration): FunctionDeclaration {
    return createFunctionDeclaration(getSynthesizedDeepClones(fn.decorators), // TODO: GH#19915 Don't think this is even legal.
    concatenate(additionalModifiers, getSynthesizedDeepClones(fn.modifiers)), getSynthesizedDeepClone(fn.asteriskToken), name, getSynthesizedDeepClones(fn.typeParameters), getSynthesizedDeepClones(fn.parameters), getSynthesizedDeepClone(fn.type), convertToFunctionBody(getSynthesizedDeepClone((fn.body!))));
}
/* @internal */
function classExpressionToDeclaration(name: string | undefined, additionalModifiers: readonly Modifier[], cls: ClassExpression): ClassDeclaration {
    return createClassDeclaration(getSynthesizedDeepClones(cls.decorators), // TODO: GH#19915 Don't think this is even legal.
    concatenate(additionalModifiers, getSynthesizedDeepClones(cls.modifiers)), name, getSynthesizedDeepClones(cls.typeParameters), getSynthesizedDeepClones(cls.heritageClauses), getSynthesizedDeepClones(cls.members));
}
/* @internal */
...
function makeConst(modifiers: readonly Modifier[] | undefined, name: string | BindingName, init: Expression): VariableStatement {
    return createVariableStatement(modifiers, createVariableDeclarationList([createVariableDeclaration(name, /*type*/ undefined, init)], NodeFlags.Const));
}
/* @internal */
function makeExportDeclaration(exportSpecifiers: ExportSpecifier[] | undefined, moduleSpecifier?: string): ExportDeclaration {
    ...
    /*modifiers*/ undefined, exportSpecifiers && createNamedExports(exportSpecifiers), moduleSpecifier === undefined ? undefined : createLiteral(moduleSpecifier));
}
