import { Diagnostics, addToSeen, getNodeId, TextSpan, SourceFile, tryCast, getTokenAtPosition, isExportSpecifier, ExportSpecifier, CodeFixContextBase, SyntaxKind, factory, filter, contains, getDiagnosticsWithinSpan, createTextSpanFromNode, findDiagnosticForNode } from "../ts";
...
function fixSingleExportDeclaration(changes: ChangeTracker, exportSpecifier: ExportSpecifier | undefined, context: CodeFixContextBase) {
    ...
    if (typeExportSpecifiers.length === exportClause.elements.length) {
        ...
        const valueExportDeclaration = factory.updateExportDeclaration(exportDeclaration, exportDeclaration.decorators, exportDeclaration.modifiers, 
