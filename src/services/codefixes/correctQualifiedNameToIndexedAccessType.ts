import { Diagnostics, SourceFile, QualifiedName, Identifier, findAncestor, getTokenAtPosition, isQualifiedName, Debug, isIdentifier, createIndexedAccessTypeNode, createTypeReferenceNode, createLiteralTypeNode, createLiteral } from "../ts";
...
registerCodeFix({
    ...
    getCodeActions(context) {
        ...
        if (!qualifiedName)
            return undefined;
        const changes = ChangeTracker.with(context, t => doChange(t, context.sourceFile, qualifiedName));
...
/* @internal */
function getQualifiedName(sourceFile: SourceFile, pos: number): (QualifiedName & {
    left: Identifier;
}) | undefined {
    ...
    return isIdentifier(qualifiedName.left) ? qualifiedName as QualifiedName & {
        left: Identifier;
    } : undefined;
}
/* @internal */
function doChange(changeTracker: ChangeTracker, sourceFile: SourceFile, qualifiedName: QualifiedName): void {
    const rightText = qualifiedName.right.text;
    const replacement = createIndexedAccessTypeNode(createTypeReferenceNode(qualifiedName.left, /*typeArguments*/ undefined), createLiteralTypeNode(createLiteral(rightText)));
