import { Diagnostics, isInterfaceDeclaration, singleElementArray, concatenate, createMap, NodeMap, addToSeen, getNodeId, isPrivateIdentifier, TypeChecker, ClassLikeDeclaration, getClassExtendsHeritageElement, find, isClassLike, InterfaceDeclaration, Identifier, EnumDeclaration, PrivateIdentifier, SourceFile, CallExpression, Program, getTokenAtPosition, isIdentifier, isPropertyAccessExpression, skipConstraint, TypeReference, isSourceFileJS, tryCast, isCallExpression, isEnumDeclaration, CodeFixContext, CodeFixAction, SyntaxKind, createIdentifier, createProperty, getFirstConstructorWithBody, createThis, Expression, createStatement, createAssignment, createPropertyAccess, Node, TypeNode, BinaryExpression, createKeywordTypeNode, createToken, PropertyDeclaration, isPropertyDeclaration, createParameter, createIndexSignature, UserPreferences, CodeFixContextBase, getAncestor, some, TypeFlags, createEnumMember, createStringLiteral, updateEnumDeclaration } from "../ts";
import { registerCodeFix, createCodeFixAction, createCombinedCodeActions, eachDiagnostic, createCodeFixActionWithoutFixAll, createMethodFromCallExpression } from "../ts.codefix";
...
registerCodeFix({
    ...
    getCodeActions(context) {
        ...
        if (info.kind === InfoKind.Enum) {
            ...
            const changes = ChangeTracker.with(context, t => addEnumMemberDeclaration(t, context.program.getTypeChecker(), token, parentDeclaration));
...
function getActionsForAddMissingMemberInJavascriptFile(context: CodeFixContext, declSourceFile: SourceFile, classDeclaration: ClassLikeDeclaration, token: Identifier | PrivateIdentifier, makeStatic: boolean): CodeFixAction | undefined {
    const changes = ChangeTracker.with(context, t => addMissingMemberInJs(t, declSourceFile, classDeclaration, token, makeStatic));
    if (changes.length === 0) {
        return undefined;
    }
...
function createAddPropertyDeclarationAction(context: CodeFixContext, declSourceFile: SourceFile, classDeclaration: ClassOrInterface, makeStatic: boolean, tokenName: string, typeNode: TypeNode): CodeFixAction {
    const changes = ChangeTracker.with(context, t => addPropertyDeclaration(t, declSourceFile, classDeclaration, tokenName, typeNode, makeStatic));
...
/* @internal */
function addPropertyDeclaration(changeTracker: ChangeTracker, declSourceFile: SourceFile, classDeclaration: ClassOrInterface, tokenName: string, typeNode: TypeNode, makeStatic: boolean): void {
    const property = createProperty(
    /*decorators*/ undefined, 
    /*modifiers*/ makeStatic ? [createToken(SyntaxKind.StaticKeyword)] : undefined, tokenName, 
...
/* @internal */
function getActionForMethodDeclaration(context: CodeFixContext, declSourceFile: SourceFile, classDeclaration: ClassOrInterface, token: Identifier | PrivateIdentifier, callExpression: CallExpression, makeStatic: boolean, inJs: boolean, preferences: UserPreferences): CodeFixAction | undefined {
    // Private methods are not implemented yet.
    if (isPrivateIdentifier(token)) {
        return undefined;
    }
    const changes = ChangeTracker.with(context, t => addMethodDeclaration(context, t, declSourceFile, classDeclaration, token, callExpression, makeStatic, inJs, preferences));
...
/* @internal */
function addMethodDeclaration(context: CodeFixContextBase, changeTracker: ChangeTracker, declSourceFile: SourceFile, typeDecl: ClassOrInterface, token: Identifier, callExpression: CallExpression, makeStatic: boolean, inJs: boolean, preferences: UserPreferences): void {
...
/* @internal */
function addEnumMemberDeclaration(changes: ChangeTracker, checker: TypeChecker, token: Identifier, enumDeclaration: EnumDeclaration) {
    /**
    ...
    changes.replaceNode(enumDeclaration.getSourceFile(), enumDeclaration, updateEnumDeclaration(enumDeclaration, enumDeclaration.decorators, enumDeclaration.modifiers, enumDeclaration.name, concatenate(enumDeclaration.members, singleElementArray(enumMember))));
}
