...
function getInfo(sourceFile: ts.SourceFile, tokenPos: number, errorCode: number, checker: ts.TypeChecker, program: ts.Program): Info | undefined {
    ...
    if (errorCode === ts.Diagnostics.Argument_of_type_0_is_not_assignable_to_parameter_of_type_1.code) {
        ...
        const signature = ts.singleOrUndefined(checker.getSignaturesOfType(checker.getTypeAtLocation(parent.parent.expression), ts.SignatureKind.Call));
    ...
    if (enumDeclaration && !(leftExpressionType.flags & ts.TypeFlags.EnumLike) && !ts.isPrivateIdentifier(token) && !isSourceFileFromLibrary(program, enumDeclaration.getSourceFile())) {
...
function isSourceFileFromLibrary(program: ts.Program, node: ts.SourceFile) {
