import { Diagnostics, concatenate, addToSeen, getNodeId, getOrUpdate, isPrivateIdentifier, ModifierFlags, isInterfaceDeclaration, Identifier, EnumDeclaration, CallExpression, PrivateIdentifier, SourceFile, ModuleDeclaration, Symbol, ObjectLiteralExpression, JsxOpeningLikeElement, TypeChecker, Program, getTokenAtPosition, SyntaxKind, isObjectLiteralExpression, isCallExpression, findIndex, singleOrUndefined, SignatureKind, isParameter, isIdentifier, arrayFrom, length, isMemberName, hasInitializer, isJsxOpeningLikeElement, getEmitScriptTarget, isPropertyAccessExpression, skipConstraint, find, isModuleDeclaration, isSourceFile, isClassLike, TypeReference, startsWithUnderscore, isSourceFileJS, tryCast, isEnumDeclaration, CodeFixContext, CodeFixAction, singleElementArray, ClassLikeDeclaration, factory, getFirstConstructorWithBody, Expression, Node, TypeNode, BinaryExpression, NodeBuilderFlags, PropertyDeclaration, isPropertyDeclaration, CodeFixContextBase, MethodDeclaration, findAncestor, isMethodDeclaration, isConstructorDeclaration, some, TypeFlags, idText, FunctionDeclaration, getQuotePreference, isJsxSpreadAttribute, map, setParent, createPropertyNameNodeForIdentifierOrLiteral, QuotePreference, Type, firstOrUndefined, SymbolFlags, NumberLiteralType, BigIntLiteralType, StringLiteralType, firstDefined, UnionType, getObjectFlags, ObjectFlags, emptyArray, or, isFunctionTypeNode, isMethodSignature, FunctionExpression, getClassLikeDeclarationOfSymbol, hasAbstractModifier, isTypeLiteralNode, ScriptTarget, __String, isJsxAttribute, filter, isIdentifierText, LanguageVariant, getCheckFlags, CheckFlags } from "../ts";
import { registerCodeFix, createCodeFixAction, ClassOrInterface, createCombinedCodeActions, eachDiagnostic, getAllSupers, createCodeFixActionWithoutFixAll, createImportAdder, createSignatureDeclarationFromCallExpression, ImportAdder, createSignatureDeclarationFromSignature, createStubbedBody } from "../ts.codefix";
...
registerCodeFix({
    ...
    getCodeActions(context) {
        ...
        if (info.kind === InfoKind.ObjectLiteral) {
            const changes = ChangeTracker.with(context, t => addObjectLiteralProperties(t, context, info));
        ...
        if (info.kind === InfoKind.JsxAttributes) {
            const changes = ChangeTracker.with(context, t => addJsxAttributes(t, context, info));
        ...
        if (info.kind === InfoKind.Function) {
            const changes = ChangeTracker.with(context, t => addFunctionDeclaration(t, context, info));
        ...
        if (info.kind === InfoKind.Enum) {
            const changes = ChangeTracker.with(context, t => addEnumMemberDeclaration(t, context.program.getTypeChecker(), info));
    ...
    getAllCodeActions: context => {
        ...
        const typeDeclToMembers = new ts.Map<ClassOrInterface, ClassOrInterfaceInfo[]>();
...
function createActionForAddMissingMemberInJavascriptFile(context: CodeFixContext, { parentDeclaration, declSourceFile, modifierFlags, token }: ClassOrInterfaceInfo): CodeFixAction | undefined {
    ...
    const changes = ChangeTracker.with(context, t => addMissingMemberInJs(t, declSourceFile, parentDeclaration, token, !!(modifierFlags & ModifierFlags.Static)));
    if (changes.length === 0) {
        return undefined;
    }
...
/* @internal */
function addMissingMemberInJs(changeTracker: ChangeTracker, declSourceFile: SourceFile, classDeclaration: ClassLikeDeclaration, token: Identifier | PrivateIdentifier, makeStatic: boolean): void {
...
function createActionsForAddMissingMemberInTypeScriptFile(context: CodeFixContext, { parentDeclaration, declSourceFile, modifierFlags, token }: ClassOrInterfaceInfo): CodeFixAction[] | undefined {
    ...
    const addPropertyDeclarationChanges = (modifierFlags: ModifierFlags) => ChangeTracker.with(context, t => addPropertyDeclaration(t, declSourceFile, parentDeclaration, memberName, typeNode, modifierFlags));
...
/* @internal */
function addPropertyDeclaration(changeTracker: ChangeTracker, declSourceFile: SourceFile, classDeclaration: ClassOrInterface, tokenName: string, typeNode: TypeNode, modifierFlags: ModifierFlags): void {
    const property = factory.createPropertyDeclaration(
        /*decorators*/ undefined,
    /*modifiers*/ modifierFlags ? factory.createNodeArray(factory.createModifiersFromModifierFlags(modifierFlags)) : undefined, tokenName, 
...
function createAddIndexSignatureAction(context: CodeFixContext, declSourceFile: SourceFile, classDeclaration: ClassOrInterface, tokenName: string, typeNode: TypeNode): CodeFixAction {
    ...
    const changes = ChangeTracker.with(context, t => t.insertNodeAtClassStart(declSourceFile, classDeclaration, indexSignature));
...
function getActionsForMissingMethodDeclaration(context: CodeFixContext, info: ClassOrInterfaceInfo): CodeFixAction[] | undefined {
    ...
    const addMethodDeclarationChanges = (modifierFlags: ModifierFlags) => ChangeTracker.with(context, t => addMethodDeclaration(context, t, call, token, modifierFlags, parentDeclaration, declSourceFile));
...
/* @internal */
function addMethodDeclaration(context: CodeFixContextBase, changes: ChangeTracker, callExpression: CallExpression, name: Identifier, modifierFlags: ModifierFlags, parentDeclaration: ClassOrInterface, sourceFile: SourceFile): void {
...
/* @internal */
function addEnumMemberDeclaration(changes: ChangeTracker, checker: TypeChecker, { token, parentDeclaration }: EnumInfo) {
    /**
    ...
    changes.replaceNode(parentDeclaration.getSourceFile(), parentDeclaration, factory.updateEnumDeclaration(parentDeclaration, parentDeclaration.decorators, parentDeclaration.modifiers, parentDeclaration.name, concatenate(parentDeclaration.members, singleElementArray(enumMember))), {
...
/* @internal */
function addFunctionDeclaration(changes: ChangeTracker, context: CodeFixContextBase, info: FunctionInfo) {
...
/* @internal */
function addJsxAttributes(changes: ChangeTracker, context: CodeFixContextBase, info: JsxAttributesInfo) {
    const importAdder = createImportAdder(context.sourceFile, context.program, context.preferences, context.host);
...
/* @internal */
function addObjectLiteralProperties(changes: ChangeTracker, context: CodeFixContextBase, info: ObjectLiteralInfo) {
    const importAdder = createImportAdder(context.sourceFile, context.program, context.preferences, context.host);
    const quotePreference = getQuotePreference(context.sourceFile, context.preferences);
...
function getUnmatchedAttributes(checker: TypeChecker, target: ScriptTarget, source: JsxOpeningLikeElement) {
    ...
    if (!length(targetProps))
        return emptyArray;
    const seenNames = new ts.Set<__String>();
