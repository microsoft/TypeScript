import { CodeFixContext, ImportDeclaration, CodeFixAction, getSourceFileOfNode, getNamespaceDeclarationNode, NamespaceImport, makeImport, getQuotePreference, getEmitModuleKind, ModuleKind, createImportEqualsDeclaration, createExternalModuleReference, SourceFile, Node, Diagnostics, SyntaxKind, findAncestor, getTokenAtPosition, CallExpression, NewExpression, TransientSymbol, isImportCall, addRange, isExpression, isNamedDeclaration, createPropertyAccess } from "../ts";
import { ChangeTracker } from "../ts.textChanges";
import { createCodeFixActionWithoutFixAll, registerCodeFix } from "../ts.codefix";
/* @internal */
...
function getCodeFixesForImportDeclaration(context: CodeFixContext, node: ImportDeclaration): CodeFixAction[] {
    ...
    if (getEmitModuleKind(opts) === ModuleKind.CommonJS) {
        ...
        /*modifiers*/ undefined, namespace.name, createExternalModuleReference(node.moduleSpecifier))));
    }
    return variations;
}
/* @internal */
function createAction(context: CodeFixContext, sourceFile: SourceFile, node: Node, replacement: Node): CodeFixAction {
    const changes = ChangeTracker.with(context, t => t.replaceNode(sourceFile, node, replacement));
...
function getImportCodeFixesForExpression(context: CodeFixContext, expr: Node): CodeFixAction[] | undefined {
    ...
    if (isExpression(expr) && !(isNamedDeclaration(expr.parent) && expr.parent.name === expr)) {
        const sourceFile = context.sourceFile;
        const changes = ChangeTracker.with(context, t => t.replaceNode(sourceFile, expr, createPropertyAccess(expr, "default"), {}));
