import { Diagnostics, getTokenAtPosition, isJSDocTemplateTag, SyntaxKind, showModuleSpecifier, CodeFixAction, cast, isInferTypeNode, isComputedPropertyName, Debug, SourceFile, Node, createKeywordTypeNode, FileTextChanges, isDeclarationWithTypeParameterChildren, ImportDeclaration, tryCast, isImportDeclaration, TypeChecker, isObjectBindingPattern, isVariableDeclarationList, isIdentifier, createIdentifier, Identifier, VariableDeclaration, isPropertyAccessExpression, isBinaryExpression, isExpressionStatement, isParameter, isImportClause, ParameterDeclaration, isMemberSymbolInBaseType } from "../ts";
import { registerCodeFix, createCodeFixAction, codeFixAll, DiagnosticAndArguments } from "../ts.codefix";
...
registerCodeFix({
    ...
    getCodeActions(context) {
        ...
        const delDestructure = ChangeTracker.with(context, t => tryDeleteFullDestructure(token, t, sourceFile, checker, sourceFiles, /*isFixAll*/ false));
        if (delDestructure.length) {
            return [createDeleteFix(delDestructure, Diagnostics.Remove_destructuring)];
        }
        const delVar = ChangeTracker.with(context, t => tryDeleteFullVariableStatement(sourceFile, token, t));
        if (delVar.length) {
        ...
        if (token.kind === SyntaxKind.InferKeyword) {
            ...
            const deletion = ChangeTracker.with(context, t => tryDeleteDeclaration(sourceFile, token, t, checker, sourceFiles, /*isFixAll*/ false));
            if (deletion.length) {
...
/* @internal */
function tryDeleteFullDestructure(token: Node, changes: ChangeTracker, sourceFile: SourceFile, checker: TypeChecker, sourceFiles: readonly SourceFile[], isFixAll: boolean): boolean {
    if (token.kind !== SyntaxKind.OpenBraceToken || !isObjectBindingPattern(token.parent))
        return false;
...
/* @internal */
function tryDeleteFullVariableStatement(sourceFile: SourceFile, token: Node, changes: ChangeTracker): boolean {
...
/* @internal */
function tryDeleteDeclaration(sourceFile: SourceFile, token: Node, changes: ChangeTracker, checker: TypeChecker, sourceFiles: readonly SourceFile[], isFixAll: boolean) {
...
/* @internal */
function deleteAssignments(changes: ChangeTracker, sourceFile: SourceFile, token: Identifier, checker: TypeChecker) {
...
/* @internal */
function tryDeleteDeclarationWorker(token: Node, changes: ChangeTracker, sourceFile: SourceFile, checker: TypeChecker, sourceFiles: readonly SourceFile[], isFixAll: boolean): void {
    const { parent } = token;
...
/* @internal */
function tryDeleteParameter(changes: ChangeTracker, sourceFile: SourceFile, p: ParameterDeclaration, checker: TypeChecker, sourceFiles: readonly SourceFile[], isFixAll: boolean): void {
    if (mayDeleteParameter(p, checker, isFixAll)) {
        if (p.modifiers && p.modifiers.length > 0
            && (!isIdentifier(p.name) || Core.isSymbolReferencedInFile(p.name, checker, sourceFile))) {
...
function mayDeleteParameter(p: ParameterDeclaration, checker: TypeChecker, isFixAll: boolean): boolean {
    ...
    switch (parent.kind) {
        case SyntaxKind.MethodDeclaration:
            ...
            if (isMemberSymbolInBaseType(symbol, checker))
                return false;
        // falls through
...
/* @internal */
function deleteUnusedArguments(changes: ChangeTracker, sourceFile: SourceFile, deletedParameter: ParameterDeclaration, sourceFiles: readonly SourceFile[], checker: TypeChecker): void {
    Core.eachSignatureCall(deletedParameter.parent, sourceFiles, checker, call => {
