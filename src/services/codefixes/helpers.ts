import { ClassLikeDeclaration, Symbol, UserPreferences, ClassElement, SymbolTracker, noop, Program, LanguageServiceHost, Node, getEmitScriptTarget, getSynthesizedDeepClone, getNameOfDeclaration, PropertyName, getModifierFlags, createNodeArray, SymbolFlags, NodeFlags, SyntaxKind, NodeBuilderFlags, createProperty, createToken, getAllAccessorDeclarations, AccessorDeclaration, isGetAccessorDeclaration, createGetAccessor, emptyArray, Debug, isSetAccessorDeclaration, getSetAccessorValueParameter, isIdentifier, idText, createSetAccessor, SignatureKind, some, getSynthesizedDeepClones, SignatureDeclaration, Signature, NodeArray, Modifier, Block, MethodDeclaration, forEach, CodeFixContextBase, CallExpression, isInterfaceDeclaration, map, isPropertyAccessExpression, createMethod, isYieldExpression, createTypeParameterDeclaration, CharacterCodes, TypeNode, ParameterDeclaration, createParameter, createKeywordTypeNode, signatureHasRestParameter, createArrayTypeNode, TypeParameterDeclaration, createBlock, createThrow, createNew, createIdentifier, createLiteral, ModifierFlags, TsConfigSourceFile, Expression, getTsConfigObjectLiteralExpression, createObjectLiteral, isObjectLiteralExpression, createPropertyAssignment, createStringLiteral, ObjectLiteralExpression, PropertyAssignment, find, isPropertyAssignment, isStringLiteral, Type, ScriptTarget, isLiteralImportTypeNode, getFirstIdentifier, getNameForExportedSymbol, createTypeReferenceNode, EntityName, Identifier, createQualifiedName } from "../ts";
...
function addNewNodeForMemberSymbol(symbol: Symbol, enclosingDeclaration: ClassLikeDeclaration, context: TypeConstructionContext, preferences: UserPreferences, importAdder: ImportAdder | undefined, addClassElement: (node: Node) => void): void {
    ...
    switch (declaration.kind) {
        ...
        case SyntaxKind.PropertyDeclaration:
            ...
            /*decorators*/ undefined, modifiers, name, optional ? createToken(SyntaxKind.QuestionToken) : undefined, typeNode, 
...
/* @internal */
function signatureToMethodDeclaration(context: TypeConstructionContext, signature: Signature, enclosingDeclaration: ClassLikeDeclaration, modifiers: NodeArray<Modifier> | undefined, name: PropertyName, optional: boolean, body: Block | undefined, importAdder: ImportAdder | undefined): MethodDeclaration | undefined {
...
/* @internal */
export function createMethodFromCallExpression(context: CodeFixContextBase, call: CallExpression, methodName: string, inJs: boolean, makeStatic: boolean, preferences: UserPreferences, contextNode: Node): MethodDeclaration {
    ...
    /*asteriskToken*/ isYieldExpression(parent) ? createToken(SyntaxKind.AsteriskToken) : undefined, methodName, 
    /*questionToken*/ undefined, 
    /*typeParameters*/ inJs ? undefined : map(typeArguments, (_, i) => createTypeParameterDeclaration(CharacterCodes.T + typeArguments!.length - 1 <= CharacterCodes.Z ? String.fromCharCode(CharacterCodes.T + i) : `T${i}`)), 
...
/* @internal */
function createMethodImplementingSignatures(signatures: readonly Signature[], name: PropertyName, optional: boolean, modifiers: readonly Modifier[] | undefined, preferences: UserPreferences): MethodDeclaration {
    ...
    if (someSigHasRestParameter) {
        ...
        /*decorators*/ undefined, 
        /*modifiers*/ undefined, createToken(SyntaxKind.DotDotDotToken), maxArgsParameterSymbolNames[maxNonRestArgs] || "rest", 
        /*questionToken*/ maxNonRestArgs >= minArgumentCount ? createToken(SyntaxKind.QuestionToken) : undefined, anyArrayType, 
        /*initializer*/ undefined);
...
/* @internal */
function createStubbedMethod(modifiers: readonly Modifier[] | undefined, name: PropertyName, optional: boolean, typeParameters: readonly TypeParameterDeclaration[] | undefined, parameters: readonly ParameterDeclaration[], returnType: TypeNode | undefined, preferences: UserPreferences): MethodDeclaration {
    return createMethod(
    /*decorators*/ undefined, modifiers, 
    /*asteriskToken*/ undefined, name, optional ? createToken(SyntaxKind.QuestionToken) : undefined, typeParameters, parameters, returnType, createStubbedMethodBody(preferences));
}
/* @internal */
function createStubbedMethodBody(preferences: UserPreferences): Block {
    return createBlock([createThrow(createNew(createIdentifier("Error"), 
...
/* @internal */
export function setJsonCompilerOptionValues(changeTracker: ChangeTracker, configFile: TsConfigSourceFile, options: [string, Expression][]) {
    ...
    if (compilerOptionsProperty === undefined) {
        changeTracker.insertNodeAtObjectStart(configFile, tsconfigObjectLiteral, createJsonPropertyAssignment("compilerOptions", createObjectLiteral(options.map(([optionName, optionValue]) => createJsonPropertyAssignment(optionName, optionValue)), /*multiLine*/ true)));
        return;
    }
