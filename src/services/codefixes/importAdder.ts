...
import { createCodeFixAction } from "../codeFixProvider";
import {
...
/** @internal */
export const fixId = "fixMissingImport";
...
/** @internal */
export type ImportFix = FixUseNamespaceImport | FixAddJsdocTypeImport | FixAddToExistingImport | FixAddNewImport | FixPromoteTypeOnlyImport;
 ...
 * @internal
 */
export const enum ImportFixKind { UseNamespace, JsdocTypeImport, AddToExisting, AddNew, PromoteTypeOnly }
 ...
 * They're also ordered in terms of increasing priority for a fix-all scenario (see `reduceAddAsTypeOnlyValues`).
 * @internal
 */
export const enum AddAsTypeOnly {
    Allowed    = 1 << 0,
    Required   = 1 << 1,
    NotAllowed = 1 << 2,
}
...
/** @internal */
export interface ImportFixBase {
...
/** @internal */
export interface FixUseNamespaceImport extends ImportFixBase, Qualification {
    readonly kind: ImportFixKind.UseNamespace;
}

/** @internal */
export interface FixAddJsdocTypeImport extends ImportFixBase {
...
/** @internal */
export interface FixAddToExistingImport extends ImportFixBase {
...
/** @internal */
export interface FixAddNewImport extends ImportFixBase {
...
/** @internal */
export function createImportAdder(sourceFile: SourceFile, program: Program, preferences: UserPreferences, host: LanguageServiceHost, useAutoImportProvider: boolean, cancellationToken?: CancellationToken): ImportAdder {
    return createImportAdderWorker(sourceFile, program, useAutoImportProvider, preferences, host, cancellationToken);
}
...
/** @internal */
export function getPromoteTypeOnlyCompletionAction(sourceFile: SourceFile, symbolToken: Identifier, program: Program, host: LanguageServiceHost, formatContext: FormatContext, preferences: UserPreferences) {
    const compilerOptions = program.getCompilerOptions();
...
/** @internal */
export function getFixInfos(context: CodeFixContextBase, errorCode: number, pos: number, useAutoImportProvider: boolean): readonly FixInfo[] | undefined {
    ...
    switch (aliasDeclaration.kind) {
        case SyntaxKind.ImportSpecifier:
            if (aliasDeclaration.isTypeOnly) {
                const sortKind = detectImportSpecifierSorting(aliasDeclaration.parent.elements, preferences);
                if (aliasDeclaration.parent.elements.length > 1 && sortKind) {
                    ...
                    const insertionIndex = getImportSpecifierInsertionIndex(aliasDeclaration.parent.elements, newSpecifier, comparer);
    ...
    function promoteImportClause(importClause: ImportClause) {
        ...
        if (convertExistingToTypeOnly) {
            ...
            if (namedImports && namedImports.elements.length > 1) {
                if (detectImportSpecifierSorting(namedImports.elements, preferences) &&
...
function doAddExistingFix(
    ...
    if (namedImports.length) {
        ...
        if (typeof preferences.organizeImportsIgnoreCase === "boolean") {
            ...
            const targetImportSorting = detectImportSpecifierSorting(existingSpecifiers, preferences);
        ...
        if (ignoreCaseForSorting === undefined) {
            ignoreCaseForSorting = detectSorting(sourceFile, preferences) === SortKind.CaseInsensitive;
        }

        const comparer = getOrganizeImportsComparer(preferences, ignoreCaseForSorting);
        ...
        const specifierSort = existingSpecifiers?.length && detectImportSpecifierSorting(existingSpecifiers, preferences);
        if (specifierSort && !(ignoreCaseForSorting && specifierSort === SortKind.CaseSensitive)) {
            for (const spec of newSpecifiers) {
                ...
                const insertionIndex = promoteFromTypeOnly && !spec.isTypeOnly
                    ? 0
                    : getImportSpecifierInsertionIndex(existingSpecifiers, spec, comparer);
...
function addNamespaceQualifier(changes: ChangeTracker, sourceFile: SourceFile, { namespacePrefix, usagePosition }: Qualification): void {
    changes.insertText(sourceFile, usagePosition, namespacePrefix + ".");
}

function addImportType(changes: ChangeTracker, sourceFile: SourceFile, { moduleSpecifier, usagePosition: position }: FixAddJsdocTypeImport, quotePreference: QuotePreference): void {
