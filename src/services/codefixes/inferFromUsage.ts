import { Diagnostics, getTokenAtPosition, Declaration, returnTrue, getNameOfDeclaration, nodeSeenTracker, Node, DiagnosticMessage, isSetAccessorDeclaration, getContainingFunction, SourceFile, Program, CancellationToken, NodeSeenTracker, LanguageServiceHost, UserPreferences, isParameterPropertyModifier, SyntaxKind, isVariableDeclaration, isPropertyDeclaration, isPropertySignature, isPropertyAccessExpression, getTypeNodeIfAccessible, createJSDocTypeTag, createJSDocTypeExpression, cast, isExpressionStatement, isParameter, isGetAccessorDeclaration, isIdentifier, Debug, VariableDeclaration, PropertyDeclaration, PropertySignature, ParameterDeclaration, FunctionLike, isInJSFile, isArrowFunction, findChildOfKind, first, createToken, last, TypeNode, createJSDocThisTag, SetAccessorDeclaration, firstOrUndefined, Type, tryCast, isVariableStatement, createJSDocReturnTag, getEmitScriptTarget, ScriptTarget, forEach, mapDefined, getJSDocType, getSynthesizedClone, setEmitFlags, EmitFlags, createJSDocParamTag, HasJSDoc, JSDocTag, flatMapToMutable, createJSDocComment, createNodeArray, emptyArray, ArrowFunction, JSDocParameterTag, JSDocReturnTag, PropertyName, Token, Identifier, PrivateIdentifier, SignatureDeclaration, UnderscoreEscapedMap, createUnderscoreEscapedMap, flatMap, isRestParameter, Expression, isRightSideOfQualifiedNameOrPropertyAccess, PrefixUnaryExpression, BinaryExpression, CaseOrDefaultClause, CallExpression, NewExpression, PropertyAccessExpression, ElementAccessExpression, PropertyAssignment, ShorthandPropertyAssignment, isExpressionNode, TypeFlags, isAssignmentExpression, escapeLeadingUnderscores, getObjectFlags, ObjectFlags, AnonymousType, UnionReduction, createMultiMap, SignatureKind, mapEntries, SymbolFlags, __String, TransientSymbol, Symbol, Signature, forEachEntry, TypeReference, singleOrUndefined, UnionOrIntersectionType, SymbolLinks, createSymbolTable, SignatureFlags } from "../ts";
import { registerCodeFix, createCodeFixAction, codeFixAll, createImportAdder, ImportAdder, tryGetAutoImportableReferenceFromImportTypeNode } from "../ts.codefix";
...
registerCodeFix({
    ...
    getCodeActions(context) {
        ...
        const changes = ChangeTracker.with(context, changes => { declaration = doChange(changes, sourceFile, token, errorCode, program, cancellationToken, returnTrue, host, preferences); });
...
/* @internal */
function annotateParameters(changes: ChangeTracker, importAdder: ImportAdder, sourceFile: SourceFile, parameterDeclaration: ParameterDeclaration, containingFunction: FunctionLike, program: Program, host: LanguageServiceHost, cancellationToken: CancellationToken): void {
    ...
    if (isInJSFile(containingFunction)) {
        ...
        if (needParens)
            changes.insertNodeBefore(sourceFile, first(containingFunction.parameters), createToken(SyntaxKind.OpenParenToken));
        ...
        if (needParens)
            changes.insertNodeAfter(sourceFile, last(containingFunction.parameters), createToken(SyntaxKind.CloseParenToken));
    }
}
/* @internal */
...
/* @internal */
function annotateJSDocThis(changes: ChangeTracker, sourceFile: SourceFile, containingFunction: FunctionLike, typeNode: TypeNode) {
...
/* @internal */
function tryReplaceImportTypeNodeWithAutoImport(typeNode: TypeNode, declaration: TypeAnnotatable, type: Type, sourceFile: SourceFile, changes: ChangeTracker, importAdder: ImportAdder, scriptTarget: ScriptTarget): boolean {
...
/* @internal */
function addJSDocTags(changes: ChangeTracker, sourceFile: SourceFile, parent: HasJSDoc, newTags: readonly JSDocTag[]): void {
