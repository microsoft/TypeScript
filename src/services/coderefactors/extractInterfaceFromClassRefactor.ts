            ...
            if (token.kind == SyntaxKind.PropertyDeclaration) {
                ...
                if (propertyDeclaration.initializer) {
                    ...
                    if (token.parent.kind === SyntaxKind.ClassDeclaration) {
                        result.push({ newText: interfaceText, span: { start: token.parent.pos, length: 0 } });
                    } else {
                        result.push({ newText: interfaceText, span: { start: token.pos, length: 0 } });
                    }

                    let firstAssignmentPosition: number = getFirstPositionOf(propertyDeclaration, SyntaxKind.FirstAssignment);
                    result.push({ newText: " : " + interfaceName, span: { start: firstAssignmentPosition, length: 0 } });
    ...
    registerCodeRefactor({
        ...
        getTextChanges: (token: Node, context: CodeFixContext): TextChange[] => {
            if (token.kind == SyntaxKind.ClassDeclaration) {
                ...
                result.push({ newText: " implements " + interfaceName, span: { start: firstPunctuationPosition, length: 0 } });
