    ...
    function getCompletionData(
        ...
        function tryGetObjectLikeCompletionSymbols(): GlobalsSearch | undefined {
            ...
            if (objectLikeContainer.kind === SyntaxKind.ObjectLiteralExpression) {
                ...
                const hasOpenStringIndexType = some(typeChecker.getIndexInfosOfType(completionsType || instantiatedType), i => !!forEachType(i.indexType, t => !!(t.flags & TypeFlags.String) || !!(t.flags & TypeFlags.TemplateLiteral)));
                const hasOtherOpenIndexType = some(typeChecker.getIndexInfosOfType(completionsType || instantiatedType), i => !!forEachType(i.indexType, t => !!(t.flags & TypeFlags.Number) || !!(t.flags & TypeFlags.ESSymbol)));
                isNewIdentifierLocation = !!hasOpenStringIndexType || !!hasOtherOpenIndexType;
                typeMembers = getPropertiesForObjectExpression(instantiatedType, completionsType, objectLikeContainer, typeChecker);
                existingMembers = objectLikeContainer.properties;

                if (typeMembers.length === 0) {
                    // If there is only a non-string indexer, we are OK to return empty completions, otherwise we should return the global completions
                    if (!hasOtherOpenIndexType || hasOpenStringIndexType) {
                        isNonContextualObjectLiteral = true;
                        return GlobalsSearch.Continue;
                    }
                }
