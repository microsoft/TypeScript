    ...
    function getCompletionData(
        ...
        function getGetClassLikeCompletionSymbols(classLikeDeclaration: ClassLikeDeclaration) {
            ...
            if (!(classElementModifierFlags & ModifierFlags.Private)) {
                ...
                const baseSymbols = flatMap(getAllSuperTypeNodes(classLikeDeclaration), baseTypeNode => {
                    const type = typeChecker.getTypeAtLocation(baseTypeNode);
                    return typeChecker.getPropertiesOfType(classElementModifierFlags & ModifierFlags.Static ? typeChecker.getTypeOfSymbolAtLocation(type.symbol, classLikeDeclaration) : type);
                });
                symbols = filterClassMembersList(baseSymbols, classLikeDeclaration.members, classElementModifierFlags);
            }
        }
