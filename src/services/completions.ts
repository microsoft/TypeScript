...
function createCompletionEntry(
    ...
    importCompletionNode: ts.Node | undefined,
...
function isClassLikeMemberCompletion(symbol: ts.Symbol, location: ts.Node): boolean {
...
function getInsertTextAndReplacementSpanForImportCompletion(name: string, importCompletionNode: ts.Node, contextToken: ts.Node | undefined, origin: SymbolOriginInfoResolvedExport, useSemicolons: boolean, options: ts.CompilerOptions, preferences: ts.UserPreferences) {
    const sourceFile = importCompletionNode.getSourceFile();
    const replacementSpan = ts.createTextSpanFromNode(ts.findAncestor(importCompletionNode, ts.or(ts.isImportDeclaration, ts.isImportEqualsDeclaration)) || importCompletionNode, sourceFile);
    ...
    const isTopLevelTypeOnly = ts.tryCast(importCompletionNode, ts.isImportDeclaration)?.importClause?.isTypeOnly || ts.tryCast(importCompletionNode, ts.isImportEqualsDeclaration)?.isTypeOnly;
    const isImportSpecifierTypeOnly = couldBeTypeOnlyImportSpecifier(importCompletionNode, contextToken);
    const topLevelTypeOnlyText = isTopLevelTypeOnly ? ` ${ts.tokenToString(ts.SyntaxKind.TypeKeyword)} ` : " ";
...
interface CompletionData {
    ...
    readonly importCompletionNode?: ts.Node;
...
function getCompletionData(
    ...
    let importCompletionNode: ts.Node | undefined;
    ...
    if (contextToken) {
        ...
        else if (!importCompletionNode && sourceFile.languageVariant === ts.LanguageVariant.JSX) {
            ...
            context => {
                exportInfo.search(
                    ...
                    (symbolName, targetFlags) => {
                        ...
                        if (!isTypeOnlyLocation && !importCompletionNode && !(targetFlags & ts.SymbolFlags.Value)) return false;
...
interface ImportStatementCompletionInfo {
    ...
    replacementNode: ts.ImportEqualsDeclaration | ts.ImportDeclaration | ts.ImportSpecifier | ts.Token<ts.SyntaxKind.ImportKeyword> | undefined;
}
