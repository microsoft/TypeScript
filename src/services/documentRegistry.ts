import { CompilerOptions, IScriptSnapshot, ScriptKind, SourceFile, Path, createMap, createGetCanonicalFileName, arrayFrom, toPath, getOrUpdate, ScriptTarget, Debug, createLanguageServiceSourceFile, updateLanguageServiceSourceFile, sourceFileAffectingCompilerOptions, getCompilerOptionValue } from "./ts";
...
export function createDocumentRegistryInternal(useCaseSensitiveFileNames?: boolean, currentDirectory = "", externalCache?: ExternalDocumentCache): DocumentRegistry {
    ...
    // for those settings.
    const buckets = createMap<ts.Map<DocumentRegistryEntry>>();
    ...
    function acquireOrUpdateDocument(fileName: string, path: Path, compilationSettings: CompilerOptions, key: DocumentRegistryBucketKey, scriptSnapshot: IScriptSnapshot, version: string, acquiring: boolean, scriptKind?: ScriptKind): SourceFile {
        const bucket = getOrUpdate<ts.Map<DocumentRegistryEntry>>(buckets, key, createMap);
