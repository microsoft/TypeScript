import { CompletionEntry, Declaration, TypeChecker, SymbolDisplayPart, forEachUnique, isJSDoc, SyntaxKind, contains, flatten, intersperse, lineBreakPart, arraysEqual, JSDoc, JSDocTag, JSDocPropertyTag, JSDocTypedefTag, getJSDocCommentsAndTags, JSDocTagInfo, getJSDocTags, JSDocComment, textPart, flatMap, buildLinkParts, JSDocImplementsTag, JSDocAugmentsTag, JSDocTemplateTag, JSDocTypeTag, JSDocCallbackTag, JSDocParameterTag, JSDocSeeTag, Node, spacePart, parameterNamePart, propertyNamePart, typeParameterNamePart, typeAliasNamePart, map, ScriptElementKind, CompletionEntryDetails, emptyArray, isIdentifier, isFunctionLike, mapDefined, isJSDocParameterTag, startsWith, SourceFile, DocCommentTemplateOptions, TextInsertion, getTokenAtPosition, findAncestor, length, hasJSFileExtension, getLineStartPositionForPosition, isWhiteSpaceSingleLine, ParameterDeclaration, forEachAncestor, ArrowFunction, FunctionDeclaration, MethodDeclaration, ConstructorDeclaration, MethodSignature, PropertyAssignment, VariableStatement, ExpressionStatement, BinaryExpression, getAssignmentDeclarationKind, AssignmentDeclarationKind, PropertyDeclaration, isFunctionExpression, isArrowFunction, isExpression, isFunctionLikeDeclaration, isBlock, forEachReturnStatement, Expression, FunctionExpression, ParenthesizedExpression, find, ClassExpression, isConstructorDeclaration } from "./ts";
import { SortText } from "./ts.Completions";
/* @internal */
const jsDocTagNames = [
    "abstract",
    "access",
...
function getDisplayPartsFromComment(comment: string | readonly JSDocComment[], checker: TypeChecker | undefined): SymbolDisplayPart[] {
    ...
    return flatMap(comment, node => node.kind === SyntaxKind.JSDocText ? [textPart(node.text)] : buildLinkParts(node, checker)) as SymbolDisplayPart[];
}
