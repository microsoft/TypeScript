 ...
 *   3) Sorting imports
 */
export function organizeImports(sourceFile: SourceFile, formatContext: FormatContext, host: LanguageServiceHost, program: Program, preferences: UserPreferences) {
...
function removeUnusedImports(oldImports: readonly ImportDeclaration[], sourceFile: SourceFile, program: Program) {
    ...
    for (const importDecl of oldImports) {
        ...
        // If a module is imported to be augmented, it’s used
            ...
            if (sourceFile.isDeclarationFile) {
                usedImports.push(createImportDeclaration(importDecl.decorators, importDecl.modifiers, 
    ...
    function isDeclarationUsed(identifier: Identifier) {
        ...
        return jsxElementsPresent && (identifier.text === jsxNamespace) || Core.isSymbolReferencedInFile(identifier, typeChecker, sourceFile);
    }
}
/* @internal */
    ...
    for (const group of [regularImports, typeOnlyImports]) {
        ...
        if (defaultImports.length === 1) {
            ...
            for (const defaultImport of defaultImports) {
                newImportSpecifiers.push(createImportSpecifier(createIdentifier("default"), (defaultImport.importClause!.name!))); // TODO: GH#18217
            }
        }
...
export function coalesceExports(exportGroup: readonly ExportDeclaration[]) {
    ...
    for (const exportGroup of [namedExports, typeOnlyExports]) {
        ...
        coalescedExports.push(updateExportDeclaration(exportDecl, exportDecl.decorators, exportDecl.modifiers, exportDecl.exportClause && (isNamedExports(exportDecl.exportClause) ?
            updateNamedExports(exportDecl.exportClause, sortedExportSpecifiers) :
            updateNamespaceExport(exportDecl.exportClause, exportDecl.exportClause.name)), exportDecl.moduleSpecifier, exportDecl.isTypeOnly));
    }
...
function updateImportDeclarationAndClause(importDeclaration: ImportDeclaration, name: Identifier | undefined, namedBindings: NamedImportBindings | undefined) {
    return updateImportDeclaration(importDeclaration, importDeclaration.decorators, importDeclaration.modifiers, updateImportClause((importDeclaration.importClause!), name, namedBindings, importDeclaration.importClause!.isTypeOnly), // TODO: GH#18217
    importDeclaration.moduleSpecifier);
}
/* @internal */
function sortSpecifiers<T extends ImportOrExportSpecifier>(specifiers: readonly T[]) {
    return stableSort(specifiers, (s1, s2) => compareIdentifiers(s1.propertyName || s1.name, s2.propertyName || s2.name) ||
