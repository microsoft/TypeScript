...
registerRefactor(refactorName, {
    ...
    getEditsForAction(context, actionName): RefactorEditInfo {
        ...
        const edits = ChangeTracker.with(context, t => doChange(context.file, context.program, Debug.checkDefined(getInfo(context), "context must have info"), t, context.cancellationToken));
...
/* @internal */
function doChange(exportingSourceFile: SourceFile, program: Program, info: Info, changes: ChangeTracker, cancellationToken: CancellationToken | undefined): void {
    changeExport(exportingSourceFile, info, changes, program.getTypeChecker());
    changeImports(program, info, changes, cancellationToken);
}
/* @internal */
function changeExport(exportingSourceFile: SourceFile, { wasDefault, exportNode, exportName }: Info, changes: ChangeTracker, checker: TypeChecker): void {
    if (wasDefault) {
        ...
        switch (exportNode.kind) {
            ...
            case SyntaxKind.VariableStatement:
                ...
                if (!Core.isSymbolReferencedInFile(exportName, checker, exportingSourceFile)) {
...
/* @internal */
function changeImports(program: Program, { wasDefault, exportName, exportingModuleSymbol }: Info, changes: ChangeTracker, cancellationToken: CancellationToken | undefined): void {
    ...
    Core.eachExportReference(program.getSourceFiles(), checker, cancellationToken, exportSymbol, exportingModuleSymbol, exportName.text, wasDefault, ref => {
