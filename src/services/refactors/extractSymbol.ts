...
export function getAvailableActions(context: RefactorContext): readonly ApplicableRefactorInfo[] {
    ...
    const usedFunctionNames: ts.Map<boolean> = createMap();
    const constantActions: RefactorActionInfo[] = [];
    const usedConstantNames: ts.Map<boolean> = createMap();
    let i = 0;
...
function extractFunctionInScope(node: Statement | Expression | Block, scope: Scope, { usages: usagesInScope, typeParameterUsages, substitutions }: ScopeUsages, exposedVariableDeclarations: readonly VariableDeclaration[], range: TargetRange, context: RefactorContext): RefactorEditInfo {
    ...
    if (isClassLike(scope)) {
        ...
        /*decorators*/ undefined, modifiers.length ? modifiers : undefined, range.facts & RangeFacts.IsGenerator ? createToken(SyntaxKind.AsteriskToken) : undefined, functionName, 
        /*questionToken*/ undefined, typeParameters, parameters, returnType, body);
    }
    else {
        newFunction = createFunctionDeclaration(
        /*decorators*/ undefined, range.facts & RangeFacts.IsAsyncFunction ? [createToken(SyntaxKind.AsyncKeyword)] : undefined, range.facts & RangeFacts.IsGenerator ? createToken(SyntaxKind.AsteriskToken) : undefined, functionName, typeParameters, parameters, returnType, body);
    }
    ...
    let call: Expression = createCall(called, callTypeArguments, // Note that no attempt is made to take advantage of type argument inference
    callArguments);
    ...
    if (exposedVariableDeclarations.length && !writes) {
        ...
        if (exposedVariableDeclarations.length === 1) {
            ...
            /*modifiers*/ undefined, createVariableDeclarationList([createVariableDeclaration(getSynthesizedDeepClone(variableDeclaration.name), /*type*/ getSynthesizedDeepClone(variableDeclaration.type), /*initializer*/ call)], // TODO (acasey): test binding patterns
            ...
            newNodes.push(createVariableStatement(
            /*modifiers*/ undefined, createVariableDeclarationList([createVariableDeclaration(createObjectBindingPattern(bindingElements), 
                /*type*/ typeLiteral, 
        ...
        if (exposedVariableDeclarations.length) {
            ...
            for (const variableDeclaration of exposedVariableDeclarations) {
                ...
                newNodes.push(createVariableStatement(
                /*modifiers*/ undefined, createVariableDeclarationList([createVariableDeclaration(variableDeclaration.symbol.name, getTypeDeepCloneUnionUndefined(variableDeclaration.type))], flags)));
            }
        }
        if (returnValueProperty) {
            // has both writes and return, need to create variable declaration to hold return value;
            newNodes.push(createVariableStatement(
            /*modifiers*/ undefined, createVariableDeclarationList([createVariableDeclaration(returnValueProperty, getTypeDeepCloneUnionUndefined(returnType))], NodeFlags.Let)));
        }
...
function extractConstantInScope(node: Expression, scope: Scope, { substitutions }: ScopeUsages, rangeFacts: RangeFacts, context: RefactorContext): RefactorEditInfo {
    ...
    if (isClassLike(scope)) {
        ...
        const localReference = createPropertyAccess(rangeFacts & RangeFacts.InStaticRegion
            ? createIdentifier(scope.name!.getText()) // TODO: GH#18217
            : createThis(), createIdentifier(localNameText));
        // Declare
        ...
        if (oldVariableDeclaration) {
            ...
            /*modifiers*/ undefined, createVariableDeclarationList([newVariableDeclaration], NodeFlags.Const));
    ...
    function transformFunctionInitializerAndType(variableType: TypeNode | undefined, initializer: Expression): {
        ...
        for (const p of initializer.parameters) {
            if (p.type) {
                ...
                if (paramType === checker.getAnyType())
                    hasAny = true;
                parameters.push(updateParameter(p, p.decorators, p.modifiers, p.dotDotDotToken, p.name, p.questionToken, p.type || checker.typeToTypeNode(paramType, scope, NodeBuilderFlags.NoTruncation), p.initializer));
            }
        }
        ...
        if (isArrowFunction(initializer)) {
            initializer = updateArrowFunction(initializer, node.modifiers, initializer.typeParameters, parameters, initializer.type || checker.typeToTypeNode(functionSignature.getReturnType(), scope, NodeBuilderFlags.NoTruncation), initializer.equalsGreaterThanToken, initializer.body);
        }
        else {
            ...
            initializer = updateFunctionExpression(initializer, node.modifiers, initializer.asteriskToken, initializer.name, initializer.typeParameters, parameters, initializer.type || checker.typeToTypeNode(functionSignature.getReturnType(), scope, NodeBuilderFlags.NoTruncation), initializer.body);
        }
        return { variableType, initializer };
    }
}
...
/* @internal */
function transformFunctionBody(body: Node, exposedVariableDeclarations: readonly VariableDeclaration[], writes: readonly UsageEntry[] | undefined, substitutions: ts.ReadonlyMap<Node>, hasReturn: boolean): {
    body: Block;
...
/* @internal */
function transformConstantInitializer(initializer: Expression, substitutions: ts.ReadonlyMap<Node>): Expression {
...
interface ScopeUsages {
    ...
    readonly typeParameterUsages: ts.Map<TypeParameter>; // Key is type ID
    readonly substitutions: ts.Map<Node>;
}
/* @internal */
...
function collectReadsAndWrites(targetRange: TargetRange, scopes: Scope[], enclosingTextRange: TextRange, sourceFile: SourceFile, checker: TypeChecker, cancellationToken: CancellationToken): ReadsAndWrites {
    ...
    const substitutionsPerScope: ts.Map<Node>[] = [];
    ...
    for (const scope of scopes) {
        ...
        functionErrorsPerScope.push(isFunctionLikeDeclaration(scope) && scope.kind !== SyntaxKind.FunctionDeclaration
