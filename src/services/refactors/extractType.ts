...
function doTypeAliasChange(changes: ChangeTracker, file: SourceFile, name: string, info: TypeAliasInfo) {
    ...
    /* modifiers */ undefined, name, typeParameters.map(id => updateTypeParameterDeclaration(id, id.name, id.constraint, /* defaultType */ undefined)), selection);
...
/* @internal */
function doTypedefChange(changes: ChangeTracker, file: SourceFile, name: string, info: Info) {
    const { firstStatement, selection, typeParameters } = info;
