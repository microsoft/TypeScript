...
registerRefactor(refactorNameForMoveToFile, {
    ...
    getAvailableActions: function getRefactorActionsToMoveToFile(
        ...
        // Use robust statement detector
        const statements = getStatementsToMove(context);
        if (!statements?.all?.length) return emptyArray;

        // Reject control-flow statements (always).
        const s = statements.all[0];
        switch (s.kind) {
        ...
        // Block refactor when selection is inside an unbraced if/else statement
        // This prevents extracting partial statements that would create invalid code
        ...
        // Only offer move-to-file when the host/preferences allow creating/updating files.
        if (context.preferences.allowTextChangesInNewFiles && statements) {
            const affectedTextRange = {
                start: {
                    line: getLineAndCharacterOfPosition(
                        file,
                        statements.all[0].getStart(file)
                    ).line,
                    offset: getLineAndCharacterOfPosition(
                        file,
                        statements.all[0].getStart(file)
                    ).character,
                },
            ...
            return [
                {
                    ...
                    description,
                    actions: [
                        { ...moveToFileAction, range: affectedTextRange },
                    ],
                },
            ];
        }
...
function error(notApplicableReason: string): RefactorEditInfo {
    return {
        edits: [],
        renameFilename: undefined,
        renameLocation: undefined,
...
/**
 * Robust check: are start & end inside the same if statement's then/else clause which lacks braces?
 */
function isInsideIfStatementWithoutBraces(context: RefactorContext): boolean {
    ...
    const endToken = getTokenAtPosition(file, endPosition);

    const startIf = findAncestor(startToken, isIfStatement);
    const endIf = findAncestor(endToken, isIfStatement);

    if (!startIf || startIf !== endIf) return false;

    const ifStmt = startIf;

    // THEN: check non-block thenStatement boundaries
    const then = ifStmt.thenStatement;
    if (then && !isBlock(then)) {
        const thenStart = then.getStart(file);
        const thenEnd = then.end;
        if (startPosition >= thenStart && endPosition <= thenEnd) return true;
    }

    // ELSE: only if plain statement (not else-if) and not a block
...
function doChange(
    ...
    const targetSourceFile = isForNewFile
        ? createFutureSourceFile(
              targetFile,
              oldFile.externalModuleIndicator
                  ? ModuleKind.ESNext
    ...
    const importAdderForOldFile = codefix.createImportAdder(
        oldFile,
        context.program,
        context.preferences,
        context.host
    );
    const importAdderForNewFile = codefix.createImportAdder(
        targetSourceFile,
        context.program,
        context.preferences,
        context.host
    ...
    getNewStatementsAndRemoveFromOldFile(
        ...
        getUsageInfo(
            ...
            isForNewFile
                ? undefined
                : getExistingLocals(
                      targetSourceFile as SourceFile,
                      toMove.all,
                      checker
                  )
        ),
...
export function getNewStatementsAndRemoveFromOldFile(
    ...
    if (!isFullSourceFile(targetFile) && prologueDirectives.length) {
        ...
        changes.insertStatementsInNewFile(
            targetFile.fileName,
            prologueDirectives,
            oldFile
        );
    ...
    const body = addExports(
        oldFile,
        toMove.all,
        arrayFrom(usage.oldFileImportsFromTargetFile.keys()),
        useEsModuleSyntax
    );
    if (isFullSourceFile(targetFile) && targetFile.statements.length > 0) {
        ...
        changes.insertStatementsInNewFile(
            targetFile.fileName,
            importAdderForNewFile.hasFixes()
                ? [SyntaxKind.NewLineTrivia, ...body]
                : body,
            oldFile
        );
    }
...
/** @internal */
export function addExportsInOldFile(
    oldFile: SourceFile,
    ...
    for (const sourceFile of program.getSourceFiles()) {
        ...
        for (const statement of sourceFile.statements) {
            forEachImportInStatement(statement, (importNode) => {
                ...
                if (
                    getStringComparer(!program.useCaseSensitiveFileNames())(
...
function forEachAliasDeclarationInImportOrRequire(
    importOrRequire: SupportedImport,
    cb: (declaration: codefix.ImportOrRequireAliasDeclaration) => void
): void {
    if (importOrRequire.kind === SyntaxKind.ImportDeclaration) {
        ...
        if (
            importOrRequire.importClause?.namedBindings?.kind ===
            SyntaxKind.NamespaceImport
        ) {
        ...
        if (
            ...
            for (const element of importOrRequire.importClause.namedBindings
                .elements) {
                cb(element);
            }
        }
        ...
        if (importOrRequire.name.kind === SyntaxKind.Identifier) {
            ...
            importOrRequire.name.kind === SyntaxKind.ObjectBindingPattern
        ) {
...
export function addImportsForMovedSymbols(
    ...
    for (const [symbol, isValidTypeOnlyUseSite] of symbols) {
        ...
        const exportKind =
            symbol.name === "default" && symbol.parent
                ? ExportKind.Default
                : ExportKind.Named;
        importAdder.addImportForNonExistentExport(
            symbolName,
            targetFileName,
            exportKind,
            symbol.flags,
...
function deleteUnusedImports(
    ...
    if (
        importDecl.kind === SyntaxKind.ImportDeclaration &&
        ...
        default:
            return Debug.assertNever(
                ...
                `Unexpected declaration kind ${
                    (d as DeclarationStatement).kind
                }`
            );
    }
}
        ...
        default:
            return Debug.assertNever(
                ...
                `Unexpected decl kind ${
                    (decl as TopLevelDeclarationStatement).kind
                }`
            );
    }
}
...
export interface UsageInfo {
    ...
    readonly oldImportsNeededByTargetFile: Map<
        Symbol,
        [boolean, codefix.ImportOrRequireAliasDeclaration | undefined]
    >;
...
function _getRangeToMove(context: RefactorContext): RangeToMove | undefined {
    const { file } = context;
...
function isTopLevelStatement(
    statement: Statement,
    sourceFile: SourceFile
): boolean {
    return statement.parent === sourceFile;
}
...
export function getStatementsToMove(
    ...
    const span = { start: startPosition, length: endPosition - startPosition };
    const selectedRange = { pos: span.start, end: span.start + span.length };

    // 1. Find the smallest statement that fully contains the selection
    const rootStatement = findAncestor(
        getTokenAtPosition(file, startPosition),
        (n) => isStatement(n)
    );

    if (!rootStatement) return undefined;

    // Reject non-top-level or nested statements directly (loop bodies, if-bodies etc.)
    if (!isTopLevelStatement(rootStatement, file)) {
        return undefined;
    }

    // Reject statements inside unbraced if/else branches
    // This prevents extracting partial statements like 'const x = 1;' from 'if (true) const x = 1;'
    if (isInsideIfStatementWithoutBraces(context)) {
        return undefined;
    }

    // 2. Collect all statements in the file
    const statements = file.statements;

    // 3. Filter statements that are fully contained in the selection
    const selectedStatements = statements.filter((stmt) =>
        rangeContainsRange(selectedRange, stmt)
    );

    // 4. Reject empty / partial selections
    if (selectedStatements.length === 0) return undefined;

    // 5. Reject selections that intersect but do not fully contain statements
    for (const stmt of statements) {
        ...
        if (intersects && !fullyInside) {
            return undefined; // partial = illegal
        }
    }

    // 6. Reject control-flow statements unconditionally
    ...
    const lastSelected = selectedStatements[selectedStatements.length - 1];
    const lastSelectedIndex = statements.indexOf(lastSelected);
    const afterLast =
        lastSelectedIndex >= 0 && lastSelectedIndex < statements.length - 1
            ? statements[lastSelectedIndex + 1]
            : undefined;

    return {
        all: selectedStatements,
        ranges: [
            {
                first: selectedStatements[0],
                afterLast,
            },
        ],
    };
...
function _isAllowedStatementToMove(statement: Statement): boolean {
...
export function getUsageInfo(
    ...
    if (jsxNamespaceSymbol) {
        ...
        oldImportsNeededByTargetFile.set(jsxNamespaceSymbol, [
            false,
            tryCast(
                jsxNamespaceSymbol.declarations?.[0],
                (d): d is codefix.ImportOrRequireAliasDeclaration =>
                    isImportSpecifier(d) ||
                    isImportClause(d) ||
                    isNamespaceImport(d) ||
    ...
    for (const statement of toMove) {
        forEachReference(
            ...
            (symbol, isValidTypeOnlyUseSite) => {
                ...
                const importedDeclaration = find(
                    symbol.declarations,
                    isInImport
                );
                if (importedDeclaration) {
                    ...
                    targetFileImportsFromOldFile.set(
                        symbol,
                        isValidTypeOnlyUseSite
                    );
                }
        ...
        forEachReference(
            ...
            (symbol, isValidTypeOnlyUseSite) => {
                if (movedSymbols.has(symbol))
                    oldFileImportsFromTargetFile.set(
                        symbol,
                        isValidTypeOnlyUseSite
                    );
...
function inferNewFileName(
    importsFromNewFile: Map<Symbol, unknown>,
    movedSymbols: Set<Symbol>
): string {
    return (
...
function forEachReference(
    node: Node,
    checker: TypeChecker,
    enclosingRange: TextRange | undefined,
                    ...
                    const exportDeclaration = firstDefined(
                        ...
                        (d) =>
                            isExportDeclaration(d)
                                ? d
                                : isExportSpecifier(d)
                                ? tryCast(d.parent.parent, isExportDeclaration)
                                : undefined
                    );
                    if (
                        exportDeclaration &&
...
/** @internal */
export function getExistingLocals(
    sourceFile: SourceFile,
    statements: readonly Statement[],
    checker: TypeChecker
): Set<Symbol> {
        ...
        forEachReference(
            ...
            (s) => {
                ...
                if (
                    ...
                    getSourceFileOfNode(symbol.valueDeclaration).path ===
                        sourceFile.path
                ) {
