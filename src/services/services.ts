...
namespace ts {
    ...
    class SourceFileObject extends NodeObject implements SourceFile {
        ...
        private computeNamedDeclarations(): Map<Declaration[]> {
            const result = Map.create<Declaration[]>();
            ...
            function getDeclarations(name: string) {
                return Map.get(result, name) || (result[name] = []);
            }
    ...
    function fixupCompilerOptions(options: CompilerOptions, diagnostics: Diagnostic[]): CompilerOptions {
        ...
        commandLineOptionsStringToEnum = commandLineOptionsStringToEnum || <CommandLineOptionOfCustomType[]>filter(optionDeclarations, o =>
            typeof o.type === "object" && !MapLike.forEach(o.type, v => typeof v !== "number"));

        options = MapLike.clone(options);

        for (const opt of commandLineOptionsStringToEnum) {
            if (!MapLike.has(options, opt.name)) {
                continue;
            }
            ...
            if (typeof value === "string") {
                ...
                if (!MapLike.forEach(opt.type, v => v === value)) {
    ...
    export function transpileModule(input: string, transpileOptions: TranspileOptions): TranspileOutput {
        ...
        sourceFile.renamedDependencies = Map.create(transpileOptions.renamedDependencies);
    ...
    export function createDocumentRegistry(useCaseSensitiveFileNames?: boolean, currentDirectory = ""): DocumentRegistry {
        ...
        // for those settings.
        const buckets = Map.create<FileMap<DocumentRegistryEntry>>();
        ...
        function reportStats() {
            const bucketInfoArray = Object.keys(buckets).filter(name => name && name.charAt(0) === "_").map(name => {
                const entries = Map.get(buckets, name);
    ...
    export function createLanguageService(host: LanguageServiceHost,
        ...
        function synchronizeHostData(): void {
            ...
            const shouldCreateNewSourceFiles = oldSettings &&
                (oldSettings.target !== newSettings.target ||
                 ...
                 !MapLike.equals(oldSettings.paths, newSettings.paths));
        ...
        function getCompletionData(fileName: string, position: number) {
            ...
            function filterNamedImportOrExportCompletionItems(exportsOfModule: Symbol[], namedImportsOrExports: ImportOrExportSpecifier[]): Symbol[] {
                const existingImportsOrExports = Map.create<boolean>();
                ...
                if (!Map.some(existingImportsOrExports)) {
                    return filter(exportsOfModule, e => e.name !== "default");
                }

                return filter(exportsOfModule, e => e.name !== "default" && !Map.get(existingImportsOrExports, e.name));
            }
            ...
            function filterObjectMembersList(contextualMemberSymbols: Symbol[], existingMembers: Declaration[]): Symbol[] {
                ...
                const existingMemberNames = Map.create<boolean>();
                ...
                return filter(contextualMemberSymbols, m => !Map.get(existingMemberNames, m.name));
            }
            ...
            function filterJsxAttributes(symbols: Symbol[], attributes: NodeArray<JsxAttribute | JsxSpreadAttribute>): Symbol[] {
                const seenNames = Map.create<boolean>();
                ...
                return filter(symbols, a => !Map.get(seenNames, a.name));
            }
        }
        ...
        function getCompletionsAtPosition(fileName: string, position: number): CompletionInfo {
            ...
            function getJavaScriptCompletionEntries(sourceFile: SourceFile, position: number, uniqueNames: Map<string>): CompletionEntry[] {
                ...
                for (const name in nameTable) if (Map.guard(nameTable, name)) {
        ...
        function getDefinitionAtPosition(fileName: string, position: number): DefinitionInfo[] {
            ...
            if (typeReferenceDirective) {
                const referenceFile = Map.get(program.getResolvedTypeReferenceDirectives(), typeReferenceDirective.fileName);
        ...
        function getDocumentHighlights(fileName: string, position: number, filesToSearch: string[]): DocumentHighlights[] {
            ...
            function getSemanticDocumentHighlights(node: Node): DocumentHighlights[] {
                ...
                function convertReferencedSymbols(referencedSymbols: ReferencedSymbol[]): DocumentHighlights[] {
                    ...
                    const fileNameToDocumentHighlights = Map.create<DocumentHighlights>();
                    ...
                    for (const referencedSymbol of referencedSymbols) {
                        for (const referenceEntry of referencedSymbol.references) {
                            ...
                            let documentHighlights = Map.get(fileNameToDocumentHighlights, fileName);
        ...
        function getReferencedSymbolsForNode(node: Node, sourceFiles: SourceFile[], findInStrings: boolean, findInComments: boolean): ReferencedSymbol[] {
            ...
            if (scope) {
                ...
                for (const sourceFile of sourceFiles) {
                    ...
                    if (Map.get(nameTable, internedName) !== undefined) {
                        result = result || [];
            ...
            function populateSearchSymbolSet(symbol: Symbol, location: Node): Symbol[] {
                ...
                forEach(typeChecker.getRootSymbols(symbol), rootSymbol => {
                    ...
                    if (rootSymbol.parent && rootSymbol.parent.flags & (SymbolFlags.Class | SymbolFlags.Interface)) {
                        getPropertySymbolsFromBaseTypes(rootSymbol.parent, rootSymbol.getName(), result, /*previousIterationSymbolsCache*/ Map.create<Symbol>());
                    }
                });
            ...
            function getPropertySymbolsFromBaseTypes(symbol: Symbol, propertyName: string, result: Symbol[],
                ...
                if (Map.has(previousIterationSymbolsCache, symbol.name)) {
                    return;
                }
    ...
    function initializeNameTable(sourceFile: SourceFile): void {
        const nameTable = Map.create<number>();
