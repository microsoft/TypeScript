namespace ts {
    ...
    function createNode<TKind extends SyntaxKind>(kind: TKind, pos: number, end: number, parent: Node) {
        const node = isNodeKind(kind) ? new NodeObject(kind, pos, end) :
            kind === SyntaxKind.Identifier ? new IdentifierObject(SyntaxKind.Identifier, pos, end) :
                kind === SyntaxKind.PrivateIdentifier ? new PrivateIdentifierObject(SyntaxKind.PrivateIdentifier, pos, end) :
                    isTokenSyntaxKind(kind) ? new TokenObject(kind, pos, end) : Debug.fail("Could not create Node by kind.");
        node.parent = parent;
