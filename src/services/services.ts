...
import { getImplementationsAtPosition, toContextSpan, FindReferencesUse, toRenameLocation, toReferenceEntry, Options, ToReferenceOrRenameEntry, findReferenceOrRenameEntries, findReferencedSymbols } from "./ts.FindAllReferences";
...
class SourceFileObject extends NodeObject implements SourceFile {
    ...
    public resolvedModules: ts.Map<ResolvedModuleFull> | undefined;
    public resolvedTypeReferenceDirectiveNames!: ts.Map<ResolvedTypeReferenceDirective>;
    public imports!: readonly StringLiteralLike[];
    public moduleAugmentations!: StringLiteral[];
    private namedDeclarations: ts.Map<Declaration[]> | undefined;
    ...
    public getNamedDeclarations(): ts.Map<Declaration[]> {
        if (!this.namedDeclarations) {
            this.namedDeclarations = this.computeNamedDeclarations();
        }
        return this.namedDeclarations;
    }
    private computeNamedDeclarations(): ts.Map<Declaration[]> {
...
export function createLanguageService(host: LanguageServiceHost, documentRegistry: DocumentRegistry = createDocumentRegistry(host.useCaseSensitiveFileNames && host.useCaseSensitiveFileNames(), host.getCurrentDirectory()), syntaxOnly = false): LanguageService {
    ...
    function dispose(): void {
        if (program) {
            forEach(program.getSourceFiles(), f => documentRegistry.releaseDocument(f.fileName, program.getCompilerOptions()));
            program = undefined!; // TODO: GH#18217
        }
        host = undefined!;
    }
    ...
    function getCompletionsAtPosition(fileName: string, position: number, options: GetCompletionsAtPositionOptions = emptyOptions): CompletionInfo | undefined {
        ...
        synchronizeHostData();
        return Completions.getCompletionsAtPosition(host, program, log, getValidSourceFile(fileName), position, fullPreferences, options.triggerCharacter);
    }
    function getCompletionEntryDetails(fileName: string, position: number, name: string, formattingOptions: FormatCodeSettings | undefined, source: string | undefined, preferences: UserPreferences = emptyOptions): CompletionEntryDetails | undefined {
        synchronizeHostData();
        return Completions.getCompletionEntryDetails(program, log, getValidSourceFile(fileName), position, { name, source }, host, ((formattingOptions && getFormatContext(formattingOptions))!), // TODO: GH#18217
    ...
    function getReferencesAtPosition(fileName: string, position: number): ReferenceEntry[] | undefined {
        synchronizeHostData();
        return getReferencesWorker(getTouchingPropertyName(getValidSourceFile(fileName), position), position, { use: FindReferencesUse.References }, toReferenceEntry);
    }
    ...
    function getFormattingEditsForRange(fileName: string, start: number, end: number, options: FormatCodeOptions | FormatCodeSettings): TextChange[] {
        const sourceFile = syntaxTreeCache.getCurrentSourceFile(fileName);
        return formatSelection(start, end, sourceFile, getFormatContext(toEditorSettings(options)));
    }
    function getFormattingEditsForDocument(fileName: string, options: FormatCodeOptions | FormatCodeSettings): TextChange[] {
        return formatDocument(syntaxTreeCache.getCurrentSourceFile(fileName), getFormatContext(toEditorSettings(options)));
    }
    function getFormattingEditsAfterKeystroke(fileName: string, position: number, key: string, options: FormatCodeOptions | FormatCodeSettings): TextChange[] {
        const sourceFile = syntaxTreeCache.getCurrentSourceFile(fileName);
        const formatContext = getFormatContext(toEditorSettings(options));
    ...
    function getCodeFixesAtPosition(fileName: string, start: number, end: number, errorCodes: readonly number[], formatOptions: FormatCodeSettings, preferences: UserPreferences = emptyOptions): readonly CodeFixAction[] {
        ...
        const formatContext = getFormatContext(formatOptions);
    ...
    function getEditsForFileRename(oldFilePath: string, newFilePath: string, formatOptions: FormatCodeSettings, preferences: UserPreferences = emptyOptions): readonly FileTextChanges[] {
        return ts.getEditsForFileRename((getProgram()!), oldFilePath, newFilePath, host, getFormatContext(formatOptions), preferences, sourceMapper);
    }
    ...
    function getRefactorContext(file: SourceFile, positionOrRange: number | TextRange, preferences: UserPreferences, formatOptions?: FormatCodeSettings): RefactorContext {
        ...
        return {
            ...
            program: getProgram()!,
            host,
            formatContext: getFormatContext((formatOptions!)),
            cancellationToken,
            preferences,
        };
    }
