...
export class ChangeTracker {
    ...
    private readonly classesWithNodesInsertedAtStart = createMap<{
        readonly node: ClassDeclaration | InterfaceDeclaration | ObjectLiteralExpression;
        readonly sourceFile: SourceFile;
    ...
    private computeIndentationForNewMember(sourceFile: SourceFile, cls: ClassLikeDeclaration | InterfaceDeclaration | ObjectLiteralExpression) {
        ...
        return SmartIndenter.findFirstNonWhitespaceColumn(getLineStartPositionForPosition(clsStart, sourceFile), clsStart, sourceFile, this.formatContext.options)
...
namespace changesToText {
    export function getTextChangesFromChanges(changes: readonly Change[], newLineCharacter: string, formatContext: FormatContext, validate: ValidateNonFormattedText | undefined): FileTextChanges[] {
        return group(changes, c => c.sourceFile.path).map(changesInFile => {
            ...
            const textChanges = normalized.map(c => createTextChange(createTextSpanFromRange(c.range), computeNewText(c, sourceFile, newLineCharacter, formatContext, validate)));
            return { fileName: sourceFile.fileName, textChanges };
        });
    }
    export function newFileChanges(oldFile: SourceFile | undefined, fileName: string, statements: readonly Statement[], newLineCharacter: string, formatContext: FormatContext): FileTextChanges {
    ...
    export function newFileChangesWorker(oldFile: SourceFile | undefined, scriptKind: ScriptKind, statements: readonly Statement[], newLineCharacter: string, formatContext: FormatContext): string {
    ...
    function getFormatCodeSettingsForWriting({ options }: FormatContext, sourceFile: SourceFile): FormatCodeSettings {
    ...
    export function getNonformattedText(node: Node, sourceFile: SourceFile | undefined, newLineCharacter: string): {
        text: string;
        node: Node;
    } {
