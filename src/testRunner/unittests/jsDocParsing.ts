import { parseJSDocTypeExpressionForTests, parseIsolatedJSDocComment, Debug, createSourceFile, ScriptTarget, SyntaxKind, JSDocTemplateTag } from "../ts";
import { Baseline } from "../Harness";
import { sourceFileToJSON } from "../Utils";
    ...
    describe("TypeExpressions", () => {
        function parsesCorrectly(name: string, content: string) {
            it(name, () => {
                ...
                Baseline.runBaseline("JSDocParsing/TypeExpressions.parsesCorrectly." + name + ".json", sourceFileToJSON(typeAndDiagnostics!.jsDocTypeExpression.type));
            });
        }
    ...
    describe("DocComments", () => {
        function parsesCorrectly(name: string, content: string) {
            it(name, () => {
                ...
                Baseline.runBaseline("JSDocParsing/DocComments.parsesCorrectly." + name + ".json", JSON.stringify(comment.jsDoc, (_, v) => v && v.pos !== undefined ? JSON.parse(sourceFileToJSON(v)) : v, 4));
            });
        }
