import { Path, arrayToSet, arrayFrom, mapDefinedIterator, CompilerOptions, NewLineKind, createProgram, ScriptTarget, createSourceFile, sys, ModuleKind, emptyOptions, Program, ScriptKind, ExpressionStatement, AsExpression, ImportDeclaration } from "../ts";
...
function verifyMissingFilePaths(missingPaths: readonly Path[], expected: readonly string[]) {
    ...
    const map = (arrayToSet(expected) as ts.Map<boolean>);
