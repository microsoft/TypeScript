...
function checkCache<T>(caption: string, program: Program, fileName: string, expectedContent: ts.Map<T> | undefined, getCache: (f: SourceFile) => ts.Map<T> | undefined, entryChecker: (expected: T, original: T) => boolean): void {
...
function mapsAreEqual<T>(left: ts.Map<T>, right: ts.Map<T>, valuesAreEqual?: (left: T, right: T) => boolean): boolean {
    if (left === right)
        return true;
    if (!left || !right)
        return false;
...
function checkResolvedModulesCache(program: Program, fileName: string, expectedContent: ts.Map<ResolvedModule | undefined> | undefined): void {
    checkCache("resolved modules", program, fileName, expectedContent, f => f.resolvedModules, checkResolvedModule);
}
function checkResolvedTypeDirectivesCache(program: Program, fileName: string, expectedContent: ts.Map<ResolvedTypeReferenceDirective> | undefined): void {
