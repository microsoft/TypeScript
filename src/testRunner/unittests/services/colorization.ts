import { TokenClass, ClassificationResult, EndOfLineState, last } from "../../ts";
...
describe("unittests:: services:: Colorization", () => {
    ...
    const languageServiceAdapter = new LanguageService.ShimLanguageServiceAdapter(/*preprocessToResolve*/ false);
    ...
    function punctuation(text: string, position?: number) {
        return createClassification(text, TokenClass.Punctuation, position);
    }
    function keyword(text: string, position?: number) {
        return createClassification(text, TokenClass.Keyword, position);
    }
    ...
    function comment(text: string, position?: number) {
        return createClassification(text, TokenClass.Comment, position);
    }
    function whitespace(text: string, position?: number) {
        return createClassification(text, TokenClass.Whitespace, position);
    }
    function identifier(text: string, position?: number) {
        return createClassification(text, TokenClass.Identifier, position);
    }
    function numberLiteral(text: string, position?: number) {
        return createClassification(text, TokenClass.NumberLiteral, position);
    }
    function stringLiteral(text: string, position?: number) {
        return createClassification(text, TokenClass.StringLiteral, position);
    }
    ...
    describe("test getClassifications", () => {
        it("returns correct token classes", () => {
            testLexicalClassification("var x: string = \"foo\" ?? \"bar\"; //Hello", EndOfLineState.None, keyword("var"), whitespace(" "), identifier("x"), punctuation(":"), keyword("string"), operator("="), stringLiteral("\"foo\""), whitespace(" "), operator("??"), stringLiteral("\"foo\""), comment("//Hello"), punctuation(";"));
        });

        it("correctly classifies a comment after a divide operator", () => {
            testLexicalClassification("1 / 2 // comment", EndOfLineState.None, numberLiteral("1"), whitespace(" "), operator("/"), numberLiteral("2"), comment("// comment"));
        });

        it("correctly classifies a literal after a divide operator", () => {
            testLexicalClassification("1 / 2, 3 / 4", EndOfLineState.None, numberLiteral("1"), whitespace(" "), operator("/"), numberLiteral("2"), numberLiteral("3"), numberLiteral("4"), operator(","));
        });

        it("correctly classifies a multiline string with one backslash", () => {
            testLexicalClassification("'line1\\", EndOfLineState.None, stringLiteral("'line1\\"), finalEndOfLineState(EndOfLineState.InSingleQuoteStringLiteral));
        });
        ...
        it("correctly classifies an unterminated single-line string with no backslashes", () => {
            testLexicalClassification("'line1", EndOfLineState.None, stringLiteral("'line1"), finalEndOfLineState(EndOfLineState.None));
        });
        ...
        it("correctly classifies the continuing line of a multiline string ending in one backslash", () => {
            testLexicalClassification("\\", EndOfLineState.InDoubleQuoteStringLiteral, stringLiteral("\\"), finalEndOfLineState(EndOfLineState.InDoubleQuoteStringLiteral));
        });
        ...
        it("correctly classifies the last line of an unterminated multiline string ending in no backslashes", () => {
            testLexicalClassification("  ", EndOfLineState.InDoubleQuoteStringLiteral, stringLiteral("  "), finalEndOfLineState(EndOfLineState.None));
        });
        ...
        it("correctly classifies the last line of a multiline string", () => {
            testLexicalClassification("'", EndOfLineState.InSingleQuoteStringLiteral, stringLiteral("'"), finalEndOfLineState(EndOfLineState.None));
        });

        it("correctly classifies an unterminated multiline comment", () => {
            testLexicalClassification("/*", EndOfLineState.None, comment("/*"), finalEndOfLineState(EndOfLineState.InMultiLineCommentTrivia));
        });

        it("correctly classifies the termination of a multiline comment", () => {
            testLexicalClassification("   */     ", EndOfLineState.InMultiLineCommentTrivia, comment("   */"), finalEndOfLineState(EndOfLineState.None));
        });

        it("correctly classifies the continuation of a multiline comment", () => {
            testLexicalClassification("LOREM IPSUM DOLOR   ", EndOfLineState.InMultiLineCommentTrivia, comment("LOREM IPSUM DOLOR   "), finalEndOfLineState(EndOfLineState.InMultiLineCommentTrivia));
        });
        ...
        it("correctly classifies a keyword after a dot", () => {
            testLexicalClassification("a.var", EndOfLineState.None, identifier("var"));
        });

        it("correctly classifies a string literal after a dot", () => {
            testLexicalClassification("a.\"var\"", EndOfLineState.None, stringLiteral("\"var\""));
        });

        it("correctly classifies a keyword after a dot separated by comment trivia", () => {
            testLexicalClassification("a./*hello world*/ var", EndOfLineState.None, identifier("a"), punctuation("."), comment("/*hello world*/"), identifier("var"));
        });
        ...
        it("classifies a keyword after a dot on previous line", () => {
            testLexicalClassification("var", EndOfLineState.None, keyword("var"), finalEndOfLineState(EndOfLineState.None));
        });

        it("classifies multiple keywords properly", () => {
            testLexicalClassification("public static", EndOfLineState.None, keyword("public"), keyword("static"), finalEndOfLineState(EndOfLineState.None));
            testLexicalClassification("public var", EndOfLineState.None, keyword("public"), identifier("var"), finalEndOfLineState(EndOfLineState.None));
        });

        it("classifies a single line no substitution template string correctly", () => {
            testLexicalClassification("`number number public string`", EndOfLineState.None, stringLiteral("`number number public string`"), finalEndOfLineState(EndOfLineState.None));
        });
        it("classifies substitution parts of a template string correctly", () => {
            testLexicalClassification("`number '${ 1 + 1 }' string '${ 'hello' }'`", EndOfLineState.None, stringLiteral("`number '${"), numberLiteral("1"), operator("+"), numberLiteral("1"), stringLiteral("}' string '${"), stringLiteral("'hello'"), stringLiteral("}'`"), finalEndOfLineState(EndOfLineState.None));
        });
        it("classifies an unterminated no substitution template string correctly", () => {
            testLexicalClassification("`hello world", EndOfLineState.None, stringLiteral("`hello world"), finalEndOfLineState(EndOfLineState.InTemplateHeadOrNoSubstitutionTemplate));
        });
        it("classifies the entire line of an unterminated multiline no-substitution/head template", () => {
            testLexicalClassification("...", EndOfLineState.InTemplateHeadOrNoSubstitutionTemplate, stringLiteral("..."), finalEndOfLineState(EndOfLineState.InTemplateHeadOrNoSubstitutionTemplate));
        });
        it("classifies the entire line of an unterminated multiline template middle/end", () => {
            testLexicalClassification("...", EndOfLineState.InTemplateMiddleOrTail, stringLiteral("..."), finalEndOfLineState(EndOfLineState.InTemplateMiddleOrTail));
        });
        it("classifies a termination of a multiline template head", () => {
            testLexicalClassification("...${", EndOfLineState.InTemplateHeadOrNoSubstitutionTemplate, stringLiteral("...${"), finalEndOfLineState(EndOfLineState.InTemplateSubstitutionPosition));
        });
        it("classifies the termination of a multiline no substitution template", () => {
            testLexicalClassification("...`", EndOfLineState.InTemplateHeadOrNoSubstitutionTemplate, stringLiteral("...`"), finalEndOfLineState(EndOfLineState.None));
        });
        it("classifies the substitution parts and middle/tail of a multiline template string", () => {
            testLexicalClassification("${ 1 + 1 }...`", EndOfLineState.InTemplateHeadOrNoSubstitutionTemplate, stringLiteral("${"), numberLiteral("1"), operator("+"), numberLiteral("1"), stringLiteral("}...`"), finalEndOfLineState(EndOfLineState.None));
        });
        ...
        it("classifies substitution expressions with curly braces appropriately", () => {
            ...
            testLexicalClassification("...${ () => { } } ${ { x: `1` } }...`", EndOfLineState.InTemplateHeadOrNoSubstitutionTemplate, stringLiteral(track("...${"), pos), punctuation(track(" ", "("), pos), punctuation(track(")"), pos), punctuation(track(" ", "=>"), pos), punctuation(track(" ", "{"), pos), punctuation(track(" ", "}"), pos), stringLiteral(track(" ", "} ${"), pos), punctuation(track(" ", "{"), pos), identifier(track(" ", "x"), pos), punctuation(track(":"), pos), stringLiteral(track(" ", "`1`"), pos), punctuation(track(" ", "}"), pos), stringLiteral(track(" ", "}...`"), pos), finalEndOfLineState(EndOfLineState.None));
        ...
        it("classifies partially written generics correctly.", () => {
            testLexicalClassification("Foo<number", EndOfLineState.None, identifier("Foo"), operator("<"), identifier("number"), finalEndOfLineState(EndOfLineState.None));

            // Looks like a cast, should get classified as a keyword.
            testLexicalClassification("<number", EndOfLineState.None, operator("<"), keyword("number"), finalEndOfLineState(EndOfLineState.None));

            // handle nesting properly.
            testLexicalClassification("Foo<Foo,Foo<number", EndOfLineState.None, identifier("Foo"), operator("<"), identifier("Foo"), operator(","), identifier("Foo"), operator("<"), identifier("number"), finalEndOfLineState(EndOfLineState.None));
        });
...
>>>>>>> Branch - a\r\n", EndOfLineState.None, comment("<<<<<<< HEAD"), keyword("class"), identifier("C"), punctuation("{"), punctuation("}"), comment("||||||| merged common ancestors\r\nclass E { }\r\n"), comment("=======\r\nclass D { }\r\n"), comment(">>>>>>> Branch - a"), finalEndOfLineState(EndOfLineState.None));
        ...
        it("'of' keyword", () => {
            testLexicalClassification("for (var of of of) { }", EndOfLineState.None, keyword("for"), punctuation("("), keyword("var"), keyword("of"), keyword("of"), keyword("of"), punctuation(")"), punctuation("{"), punctuation("}"), finalEndOfLineState(EndOfLineState.None));
        });
    });
});
