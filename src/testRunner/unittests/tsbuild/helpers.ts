...
type ReadableProgramBuildInfoFileInfo = Omit<ts.BuilderState.FileInfo, "impliedFormat"> & { impliedFormat: string | undefined; };
...
> & {
    ...
    fileInfos: ts.MapLike<ReadableProgramBuildInfoFileInfo>;
...
type ReadableProgramBundleEmitBuildInfo = Omit<ts.ProgramBundleEmitBuildInfo, "fileInfos"> & {
    fileInfos: ts.MapLike<string>;
};
...
function generateBuildInfoProgramBaseline(sys: ts.System, buildInfoPath: string, buildInfo: ts.BuildInfo) {
    ...
    if (buildInfo.program && ts.isProgramBundleEmitBuildInfo(buildInfo.program)) {
        ...
        buildInfo.program?.fileInfos?.forEach((fileInfo, index) => fileInfos[toFileName(index + 1 as ts.ProgramBuildInfoFileId)] = fileInfo);
        program = {
            ...buildInfo.program,
            fileInfos
        };
        ...
        buildInfo.program?.fileInfos?.forEach((fileInfo, index) => fileInfos[toFileName(index + 1 as ts.ProgramBuildInfoFileId)] = toReadableFileInfo(fileInfo));
        ...
        program = buildInfo.program && {
            ...
            affectedFilesPendingEmit: buildInfo.program.affectedFilesPendingEmit?.map(([fileId, emitKind]) => [
                toFileName(fileId),
                emitKind === ts.BuilderFileEmit.DtsOnly ? "DtsOnly" :
                    emitKind === ts.BuilderFileEmit.Full ? "Full" :
                        ts.Debug.assertNever(emitKind)
            ]),
    ...
    function toReadableFileInfo(fileInfo: ts.ProgramBuildInfoFileInfo): ReadableProgramBuildInfoFileInfo {
        const info = ts.toBuilderStateFileInfo(fileInfo);
        return {
            ...info,
            ...
            if (!isReadableProgramBundleEmitBuildInfo(incrementalReadableBuildInfo?.program)) {
                ...
                if (incrementalReadableBuildInfo?.program?.affectedFilesPendingEmit) {
                    ...
                    incrementalReadableBuildInfo.program.affectedFilesPendingEmit.forEach(([actualFile]) => {
                        expectedIndex = ts.findIndex((cleanReadableBuildInfo!.program! as ReadableProgramMultiFileEmitBuildInfo).affectedFilesPendingEmit, ([expectedFile]) => actualFile === expectedFile, expectedIndex);
                        if (expectedIndex === -1) {
                            addBaseline(
...
function getBuildInfoForIncrementalCorrectnessCheck(text: string | undefined): {
    ...
    let sanitizedFileInfos: ts.MapLike<ReadableProgramBuildInfoFileInfo | string> | undefined;
    if (readableBuildInfo.program?.fileInfos) {
        ...
        for (const id in readableBuildInfo.program.fileInfos) {
            if (ts.hasProperty(readableBuildInfo.program.fileInfos, id)) {
                ...
                sanitizedFileInfos[id] = ts.isString(info) ? info : { ...info, signature: undefined };
            }
        }
    }
    return {
