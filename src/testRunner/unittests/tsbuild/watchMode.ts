import { TestFSWithWatch, BuildOptions, createSolutionBuilderHost, emptyArray, createSolutionBuilderWithWatchHost, createSolutionBuilderWithWatch, projectSystem, SolutionBuilder, EmitAndSemanticDiagnosticsBuilderProgram, ResolvedConfigFilePath, ConfigFileProgramReloadLevel, arrayFrom, mapDefinedIterator, Path, libContent } from "../../ts";
import { createWatchedSystem, WatchedSystem, File, libFile, verifyTscWatch, checkWatchedFiles, checkWatchedDirectories, checkOutputErrorsInitial, checkWatchedFilesDetailed, checkWatchedDirectoriesDetailed, createWatchOfConfigFile, Watch, checkArray, checkOutputErrorsIncremental, checkProgramActualFiles, replaceFileText, projectRoot } from "../../ts.tscWatch";
import { createProjectService, TestProjectService, checkNumberOfProjects, getTypeRootsFromLocation } from "../../ts.projectSystem";
import { dedent } from "../../Utils";
import * as ts from "../../ts";
import projectsLocation = ts.TestFSWithWatch.tsbuildProjectsLocation;
import getFilePathInProject = ts.TestFSWithWatch.getTsBuildProjectFilePath;
...
let x: string = 10;`);
    ...
    describe("tsc-watch and tsserver works with project references", () => {
        describe("invoking when references are already built", () => {
            ...
            function createSolutionOfProject(allFiles: readonly File[], currentDirectory: string, solutionBuilderconfig: string, getOutputFileStamps: (host: TsBuildWatchSystem) => readonly OutputFileStamp[]) {
            ...
            function createSolutionAndWatchModeOfProject(allFiles: readonly File[], currentDirectory: string, solutionBuilderconfig: string, watchConfig: string, getOutputFileStamps: (host: TsBuildWatchSystem) => readonly OutputFileStamp[]) {
                // Build the composite project
            ...
            function createSolutionAndServiceOfProject(allFiles: readonly File[], currentDirectory: string, solutionBuilderconfig: string, openFileName: string, getOutputFileStamps: (host: TsBuildWatchSystem) => readonly OutputFileStamp[]) {
                // Build the composite project
                const { host, solutionBuilder } = createSolutionOfProject(allFiles, currentDirectory, solutionBuilderconfig, getOutputFileStamps);
                // service
            ...
            function checkProjectActualFiles(service: TestProjectService, configFile: string, expectedFiles: readonly string[]) {
            ...
            describe("on sample project", () => {
                ...
                const expectedWatchedDirectoriesRecursive = getTypeRootsFromLocation(projectPath(SubProject.tests));
                ...
                function verifyWatches(host: TsBuildWatchSystem, withTsserver?: boolean) {
                    verifyWatchesOfProject(host, withTsserver ?
                        [...core.slice(0, core.length - 1), ...logic, tests[0], libFile].map(f => f.path.toLowerCase()) :
                        [core[0], logic[0], ...tests, libFile].map(f => f.path).concat([coreIndexDts, coreAnotherModuleDts, logicIndexDts].map(f => f.toLowerCase())), expectedWatchedDirectoriesRecursive);
                }
                function verifyScenario(edit: (host: TsBuildWatchSystem, solutionBuilder: SolutionBuilder<EmitAndSemanticDiagnosticsBuilderProgram>) => void, expectedProgramFilesAfterEdit: () => readonly string[], expectedProjectFilesAfterEdit: () => readonly string[]) {
                    it("with tsc-watch", () => {
...
export function gfoo() {
            ...
            describe("on transitive references", () => {
                ...
                function verifyWatchState(host: TsBuildWatchSystem, watch: Watch, expectedProgramFiles: readonly string[], expectedWatchedFiles: readonly string[], expectedWatchedDirectoriesRecursive: readonly string[], dependencies: readonly [string, readonly string[]][], expectedWatchedDirectories?: readonly string[]) {
                ...
                function verifyTransitiveReferences(multiFolder: boolean) {
                    ...
                    const expectedWatchedDirectoriesRecursive = [
                        ...(multiFolder ? [
                            getFilePathInProject(project, "a"),
                            getFilePathInProject(project, "b"),
                        ] : []),
                        getFilePathInProject(project, "refs"),
                        ...getTypeRootsFromLocation(multiFolder ? getFilePathInProject(project, "c") : getProjectPath(project))
                    ].map(s => s.toLowerCase());
                    ...
                    function verifyProject(host: TsBuildWatchSystem, service: TestProjectService, orphanInfos?: readonly string[]) {
                    ...
                    function verifyServerState({ host, service, expectedProjectFiles, expectedProjectWatchedFiles, expectedWatchedDirectoriesRecursive, orphanInfos }: VerifyServerState) {
                        ...
                        const actualOrphan = arrayFrom(mapDefinedIterator(service.filenameToScriptInfo.values(), v => v.containingProjects.length === 0 ? v.fileName : undefined));
                    ...
                    function verifyScenario({ edit, expectedEditErrors, expectedProgramFiles, expectedProjectFiles, expectedWatchedFiles, expectedProjectWatchedFiles, expectedWatchedDirectoriesRecursive, dependencies, revert, orphanInfosAfterEdit, orphanInfosAfterRevert }: VerifyScenario) {
                        ...
                        if (!multiFolder)
                            return; // With side by side file open is in inferred project without any settings
                        it("with tsserver", () => {
                            const { host, solutionBuilder, service } = createSolutionAndService();
                            edit(host, solutionBuilder);
                            host.checkTimeoutQueueLengthAndRun(2);
                ...
                describe("when config files are side by side", () => {
                    ...
                    it("when referenced project uses different module resolution", () => {
                        ...
                        const expectedWatchedDirectoriesRecursive = [
                            getFilePathInProject(project, "refs"),
                            ...getTypeRootsFromLocation(getProjectPath(project))
                        ].map(s => s.toLowerCase());
                        const defaultDependencies: readonly [string, readonly string[]][] = [
                            [aDts, [aDts]],
                            [bDts, [bDts, aDts]],
