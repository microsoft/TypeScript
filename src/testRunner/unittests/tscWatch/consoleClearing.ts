import { File, WatchedSystem, verifyTscWatch, createWatchedSystem, libFile, createWatchOfConfigFile, runWatchBaseline } from "../../ts.tscWatch";
import { emptyArray, CompilerOptions, TestFSWithWatch } from "../../ts";
describe("unittests:: tsc-watch:: console clearing", () => {
    ...
    describe("when preserveWatchOutput is true in config file", () => {
        ...
        it("using createWatchOfConfigFile ", () => {
            const sys = TestFSWithWatch.changeToHostTrackingWrittenFiles(createWatchedSystem(files));
