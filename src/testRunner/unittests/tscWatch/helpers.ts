...
export function checkNormalBuildErrors(host: WatchedSystem, errors: readonly Diagnostic[] | readonly string[], reportErrorSummary?: boolean) {
    checkOutputErrors(host, [
        ...map(errors, hostOutputDiagnostic),
        ...reportErrorSummary ?
            [hostOutputWatchDiagnostic(getErrorSummaryText(errors.length, host.newLine))] : emptyArray
    ]);
}
...
function tscWatchCompile(input: TscWatchCompile) {
    it("Generates files matching the baseline", () => {
        const sys = TestFSWithWatch.changeToHostTrackingWrittenFiles(patchHostForBuildInfoReadWrite(input.sys()));
        const { scenario, subScenario, commandLineArgs, changes, baselineSourceMap } = input;
        const { cb, getPrograms } = commandLineCallbacks(sys);
        executeCommandLine(sys, cb, commandLineArgs, input.maxNumberOfFilesToIterateForInvalidation);
        runWatchBaseline({
            scenario,
            subScenario,
            commandLineArgs,
...
export function runWatchBaseline({ scenario, subScenario, commandLineArgs, getPrograms, sys, baselineSourceMap, changes }: RunWatchBaseline) {
    ...
    Baseline.runBaseline(`${isBuild(commandLineArgs) ? "tsbuild/watchMode" : "tscWatch"}/${scenario}/${subScenario.split(" ").join("-")}.js`, baseline.join("\r\n"));
}
