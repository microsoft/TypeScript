import { File, WatchedSystem, createWatchedSystem, SystemSnap, watchBaseline, libFile } from "../../ts.tscWatch";
import { TestFSWithWatch, emptyArray, commandLineCallbacks, isBuild, executeCommandLine, createDiagnosticReporter, parseConfigFileWithSystem, performIncrementalCompilation, getConfigFileParsingDiagnostics, noop, createIncrementalProgram, createIncrementalCompilerHost, ModuleKind, Path, Diagnostics } from "../../ts";
import { patchHostForBuildInfoReadWrite } from "../../fakes";
...
describe("unittests:: tsc-watch:: emit file --incremental", () => {
    ...
    function verifyIncrementalWatchEmitWorker({ subScenario, files, optionsToExtend, modifyFs }: VerifyIncrementalWatchEmitInput, incremental: boolean) {
        const sys = TestFSWithWatch.changeToHostTrackingWrittenFiles(patchHostForBuildInfoReadWrite(createWatchedSystem(files(), { currentDirectory: project })));
        if (incremental)
