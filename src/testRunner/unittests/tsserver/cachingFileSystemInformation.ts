import { countWhere, startsWith, directorySeparator, MultiMap, createMultiMap, arrayFrom, TestFSWithWatch, ModuleKind, getDirectoryPath, returnTrue, ScriptTarget, Diagnostics, flattenDiagnosticMessageText, forEachAncestorDirectory, combinePaths, createMap, map, server, Path, mapDefined, forEach, find, filter } from "../../ts";
import { TestServerHost, File, createServerHost, createProjectService, checkNumberOfProjects, mapCombinedPathsInAncestor, nodeModulesAtTypes, nodeModules, createSession, checkNumberOfConfiguredProjects, checkProjectActualFiles, makeSessionRequest, protocol, libFile, getNodeModuleDirectories, checkWatchedFiles, checkWatchedDirectories, getTypeRootsFromLocation } from "../../ts.projectSystem";
...
describe("unittests:: tsserver:: CachingFileSystemInformation:: tsserverProjectSystem CachingFileSystemInformation", () => {
    ...
    function createCallsTrackingHost(host: TestServerHost) {
        ...
        function verifyCalledOnEachEntry(callback: CalledMaps, expectedKeys: ts.Map<number>) {
