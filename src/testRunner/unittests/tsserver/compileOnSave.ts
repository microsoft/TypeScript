...
describe("unittests:: tsserver:: compileOnSave:: affected list", () => {
    ...
    function createSession(host: ServerHost, typingsInstaller?: ITypingsInstaller): Session {
    ...
    describe("for changes in declaration files", () => {
        function testDTS(dtsFileContents: string, tsFileContents: string, opts: CompilerOptions, expectDTSEmit: boolean) {
            ...
            session.executeCommand((<projectSystem.protocol.OpenRequest>{
                seq: 1,
                type: "request",
                command: "open",
            ...
            const { response } = session.executeCommand((<projectSystem.protocol.CompileOnSaveAffectedFileListRequest>{
                seq: 3,
                type: "request",
            ...
            const { response: response2 } = session.executeCommand((<projectSystem.protocol.CompileOnSaveAffectedFileListRequest>{
                seq: 4,
                type: "request",
                command: "compileOnSaveAffectedFileList",
                arguments: { file: f2.path }
    ...
    describe("tsserverProjectSystem emit with outFile or out setting", () => {
        function test(opts: CompilerOptions, expectedUsesOutFile: boolean) {
            ...
            assert.equal((<projectSystem.protocol.CompileOnSaveAffectedFileListSingleProject[]>response).length, 1, "expected output for 1 project");
            assert.equal((<projectSystem.protocol.CompileOnSaveAffectedFileListSingleProject[]>response)[0].fileNames.length, 2, "expected output for 1 project");
            assert.equal((<projectSystem.protocol.CompileOnSaveAffectedFileListSingleProject[]>response)[0].projectUsesOutFile, expectedUsesOutFile, "usesOutFile");
        }
