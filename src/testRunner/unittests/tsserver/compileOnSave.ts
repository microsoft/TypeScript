...
describe("unittests:: tsserver:: compileOnSave:: affected list", () => {
    ...
    describe("tsserverProjectSystem emit with outFile or out setting", () => {
        function test(opts: ts.CompilerOptions, expectedUsesOutFile: boolean) {
            ...
            assert.equal((response as ts.projectSystem.protocol.CompileOnSaveAffectedFileListSingleProject[]).length, 1, "expected output for 1 project");
            assert.equal((response as ts.projectSystem.protocol.CompileOnSaveAffectedFileListSingleProject[])[0].fileNames.length, 2, "expected output for 1 project");
            assert.equal((response as ts.projectSystem.protocol.CompileOnSaveAffectedFileListSingleProject[])[0].projectUsesOutFile, expectedUsesOutFile, "usesOutFile");
        }
