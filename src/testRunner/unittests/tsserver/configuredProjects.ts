...
import { getDirectoryPath, combinePaths, Path, find, createTextSpan, map, mapDefined, emptyArray } from "../../ts";
import { projectRoot } from "../../ts.tscWatch";
import { maxProgramSizeForNonTsFiles, toNormalizedPath, ProjectLanguageServiceStateEvent, NormalizedPath, ProjectService } from "../../ts.server";
...
declare var console: {
    ...
    describe("when creating new file", () => {
        ...
        interface VerifySession {
            ...
            checkProjectBeforeError: (service: ProjectService) => void;
            checkProjectAfterError: (service: ProjectService) => void;
        }
        ...
        function checkFooBarInInferredProject(service: ProjectService) {
        ...
        function checkFooBarInConfiguredProject(service: ProjectService) {
...
describe("unittests:: tsserver:: ConfiguredProjects:: non-existing directories listed in config file input array", () => {
    it("should be tolerated without crashing the server", () => {
        ...
        assert.isTrue(inferredProject.containsFile((<NormalizedPath>file1.path)));
        assert.isFalse(projectService.configuredProjects.get(configFile.path)!.containsFile((<NormalizedPath>file1.path)));
    });
