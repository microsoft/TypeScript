...
import { NormalizedPath, toNormalizedPath } from "../../ts.server";
describe("unittests:: tsserver:: format settings", () => {
    it("can be set globally", () => {
        ...
        const defaultSettings = projectService.getFormatCodeOptions((f1.path as NormalizedPath));
        // set global settings
        ...
        const s1 = projectService.getFormatCodeOptions(toNormalizedPath(f1.path));
        ...
        const s2 = projectService.getFormatCodeOptions(toNormalizedPath(f1.path));
        ...
        // get format options for file - should be equal to new per-file settings
        const s3 = projectService.getFormatCodeOptions(toNormalizedPath(f1.path));
