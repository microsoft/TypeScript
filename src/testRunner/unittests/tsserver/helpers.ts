...
function patchHostTimeouts(host: ts.TestFSWithWatch.TestServerHostTrackingWrittenFiles, session: TestSession | TestProjectService) {
...
export class TestSession extends ts.server.Session {
    ...
    public testhost: ts.TestFSWithWatch.TestServerHostTrackingWrittenFiles;
    ...
    constructor(opts: TestSessionOptions) {
        ...
        this.testhost = ts.TestFSWithWatch.changeToHostTrackingWrittenFiles(this.host as TestServerHost);
    ...
    baselineHost(title: string) {
        if (!this.logger.hasLevel(ts.server.LogLevel.verbose)) return;
