...
describe("unittests:: tsserver:: Inferred projects", () => {
    it("create inferred project", () => {
        ...
        ts.projectSystem.baselineTsserverLogs("inferredProjects", "create inferred project", projectService);
    });
    ...
    it("regression test - should infer typeAcquisition for inferred projects when set undefined", () => {
        ...
        ts.projectSystem.checkProjectActualFiles(inferredProject, [file1.path]);
    ...
    it("Setting compiler options for inferred projects when there are no open files should not schedule any refresh", () => {
        const host = ts.projectSystem.createServerHost([ts.projectSystem.commonFile1, ts.projectSystem.libFile]);
        const projectService = ts.projectSystem.createProjectService(host);
        projectService.setCompilerOptionsForInferredProjects({
            allowJs: true,
            target: ts.ScriptTarget.ES2015
