import { ProjectFilesWithTSDiagnostics, protocol, CommandNames, Logger, Msg } from "../../ts.server";
import { Diagnostic, flattenDiagnosticMessageText, zipWith, startsWith, getBaseFileName, find, noop, returnUndefined, projectSystem, Path, Diagnostics, formatStringFromArgs, getDirectoryPath, emptyArray, ModuleKind, arrayToMap, identity, map, mapDefined } from "../../ts";
import { createServerHost, libFile, createSession, toExternalFiles, checkNumberOfProjects, openFilesForSession, configuredProjectAt, createProjectService, checkProjectRootFiles, File, verifyGetErrRequest, createDiagnostic, checkCompleteEvent, checkProjectActualFiles, closeFilesForSession, ConfigFileDiagnostic, TestServerEventManager, executeSessionRequest, protocolTextSpanFromSubstring, Folder, checkNumberOfConfiguredProjects, checkWatchedFilesDetailed, checkWatchedDirectoriesDetailed, checkWatchedDirectories } from "../../ts.projectSystem";
import { projectRoot } from "../../ts.tscWatch";
describe("unittests:: tsserver:: Project Errors", () => {
    function checkProjectErrors(projectFiles: ProjectFilesWithTSDiagnostics, expectedErrors: readonly string[]): void {
    ...
    function checkDiagnosticsWithLinePos(errors: protocol.DiagnosticWithLinePosition[], expectedErrors: string[]) {
        assert.equal(errors ? errors.length : 0, expectedErrors.length, `expected ${expectedErrors.length} error in the list`);
        if (expectedErrors.length) {
    ...
    it("external project - diagnostics for missing files", () => {
        ...
        const compilerOptionsRequest: protocol.CompilerOptionsDiagnosticsRequest = {
            type: "request",
            command: CommandNames.CompilerOptionsDiagnosticsFull,
            seq: 2,
        ...
        };
        {
            ...
            const diags = (session.executeCommand(compilerOptionsRequest).response as protocol.DiagnosticWithLinePosition[]);
    ...
    it("configured projects - diagnostics for missing files", () => {
        ...
        let diags = (session.executeCommand(compilerOptionsRequest).response as protocol.DiagnosticWithLinePosition[]);
        checkDiagnosticsWithLinePos(diags, ["File '/a/b/applib.ts' not found."]);
        host.reloadFS([file1, file2, config, libFile]);
        checkNumberOfProjects(projectService, { configuredProjects: 1 });
        diags = (session.executeCommand(compilerOptionsRequest).response as protocol.DiagnosticWithLinePosition[]);
        checkDiagnosticsWithLinePos(diags, []);
    });
...
describe("unittests:: tsserver:: Project Errors dont include overwrite emit error", () => {
    it("for inferred project", () => {
        ...
        const diags = (session.executeCommand((<protocol.CompilerOptionsDiagnosticsRequest>{
            type: "request",
            command: CommandNames.CompilerOptionsDiagnosticsFull,
            seq: 2,
            arguments: { projectFileName: projectName }
        })).response as readonly projectSystem.protocol.DiagnosticWithLinePosition[]);
        ...
        const diagsAfterUpdate = (session.executeCommand((<protocol.CompilerOptionsDiagnosticsRequest>{
            type: "request",
            command: CommandNames.CompilerOptionsDiagnosticsFull,
            seq: 4,
            arguments: { projectFileName: projectName }
    ...
    it("for external project", () => {
        ...
        projectService.openExternalProject((<projectSystem.protocol.ExternalProject>{
            projectFileName,
        ...
        assert.isTrue(diags.length === 0);
        session.executeCommand((<protocol.OpenExternalProjectRequest>{
            type: "request",
            command: CommandNames.OpenExternalProject,
            seq: 3,
            arguments: {
                projectFileName,
                rootFiles: externalFiles,
...
describe("unittests:: tsserver:: Project Errors reports Options Diagnostic locations correctly with changes in configFile contents", () => {
    it("when options change", () => {
        ...
        const diags = (session.executeCommand((<protocol.SemanticDiagnosticsSyncRequest>{
            type: "request",
        ...
        const diagsAfterEdit = (session.executeCommand((<protocol.SemanticDiagnosticsSyncRequest>{
            type: "request",
            command: CommandNames.SemanticDiagnosticsSync,
            seq: 2,
            arguments: { file: configFile.path, projectFileName: projectName, includeLinePosition: true }
        ...
        function verifyDiagnostic(beforeEditDiag: protocol.DiagnosticWithLinePosition, afterEditDiag: protocol.DiagnosticWithLinePosition) {
...
describe("unittests:: tsserver:: Project Errors with config file change", () => {
    it("Updates diagnostics when '--noUnusedLabels' changes", () => {
        ...
        host.runQueuedTimeoutCallbacks();
        const response = (executeSessionRequest<projectSystem.protocol.SemanticDiagnosticsSyncRequest, projectSystem.protocol.SemanticDiagnosticsSyncResponse>(session, projectSystem.protocol.CommandTypes.SemanticDiagnosticsSync, { file: aTs.path }) as projectSystem.protocol.Diagnostic[] | undefined);
        assert.deepEqual<projectSystem.protocol.Diagnostic[] | undefined>(response, [
            {
    ...
    it("should report error when json is not root file found by tsconfig", () => {
        ...
        verifyGetErrRequest({
            ...
            expected: [{
                    ...
                    syntax: [],
                    semantic: [
                        createDiagnostic(span.start, span.end, Diagnostics.File_0_is_not_listed_within_the_file_list_of_project_1_Projects_must_list_all_files_or_use_an_include_pattern, [blabla.path, tsconfig.path])
                    ],
                    suggestion: []
                }]
        });
...
describe("unittests:: tsserver:: Project Errors with npm install when", () => {
    function verifyNpmInstall(timeoutDuringPartialInstallation: boolean) {
        ...
        const moduleFile: File = {
            path: `${projectRoot}/node_modules/@angular/core/index.d.ts`,
        ...
        const session = createSession(host, { canUseEvents: true });
        const service = session.getProjectService();
        openFilesForSession([{ file: main, projectRootPath: projectRoot }], session);
        const span = protocolTextSpanFromSubstring(main.content, `'@angular/core'`);
        const moduleNotFoundErr: projectSystem.protocol.Diagnostic[] = [
            createDiagnostic(span.start, span.end, Diagnostics.Cannot_find_module_0, ["@angular/core"])
        ];
        const expectedRecursiveWatches = arrayToMap([`${projectRoot}`, `${projectRoot}/src`, `${projectRoot}/node_modules`, `${projectRoot}/node_modules/@types`], identity, () => 1);
        verifyProject();
        verifyErrors(moduleNotFoundErr);
        let npmInstallComplete = false;
        // Simulate npm install
        let filesAndFoldersToAdd: (File | Folder)[] = [
            { path: `${projectRoot}/node_modules` },
            { path: `${projectRoot}/node_modules/.staging` },
            { path: `${projectRoot}/node_modules/.staging/@babel` },
            { path: `${projectRoot}/node_modules/.staging/@babel/helper-plugin-utils-a06c629f` },
            { path: `${projectRoot}/node_modules/.staging/core-js-db53158d` },
        ];
        verifyWhileNpmInstall({ timeouts: 2, semantic: moduleNotFoundErr });
        filesAndFoldersToAdd = [
            { path: `${projectRoot}/node_modules/.staging/@angular/platform-browser-dynamic-5efaaa1a` },
            { path: `${projectRoot}/node_modules/.staging/@angular/cli-c1e44b05/models/analytics.d.ts`, content: `export const x = 10;` },
            { path: `${projectRoot}/node_modules/.staging/@angular/core-0963aebf/index.d.ts`, content: `export const y = 10;` },
        ];
        ...
        host.deleteFolder(`${projectRoot}/node_modules/.staging`, /*recursive*/ true);
        npmInstallComplete = true;
        projectFiles.push(moduleFile);
        // Additional watch for watching script infos from node_modules
        expectedRecursiveWatches.set(`${projectRoot}/node_modules`, 2);
        verifyWhileNpmInstall({ timeouts: 2, semantic: [] });
        function verifyWhileNpmInstall({ timeouts, semantic }: {
            timeouts: number;
            semantic: projectSystem.protocol.Diagnostic[];
        }) {
        ...
        function verifyErrors(semantic: projectSystem.protocol.Diagnostic[], existingTimeouts?: number) {
            verifyGetErrRequest({
                session,
                host,
                expected: [{
