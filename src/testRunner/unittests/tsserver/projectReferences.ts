...
`
        ...
        type ActionInfoSpreader<Req = protocol.Request, Response = {}> = [ActionKey, // Key to get initial value and pass this value to spread function
        ...
        function checkProject(session: TestSession, verifiers: readonly DocumentPositionMapperVerifier[], noDts?: true) {
            ...
            verifiers.forEach(({ configFile, expectedProjectActualFiles }) => {
                checkProjectActualFiles((service.configuredProjects.get(configFile.path.toLowerCase())!), noDts ?
        ...
        interface VerifyDocumentPositionMapper {
            session: TestSession;
            dependencyMap: ScriptInfo | undefined;
        ...
        interface VerifyAllFnAction {
            ...
            actionKey: ActionKey;
            sourceMapPath?: ScriptInfo["sourceMapFilePath"];
            dependencyMap?: ScriptInfo | undefined;
            documentPositionMapper?: ScriptInfo["documentPositionMapper"];
        }
        interface VerifyAllFnActionResult {
            ...
            actionKey: ActionKey;
            dependencyMap: ScriptInfo | undefined;
            documentPositionMapper: ScriptInfo["documentPositionMapper"] | undefined;
        }
        ...
        function verifyScriptInfoCollection(session: TestSession, host: TestServerHost, verifiers: readonly DocumentPositionMapperVerifier[], { dependencyMap, documentPositionMapper, actionInfos, actionKey }: VerifyAllFnActionResult) {
        ...
        function verifyScenarioAndScriptInfoCollection(session: TestSession, host: TestServerHost, verifiers: readonly DocumentPositionMapperVerifier[], actionKey: ActionKey, noDts?: true) {
        ...
        function verifyScenarioWithChangesWorker({ scenarioName, verifier, withRefs, disableSourceOfProjectReferenceRedirect, change, afterChangeActionKey }: VerifyScenarioWithChanges, timeoutBeforeAction: boolean) {
            it(scenarioName, () => {
        ...
        function verifyScenarioWhenFileNotPresent({ scenarioName, verifier, withRefs, disableSourceOfProjectReferenceRedirect, fileLocation, fileNotPresentKey, fileCreatedKey, fileDeletedKey, noDts }: VerifyScenarioWhenFileNotPresent) {
        ...
        function verifyScenarioWorker({ mainScenario, verifier }: VerifyScenario, withRefs: boolean, disableSourceOfProjectReferenceRedirect?: true) {
            ...
            verifyScenarioWithChanges({
                ...
                withRefs,
                disableSourceOfProjectReferenceRedirect,
                change: (_host, session, verifiers) => verifiers.forEach(verifier => session.executeCommandSeq<protocol.ChangeRequest>({
...
`
            ...
            const host = createHost([commonConfig, keyboardTs, keyboardTestTs, srcConfig, terminalTs, libFile], [srcConfig.path]);
            const session = createSession(host);
    ...
    describe("when references are monorepo like with symlinks", () => {
        ...
        function verifySession({ bPackageJson, aTest, bFoo, bBar, bSymlink }: Packages, alreadyBuilt: boolean, extraOptions: CompilerOptions) {
            ...
            checkProjectActualFiles(project, [aConfig.path, aTest.path, bFoo.path, bBar.path, libFile.path]);
            verifyGetErrRequest({
                host,
                session,
                expected: [
