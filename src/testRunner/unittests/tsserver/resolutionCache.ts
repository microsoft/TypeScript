import { TestServerHost, createServerHost, createProjectService, TestTypingsInstaller, checkProjectActualFiles, configuredProjectAt, createSession, openFilesForSession, makeSessionRequest, verifyDiagnostics, verifyNoDiagnostics, File, libFile, checkNumberOfProjects, verifyGetErrRequest, createDiagnostic, checkProjectUpdatedInBackgroundEvent, checkErrorMessage, checkNoDiagnosticEvents, checkCompleteEvent, toExternalFiles, nodeModules, nodeModulesAtTypes, checkWatchedFiles, checkWatchedDirectories, getConfigFilesToWatch, getTypeRootsFromLocation, checkWatchedFilesDetailed, checkWatchedDirectoriesDetailed } from "../../ts.projectSystem";
import { ModuleResolutionHost, Diagnostics, projectSystem, combinePaths, getDirectoryPath, forEachAncestorDirectory, removeFileExtension, normalizePath, arrayToSet, emptyArray, mapDefined, arrayFrom, ScriptKind, createMap, ResolutionCacheHost, Debug } from "../../ts";
...
describe("unittests:: tsserver:: resolutionCache:: tsserverProjectSystem add the missing module file for inferred project", () => {
    it("should remove the `module not found` error", () => {
        ...
        let diags = (session.executeCommand(getErrRequest).response as protocol.Diagnostic[]);
        ...
        // Recheck
        diags = (session.executeCommand(getErrRequest).response as protocol.Diagnostic[]);
