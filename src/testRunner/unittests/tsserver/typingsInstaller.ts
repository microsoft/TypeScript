import { MapLike, createMap, emptyArray, TypeAcquisition, SortedReadonlyArray, ModuleResolutionKind, versionMajorMinor, combinePaths, TestFSWithWatch, getDirectoryPath, Path, emptyMap, createMapFromTemplate, JsTyping, Version, arrayIterator, ModuleKind, ScriptTarget, JsxEmit, ScriptKind } from "../../ts";
...
import { ServerHost, Project, protocol, SetTypings, InvalidateCachedTypings, BeginInstallTypes, EndInstallTypes, EventBeginInstallTypes, EventEndInstallTypes } from "../../ts.server";
...
interface InstallerParams {
    ...
    typesRegistry?: ts.Map<MapLike<string>>;
}
...
describe("unittests:: tsserver:: typingsInstaller:: General functionality", () => {
    ...
    it("should pick typing names from non-relative unresolved imports", () => {
        ...
        proj.updateGraph();
        assert.deepEqual(proj.cachedUnresolvedImportsPerFile.get((<Path>f1.path)), ["foo", "foo", "foo", "@bar/router", "@bar/common", "@bar/common"]);
        installer.installAll(/*expectedCount*/ 1);
    });
