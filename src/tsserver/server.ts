import { MapLike, normalizeSlashes, combinePaths, versionMajorMinor, Debug, server, noop, perfLogger, createMap, ApplyCodeActionCommandResult, JsTyping, getDirectoryPath, TypeAcquisition, SortedReadonlyArray, createMapFromTemplate, assertType, CharacterCodes, stripQuotes, WatchedFile, missingFileModifiedTime, FileWatcherEventKind, onWatchedFileStat, FileWatcherCallback, unorderedRemoveItem, directorySeparator, toFileNameLowerCase, getRootLength, getNodeMajorVersion, FileWatcher, DirectoryWatcherCallback, WatchOptions, resolveJSModule, validateLocaleAndSetLanguage, setStackTraceLimit, version } from "./ts";
import { LogLevel, Msg, nowString, ITypingsInstaller, ProjectService, ServerHost, Event, InstallPackageOptionsWithProject, InstallPackageRequest, Arguments, Project, TypingInstallerRequestUnion, createInstallTypingsRequest, stringifyIndented, TypesRegistryResponse, PackageInstalledResponse, SetTypings, InvalidateCachedTypings, BeginInstallTypes, EndInstallTypes, InitializationFailedResponse, EventTypesRegistry, ActionPackageInstalled, EventInitializationFailed, protocol, EventBeginInstallTypes, EventEndInstallTypes, ActionInvalidate, ActionSet, Session, nullTypingsInstaller, formatMessage, toEvent, findArgument, RequireResult, ServerCancellationToken, nullCancellationToken, hasArgument, emptyArray } from "./ts.server";
...
class Logger implements server.Logger {
    ...
    constructor(private readonly logFilename: string, private readonly traceToConsole: boolean, private readonly level: LogLevel) {
        if (this.logFilename) {
            try {
...
class NodeTypingsInstaller implements ITypingsInstaller {
    ...
    private typesRegistryCache: ts.Map<MapLike<string>> | undefined;
