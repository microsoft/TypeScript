import { MapLike, noop, resolveModuleName, combinePaths, ModuleResolutionKind, compareStringsCaseInsensitive, Comparison, FileWatcher, JsTyping, createMap, GetCanonicalFileName, Path, createGetCanonicalFileName, clearMap, closeFileWatcher, hasProperty, getBaseFileName, getProperty, Version, mapDefined, mangleScopedPackageName, version, versionMajorMinor, getDirectoryPath, WatchOptions, copyEntries, FileWatcherEventKind, fileExtensionIs, Extension, containsPath, directorySeparator } from "./ts";
import { InstallTypingHost, CloseProject, DiscoverTypings, BeginInstallTypes, EventBeginInstallTypes, EndInstallTypes, EventEndInstallTypes, ActionInvalidate, SetTypings, ActionSet, InvalidateCachedTypings } from "./ts.server";
...
type ProjectWatchers = ts.Map<FileWatcher> & {
    isInvoked?: boolean;
};
export abstract class TypingsInstaller {
    private readonly packageNameToTypingLocation: ts.Map<JsTyping.CachedTyping> = createMap<JsTyping.CachedTyping>();
    private readonly missingTypingsSet: ts.Map<true> = createMap<true>();
    private readonly knownCachesSet: ts.Map<true> = createMap<true>();
    ...
    abstract readonly typesRegistry: ts.Map<MapLike<string>>;
