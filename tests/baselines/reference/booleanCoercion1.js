...
var Enum;
(function (Enum) {
    Enum[Enum["EnumValue"] = 0] = "EnumValue";
})(Enum || (Enum = {}));
class Class {
}
function testExoression(typeParameter) {
    // if
    if (anyValue) { }
    if (unknownValue) { }
    if (undefinedValue) { }
...
function middleExpression(typeParameter) {
    // for
    for (; anyValue;) { }
    for (; unknownValue;) { }
    for (; undefinedValue;) { }
...
function firstOperand(typeParameter) {
    // conditional
    anyValue ? 1 : 2;
    unknownValue ? 1 : 2;
    undefinedValue ? 1 : 2;
...
function notOperand(typeParameter) {
    // not
    !anyValue;
    !unknownValue;
    !undefinedValue;
...
function questionOrAssignment(typeParameter) {
    // question
    anyValue ?? 1;
    unknownValue ?? 1;
    undefinedValue ?? 1;
