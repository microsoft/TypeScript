...
function f52<T>(obj: { [x: string]: boolean }, k: Exclude<keyof T, symbol>, s: string, n: number): void {
    const x1 = obj[s];
    const x2 = obj[n];
    const x3 = obj[k];
}

function f53<T, K extends Exclude<keyof T, symbol>>(obj: { [x: string]: boolean }, k: K, s: string, n: number): void {
    const x1 = obj[s];
    const x2 = obj[n];
    const x3 = obj[k];
}
...
function f80<T extends { a: { x: any } }>(obj: T): void {
    let a1 = obj.a;  // { x: any }
    let a2 = obj['a'];  // { x: any }
...
function f81<T extends { a: { x: any } }>(obj: T): T["a"]["x"] {
...
function f83<T extends { [x: string]: { x: any } }, K extends keyof T>(obj: T, key: K): T[K]["x"] {
...
var empty: unknown = one(() => {}) // inferred as {}, expected
...
let result: string[] = dispatchMethod<SomeMethodDescriptor>("someMethod", ["hello", 35]);
...
function onChangeGenericFunction<T>(handler: Handler<T & {preset: number}>): void {
...
const cf1 = <T extends { [P in K]: string; } & { cool: string; }, K extends keyof T>(t: T, k: K): void =>
{
    const s: string = t[k];
    t.cool;
};
...
//# sourceMappingURL=keyofAndIndexedAccess.d.ts.map
/// [Errors] ////
