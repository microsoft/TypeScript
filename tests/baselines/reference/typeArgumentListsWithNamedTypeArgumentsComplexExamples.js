//// [typeArgumentListsWithNamedTypeArgumentsComplexExamples.ts]
...
//// [typeArgumentListsWithNamedTypeArgumentsComplexExamples.js]
var assumingNotAllowed = testNamingOtherParameters({ a: "test" }); // Error, cannot find `A`
var result1 = stillDefaultsIfNoInference({ b: "test" }); // expect result1 type is {a: string, b: string, c: object, x: {}}
...
var expectAllowed3 = testConstraints2({ a: ["x", "y"] }); // OK - inference fails, but that just makes A = string, whcih still passes
var expectError2 = testConstraints2({ b: ["x", "y", "z"] }); // error "z" not in "x" | "y"
var expectAllowed4 = complexConstraints({ a: ["x"], c: ["x", "y"] }); // OK { a: ("x" | "y" | "z")[], b: ("x" | "y" | "z")[], c: ("x" | "y")[] }
// OK because B inferred to be "x" but that conflicts with C as "x" | "y" so inference fails - A and C are provided,
// B becomes its constraint, A, or "x" | "y" | "z", and the call successfully resolves
var expectAlllowed5 = complexConstraints({ b: ["x"] });
var expectError3 = complexConstraints({ c: ["y"] }); // error "y" does not extend "x"
var expectAllowed6 = "test"; // R defaults to any, so this is fine
var expectAllowed7 = 35; // As is this
var expectError4 = "test"; // Didn't fulfill R, issues error
