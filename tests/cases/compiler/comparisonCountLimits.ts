...
function get11BitsOf<T extends string, U extends string, FallbackPrefix extends string>() {
    type Bit = T | U; // 2^1
    type HalfNibble = `${Bit}${Bit}`; // 2^2
    type Nibble = `${HalfNibble}${HalfNibble}`; // 2^4
    type Byte = `${Nibble}${Nibble}`; // 2^8
    type ElevenBits = `${Byte}${HalfNibble}${Bit}`; // 2^11 (approx. 4 million comparisons if compared naively)
...
let a = get11BitsOf<"0", "1", "a" | "b">();
const b = get11BitsOf<"a", "b", never>();
