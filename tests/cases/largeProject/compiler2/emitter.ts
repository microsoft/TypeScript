...
module TypeScript2 {
    ...
    export class Emitter {
        ...
        public writeCaptureThisStatement(ast: AST2) {
            this.emitIndent();
        ...
        public emitParensAndCommentsInPlace(ast: AST2, pre: boolean) {
        ...
        public emitObjectLiteral(content: ASTList2) {
            this.writeLineToOutput("{");
        ...
        public emitArrayLiteral(content: ASTList2) {
            this.writeToOutput("[");
            if (content) {
                this.writeLineToOutput("");
                this.indenter.increaseIndent();
        ...
        public emitNew(target: AST2, args: ASTList2) {
        ...
        private getConstantValue(init: AST2): number {
            if (init) {
                if (init.nodeType === NodeType.NumberLit) {
                    ...
                    var binop = <BinaryExpression2>init;
        ...
        public tryEmitConstant(dotExpr: BinaryExpression2) {
            if (!this.emitOptions.propagateConstants) {
                return false;
            }
            var propertyName = <Identifier2>dotExpr.operand2;
        ...
        public emitCall(callNode: CallExpression, target: AST2, args: ASTList2) {
            if (!this.emitSuperCall(callNode)) {
                if (!hasFlag(callNode.flags, ASTFlags.ClassBaseConstructorCall)) {
                    ...
                    this.indenter.decreaseIndent();
                    this.indenter.decreaseIndent();
                    var constructorCall = new ASTList2();
        ...
        public emitConstructorCalls(bases: ASTList2, classDecl: TypeDeclaration) {
            if (bases == null) {
                return;
            }
        ...
        public emitInnerFunction(funcDecl: FuncDecl, printName: boolean, isMember: boolean,
            bases: ASTList2, hasSelfRef: boolean, classDecl: TypeDeclaration) {
            ...
            if (funcDecl.isConstructor && hasFlag(funcDecl.fncFlags, FncFlags.ClassMethod) && !classPropertiesMustComeAfterSuperCall) {
                ...
                var nProps = (<ASTList2>this.thisClassNode.members).members.length;

                for (var i = 0; i < nProps; i++) {
                    if ((<ASTList2>this.thisClassNode.members).members[i].nodeType == NodeType.VarDecl) {
                        var varDecl = <VarDecl>(<ASTList2>this.thisClassNode.members).members[i];
                    ...
                    if (this.emitOptions.ioHost) {
                        ...
                        if (switchToForwardSlashes(modFilePath) != switchToForwardSlashes(this.emittingFileName)) {
                            ...
                            if (prevSourceMapper != null) {
                                ...
                                this.setSourceMappings(new TypeScript2.SourceMapper(tsModFileName, this.emittingFileName, this.outfile, sourceMappingFile, this.errorReporter, this.emitOptions.emitFullSourceMapPath));
                                this.emitState.column = 0;
        ...
        public emitIndex(operand1: AST2, operand2: AST2) {
        ...
        public emitJavascriptName(name: Identifier2, addThis: boolean) {
            var sym = name.sym;
        ...
        public emitJavascriptStatements(stmts: AST2, emitEmptyBod: boolean) {
            if (stmts) {
                if (stmts.nodeType != NodeType.Block) {
                    var hasContents = (stmts && (stmts.nodeType != NodeType.List || ((<ASTList2>stmts).members.length > 0)));
                    if (emitEmptyBod || hasContents) {
                        var hasOnlyBlockStatement = ((stmts.nodeType == NodeType.Block) ||
                            ((stmts.nodeType == NodeType.List) && ((<ASTList2>stmts).members.length == 1) && ((<ASTList2>stmts).members[0].nodeType == NodeType.Block)));
        ...
        public emitBareJavascriptStatements(stmts: AST2, emitClassPropertiesAfterSuperCall: boolean = false) {
            // just the statements without enclosing curly braces
            if (stmts.nodeType != NodeType.Block) {
                if (stmts.nodeType == NodeType.List) {
                    var stmtList = <ASTList2>stmts;
        ...
        public recordSourceMappingStart(ast: ASTSpan2) {
        ...
        public recordSourceMappingEnd(ast: ASTSpan2) {
            if (this.sourceMapper && isValidAstNode(ast)) {
        ...
        public emitJavascriptList(ast: AST2, delimiter: string, tokenId: TokenID, startLine: boolean, onlyStatics: boolean, emitClassPropertiesAfterSuperCall: boolean = false, emitPrologue? = false, requiresExtendsBlock?: boolean) {
            if (ast == null) {
                return;
            }
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    if (i == 1 && emitClassPropertiesAfterSuperCall) {
                        ...
                        for (var iMember = 0; iMember < nProps; iMember++) {
                            if ((<ASTList2>this.thisClassNode.members).members[iMember].nodeType == NodeType.VarDecl) {
                                var varDecl = <VarDecl>(<ASTList2>this.thisClassNode.members).members[iMember];
                                if (!hasFlag(varDecl.varFlags, VarFlags.Static) && varDecl.init) {
                                    this.emitIndent();
                                    this.emitJavascriptVarDecl(varDecl, TokenID.Tilde);
                                    this.writeLineToOutput("");
        ...
        public emitJavascript(ast: AST2, tokenId: TokenID, startLine: boolean) {
            if (ast == null) {
                return;
            }
        ...
        public emitPrototypeMember(member: AST2, className: string) {
        ...
        public emitAddBaseMethods(className: string, base: Type2, classDecl: TypeDeclaration): void {
            if (base.members) {
        ...
        public emitJavascriptClass(classDecl: ClassDeclaration) {
            if (!hasFlag(classDecl.varFlags, VarFlags.Ambient)) {
                ...
                var baseNameDecl: AST2 = null;
                var baseName: AST2 = null;
                ...
                if (constrDecl) {
                    ...
                    var members = (<ASTList2>this.thisClassNode.members).members
                ...
                for (var j = 0; j < membersLen; j++) {
                    ...
                    var memberDecl: AST2 = classDecl.members.members[j];
        ...
        public emitSuperCall(callEx: CallExpression): boolean {
            if (callEx.target.nodeType == NodeType.Dot) {
                var dotNode = <BinaryExpression2>callEx.target;
