...
module TypeScript2 {
    ...
    export class ErrorReporter {
        ...
        public writePrefix(ast: AST2): void {
            if (ast) {
                this.setError(ast);
            }
            else {
        ...
        public reportError(ast: AST2, message: string) {
        ...
        public emitterError(ast: AST2, message: string) {
            this.reportError(ast, message);
            // Emitter errors are not recoverable, stop immediately
            throw Error("EmitError");
        }

        public duplicateIdentifier(ast: AST2, name: string) {
            this.reportError(ast, "Duplicate identifier '" + name + "'");
        }

        public showRef(ast: AST2, text: string, symbol: Symbol) {
        ...
        public unresolvedSymbol(ast: AST2, name: string) {
            this.reportError(ast, "The name '" + name + "' does not exist in the current scope");
        }

        public symbolDoesNotReferToAValue(ast: AST2, name: string): void {
            this.reportError(ast, "The name '" + name + "' does not refer to a value");
        }

        public styleError(ast: AST2, msg: string): void {
        ...
        public simpleError(ast: AST2, msg: string): void {
        ...
        public indexLHS(ast: BinaryExpression2, scope: SymbolScope): void {
        ...
        public incompatibleTypes(ast: AST2, t1: Type2, t2: Type2, op: string, scope: SymbolScope, comparisonInfo?:TypeComparisonInfo) {
            if (!t1) {
        ...
        public expectedClassOrInterface(ast: AST2): void {
            this.simpleError(ast, "Expected var, class, interface, or module");
        }

        public unaryOperatorTypeError(ast: AST2, op: string, type: Type2) {
