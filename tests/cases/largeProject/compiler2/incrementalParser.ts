...
module TypeScript2 {
    ...
    export class IncrementalParser {
        ...
        constructor (private logger: TypeScript2.ILogger2) {
        ...
        public getEnclosingScopeContextIfSingleScopeEdit(previousScript: Script, scriptId: string, newSourceText: ISourceText, editRange: ScriptEditRange): EnclosingScopeContext {
            ...
            var scope1 = TypeScript2.findEnclosingScopeAt(this.logger, previousScript, newSourceText, editRange.minChar, false/*isMemberCompletion*/);
            var scope2 = TypeScript2.findEnclosingScopeAt(this.logger, previousScript, newSourceText, editRange.limChar, false/*isMemberCompletion*/);
        ...
        public attemptIncrementalUpdateUnit(previousScript: Script, scriptId: string, newSourceText: ISourceText, editRange: ScriptEditRange): UpdateUnitResult {
            ...
            var parseErrors: TypeScript2.ErrorEntry[] = [];
            var errorCapture = function(minChar: number, charLen: number, message: string, unitIndex: number): void {
                parseErrors.push(new TypeScript2.ErrorEntry(unitIndex, minChar, minChar + charLen, message));
            };

            var quickParseResult = TypeScript2.quickParse(this.logger, scope1.scopeStartAST, newSourceText, scope1.scopeStartAST.minChar, scope1.scopeStartAST.minChar + newScopeLength, errorCapture);
            if (quickParseResult.endLexState != TypeScript2.LexState.Start) {
            ...
            return TypeScript2.UpdateUnitResult.singleScopeEdits(previousScript, scriptFragment, oldScope, newScope, editRange, parseErrors);
        }

        public mergeTrees(updateResult: UpdateUnitResult): void {
            TypeScript2.timeFunction(this.logger, "mergeTrees()", () => {
        ...
        private replaceAST(script: TypeScript2.AST2, oldAst: TypeScript2.AST2, newAst: TypeScript2.AST2) {
            var pre = (cur: TypeScript2.AST2, parent: TypeScript2.AST2, walker: TypeScript2.IAstWalker) => {
                if (cur === oldAst) {
        ...
        private mergeLocationInfo(script: TypeScript2.Script, partial: TypeScript2.Script, editRange: ScriptEditRange) {
        ...
        private applyDeltaPosition(ast: TypeScript2.AST2, start: number, delta: number) {
            var applyDelta = (ast: TypeScript2.AST2) => {
            ...
            var applyDeltaToComments = (comments: TypeScript2.Comment[]) => {
            ...
            TypeScript2.getAstWalkerFactory().walk(ast, pre);
        }
    }
}
