...
module TypeScript2 {
    ...
    export class Parser {
        private varLists: ASTList2[] = [];
        private scopeLists: ASTList2[] = [];
        private staticsLists: ASTList2[] = [];
        ...
        private anonId = new Identifier2("_anonymous");
        ...
        private pushStmt(stmt: Statement, labels: ASTList2) {
        ...
        private resolveJumpTarget(jump: Jump): void {
            var resolvedTarget = AST2.getResolvedIdentifierName(jump.target);
                ...
                if (jump.target) {
                    if (info.labels && (info.labels.members.length > 0)) {
                        for (var j = 0, labLen = info.labels.members.length; j < labLen; j++) {
                            var Label2 = <Label2>info.labels.members[j];
                            if (Label2.id.text == resolvedTarget) {
            ...
            // no luck
            if (jump.target) {
                this.reportParseError("could not find enclosing statement with Label2 " + jump.target);
            }
            else {
        ...
        private pushDeclLists() {
            this.staticsLists.push(new ASTList2());
            this.varLists.push(new ASTList2());
            this.scopeLists.push(new ASTList2());
        }
        ...
        private parseEnumDecl(errorRecoverySet: ErrorRecoverySet, modifiers: Modifiers): ModuleDeclaration {
            ...
            this.pushDeclLists();
            var members = new ASTList2();
            members.minChar = membersMinChar;
            var mapDecl = new VarDecl(new Identifier2("_map"), 0);
            ...
            mapDecl.init = new UnaryExpression2(NodeType.ArrayLit, null);
            members.append(mapDecl);
            var lastValue: NumberLiteral = null;
            var memberNames: Identifier2[] = [];
            for (; ;) {
                var minChar = this.scanner.startPos;
                var limChar;
                var memberName: Identifier2 = null;
                var memberValue: AST2 = null;
                ...
                if ((this.currentToken.tokenId == TokenID.Identifier) || convertTokToIDName(this.currentToken)) {
                    memberName = Identifier2.fromToken(this.currentToken);
                    ...
                    if (this.errorRecovery) {
                        memberName = new MissingIdentifier2();
                        memberName.minChar = this.scanner.startPos;
                ...
                if (this.currentToken.tokenId == TokenID.Equals) {
                    ...
                    var map: BinaryExpression2 =
                        new BinaryExpression2(NodeType.Asg,
                                             new BinaryExpression2(NodeType.Index,
                                                                  new Identifier2("_map"),
                                                                  memberValue),
                ...
                if (memberValue.nodeType == NodeType.NumberLit) {
                    ...
                    // as well.
                    var binop = <BinaryExpression2>memberValue;
                    ...
                    // as well.
                    var nameNode = <Identifier2>memberValue;
        ...
        private parseDottedName(enclosedList: AST2[]): void {
            this.currentToken = this.scanner.scan();
            if ((this.currentToken.tokenId == TokenID.Identifier) || convertTokToID(this.currentToken, this.strictMode)) {
                var id = Identifier2.fromToken(this.currentToken);
                    ...
                    if (this.currentToken.tokenId == TokenID.OpenParen) {
                        ...
                        if (this.currentToken.tokenId == TokenID.StringLiteral || this.currentToken.tokenId == TokenID.Identifier || convertTokToID(this.currentToken, this.strictMode)) {
                            ...
                            if (this.currentToken.tokenId == TokenID.StringLiteral) {
                                ...
                                alias = Identifier2.fromToken(this.currentToken);
                                alias.minChar = this.scanner.startPos;
                                alias.limChar = this.scanner.pos;

                                if (!this.isValidImportPath((<Identifier2>alias).text)) {
        ...
        private parseModuleDecl(errorRecoverySet: ErrorRecoverySet, modifiers: Modifiers, preComments: Comment[]): ModuleDeclaration {
            ...
            if (enclosedList && (enclosedList.length > 0)) {
                ...
                var innerName = <Identifier2>enclosedList[len - 1];
                ...
                this.popDeclLists();
                var outerModBod: ASTList2;
                for (var i = len - 2; i >= 0; i--) {
                    outerModBod = new ASTList2();
                    outerModBod.append(innerDecl);
                    innerName = <Identifier2>enclosedList[i];
                    innerDecl = new ModuleDeclaration(innerName, outerModBod, new ASTList2(),
                                                new ASTList2(), endingToken);
                ...
                moduleDecl = new ModuleDeclaration(<Identifier2>name, outerModBod, new ASTList2(),
                                            new ASTList2(), endingToken);
            }
            else {
                moduleDecl = new ModuleDeclaration(<Identifier2>name, moduleBody, this.topVarList(), this.topScopeList(), endingToken);
        ...
        private parseTypeReferenceTail(errorRecoverySet: ErrorRecoverySet, minChar: number, term: AST2): TypeReference {
        ...
        private parseNamedType(errorRecoverySet: ErrorRecoverySet, minChar: number, term: AST2, tail: boolean): AST2 {
            ...
            if (this.currentToken.tokenId == TokenID.Dot) {
                ...
                if ((this.currentToken.tokenId == TokenID.Identifier) || ((!this.errorRecovery || !this.scanner.lastTokenHadNewline()) && convertTokToID(this.currentToken, this.strictMode))) {
                    var op2 = Identifier2.fromToken(this.currentToken);
                    op2.minChar = this.scanner.startPos;
                    op2.limChar = this.scanner.pos;
                    var dotNode = new BinaryExpression2(NodeType.Dot, term, op2);
                    ...
                    if (this.errorRecovery) {
                        ...
                        var eop2 = new MissingIdentifier2();
                        eop2.minChar = this.scanner.pos;
                        eop2.limChar = this.scanner.pos;
                        var edotNode = new BinaryExpression2(NodeType.Dot, term, eop2);
        ...
        private parseTypeReference(errorRecoverySet: ErrorRecoverySet, allowVoid: boolean): AST2 {
            ...
            switch (this.currentToken.tokenId) {
                ...
                case TokenID.String: {
                    ...
                    var predefinedIdentifier = new Identifier2(text);
        ...
        private parseFunctionStatements(errorRecoverySet: ErrorRecoverySet,
            ...
            if (!requiresSignature) {
                if (!wasShorthand || isAnonLambda) {
                    funcDecl.endingToken = new ASTSpan2();
        ...
        private transformAnonymousArgsIntoFormals(formals: ASTList2, argList: AST2) : boolean {
            ...
            var translateBinExOperand = (operand: AST2) : boolean => {
                if (operand.nodeType == NodeType.Comma) {
                    ...
                    var opArg = operand.nodeType == NodeType.Asg ? (<BinaryExpression2>operand).operand1 : operand;

                    var arg = new ArgDecl(<Identifier2>opArg);
                    ...
                    if (operand.nodeType == NodeType.Asg) {
                        arg.init = (<BinaryExpression2>operand).operand2;
                    }
            ...
            if (argList) {
                if (argList.nodeType == NodeType.Comma) {
                    var commaList = <BinaryExpression2> argList;
        ...
        private parseFncDecl(errorRecoverySet: ErrorRecoverySet,
            ...
            if (!isMethod && !isStatic && !indexer && !lambdaArgContext) {
                ...
                if ((this.currentToken.tokenId != TokenID.Identifier) && (!convertTokToID(this.currentToken, this.strictMode))) {
                    if (isDecl) {
                        ...
                        return new IncompleteAST2(fnMin, this.scanner.pos);
                    }
                }
                else {
                    name = Identifier2.fromToken(this.currentToken);
            ...
            var args: ASTList2 = new ASTList2();
        ...
        private convertToTypeReference(ast: AST2): TypeReference {
            ...
            switch (ast.nodeType) {
                ...
                case NodeType.Name:
                    ...
                    return result;
                case NodeType.Index: {
                    var expr = <BinaryExpression2>ast;
                    ...
                    if (result) {
                        ...
                        var etr = <TypeReference>new AST2(NodeType.Error);
                        return etr;
                    }
                }
            }
        ...
        private parseArgList(errorRecoverySet: ErrorRecoverySet): ASTList2 {
            var args: ASTList2 = new ASTList2();
        ...
        private parseBaseList(extendsList: ASTList2,
            ...
            for (; ;) {
                ...
                var baseName: Identifier2 = null;
                if ((this.currentToken.tokenId == TokenID.Identifier) || convertTokToID(this.currentToken, this.strictMode)) {
                    var minChar = this.scanner.startPos;
                    baseName = Identifier2.fromToken(this.currentToken);
                    baseName.minChar = minChar;
                    baseName.limChar = this.scanner.pos;
                    baseName = <Identifier2>this.parseNamedType(errorRecoverySet | ErrorRecoverySet.LCurly,
                                            minChar, baseName, false);
                }
                else {
                    this.reportParseError("Expected base name");
                    if (this.errorRecovery) {
                        baseName = new MissingIdentifier2();
        ...
        private parseClassDecl(errorRecoverySet: ErrorRecoverySet, minChar: number, modifiers: Modifiers): ClassDeclaration {
            ...
            var extendsList: ASTList2 = null;
            var implementsList: ASTList2 = null;
            var requiresSignature = false;

            if ((this.currentToken.tokenId == TokenID.Extends) ||
                (this.currentToken.tokenId == TokenID.Implements)) {
                extendsList = new ASTList2();
                implementsList = new ASTList2();
            ...
            var classDecl = new ClassDeclaration(name, new ASTList2(), extendsList, implementsList);
        ...
        private parseClassElements(classDecl: ClassDeclaration, errorRecoverySet: ErrorRecoverySet, parentModifiers: Modifiers) {
            ...
            while (!(this.currentToken.tokenId == TokenID.CloseBrace || this.currentToken.tokenId == TokenID.EndOfFile)) {
                ...
                if (this.currentToken.tokenId == TokenID.Get) {
                    ...
                    var id = wasGetOrSetId ? new Identifier2(idText) : Identifier2.fromToken(this.currentToken);
                    id.minChar = this.scanner.startPos;
                    id.limChar = this.scanner.pos;
            ...
            if (this.currentToken.tokenId == TokenID.CloseBrace) {
                classDecl.endingToken = new ASTSpan2();
        ...
        private parseClassMemberFunctionDeclaration(methodName: Identifier2, minChar: number, errorRecoverySet: ErrorRecoverySet, modifiers: Modifiers) {
            ...
            var ast: AST2 = this.parseFncDecl(errorRecoverySet, true, isAmbient, true, methodName, false, isStatic, isAmbient, modifiers, null, true);
        ...
        private parseTypeMember(errorRecoverySet: ErrorRecoverySet): AST2 {
            var minChar = this.scanner.startPos;
        ...
        private parseTypeMemberList(errorRecoverySet: ErrorRecoverySet, members: ASTList2) {
        ...
        private makeVarDecl(id: Identifier2, nest: number): VarDecl {
        ...
        private parsePropertyDeclaration(
            ...
            if (this.currentToken.tokenId == TokenID.OpenParen && !wasAccessorID) {
                ...
                if (this.errorRecovery) {
                    ...
                    var epd = new VarDecl(new MissingIdentifier2(), this.nestingLevel);
                ...
                if (wasAccessorID) {
                    text = Identifier2.fromToken(this.prevIDTok);
                    ...
                    text = Identifier2.fromToken(this.currentToken);
        ...
        private parseVariableDeclaration(
            ...
            while (true) {
                if ((this.currentToken.tokenId != TokenID.Identifier) && (!convertTokToID(this.currentToken, this.strictMode))) {
                    ...
                    if (this.errorRecovery) {
                        varDecl = new VarDecl(new MissingIdentifier2(), this.nestingLevel);
                        varDecl.minChar = minChar;
                        this.skip(errorRecoverySet);
                        varDecl.flags |= ASTFlags.Error;
                        varDecl.limChar = this.scanner.lastTokenLimChar();
                ...
                var varDeclName = Identifier2.fromToken(this.currentToken)
        ...
        private parseMemberList(errorRecoverySet: ErrorRecoverySet): ASTList2 {
            var elements = new ASTList2();
            if (this.currentToken.tokenId == TokenID.CloseBrace) {
                return elements;
            }

            var idHint: string = null;
            var memberName: AST2 = null;
            var memberExpr: AST2 = null;
            var member: BinaryExpression2 = null;
            var minChar = this.scanner.startPos;
            var isSet = false;
            ...
            for (; ;) {
                ...
                if (this.currentToken.tokenId == TokenID.Get || this.currentToken.tokenId == TokenID.Set) {
                    ...
                    if ((this.currentToken.tokenId == TokenID.Identifier) || convertTokToIDName(this.currentToken)) {
                        ...
                        memberName = Identifier2.fromToken(getSetTok);
                ...
                if (accessorPattern) {
                    ...
                    memberExpr = funcDecl;
                    member = new BinaryExpression2(NodeType.Member, memberName, memberExpr);
        ...
        private parseArrayList(errorRecoverySet: ErrorRecoverySet): ASTList2 {
            var elements: ASTList2 = null;
            if (this.currentToken.tokenId == TokenID.CloseBracket) {
                return elements;
            }
            else {
                elements = new ASTList2();
            ...
            for (; ;) {
                if ((this.currentToken.tokenId == TokenID.Comma) ||
                    ...
                    arg = new AST2(NodeType.EmptyExpr);
                }
                else {
        ...
        private parseArrayLiteral(errorRecoverySet: ErrorRecoverySet): UnaryExpression2 {
            var arrayLiteral: UnaryExpression2 = null;
            arrayLiteral = new UnaryExpression2(NodeType.ArrayLit,
        ...
        private parseTerm(errorRecoverySet: ErrorRecoverySet, allowCall: boolean, typeContext: TypeContext, inCast: boolean): AST2 {
            ...
            switch (this.currentToken.tokenId) {
                ...
                case TokenID.Any:
                case TokenID.String:
                    var tid = new Identifier2(tokenTable[this.currentToken.tokenId].text);
                    ...
                    break;
                case TokenID.This:
                    ast = new AST2(NodeType.This);
                    ast.minChar = minChar;
                    this.currentToken = this.scanner.scan();
                    limChar = this.scanner.lastTokenLimChar();
                    break;
                case TokenID.Super:
                    ast = new AST2(NodeType.Super);
                    ast.minChar = minChar;
                    this.currentToken = this.scanner.scan();
                    limChar = this.scanner.lastTokenLimChar();
                    break;
                case TokenID.True:
                    ast = new AST2(NodeType.True);
                    this.currentToken = this.scanner.scan();
                    ast.minChar = minChar;
                    break;
                case TokenID.False:
                    ast = new AST2(NodeType.False);
                    this.currentToken = this.scanner.scan();
                    ast.minChar = minChar;
                    break;
                case TokenID.Null:
                    ast = new AST2(NodeType.Null);
                    this.currentToken = this.scanner.scan();
                    ast.minChar = minChar;
                    break;
                case TokenID.New:
                    ...
                    if (target.nodeType == NodeType.Error || (target.nodeType == NodeType.Index && (<BinaryExpression2>target).operand1.nodeType == NodeType.TypeRef)) {
            ...
            if (ast == null) {
                switch (this.currentToken.tokenId) {
                    ...
                    // TODO: rescan regex for TokenID.Div and AsgDiv
                        ...
                        this.checkCurrentToken(TokenID.CloseBrace, errorRecoverySet);
                        ast = new UnaryExpression2(NodeType.ObjectLit, members);
                        ast.minChar = minChar;
                        limChar = this.scanner.lastTokenLimChar();
                        members.minChar = minChar;
                    ...
                    case TokenID.LessThan:
                        minChar = this.scanner.startPos;
                        this.currentToken = this.scanner.scan();
                        var term: AST2 = this.parseTypeReference(ErrorRecoverySet.BinOp, false);
                        this.checkCurrentToken(TokenID.GreaterThan, errorRecoverySet);
                        ast = new UnaryExpression2(NodeType.TypeAssertion, this.parseExpr(errorRecoverySet, OperatorPrecedence.Unary, false, TypeContext.NoTypes));
                        (<UnaryExpression2>ast).castTerm = term;
                        break;
                    ...
                    default:
                        if (this.prevExpr && hasFlag(this.prevExpr.flags, ASTFlags.PossibleOptionalParameter)) {
                            ...
                            if (this.errorRecovery) {
                                var ident = new MissingIdentifier2();
            ...
            if (postFix) {
                if (sawId && (postFix.nodeType == NodeType.Index)) {
                    var binExpr = <BinaryExpression2>postFix;
                ...
                return new AST2(NodeType.Error);
            }
        ...
        private parseExpr(errorRecoverySet: ErrorRecoverySet, minPrecedence: number, allowIn: boolean,
            ...
            if ((tokenInfo != undefined) && (tokenInfo.unopNodeType != NodeType.None)) {
                ...
                if ((tokenInfo.unopNodeType == NodeType.Pos) &&
                    ...
                    ast = new UnaryExpression2(tokenInfo.unopNodeType, tempExpr);
                ...
                if (ast.nodeType == NodeType.Name) {
                    ...
                    if (this.inferPropertiesFromThisAssignment && 
                        (this.currentToken.tokenId == TokenID.Colon || this.currentToken.tokenId == TokenID.Equals) &&
                         ...
                         (<BinaryExpression2>ast).operand1.nodeType == NodeType.This) {

                        if ((<BinaryExpression2>ast).operand2.nodeType == NodeType.Name) {
                            var op2ID: Identifier2 = (<Identifier2>(<BinaryExpression2>ast).operand2);
                    ...
                    if (!subsumedExpr) {
                        ...
                        while (temp.nodeType == NodeType.Dot) {
                            var binExpr = <BinaryExpression2>temp;
                ...
                if ((!this.scanner.lastTokenHadNewline()) &&
                    ...
                    canAssign = false;
                    var operand = ast;
                    ast = new UnaryExpression2((this.currentToken.tokenId == TokenID.PlusPlus) ? NodeType.IncPost : NodeType.DecPost, operand);
            ...
            for (; ;) {
                ...
                if (tokenInfo.binopNodeType == NodeType.ConditionalExpression) {
                    ...
                    var binExpr2: BinaryExpression2;

                    binExpr2 = new BinaryExpression2(tokenInfo.binopNodeType, ast,
        ...
        private parsePostfixOperators(errorRecoverySet: ErrorRecoverySet, ast: AST2, allowCall: boolean, inNew: boolean,
            ...
            if (!ast) {
                ast = new AST2(NodeType.EmptyExpr);
                ast.isParenthesized = true;
            }
            ...
            for (; ;) {
                switch (this.currentToken.tokenId) {
                    ...
                    case TokenID.OpenBracket:
                        ...
                        if (this.currentToken.tokenId == TokenID.CloseBracket) {
                            if (hasFlag(typeContext, TypeContext.ArraySuffix)) {
                                ...
                                if (ast.nodeType == NodeType.TypeRef) {
                                    ...
                                    ast = new BinaryExpression2(NodeType.Index, ast, null);
                                }
                                ast.limChar = this.scanner.pos;
                        ...
                        ast = new BinaryExpression2(NodeType.Index, ast,
                    ...
                    case TokenID.Dot: {
                        ...
                        ast = new BinaryExpression2(NodeType.Dot, ast, name);
                        ast.minChar = lhsMinChar;
                        ast.limChar = this.scanner.lastTokenLimChar();
                        break;
                    }
        ...
        private parseCatch(errorRecoverySet: ErrorRecoverySet, parentModifiers: Modifiers): Catch {
            ...
            if ((this.currentToken.tokenId != TokenID.Identifier) || convertTokToID(this.currentToken, this.strictMode)) {
                ...
                if (this.errorRecovery) {
                    ...
                    var ecatch = new Catch(new VarDecl(new MissingIdentifier2(), this.nestingLevel),
            ...
            var param = new VarDecl(Identifier2.fromToken(this.currentToken), this.nestingLevel);
        ...
        private parseTryCatchFinally(errorRecoverySet: ErrorRecoverySet, parentModifiers: Modifiers, labelList: ASTList2): AST2 {
            var tryPart: AST2 = new Try(null);
            ...
            if (this.currentToken.tokenId != TokenID.Finally) {
                if (tc == null) {
                    ...
                    if (this.errorRecovery) {
                        var etf = new TryFinally(tryPart, new Finally(new AST2(NodeType.Empty)));
                        etf.flags |= ASTFlags.Error;
                        etf.minChar = this.scanner.startPos;
                        etf.limChar = this.scanner.pos;
                        return etf;
                    }
                    return new TryFinally(tryPart, new Finally(new AST2(NodeType.Empty)));
                }
                else {
                    return tc;
                }
        ...
        private parseStatement(errorRecoverySet: ErrorRecoverySet, allowedElements: AllowedElements, parentModifiers: Modifiers): AST2 {
            var ast: AST2 = null;
            var labelList: ASTList2 = null;
            var astList: ASTList2 = null;
            var temp: AST2;
            var modifiers = Modifiers.None;
            var minChar = this.scanner.startPos;
            var forInOk = false;
            var needTerminator = false;
            var fnOrVar: AST2 = null;
            var preComments = this.parseComments();
            ...
            for (; ;) {
                switch (this.currentToken.tokenId) {
                    ...
                    case TokenID.Interface:
                        ...
                        break;
                    case TokenID.Var:
                        var declAst: AST2 = this.parseVariableDeclaration(errorRecoverySet | ErrorRecoverySet.StmtStart, modifiers,
                                                     true, false);
                        if (declAst.nodeType == NodeType.VarDecl) {
                            ast = declAst;
                        }
                        else {
                            ast = new Block(<ASTList2>declAst, false);
                        }
                        needTerminator = true;
                        break;
                    case TokenID.Static:
                    ...
                    case TokenID.Switch: {
                        ...
                        switchStmt.caseList = new ASTList2();
                    ...
                    case TokenID.Do: {
                        ...
                        this.popStmt();
                        doStmt.whileAST = new Identifier2("while");
                    ...
                    case TokenID.OpenBrace: {
                        ...
                        minChar = this.scanner.startPos;
                        this.currentToken = this.scanner.scan();
                        var block = new Block(new ASTList2(), true);
                    ...
                    case TokenID.Throw:
                        ...
                        ast = new UnaryExpression2(NodeType.Throw, temp);
                        ast.limChar = this.scanner.lastTokenLimChar();
                        needTerminator = true;
                        break;
                    case TokenID.Enum:
                    ...
                    default:
                        if (modifiers != Modifiers.None) {
                            this.reportParseError("modifiers can not appear before an expression statement or Label2");
                        }
                        minChar = this.scanner.startPos;
                        ...
                        if (this.scanner.pos == svPos) {
                            ...
                            ast = temp;
                                        ...
                                        temp && (temp.nodeType == NodeType.Name)) {
                                            // It's a Label2
                            if (labelList == null) {
                                labelList = new ASTList2();
                            }
                            labelList.append(new Label2(<Identifier2>temp));
                            this.currentToken = this.scanner.scan();
                        }
                        else {
        ...
        private okAmbientModuleMember(ast: AST2) {
            var nt = ast.nodeType;
        ...
        public quickParse(sourceText: ISourceText, filename: string, unitIndex: number): QuickParseResult {
            ...
            var svGenTarget = TypeScript2.moduleGenTarget;
            try {
                TypeScript2.moduleGenTarget = TypeScript2.ModuleGenTarget.Local;
                ...
                TypeScript2.moduleGenTarget = svGenTarget;
            }
        }
        ...
        public parse(sourceText: ISourceText, filename: string, unitIndex: number, allowedElements = AllowedElements.Global): Script {
            ...
            if (moduleGenTarget != ModuleGenTarget.Local && this.hasTopLevelImportOrExport) {
                ...
                var id: Identifier2 = new Identifier2(correctedFileName);
    ...
    export function quickParse(logger: TypeScript2.ILogger2, scopeStartAST: AST2, sourceText: ISourceText, minChar: number, limChar: number,
        ...
        logger.log("Quick parse range (" + minChar + "," + limChar + "): \"" + TypeScript2.stringToLiteral(fragment, 100) + "\"");
