...
func (p *typedPathsPass) run() (any, error) {
	...
	for c := range in.Root().Preorder((*ast.CallExpr)(nil)) {
		...
		arg := call.Args[0]
		if value := p.pass.TypesInfo.Types[arg].Value; value != nil && value.Kind() == constant.String {
			path := constant.StringVal(value)
			if path == "" {
				continue
			}
			if valid, requirement, checked := validateTypedPathConstant(target, path); checked {
				if valid {
					continue
				}
				p.pass.Reportf(
					call.Pos(),
					"constant converted to %s must be %s",
					target,
					requirement,
				)
				continue
		...
		diag := analysis.Diagnostic{
			Pos:     call.Pos(),
			End:     call.End(),
			Message: fmt.Sprintf("conversion from %s to %s adds path invariants; use a tspath constructor", sourceName, target),
		}
		if p.pass.TypesInfo.Types[arg].Value != nil {
			diag.SuggestedFixes = p.typedPathConstructorFix(call, arg, target)
		}
		p.pass.Report(diag)
	}
	...
	for c := range in.Root().Preorder(
		...
		if valid, requirement, checked := validateTypedPathConstant(target, constant.StringVal(typeAndValue.Value)); checked {
			if valid {
				continue
			}
			p.pass.Reportf(
		...
		p.pass.Report(analysis.Diagnostic{
			Pos:            expr.Pos(),
			End:            expr.End(),
			Message:        fmt.Sprintf("implicit conversion of non-empty constant to %s adds path invariants; use a tspath constructor", target),
			SuggestedFixes: p.typedPathConstructorFix(expr, expr, target),
		})
	}
...
func (p *typedPathsPass) typedPathConstructorFix(replace ast.Expr, value ast.Expr, target string) []analysis.SuggestedFix {
	constructor := ""
	switch target {
	case "RootedPath":
		constructor = "RootedPathFromNormalized"
	case "RootedFilePath":
		constructor = "RootedFilePathFromNormalized"
	case "RootedDirectoryPath":
		constructor = "RootedDirectoryPathFromNormalized"
	case "RelativePath":
		constructor = "RelativePathFromNormalized"
	case "PathKey":
		constructor = "PathKeyFromCanonical"
	case "ModuleSpecifier":
		constructor = "ToModuleSpecifier"
	case "SourceMapLocation":
		constructor = "ToSourceMapLocation"
	case "FileSpec":
		constructor = "ToFileSpec"
	case "PathPattern":
		constructor = "ToPathPattern"
	default:
		return nil
	}
	if valueConstant := p.pass.TypesInfo.Types[value].Value; valueConstant != nil && valueConstant.Kind() == constant.String {
		if valid, _, checked := validateTypedPathConstant(target, constant.StringVal(valueConstant)); checked && !valid {
			return nil
		}
	}
	replacement := fmt.Sprintf("tspath.%s(%s)", constructor, expressionText(p.pass.Fset, value))
	return []analysis.SuggestedFix{{
		Message: "Use a typed path constructor",
