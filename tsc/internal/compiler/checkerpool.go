...
type CheckerPool interface {
	...
	// ForEachCheckerGroupDo runs one task per checker rather than one per file, so the files a
	// checker owns are visited together and in the order given. Whether the checker is held for
	// the whole group or taken again for each file is the pool's to decide.
