...
func (mapper *projectReferenceFileMapper) FS() vfs.FS {
	return mapper.fs
}

func (mapper *projectReferenceFileMapper) GetCurrentDirectory() tspath.RootedDirectoryPath {
	return mapper.opts.Config.BaseDirectory()
}
...
func (mapper *projectReferenceFileMapper) getSourceToDtsIfSymlink(file ast.HasFileName) *tsoptions.SourceOutputAndProjectReference {
	...
	if mapper.loader != nil && mapper.opts.Config.CompilerOptions().PreserveSymlinks == core.TSTrue {
		...
		if !fileName.ContainsLowercaseDirectorySequence("/node_modules/") {
			...
			realDeclarationPath := mapper.loader.caseSensitivity.PathKey(mapper.fs.Realpath(fileName.AsPath()))
