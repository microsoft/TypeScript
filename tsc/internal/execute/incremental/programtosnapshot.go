...
type toProgramSnapshot struct {
	...
	snapshot    *snapshot
	// reuseReferences says every file references what it did in oldSnapshot; see
	// NewProgramFromPriorState.
	reuseReferences   bool
...
func (t *toProgramSnapshot) computeProgramFileChanges() {
	...
	for _, file := range files {
		wg.Queue(func() {
			...
			newReferences := t.referencesOf(file)
...
// referencesOf returns what a file references, from the previous program's map where that map
// still describes it, and by resolving its imports through a type checker where it does not.
func (t *toProgramSnapshot) referencesOf(file *ast.SourceFile) *collections.Set[tspath.Path] {
	if t.reuseReferences {
		// Only for a file the previous program had. No entry in the map means it referenced
		// nothing, which is an answer, so the file being known is what decides this rather than
		// the lookup succeeding.
		if _, known := t.oldSnapshot.fileInfos.Load(file.Path()); known {
			references, _ := t.oldSnapshot.referencedMap.getReferences(file.Path())
			return references
		}
	}
	return getReferencedFiles(t.program, file)
}
...
func getReferencedFiles(program *compiler.Program, file *ast.SourceFile) *collections.Set[tspath.Path] {
	...
	// The project system hands out checkers by what the caller wants them for, and this is the
	// work of checking rather than a query: a query checker is never handed back by a sweep, so
	// resolving every file's imports through one would leave a share of the program's types in
	// each of the slots kept for hovers and completions until they idled out. The command line's
	// pool does not distinguish, so this says nothing to it.
	checkerCtx := core.WithCheckerLifetime(context.TODO(), core.CheckerLifetimeDiagnostics)
	checker, done := program.GetTypeCheckerForFileExclusive(checkerCtx, file)
	defer done()
