...
func GetFileMapWithBuild(files FileMap, commandLineArgs []string) FileMap {
	...
	sys.fs.writtenFiles.Range(func(fileName tspath.RootedFilePath) bool {
		if text, ok := sys.fsFromFileMap().ReadFile(fileName); ok {
			files[fileName.AsString()] = text
		}
		return true
	})
	return files
...
func newTestSys(tscInput *tscInput, forIncrementalCorrectness bool) *TestSys {
	...
	sys.mockWatchBackend.DirectoryExists = func(path tspath.RootedDirectoryPath) bool {
		return sys.fs.FS.DirectoryExists(path)
	}
	sys.mockWatchBackend.CaseSensitivity = caseSensitivity
	sys.fsDiffer = &fsbaselineutil.FSDiffer{
		FS:           sys.fs.FS.(iovfs.FsWithSys),
		DefaultLibs:  func() *collections.SyncSet[tspath.RootedPath] { return sys.fs.defaultLibs },
...
func (s *TestSys) ensureLibPathExists(path string) {
	...
	if _, ok := s.fsFromFileMap().ReadFile(fileName); !ok {
		if s.fs.defaultLibs == nil {
			s.fs.defaultLibs = &collections.SyncSet[tspath.RootedPath]{}
		}
		s.fs.defaultLibs.Add(fileName.AsPath())
