...
func watchResolutionDirectory(t *testing.T) string {
	...
	dir, err := os.MkdirTemp(root, ".watch-resolution-") //nolint:usetesting // Keep fixtures under the checkout to avoid temporary-directory aliases.
	assert.NilError(t, err)
...
func TestWatchResolutionNamespaceChanges(t *testing.T) {
	...
	for _, test := range []struct {
		directory, childFirst bool
	}{
		{false, false},
		{true, false},
		{true, true},
	} {
		t.Run(fmt.Sprintf("directory=%v/childFirst=%v", test.directory, test.childFirst), func(t *testing.T) {
			t.Parallel()
			...
			changes := wm.DrainEvents()
			assert.Assert(t, slices.Contains(changes.Affected, name))
			assert.Assert(t, slices.Contains(changes.Affected, link))
			slices.Sort(changes.Affected)
			if test.childFirst {
				slices.Reverse(changes.Affected)
			}
