...
func TestUnregisteredEventsRetainWatchAliases(t *testing.T) {
	...
	filesystem := &comparerWatchFS{countingWatchFS: &countingWatchFS{
		FS: vfstest.FromMap(map[string]string{"/repo/src/file.ts": ""}, true),
	}}
	wm := NewWatchManager(io.Discard, filesystem.DirectoryExists, filesystem)
	assert.NilError(t, wm.ReconcileWatches([]string{"/repo/src/file.ts"}, map[string]bool{"/repo": true}, nil))
	aliases := wm.aliases
	...
	for _, name := range []string{"/repo/src/file.js", "/repo/new/file.ts", "/unrelated"} {
		for _, kind := range []fswatch.EventKind{fswatch.EventUpdate, fswatch.EventDelete} {
			wm.onWatchEvents([]fswatch.Event{{Path: name, Kind: kind}}, nil)
	...
	wm.onWatchEvents([]fswatch.Event{{Path: "/repo/src", Kind: fswatch.EventUpdate}}, nil)
