...
func getPackageRealpathFuncs(fs vfs.FS, packageDirectory tspath.RootedDirectoryPath) (toRealpath, toSymlink func(tspath.RootedFilePath) tspath.RootedFilePath) {
	...
	toRealpath = func(fileName tspath.RootedFilePath) tspath.RootedFilePath {
		...
		if directoryPackage := module.NodeModulePackageRootForDirectory(directory); directoryPackage != filePackageDir {
			if fs.DirectoryExists(directory) {
				filePackageDir = directoryPackage
			}
		}
		if realDir, ok := dirCache[filePackageDir]; ok {
			if realDir == filePackageDir {
				return fileName
			}
			relative, _ := fileName.RelativeTo(filePackageDir)
			return realDir.ResolveRelativeFile(relative)
		}
		realDir := tspath.RootedDirectoryPathFromPath(fs.Realpath(filePackageDir.AsPath()))
...
func getModuleResolver(host RegistryCloneHost, currentDirectory tspath.RootedDirectoryPath, realpath func(tspath.RootedFilePath) tspath.RootedFilePath, opts module.ResolverOptions) *module.Resolver {
	...
	return module.NewResolverWithOptions(fs, currentDirectory, core.EmptyCompilerOptions, "", "", opts)
}
