...
func TestTryRemoveIndexFileName(t *testing.T) {
	...
	assert.Equal(t, tryRemoveIndexFileName(tspath.RootedFilePath("/project/index.ts")), tspath.RootedPath("/project"))
	assert.Equal(t, tryRemoveIndexFileName(tspath.RootedFilePath("/index.ts")), tspath.RootedPath(""))
	assert.Equal(t, tryRemoveIndexFileName(tspath.RootedFilePath("c:/index.ts")), tspath.RootedPath("c:/"))
	assert.Equal(t, tryRemoveIndexFileName(tspath.RootedFilePath("^/index.ts")), tspath.RootedPath(""))
}
...
func TestCreatePathUpdaterCaseFoldingShrinksOldPath(t *testing.T) {
	...
	oldPath := tspath.RootedFilePath("/a/\u212A\u212A\u212A\u212A")
	newPath := tspath.RootedFilePath("/a/new")
	updater := l.createPathUpdater(
		tspath.RootedPath(oldPath),
		tspath.RootedPath(newPath),
	)

	updated, ok := updater(tspath.RootedFilePath("/a/kkkk/x.ts"))
	assert.Assert(t, ok)
	assert.Equal(t, updated, tspath.RootedFilePath("/a/new/x.ts"))
}
