...
func (p *progressRecorder) record(req *lsproto.RequestMessage) {
	...
	case workDone.Value.Report != nil:
		...
		if pct := workDone.Value.Report.Percentage; pct != nil {
			p.percentages = append(p.percentages, *pct)
		}
...
func (p *progressRecorder) workDonePercentages() []uint32 {
	p.mu.Lock()
	defer p.mu.Unlock()
	return append([]uint32(nil), p.percentages...)
}
...
func startWorkspaceDiagnosticsClient(t *testing.T, files map[string]string) (*lsptestutil.LSPClient, *progressRecorder) {
	...
	initMsg, initResult, ok := client.SendRequest(t, lsproto.InitializeInfo, &lsproto.InitializeParams{
...
func pullWorkspaceDiagnostics(t *testing.T, client *lsptestutil.LSPClient, params *lsproto.WorkspaceDiagnosticParams) *lsproto.WorkspaceDiagnosticReport {
	t.Helper()
	msg, resp, ok := client.SendRequest(t, lsproto.WorkspaceDiagnosticInfo, params)
...
// What a pull reports is what the program has, so a project narrows it the way it narrows a build:
// with the tsconfig's exclude.
func TestWorkspaceDiagnosticsHonorsTsconfigExclude(t *testing.T) {
	t.Parallel()
	...
	files := map[string]string{
		"/home/projects/tsconfig.json": `{"exclude": ["vendor"]}`,
		"/home/projects/open.ts":       "export const opened = 1;\n",
...
// Dependencies stay out without any exclude to say so: an imported package and an automatically
// included @types package are both in the program, and both have errors, and neither is the user's
// code to fix.
func TestWorkspaceDiagnosticsLeavesOutDependencies(t *testing.T) {
	t.Parallel()
	...
	files := map[string]string{
		...
		"/home/projects/node_modules/dep/package.json":      `{"name": "dep", "types": "index.d.ts"}`,
		"/home/projects/node_modules/dep/index.d.ts":        "export declare const z: string;\nexport declare const bad: string = 1;\n",
		"/home/projects/node_modules/@types/x/package.json": `{"name": "@types/x", "types": "index.d.ts"}`,
		"/home/projects/node_modules/@types/x/index.d.ts":   "declare const globalBad: string = 1;\n",
	}
...
func TestWorkspaceDiagnosticsSupersedesOverlappingPulls(t *testing.T) {
	...
	for i := range waiters {
		waiters[i] = client.SendRequestAsync(t, lsproto.WorkspaceDiagnosticInfo, &lsproto.WorkspaceDiagnosticParams{})
	}
...
// A project reports only once it is wholly checked, and a workspace can be a single project, so
// progress paced by reports would sit at nothing for the whole run and then jump to the end. It
// has to be paced by the files the checkers get through.
func TestWorkspaceDiagnosticsProgressAdvancesWhileCheckingOneProject(t *testing.T) {
	t.Parallel()
	...
	// One project, enough files that a file is worth well under a percent of the whole.
	files := map[string]string{"/home/projects/tsconfig.json": `{}`}
	files["/home/projects/open.ts"] = "export const opened = 1;\n"
	for i := range 60 {
		files[fmt.Sprintf("/home/projects/f%d.ts", i)] = fmt.Sprintf("export const v%d: number = %d;\n", i, i)
	}

	client, progress := initWorkspaceDiagnosticsClient(t, files)
	client.SendNotification(t, lsproto.TextDocumentDidOpenInfo, &lsproto.DidOpenTextDocumentParams{
		TextDocument: &lsproto.TextDocumentItem{
			Uri: "file:///home/projects/open.ts", LanguageId: "typescript", Version: 1,
			Text: files["/home/projects/open.ts"],
	...
	report := pullWorkspaceDiagnostics(t, client, &lsproto.WorkspaceDiagnosticParams{
		WorkDoneToken:     &lsproto.IntegerOrString{String: new("wsdiag-progress")},
		PreviousResultIds: []lsproto.PreviousResultId{},
	})
	assert.Assert(t, len(report.Items) > 0, "the pull reports the workspace")

	percentages := progress.workDonePercentages()
	assert.Assert(t, len(percentages) > 0, "expected progress reports, got none")
	for i, pct := range percentages {
		assert.Assert(t, pct <= 100, "percentage out of range: %d", pct)
		if i > 0 {
			assert.Assert(t, pct > percentages[i-1], "progress must move forwards, got %v", percentages)
		}
	}
	assert.Assert(t, percentages[0] < 100,
		"progress must advance while the single project is checked, not jump to the end: %v", percentages)
}
