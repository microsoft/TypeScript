...
func (handlers handlerMap) registerWorkspaceDiagnosticHandler() {
	handlers[lsproto.WorkspaceDiagnosticInfo.Method] = func(s *Server, ctx context.Context, req *lsproto.RequestMessage) (func() error, error) {
		...
		params, err := req.UnmarshalParams[*lsproto.WorkspaceDiagnosticParams]()
		if err != nil {
			return nil, err
		}
...
// its diagnostics checkers, so checking one project already uses several checkers; projects run
// concurrently on top of that, bounded so the two together do not take the machine.
...
//
// The program checks them all in one call, so the work is shared across the project's checkers
// rather than being driven a file at a time from here; the trade is that a project reports once it
// is done rather than streaming as each of its files finishes.
func (r *workspaceDiagnosticsRun) checkProject(snapshot *project.Snapshot, pf *workspaceDiagnosticsProject) bool {
	...
	ctx := project.WithCheckProgress(r.ctx, func(checked, total int) {
		...
		// Called from the checkers, a batch of files at a time. reportProgress says nothing
		// unless the percentage moved, which bounds the notifications to a hundred on top of that.
		r.reportProgress()
	})
		...
		if pf.files[j] != nil {
			...
			if full := report.FullDocumentDiagnosticReport; full != nil && full.ResultId != nil {
				r.cache.store(lsconv.FileNameToDocumentURI(pf.files[j].FileName()), workspaceDiagnosticsCacheEntry{
					project:    tspath.Path(pf.project.Id()),
...
func (r *workspaceDiagnosticsRun) flush() {
	...
	_ = r.server.sendNotification(lsproto.WorkspaceDiagnosticPartialResultInfo, &lsproto.WorkspaceDiagnosticPartialResultParams{
...
func (r *workspaceDiagnosticsRun) beginProgress() {
	...
	case r.server.clientCapabilities.Window.WorkDoneProgress:
		...
		if err := r.server.sendClientRequestFireAndForget(lsproto.WindowWorkDoneProgressCreateInfo, &lsproto.WorkDoneProgressCreateParams{
			Token: token,
		}); err != nil {
			r.progressToken = nil
			return
