...
//
// The snapshot rather than the program generations, which is what decides whether a file needs
// checking: what a pull reports also turns on which documents are open, which projects are loaded,
// and what is on disk. Those all move the snapshot, and the per-file cache already answers an
// unnecessary sweep without checking anything.
...
// workspaceDiagnosticsSettings is the settings an entry was computed under. It compares every
// preference rather than the ones known to matter: one that changes what a diagnostic says but was
// left off such a list would strand errors in the client's problem list, which is worse than the
...
// useSettings discards the cache if the settings behind it changed. It compares the snapshot's
// copy rather than reacting to a configuration notification, so a pull already in flight cannot
// repopulate under settings that have moved on.
...
// workspaceDiagnosticsCacheStats describes what the cache holds, so a pull that takes longer than
// expected can be read against what it was able to skip.
