...
func TestResolveModuleNameTrailingSlash(t *testing.T) {
	...
	resolver := module.NewResolver(host.fs, host.cwd, opts, "", "", nil)

	for _, name := range []string{"pkg", "pkg/"} {
		r, _ := resolver.ResolveModuleName(name, tspath.RootedFilePath("/repo/src/file.ts"), core.ModuleKindESNext, nil)
		if !r.IsResolved() {
...
func TestResolvedModuleCarriesCaseInsensitivePath(t *testing.T) {
	...
	resolved, _ := resolver.ResolveModuleName("Pkg", tspath.RootedFilePath("/Repo/src/file.ts"), core.ModuleKindESNext, nil)
	if !resolved.IsResolved() {
