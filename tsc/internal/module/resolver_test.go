...
type resolutionTestInputs struct {
	fs  vfs.FS
	cwd tspath.RootedDirectoryPath
}

func (i *resolutionTestInputs) FS() vfs.FS {
	return i.fs
}

func (i *resolutionTestInputs) GetCurrentDirectory() tspath.RootedDirectoryPath {
	return i.cwd
}
...
func TestResolveModuleNameTrailingSlash(t *testing.T) {
	...
	for _, name := range []string{"pkg", "pkg/"} {
		r, _, _ := resolver.ResolveModuleName(name, tspath.RootedFilePath("/repo/src/file.ts"), core.ModuleKindESNext, nil)
		if !r.IsResolved() {
...
func TestResolveDynamicModuleNameUsingRootDirs(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			host := &resolutionTestInputs{fs: fs, cwd: "^/~ts-uri~/custom/ts-nul-authority/"}
			opts := &core.CompilerOptions{
				ModuleResolution: core.ModuleResolutionKindBundler,
				Module:           core.ModuleKindESNext,
				Target:           core.ScriptTargetESNext,
			...
			resolver := module.NewResolver(module.ResolverOptions{Host: host, CompilerOptions: opts})
			resolved, _, _ := resolver.ResolveModuleName(
				"./~ts-uri-escape~file",
...
func TestRootDirsEscapesRootLikeDynamicSegments(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			sourceFile: "^/~ts-uri~/custom/ts-nul-authority/src/c:/main.ts",
			targetFile: "^/~ts-uri~/custom/ts-nul-authority/generated/~ts-uri-escape~633a~/dep.ts",
		},
		{
			sourceFile: "^/~ts-uri~/custom/ts-nul-authority/src/^/main.ts",
			targetFile: "^/~ts-uri~/custom/ts-nul-authority/generated/~ts-uri-escape~5e~/dep.ts",
		},
	} {
		t.Run(test.sourceFile.AsString(), func(t *testing.T) {
			...
			resolver := module.NewResolver(module.ResolverOptions{Host: host, CompilerOptions: opts})
			resolved, _, _ := resolver.ResolveModuleName("./dep", test.sourceFile, core.ModuleKindESNext, nil)
...
func TestRootDirsPreservesRelativeDiskSuffixes(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.sourceFile.AsString(), func(t *testing.T) {
			...
			host := &resolutionTestInputs{fs: fs, cwd: "c:/"}
			opts := &core.CompilerOptions{
				ModuleResolution: core.ModuleResolutionKindBundler,
				Module:           core.ModuleKindESNext,
				Target:           core.ScriptTargetESNext,
...
func TestRootDirsPreservesRelativeDirectoryIntent(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			sourceFile: "c:/src/^/main.ts",
			targetRoot: "^/~ts-uri~/custom/ts-nul-authority/generated",
			targetFile: "^/~ts-uri~/custom/ts-nul-authority/generated/~ts-uri-escape~5e~/pkg/index.ts",
		},
		{
			...
			sourceFile: "c:/src/main.ts",
			targetRoot: "^/~ts-uri~/custom/ts-nul-authority/generated",
			targetFile: "^/~ts-uri~/custom/ts-nul-authority/generated/index.ts",
		},
		{
			name:       "empty suffix to disk",
			sourceRoot: "^/~ts-uri~/custom/ts-nul-authority/src",
			sourceFile: "^/~ts-uri~/custom/ts-nul-authority/src/main.ts",
			targetRoot: "d:/generated",
		...
		},
		{
			...
			sourceRoot: "^/~ts-uri~/custom/ts-nul-authority/src",
			sourceFile: "^/~ts-uri~/custom/ts-nul-authority/src/~ts-uri-escape~~/main.ts",
			targetRoot: "^/~ts-uri~/custom/ts-nul-authority/generated",
			targetFile: "^/~ts-uri~/custom/ts-nul-authority/generated/~ts-uri-escape~~/index.ts",
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			...
			resolver := module.NewResolver(module.ResolverOptions{Host: host, CompilerOptions: opts})
			resolved, _, _ := resolver.ResolveModuleName(moduleName, test.sourceFile, core.ModuleKindCommonJS, nil)
			if !resolved.IsResolved() || resolved.ResolvedFileName != test.targetFile {
				t.Errorf("resolved file = %q, expected %q", resolved.ResolvedFileName, test.targetFile)
			}
		})
...
func TestResolveDynamicPackageJSONPath(t *testing.T) {
	...
	resolved, _, _ := resolver.ResolveModuleName(
		"pkg",
		tspath.RootedFilePath("^/~ts-uri~/custom/ts-nul-authority/src/main.ts"),
		core.ModuleKindESNext,
		nil,
	)
	if !resolved.IsResolved() {
...
func TestResolvedModuleCarriesCaseInsensitivePath(t *testing.T) {
	...
	resolved, _, _ := resolver.ResolveModuleName("Pkg", tspath.RootedFilePath("/Repo/src/file.ts"), core.ModuleKindESNext, nil)
	if !resolved.IsResolved() {
...
func TestResolvedTypeReferenceDirectiveCarriesPath(t *testing.T) {
	...
	resolved, _ := resolver.ResolveTypeReferenceDirective(
		"pkg",
		tspath.RootedFilePath("/repo/src/file.ts"),
...
func TestDynamicExportPatternEntrypointUsesLogicalName(t *testing.T) {
	...
	for _, test := range []struct {
	...
	}{
		{
			...
			importName:      "Pkg/~ts-uri-escape~value",
			moduleSpecifier: "Pkg/~ts-uri-spec~7e74732d7572692d6573636170657e76616c7565~",
		},
		{
			name:            "reserved target pattern",
			packageJSON:     `{"name":"Pkg","exports":{"./*":"./~ts-uri-escape~*.d.ts"}}`,
		...
		t.Run(test.name, func(t *testing.T) {
			...
			const sourceFile = "^/~ts-uri~/custom/ts-nul-authority/src/main.ts"
			targetFile := "^/~ts-uri~/custom/ts-nul-authority/node_modules/Pkg/~ts-uri-escape~7e74732d7572692d6573636170657e76616c7565~.d.ts"
			if test.name == "reserved wildcard with trailer" {
				targetFile = "^/~ts-uri~/custom/ts-nul-authority/node_modules/Pkg/~ts-uri-escape~7e74732d7572692d6573636170657e76616c75652d696d706c~.d.ts"
			}
			fs := vfstest.FromMap(map[string]string{
				sourceFile: "",
				"^/~ts-uri~/custom/ts-nul-authority/node_modules/Pkg/package.json": test.packageJSON,
				targetFile: "export const value: number;",
			}, tspath.CaseSensitive)
			host := &resolutionTestInputs{fs: fs, cwd: "^/~ts-uri~/custom/ts-nul-authority/"}
			opts := &core.CompilerOptions{
...
func TestGeneratedDynamicEntrypointSpecifierResolvesEncodedFile(t *testing.T) {
	...
	_, _, _ = resolver.ResolveModuleName(
		"Pkg",
		tspath.RootedFilePathFromNormalized(sourceFile),
		core.ModuleKindESNext,
		nil,
...
func TestDynamicEntrypointsEncodePhysicalPackageNameAndRootLikePath(t *testing.T) {
	...
	const (
		sourceFile          = "^/~ts-uri~/custom/ts-nul-authority/src/main.ts"
		physicalPackageName = "~ts-uri-escape~7e74732d7572692d6573636170657e706b672e6a73~"
		sourcePackageName   = "~ts-uri-spec~7e74732d7572692d6573636170657e706b672e6a73~"
		packageDirectory    = "^/~ts-uri~/custom/ts-nul-authority/node_modules/" + physicalPackageName
		targetFile          = packageDirectory + "/~ts-uri-escape~633a~/value.d.ts"
		moduleSpecifier     = sourcePackageName + "/~ts-uri-spec~633a~/value.d.ts"
	)
	fs := vfstest.FromMap(map[string]string{
		sourceFile:                         "",
		packageDirectory + "/package.json": `{"name":"~ts-uri-escape~pkg.js"}`,
		targetFile:                         "export const value: number;",
	}, tspath.CaseSensitive)
	host := &resolutionTestInputs{fs: fs, cwd: "^/~ts-uri~/custom/ts-nul-authority/"}
	opts := &core.CompilerOptions{
...
func TestDynamicExportPatternPreservesStaticSourcePrefix(t *testing.T) {
	...
	const (
		sourceFile       = "^/~ts-uri~/custom/ts-nul-authority/src/main.ts"
		packageDirectory = "^/~ts-uri~/custom/ts-nul-authority/node_modules/Pkg"
		targetFile       = packageDirectory + "/foo.d.ts"
		moduleSpecifier  = "Pkg/~ts-uri-escape~prefix/foo"
	)
	fs := vfstest.FromMap(map[string]string{
		sourceFile:                         "",
		packageDirectory + "/package.json": `{"name":"Pkg","exports":{"./~ts-uri-escape~prefix/*":"./*.d.ts"}}`,
		targetFile:                         "export const value: number;",
	}, tspath.CaseSensitive)
	host := &resolutionTestInputs{fs: fs, cwd: "^/~ts-uri~/custom/ts-nul-authority/"}
	opts := &core.CompilerOptions{
...
func TestDynamicExportPatternKeepsPackageForRootLikeMatch(t *testing.T) {
	...
	const (
		...
		packageDirectory = "^/~ts-uri~/custom/ts-nul-authority/node_modules/Pkg"
		targetFile       = packageDirectory + "/~ts-uri-escape~633a~/value.d.ts"
		moduleSpecifier  = "Pkg/~ts-uri-spec~633a~/value"
	)
	fs := vfstest.FromMap(map[string]string{
		sourceFile:                         "",
...
func TestDiskExportPatternKeepsReservedPrefixLiteral(t *testing.T) {
	...
	const targetFile = "/repo/node_modules/pkg/~ts-uri-escape~666f6f~.d.ts"
	fs := vfstest.FromMap(map[string]string{
		"/repo/src/main.ts":                   "",
	...
	resolved, _, _ := resolver.ResolveModuleName(
		...
		tspath.RootedFilePath("/repo/src/main.ts"),
		core.ModuleKindESNext,
		nil,
	)
	if !resolved.IsResolved() {
	...
	entrypoints := resolver.GetEntrypointsFromPackageJsonInfo(packageJson, "pkg", false)
	if len(entrypoints) != 1 || entrypoints[0].ModuleSpecifier.AsString() != "pkg/~ts-uri-escape~666f6f~" {
...
func TestDynamicExportPatternUnderExceptionalParent(t *testing.T) {
	...
	const (
		sourceFile       = "^/~ts-uri~/custom/ts-nul-authority/work/~ts-uri-escape~2e2e~/src/main.ts"
		packageDirectory = "^/~ts-uri~/custom/ts-nul-authority/work/~ts-uri-escape~2e2e~/node_modules/Pkg"
		targetFile       = packageDirectory + "/foo.d.ts"
	)
	fs := vfstest.FromMap(map[string]string{
		sourceFile:                         "",
...
func TestResolveModuleNameTrailingSlashRace(t *testing.T) {
	...
	for _, name := range []string{"pkg", "pkg/"} {
		...
		wg.Go(func() {
			r, _, _ := resolver.ResolveModuleName(name, tspath.RootedFilePathFromNormalized(containingFile), core.ModuleKindESNext, nil)
...
func TestResolveSubpathNilContentsRace(t *testing.T) {
	...
	for _, containingFile := range []string{"/repo/src/a/file.ts", "/repo/src/b/file.ts"} {
		wg.Go(func() {
			...
			r, _, _ := resolver.ResolveModuleName("pkg/sub", tspath.RootedFilePathFromNormalized(containingFile), core.ModuleKindESNext, nil)
...
func TestResolvePeerDependencyNilContentsRace(t *testing.T) {
	...
	for _, containingFile := range []string{"/repo/src/a/file.ts", "/repo/src/b/file.ts"} {
		wg.Go(func() {
			...
			r, _, _ := resolver.ResolveModuleName("pkg", tspath.RootedFilePathFromNormalized(containingFile), core.ModuleKindESNext, nil)
			resolved = r.IsResolved()
		})
	}
