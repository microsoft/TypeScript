...
func TestIsPackageMainFilePreservesDirectoryIntent(t *testing.T) {
	t.Parallel()
	packageRoot := tspath.RootedDirectoryPath("/project/node_modules/pkg")

	if isPackageMainFile(tspath.RootedFilePath("/project/node_modules/pkg/types.d.ts"), packageRoot, "./types/", "", tspath.CaseSensitive) {
		t.Fatal("slash-terminated package entrypoint must not match the sibling declaration file")
	}
	if !isPackageMainFile(tspath.RootedFilePath("/project/node_modules/pkg/types/index.d.ts"), packageRoot, "./types/", "", tspath.CaseSensitive) {
...
func TestProcessEndingChecksRootedFilePath(t *testing.T) {
	...
	result := processEnding(
		tspath.ToModuleSpecifier("./lib/index.ts"),
	...
	if !slices.Contains(host.fileExistsCalls, tspath.RootedFilePath("/project/src/lib.ts")) {
