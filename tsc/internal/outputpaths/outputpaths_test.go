...
func TestGetSourceFileNameInNewDirSourceMatchesCommonDirectory(t *testing.T) {
	...
	actual := outputpaths.GetSourceFileNameInNewDir(
		tspath.RootedFilePath("/project/src"),
		tspath.RootedDirectoryPath("/project/out"),
...
func TestGetSourceFileNameInNewDirCanonicalizationShrinksCommonDirectory(t *testing.T) {
	...
	actual := outputpaths.GetSourceFileNameInNewDir(
		tspath.RootedFilePath("/kkkk/a.ts"),
		tspath.RootedDirectoryPath("/out"),
		tspath.RootedDirectoryPath("/\u212A\u212A\u212A\u212A"),
...
func TestGetOutputFileNameAcrossRoots(t *testing.T) {
	...
	input := tspath.RootedFilePath("d:/shared.ts")
	options := &core.CompilerOptions{
		OutDir: tspath.RootedDirectoryPath("c:/dist"),
	}
	host := outputPathsHost{
		commonSourceDirectory: tspath.RootedDirectoryPath("c:/src"),
	}
