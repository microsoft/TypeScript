...
func TestPackageDirectoryCacheIdentityPreservesPresentation(t *testing.T) {
	...
	upper := cache.PackageDirectory(tspath.RootedDirectoryPath("/Repo/Package"))
	lower := cache.PackageDirectory(tspath.RootedDirectoryPath("/repo/package"))
...
func TestForEachAncestorDirectoryStoppingAtGlobalCache(t *testing.T) {
	...
	cache := packagejson.NewInfoCache(tspath.CaseInsensitive)
	start := cache.PackageDirectory(tspath.RootedDirectoryPath("/Repo/Project/src"))
	var names []string
	var keys []string
	packagejson.ForEachAncestorDirectoryStoppingAtGlobalCache(
		tspath.RootedDirectoryPath("/Repo"),
		start,
