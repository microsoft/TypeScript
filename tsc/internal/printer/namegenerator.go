	...
	if g.Context != nil {
		if autoGenerate, ok := g.Context.autoGenerate[name]; ok {
			if autoGenerate.Kind() == GeneratedIdentifierFlagsNode {
				...
				return g.generateNameForNodeCached(g.getNodeForGeneratedName(name), ast.IsPrivateIdentifier(name), autoGenerate.Flags, autoGenerate.Prefix, autoGenerate.Suffix)
			} else {
...
func (g *nameGenerator) generateNameForNode(node *ast.Node, privateName bool, flags GeneratedIdentifierFlags, prefix string, suffix string) string {
	...
	case ast.KindIdentifier, ast.KindPrivateIdentifier:
		return g.makeUniqueName(
			g.GetTextOfNode(node),
			nil, /*checkFn*/
			flags&GeneratedIdentifierFlagsOptimistic != 0,
			flags&GeneratedIdentifierFlagsReservedInNestedScopes != 0,
			privateName,
			prefix,
			suffix,
		)
	...
	if g.Context != nil {
		if autoGenerate, ok := g.Context.autoGenerate[name]; ok {
			...
			case GeneratedIdentifierFlagsAuto:
				return g.makeTempVariableName(tempFlagsAuto, autoGenerate.Flags&GeneratedIdentifierFlagsReservedInNestedScopes != 0, ast.IsPrivateIdentifier(name), autoGenerate.Prefix, autoGenerate.Suffix)
			case GeneratedIdentifierFlagsLoop:
				// Debug.assertNode(name, isIdentifier);
				return g.makeTempVariableName(tempFlags_i, autoGenerate.Flags&GeneratedIdentifierFlagsReservedInNestedScopes != 0, false /*privateName*/, autoGenerate.Prefix, autoGenerate.Suffix)
			case GeneratedIdentifierFlagsUnique:
				return g.makeUniqueName(
					name.Text(),
					core.IfElse(autoGenerate.Flags&GeneratedIdentifierFlagsFileLevel != 0, g.IsFileLevelUniqueNameInCurrentFile, nil),
					autoGenerate.Flags&GeneratedIdentifierFlagsOptimistic != 0,
					autoGenerate.Flags&GeneratedIdentifierFlagsReservedInNestedScopes != 0,
