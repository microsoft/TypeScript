...
func TestBulkCacheInvalidation(t *testing.T) {
	...
	t.Run("large number of changes outside node_modules causes project reevaluation", func(t *testing.T) {
		...
		assert.Equal(t, initialProject.ConfigFileName(), "/project/tsconfig.json", "Should initially use root tsconfig")
		...
		assert.Equal(t, newProject.ConfigFileName(), "/project/src/utils/tsconfig.json", "Should now use nested tsconfig after bulk invalidation")
	...
	t.Run("config file names cache", func(t *testing.T) {
		...
		"include": ["src/**/*"]
			...
			if expectConfigDiscovery {
				...
				assert.Equal(t, newProject.ConfigFileName(), "/project/tsconfig.json", "Should use the newly discovered tsconfig")
			} else {
