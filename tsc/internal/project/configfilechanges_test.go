...
func TestConfigFileChanges(t *testing.T) {
	...
	t.Run("config file creation then deletion", func(t *testing.T) {
		...
		assert.Equal(t, snapshot.GetDefaultProject(lsproto.DocumentUri("file:///src/subfolder/foo.ts")).ConfigFileName().AsString(), "/src/subfolder/tsconfig.json")
		...
		assert.NilError(t, err)
		snapshot = session.Snapshot()
		assert.Equal(t, snapshot.GetDefaultProject(lsproto.DocumentUri("file:///src/subfolder/foo.ts")).ConfigFileName().AsString(), "/src/tsconfig.json")
