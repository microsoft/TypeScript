...
func TestExtendedConfigCacheOwnership(t *testing.T) {
	...
	ownerCount := func(session *Session, path tspath.Path) int {
		entry, ok := session.host.extendedConfigCache.entries.Load(path)
		if !ok {
			return 0
		}
	...
	assertNoEntry := func(t *testing.T, session *Session, fileName string) {
		t.Helper()
		path := session.host.toPath(fileName)
		_, ok := session.host.extendedConfigCache.entries.Load(path)
	...
	expectedExtendedOwnerCounts := func(session *Session, snapshot *Snapshot) map[tspath.Path]int {
		...
		for _, cfg := range snapshot.ConfigFileRegistry.configs {
			...
			for _, file := range cfg.commandLine.ExtendedSourceFiles() {
				result[session.host.toPath(file)]++
			}
		}
		return result
	}
	...
	t.Run("project system dedupes case-only extends via cache", func(t *testing.T) {
		...
		assert.Equal(t, session.host.toPath(extended[0]), session.host.toPath("/project/shared.json"))
	})
	...
	t.Run("transitive extended config ownership with new project", func(t *testing.T) {
		...
		baseEntry, baseOk := session.host.extendedConfigCache.entries.Load("/user/username/projects/shared/tsconfig.base.json")
		commonEntry, commonOk := session.host.extendedConfigCache.entries.Load("/user/username/projects/shared/tsconfig.common.json")
