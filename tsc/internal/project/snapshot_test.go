...
func TestSnapshot(t *testing.T) {
	...
	t.Run("document snapshots refresh default and synthetic programs", func(t *testing.T) {
		...
		temporarySnapshot, err := session.CloneSnapshotWithTemporaryFile(ctx, baseSnapshot, nil, uri, temporaryText)
		assert.NilError(t, err)
		...
		assert.NilError(t, session.fs.WriteFile(fileName, diskText))
