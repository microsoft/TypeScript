	...
	return func(data *collections.SyncMap[tspath.Path, string]) PatternsAndIgnored {
		...
		if data != nil {
			data.Range(func(path tspath.Path, fileName string) bool {
				...
				if workspaceDirectoryPath.ContainsPath(path) {
					...
					directoryPath := path[:idx+len("/node_modules")]
					// Canonicalization can change byte lengths, but not path component counts.
					components := tspath.GetNormalizedPathComponents(fileName, currentDirectory)
					count := len(tspath.GetPathComponents(string(directoryPath), ""))
					nodeModulesDirectories[directoryPath] = tspath.GetPathFromPathComponents(components[:count])
				} else {
		...
		if len(externalDirectories) > 0 {
			externalDirectoryParents, ignoredExternalDirs := tspath.GetCommonParents(
				...
				tspath.ComparePathsOptions{CurrentDirectory: currentDirectory, UseCaseSensitiveFileNames: useCaseSensitiveFileNames},
			)
