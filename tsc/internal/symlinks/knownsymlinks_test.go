...
func TestSetDirectory(t *testing.T) {
	t.Parallel()
	cache := NewKnownSymlinks(tspath.CaseSensitive)
	symlinkPath := tspath.CaseSensitive.PathKey(tspath.RootedPath("/test/symlink"))
	realDirectory := &KnownDirectoryLink{
		Real:     tspath.RootedDirectoryPath("/real/path"),
		RealPath: tspath.CaseSensitive.PathKey(tspath.RootedPath("/real/path")),
	}
	...
	if stored.Symlink != tspath.RootedDirectoryPath("/test/symlink") {
...
func TestKnownDirectoryLinkPreservesChildSpelling(t *testing.T) {
	...
	symlink := tspath.RootedDirectoryPath("/Project/Node_Modules/pkg")
	symlinkPath := tspath.CaseInsensitive.PathKey(symlink.AsPath())
	cache.SetDirectory(symlink, symlinkPath, &KnownDirectoryLink{
		Real:     tspath.RootedDirectoryPath("/Real/Package"),
		RealPath: tspath.PathKey("/real/package"),
	})
	...
	resolved, ok := link.ResolveFilePath(
		tspath.RootedFilePath("/PROJECT/node_modules/pkg/Src/File.ts"),
		tspath.CaseInsensitive,
	)
	if !ok {
		t.Fatal("Expected child path to resolve through directory link")
	}
	if resolved != tspath.RootedFilePath("/Real/Package/Src/File.ts") {
...
func TestSetFile(t *testing.T) {
	t.Parallel()
	cache := NewKnownSymlinks(tspath.CaseSensitive)
	symlink := tspath.RootedFilePath("/test/symlink/file.ts")
	symlinkPath := tspath.CaseSensitive.PathKey(tspath.RootedPath(symlink))
	realpath := tspath.RootedFilePath("/real/path/file.ts")
...
func TestProcessResolution(t *testing.T) {
	...
	cache.ProcessResolution(tspath.RootedFilePath("/original"), "")
	cache.ProcessResolution("", tspath.RootedFilePath("/resolved"))

	// Test with valid paths
	originalPath := tspath.RootedFilePath("/test/original/file.ts")
	resolvedPath := tspath.RootedFilePath("/test/resolved/file.ts")
...
func TestSetSymlinksFromResolutions(t *testing.T) {
	...
	resolvedModules := []struct {
	...
	}{
		{
			...
			filePath:     tspath.CaseSensitive.PathKey(tspath.RootedPath("/test/source.ts")),
		},
		{
