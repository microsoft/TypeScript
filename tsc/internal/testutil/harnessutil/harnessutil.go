...
type CompilationResult struct {
	...
	HarnessOptions   *HarnessOptions
	JS               collections.OrderedMap[tspath.RootedFilePath, *TestFile]
	DTS              collections.OrderedMap[tspath.RootedFilePath, *TestFile]
	Maps             collections.OrderedMap[tspath.RootedFilePath, *TestFile]
	Symlinks         map[string]string
	Repeat           func(TestConfiguration) *CompilationResult
	outputs          []*TestFile
	inputs           []*TestFile
	inputsAndOutputs collections.OrderedMap[tspath.RootedFilePath, *CompilationOutput]
	Trace            string
	Host             compiler.CompilerHost
	currentDirectory tspath.RootedDirectoryPath
}
...
func newCompilationResult(
	...
	if fs != nil && program != nil {
		...
		var js, dts, maps collections.OrderedMap[tspath.RootedFilePath, *TestFile]
		outputPaths := make(map[*TestFile]tspath.RootedFilePath)
		for _, output := range fs.recordedOutputs() {
			document := output.file
			outputPaths[document] = output.path
			if output.path.HasJSFileExtension() ||
				output.path.HasJSONFileExtension() {
				js.Set(output.path, document)
			} else if output.path.IsDeclarationFile() {
				dts.Set(output.path, document)
			} else if output.path.ExtensionIs(".map") {
				maps.Set(output.path, document)
			}
		}
		...
		for _, sourceFile := range program.GetSourceFiles() {
			...
			if !fileName.IsDeclarationFile() {
				...
				jsPath := c.getOutputPath(fileName, extname)
				dtsPath := c.getOutputPath(fileName, tspath.GetDeclarationEmitExtensionForPath(fileNameString))
				mapPath := c.getOutputPath(fileName, extname+".map")
				outputs := &CompilationOutput{
					Inputs: []*TestFile{input},
					JS:     js.GetOrZero(jsPath),
					DTS:    dts.GetOrZero(dtsPath),
					Map:    maps.GetOrZero(mapPath),
				}
				c.inputsAndOutputs.Set(fileName, outputs)
				if outputs.JS != nil {
					c.inputsAndOutputs.Set(jsPath, outputs)
					c.JS.Set(jsPath, outputs.JS)
					js.Delete(jsPath)
					c.outputs = append(c.outputs, outputs.JS)
				}
				if outputs.DTS != nil {
					c.inputsAndOutputs.Set(dtsPath, outputs)
					c.DTS.Set(dtsPath, outputs.DTS)
					dts.Delete(dtsPath)
					c.outputs = append(c.outputs, outputs.DTS)
				}
				if outputs.Map != nil {
					c.inputsAndOutputs.Set(mapPath, outputs)
					c.Maps.Set(mapPath, outputs.Map)
					maps.Delete(mapPath)
...
func (c *CompilationResult) getOutputPath(filePath tspath.RootedFilePath, ext string) tspath.RootedFilePath {
	outputPath := filePath
	...
	if ext == outputPath.DeclarationEmitExtension() {
		return outputpaths.ChangeToDeclarationExtension(outputPath, c.Program.Program())
	}
	return outputPath.ChangeExtension(ext)
}
...
func (c *CompilationResult) GetInputsAndOutputsForFile(path string) *CompilationOutput {
	return c.inputsAndOutputs.GetOrZero(c.currentDirectory.ResolveFile(path))
}
...
func (c *CompilationResult) GetSourceMapRecord() string {
	...
	for _, sourceMapData := range c.Result.SourceMaps {
		...
		if tspath.IsDeclarationFileName(sourceMapData.GeneratedFile) {
			currentFile = c.DTS.GetOrZero(tspath.ToRootedFilePath(sourceMapData.GeneratedFile, c.currentDirectory))
		} else {
			currentFile = c.JS.GetOrZero(tspath.ToRootedFilePath(sourceMapData.GeneratedFile, c.currentDirectory))
		}
