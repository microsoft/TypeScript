...
type ExtendedConfigCache interface {
	GetExtendedConfig(fileName tspath.RootedFilePath, path tspath.PathKey, resolutionStack []tspath.PathKey, fs vfs.FS) *ExtendedConfigCacheEntry
}
...
func getExtendsConfigPathOrArray(
	...
	if reflect.TypeOf(value).Kind() == reflect.String {
		val, err := getExtendsConfigPath(value.(string), fs, newBase, valueExpression, sourceFile)
		if val != "" {
	...
	if reflect.TypeOf(value).Kind() == reflect.Slice {
		for index, fileName := range value.([]any) {
			...
			if reflect.TypeOf(fileName).Kind() == reflect.String {
				val, err := getExtendsConfigPath(fileName.(string), fs, newBase, expression, sourceFile)
				if val != "" {
					extendedConfigPathArray = append(extendedConfigPathArray, val)
				}
				errors = append(errors, err...)
...
func getExtendsConfigPath(
	...
	if tspath.IsRootedDiskPath(extendedConfig) || strings.HasPrefix(extendedConfig, "./") || strings.HasPrefix(extendedConfig, "../") {
		...
		if !fs.FileExists(extendedConfigPath) && !extendedConfigPath.ExtensionIs(tspath.ExtensionJson) {
			extendedConfigPath = extendedConfigPath.AppendSuffix(tspath.ExtensionJson)
			if !fs.FileExists(extendedConfigPath) {
	...
	if resolved := module.ResolveConfig(extendedConfig, basePath.ResolveFile("tsconfig.json"), fs); resolved.IsResolved() {
...
func ParseJsonSourceFileConfigFileContent(
	...
	result := parseJsonConfigFileContentWorker(nil /*json*/, sourceFile, fs, basePath, existingOptions, existingOptionsRaw, sourceFile.SourceFile.FileName(), resolutionStack, extendedConfigCache)
	// tracing?.pop();
	return result
}
...
// fs: File system used to resolve and enumerate config inputs.
// basePath: A root directory to resolve relative path entries in the config file to. e.g. outDir
func ParseJsonConfigFileContent(json any, fs vfs.FS, basePath tspath.RootedDirectoryPath, existingOptions *core.CompilerOptions, configFileName tspath.RootedFilePath, resolutionStack []tspath.PathKey, extendedConfigCache ExtendedConfigCache) *ParsedCommandLine {
	...
	result := parseJsonConfigFileContentWorker(jsonObject, nil /*sourceFile*/, fs, basePath, existingOptions, nil /*existingOptionsRaw*/, configFileName, resolutionStack, extendedConfigCache)
	return result
}
...
func getExtendedConfig(
	...
	var errors []*ast.Diagnostic
	extendedConfigPath := fs.CaseSensitivity().PathKey(tspath.RootedPath(extendedConfigFileName))
	...
	if extendedConfigCache != nil && !slices.Contains(resolutionStack, extendedConfigPath) {
		cacheEntry = extendedConfigCache.GetExtendedConfig(extendedConfigFileName, extendedConfigPath, resolutionStack, fs)
	} else {
		cacheEntry = ParseExtendedConfig(extendedConfigFileName, extendedConfigPath, resolutionStack, fs, extendedConfigCache)
	}
...
func ParseExtendedConfig(
	...
	extendedResult, readErrors := readJsonConfigFile(fileName, path, fs.ReadFile)
	...
	entry.extendedConfig, parseErrors = parseConfig(nil, extendedResult, fs, fileName.Directory(), fileName, resolutionStack, extendedConfigCache)
...
func parseConfig(
	...
	if json != nil {
		ownConfig, err = parseOwnConfigOfJson(json, fs, basePath, configFileName)
	} else {
		ownConfig, err = parseOwnConfigOfJsonSourceFile(tsconfigToSourceFile(sourceFile), fs, basePath, configFileName)
	}
	errors = append(errors, err...)
	...
	applyExtendedConfig := func(result *extendsResult, extendedConfigPath tspath.RootedFilePath) {
		extendedConfig, extendedErrors := getExtendedConfig(sourceFile, extendedConfigPath, fs, resolutionStack, extendedConfigCache, result)
...
func parseJsonConfigFileContentWorker(
	...
	var errors []*ast.Diagnostic
	parsedConfig, errors := parseConfig(json, sourceFile, fs, baseDirectory, configFileName, resolutionStack, extendedConfigCache)
	...
	if len(contentMappers) > 0 && !(parsedConfig.options != nil && parsedConfig.options.RunExternalCode.IsTrue()) {
		...
		for j, mapper := range contentMappers {
			manifest, packageDirectory, diagnostic := resolveContentMapperManifest(fs, containingFile, mapper.Package)
...
func GetParsedCommandLineOfConfigFile(
	...
	return GetParsedCommandLineOfConfigFilePath(configFileName, fs.CaseSensitivity().PathKey(tspath.RootedPath(configFileName)), options, optionsRaw, fs, extendedConfigCache)
}
...
func GetParsedCommandLineOfConfigFilePath(
	...
	errors := []*ast.Diagnostic{}
	configFileText, errors := tryReadFile(configFileName, fs.ReadFile, errors)
	if len(errors) > 0 {
