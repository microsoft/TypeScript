...
func TestToRootedFilePath(t *testing.T) {
	...
	assert.Equal(
		...
		RootedDirectoryPath("^/~ts-uri~/custom/authority?query/"),
	)
}
...
func TestEncodedDynamicPathsPreserveOpaqueIdentity(t *testing.T) {
	...
	const root = "^/~ts-uri~/custom/authority"
	...
	assert.Assert(
		t,
		!PathKey("^/~ts-uri~/custom/Authority/src").ContainsPath(
...
func TestDynamicPathKeyCaseInsensitiveKeyPreservesIdentity(t *testing.T) {
	...
	upper := CaseInsensitive.PathKey(RootedPathFromNormalized("^/~ts-uri~/custom/ts-nul-authority/Foo.ts"))
	lower := CaseInsensitive.PathKey(RootedPathFromNormalized("^/~ts-uri~/custom/ts-nul-authority/foo.ts"))
...
func TestRootedPathCompare(t *testing.T) {
	t.Parallel()

	assert.Equal(t, RootedPathFromNormalized("/a").Compare(RootedPathFromNormalized("/b")), -1)
	assert.Equal(t, RootedFilePathFromNormalized("/a").Compare(RootedFilePathFromNormalized("/a")), 0)
	assert.Equal(t, RootedDirectoryPathFromNormalized("/b").Compare(RootedDirectoryPathFromNormalized("/a")), 1)
}
...
func TestForEachAncestorDirectoryPath(t *testing.T) {
	...
	RootedDirectoryPathFromNormalized("/project/src/lib").ForEachAncestorDirectory(func(directory RootedDirectoryPath) (struct{}, bool) {
