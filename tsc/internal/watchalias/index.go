...
func New(filesystem vfs.FS) *Index {
	index := &Index{filesystem: filesystem}
...
// Add registers a spelling and its ancestors if native comparison is enabled.
// Volume queries are cached per directory. Only missing directories inherit
// their nearest existing ancestor's comparer. Other errors are returned, with no
// partial alias registration. Retrying Add after an error requires a new index.
