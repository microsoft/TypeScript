...
func TestIdentityRegistrationNativeSpelling(t *testing.T) {
	t.Parallel()
	native := nativeComparer(t)
	filesystem := &comparerFS{get: func(string) (fswatch.PathComparer, error) { return native, nil }}
	index := New(filesystem)
	name := "/project/straße/İ.ts"
	assert.NilError(t, index.Register(Registration{Name: name, Realpath: name, Dependency: true}))
	queries := len(filesystem.calls)
	changes := index.Match(map[string]fswatch.EventKind{"/project/STRASSE/i\u0307.ts": fswatch.EventUpdate})
	assert.Equal(t, changes.Changes[name], fswatch.EventUpdate)
	assert.DeepEqual(t, changes.Affected, []string{name})
	assert.Equal(t, len(filesystem.calls), queries)
}
