//// [tests/cases/compiler/freshNegatedTypeWidening.ts] ////

//// [freshNegatedTypeWidening.ts]
interface BaseNode {
    kind: number;
}
...
//// [freshNegatedTypeWidening.js]
"use strict";
function assertWrappedNode(node) {
    if (!isWrappedNode(node)) {
        return node;
    }
    return node;
...
function skipSpecialNodes(node) {
    while (isSpecialNode(node)) {
        node = { kind: 0 };
    }
    return node;
}
function isOrdinaryNode(node) {
    return !isSpecialNode(node);
}
if (!isOrdinaryNode(candidate)) {
    candidate.kind;
...
declare function isWrappedNode(node: BaseNode): node is WrappedNode;
declare function assertWrappedNode(node: BaseNode): WrappedNode;
declare const explicitlyUnwrapped: BaseNode & not WrappedNode;
declare function skipWrappers(node: WrappedNode): WrappedNode;
declare function skipWrappers(node: BaseNode): BaseNode;
interface NamedNode extends BaseNode {
    name: string;
}
declare function isSpecialNode(node: BaseNode): node is WrappedNode | NamedNode;
declare function skipSpecialNodes(node: WrappedNode): WrappedNode;
declare function skipSpecialNodes(node: BaseNode): BaseNode;
declare function isOrdinaryNode(node: BaseNode): boolean;
declare const candidate: BaseNode;
declare function assertString(value: {}): string;
declare const explicitlyNotString: {} & not string;
declare function skipStrings(value: string): string;
declare function skipStrings(value: {}): {};
declare function isNotString(value: {}): boolean;
declare const nonNullValue: {};
